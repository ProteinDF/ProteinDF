...
def main():
    ...
    # set mulliken pop
    set_mulliken_pop(dfdata, dfdata.get_number_of_iterations())
    ...
    if (os.path.isfile(dfparam_path) == True):
        ...
        if (data.has_key("model") == True):
            ...
            if (model.has_key("coordinates") == True):
                atom_list = model["coordinates"]["_"]
                for index, data in enumerate(atom_list):
                    atom = BrAtom()
                    atom.set_element(data["symbol"])
                    atom.move_to(BrPosition(data["coord"]))
                    if (atom.get_atomic_number() == 0):
                        atom.set_charge(data["charge"])
                    dfdata.set_atom(index, atom)
...
def set_mulliken_pop(dfdata, iteration = None):
    if (iteration == None):
        for i in range(1, (dfdata.get_number_of_iterations() +1)):
            set_mulliken_pop(dfdata, i)
    else:
        dfPop = DfPopulation()
        mtx = dfPop.get_atom_populations()
        for atom_index in range(0, dfdata.get_number_of_atoms()):
            population = mtx.get(atom_index, 0)
            dfdata.set_mulliken_atom_population(iteration, atom_index, population)
