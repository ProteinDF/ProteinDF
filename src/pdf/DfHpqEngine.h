...
public:
    ...
    public:
        Query(int a_bar, int b_bar, int a, int b)
            : shellTypeA_bar(a_bar), shellTypeB_bar(b_bar),
              shellTypeA(a), shellTypeB(b) {
        }
    ...
    struct KinState {
        ...
        int index() const {
            const int answer = (( a_bar *HPQ_B_BAR_MAX
                                 +b_bar)*HPQ_A_MAX
                                 +a)    *HPQ_B_MAX +b;
            return answer;
        }
...
public:
    ...
    void calcKineticPart(const Query& query,
                         ...
                         const PGTOs& PGTOsA,
                         const PGTOs& PGTOsB);
    void calcNuclearAttractionPart(const Query& query,
                                   const TlPosition& A,
                                   const TlPosition& B,
                                   const PGTOs& PGTOsA,
                                   const PGTOs& PGTOsB,
...
private:
    ...
    void calcNuc(const int a_bar, const int b_bar, const int a, const int b, const int m);
    ...
    void transform6Dto5D_i(const int I_, const int J_,
                           const int J,
...
template<typename StateType, typename DataType>
void DfHpqEngine::addResultsToOutputBuffer(const DataType& fromBuf, double* toBuf)
{
        ...
        for (int index_b_bar = 0; index_b_bar < amvsBbar.size(); ++index_b_bar) {
            ...
            for (int index_a = 0; index_a < amvsA.size(); ++index_a) {
                ...
                for (int index_b = 0; index_b < amvsB.size(); ++index_b) {
                    ...
                    const int stateAmvsIndex = this->index(amvAbar, amvBbar, amvA, amvB);
