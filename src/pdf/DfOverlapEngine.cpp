...
/// = 3 * 3 * 3 * 6(d) * 6 * 6
const int DfOverlapEngine::OUTPUT_BUFFER_SIZE = 5832;
                 ...
                 const PGTOs& PGTOsC)
{
    ...
    const OvpState state(0, 0, 0, 0, 0, 0);
    ...
    const int numOfBatches = numOfPGTOsA * numOfPGTOsB * numOfPGTOsC;
    ...
    for (int indexA = 0; indexA < numOfPGTOsA; ++indexA) {
        ...
        for (int indexB = 0; indexB < numOfPGTOsB; ++indexB) {
            ...
            for (int indexC = 0; indexC < numOfPGTOsC; ++indexC) {
                ...
                const double zeta = zetaAB + zetaC;
                const double invZeta = 1.0 / zeta;
                const TlPosition G = (zetaA * A + zetaB * B + zetaC * C) * invZeta;
                const double G2 = G.squareDistanceFrom();
                const double kappa_exp = zeta * (G2 - (zetaA * A2 + zetaB * B2 + zetaC * C2) * invZeta);
                if (kappa_exp < DfOverlapEngine::MAX_EXPONENT) {
                    const double kappa = std::exp(kappa_exp);
                    
                    const double base = M_PI * invZeta;
                    const double sss = coefA * coefB * coefC * base * std::sqrt(base) * kappa;
    ...
    this->calc(this->a_bar_, this->b_bar_, this->c_bar_,
               this->a_, this->b_, this->c_);
                 ...
                 const int a, const int b, const int c)
{
    ...
    //                              a, b, c)
    //           << std::endl;
    if ((a_bar < 0) || (b_bar < 0) || (c_bar < 0) ||
        (a < 0) || (b < 0) || (c < 0)) {
        return;
    }
    
    const OvpState state(a_bar, b_bar, c_bar, a, b, c);
    const int stateIndex = state.index();
    ...
    if (a_bar > 0) {
        this->calc(a_bar-1, b_bar, c_bar, a+1, b,   c);
        this->calc(a_bar-1, b_bar, c_bar, a-1, b,   c);
        this->gradA(state);
    } else if (b_bar > 0) {
        this->calc(a_bar, b_bar-1, c_bar, a, b+1,   c);
        this->calc(a_bar, b_bar-1, c_bar, a, b-1,   c);
        this->gradB(state);
    } else if (c_bar > 0) {
        this->calc(a_bar, b_bar, c_bar-1, a, b,   c+1);
        this->calc(a_bar, b_bar, c_bar-1, a, b,   c-1);
        this->gradC(state);
    } else if (a > 0) {
        this->calc(a_bar, b_bar, c_bar, a-1, b,   c);
        this->calc(a_bar, b_bar, c_bar, a-2, b,   c);
        this->calc(a_bar, b_bar, c_bar, a-1, b-1, c);
        this->calc(a_bar, b_bar, c_bar, a-1, b,   c-1);
        this->EQ20A(state);
    } else if (b > 0) {
        this->calc(a_bar, b_bar, c_bar, a,   b-1, c);
        this->calc(a_bar, b_bar, c_bar, a-1, b-2, c);
        this->calc(a_bar, b_bar, c_bar, a,   b-1, c);
        this->calc(a_bar, b_bar, c_bar, a,   b-1, c-1);
        this->EQ20B(state);
    } else if (c > 0) {
        this->calc(a_bar, b_bar, c_bar, a,   b,   c-1);
        this->calc(a_bar, b_bar, c_bar, a-1, b-1, c);
        this->calc(a_bar, b_bar, c_bar, a,   b-1, c);
        this->calc(a_bar, b_bar, c_bar, a,   b-1, c-2);
        this->EQ20C(state);
    } else {
                ...
                for (int index_a = 0; index_a < amvsA.size(); ++index_a) {
                    ...
                    for (int index_b = 0; index_b < amvsB.size(); ++index_b) {
                        ...
                        for (int index_c = 0; index_c < amvsC.size(); ++index_c) {
                            ...
                            const int stateAmvsIndex = this->index(amvAbar, amvBbar, amvCbar,
                                                                   amvA, amvB, amvC);
...
void DfOverlapEngine::transform6Dto5D()
{
    ...
    if (a == 2) {
        this->transform6Dto5D_i(I_, J_, K_, J, K, this->WORK, pBuf);
        I = 5;
        const std::size_t end = I_ * J_ * K_ * I * J * K;
        std::copy(pBuf, pBuf + end, this->WORK);
    }
    if (b == 2) {
        this->transform6Dto5D_j(I_, J_, K_, I, K, this->WORK, pBuf);
        J = 5;
        const std::size_t end = I_ * J_ * K_ * I * J * K;
        std::copy(pBuf, pBuf + end, this->WORK);
    }
    if (c == 2) {
        this->transform6Dto5D_k(I_, J_, K_, I, J, this->WORK, pBuf);
        K = 5;
        const std::size_t end = I_ * J_ * K_ * I * J * K;
        std::copy(pBuf, pBuf + end, this->WORK);
    }
...
void DfOverlapEngine::transform6Dto5D_i(const int I_, const int J_, const int K_,
    ...
    for (int i_ = 0; i_ < I_; ++i_) {
        for (int j_ = 0; j_ < J_; ++j_) {
            for (int k_ = 0; k_ < K_; ++k_) {
                ...
                for (int j = 0; j < J; ++j) {
                    for (int k = 0; k < K; ++k) {
                        const double xx = pInput[((((i_*J_ +j_)*K_ +k_)*6 +0)*J +j)*K +k];
                        const double xy = pInput[((((i_*J_ +j_)*K_ +k_)*6 +1)*J +j)*K +k];
                        const double xz = pInput[((((i_*J_ +j_)*K_ +k_)*6 +2)*J +j)*K +k];
                        const double yy = pInput[((((i_*J_ +j_)*K_ +k_)*6 +3)*J +j)*K +k];
                        const double yz = pInput[((((i_*J_ +j_)*K_ +k_)*6 +4)*J +j)*K +k];
                        const double zz = pInput[((((i_*J_ +j_)*K_ +k_)*6 +5)*J +j)*K +k];

                        const int xy_5d   = ((((i_*J_ +j_)*K_ +k_)*5 +0)*J +j)*K +k;
                        const int xz_5d   = ((((i_*J_ +j_)*K_ +k_)*5 +1)*J +j)*K +k;
                        const int yz_5d   = ((((i_*J_ +j_)*K_ +k_)*5 +2)*J +j)*K +k;
                        const int xxyy_5d = ((((i_*J_ +j_)*K_ +k_)*5 +3)*J +j)*K +k;
                        const int rr_5d   = ((((i_*J_ +j_)*K_ +k_)*5 +4)*J +j)*K +k;
                        pOutput[xy_5d] = xy;
                        pOutput[xz_5d] = xz;
                        pOutput[yz_5d] = yz;
...
void DfOverlapEngine::transform6Dto5D_j(const int I_, const int J_, const int K_,
            ...
            for (int k_ = 0; k_ < K_; ++k_) {
                ...
                for (int i = 0; i < I; ++i) {
                    for (int k = 0; k < K; ++k) {
                        ...
                        const double xx = pInput[((((i_*J_ +j_)*K_ +k_)*I +i)*6 +0)*K +k];
                        const double xy = pInput[((((i_*J_ +j_)*K_ +k_)*I +i)*6 +1)*K +k];
                        const double xz = pInput[((((i_*J_ +j_)*K_ +k_)*I +i)*6 +2)*K +k];
                        const double yy = pInput[((((i_*J_ +j_)*K_ +k_)*I +i)*6 +3)*K +k];
                        const double yz = pInput[((((i_*J_ +j_)*K_ +k_)*I +i)*6 +4)*K +k];
                        const double zz = pInput[((((i_*J_ +j_)*K_ +k_)*I +i)*6 +5)*K +k];
                        
                        const int xy_5d   = ((((i_*J_ +j_)*K_ +k_)*I +i)*5 +0)*K +k;
                        const int xz_5d   = ((((i_*J_ +j_)*K_ +k_)*I +i)*5 +1)*K +k;
                        const int yz_5d   = ((((i_*J_ +j_)*K_ +k_)*I +i)*5 +2)*K +k;
                        const int xxyy_5d = ((((i_*J_ +j_)*K_ +k_)*I +i)*5 +3)*K +k;
                        const int rr_5d   = ((((i_*J_ +j_)*K_ +k_)*I +i)*5 +4)*K +k;
                        pOutput[xy_5d] = xy;
                        pOutput[xz_5d] = xz;
                        pOutput[yz_5d] = yz;
                        pOutput[xxyy_5d] = 0.5*(xx-yy);
...
void DfOverlapEngine::transform6Dto5D_k(const int I_, const int J_, const int K_,
    ...
    for (int i_ = 0; i_ < I_; ++i_) {
        for (int j_ = 0; j_ < J_; ++j_) {
            for (int k_ = 0; k_ < K_; ++k_) {
                ...
                for (int i = 0; i < I; ++i) {
                    for (int j = 0; j < J; ++j) {
                        const double xx = pInput[((((i_*J_ +j_)*K_ +k_)*I +i)*J +j)*6 +0];
                        const double xy = pInput[((((i_*J_ +j_)*K_ +k_)*I +i)*J +j)*6 +1];
                        const double xz = pInput[((((i_*J_ +j_)*K_ +k_)*I +i)*J +j)*6 +2];
                        const double yy = pInput[((((i_*J_ +j_)*K_ +k_)*I +i)*J +j)*6 +3];
                        const double yz = pInput[((((i_*J_ +j_)*K_ +k_)*I +i)*J +j)*6 +4];
                        const double zz = pInput[((((i_*J_ +j_)*K_ +k_)*I +i)*J +j)*6 +5];
                        
                        const int xy_5d   = ((((i_*J_ +j_)*K_ +k_)*I +i)*J +j)*5 +0;
                        const int xz_5d   = ((((i_*J_ +j_)*K_ +k_)*I +i)*J +j)*5 +1;
                        const int yz_5d   = ((((i_*J_ +j_)*K_ +k_)*I +i)*J +j)*5 +2;
                        const int xxyy_5d = ((((i_*J_ +j_)*K_ +k_)*I +i)*J +j)*5 +3;
                        const int rr_5d   = ((((i_*J_ +j_)*K_ +k_)*I +i)*J +j)*5 +4;
                        pOutput[xy_5d] = xy;
                        pOutput[xz_5d] = xz;
                        pOutput[yz_5d] = yz;
                        pOutput[xxyy_5d] = 0.5*(xx-yy);
...
void DfOverlapEngine::EQ20A(const OvpState& state)
{
    ...
    this->OVP_[stateIndex].resize(numOfAmvsAbar * numOfAmvsBbar * numOfAmvsCbar
                                  * numOfAmvsA * numOfAmvsB * numOfAmvsC);
                ...
                for (int amvA_index = 0; amvA_index < numOfAmvsA; ++amvA_index) {
                    ...
                    for (int amvB_index = 0; amvB_index < numOfAmvsB; ++amvB_index) {
                        ...
                        for (int amvC_index = 0; amvC_index < numOfAmvsC; ++amvC_index) {
                            ...
                            // batch -------------------------------------------------------
                            const OvpState state1(aBar, bBar, cBar, a-1, b, c);
                            const std::size_t state1_index = state1.index();
                            const int index1 = this->index(amvAbar, amvBbar, amvCbar, amvA1, amvB, amvC);
                            
                            std::size_t state2_index = 0;
                            int index2 = 0;
                            if (amvA2.isExist() == true) {
                                const OvpState state2(aBar, bBar, cBar, a-2, b, c);
                                state2_index = state2.index();
                                index2 = this->index(amvAbar, amvBbar, amvCbar, amvA2, amvB, amvC);
                            }
                            
                            std::size_t state3_index = 0;
                            int index3 = 0;
                            if (amvB1.isExist() == true) {
                                const OvpState state3(aBar, bBar, cBar, a-1, b-1, c);
                                state3_index = state3.index();
                                index3 = this->index(amvAbar, amvBbar, amvCbar, amvA1, amvB1, amvC);
                            }
                            
                            std::size_t state4_index = 0;
                            int index4 = 0;
                            if (amvC1.isExist() == true) {
                                const OvpState state4(aBar, bBar, cBar, a-1, b, c-1);
                                state4_index = state4.index();
                                index4 = this->index(amvAbar, amvBbar, amvCbar, amvA1, amvB, amvC1);
                            }
                            
                            const int index = this->index(amvAbar, amvBbar, amvCbar, amvA, amvB, amvC);
                            this->OVP_[stateIndex][index].resize(numOfBatches);
                ...
                for (int amvB_index = 0; amvB_index < numOfAmvsB; ++amvB_index) {
                    ...
                    for (int amvA_index = 0; amvA_index < numOfAmvsA; ++amvA_index) {
                        ...
                        for (int amvC_index = 0; amvC_index < numOfAmvsC; ++amvC_index) {
                            ...
                            // batch -------------------------------------------------------
                            const OvpState state1(aBar, bBar, cBar, a, b-1, c);
                            const std::size_t state1_index = state1.index();
                            const int index1 = this->index(amvAbar, amvBbar, amvCbar, amvA, amvB1, amvC);
                            
                            std::size_t state2_index = 0;
                            int index2 = 0;
                            if (amvA1.isExist() == true) {
                                const OvpState state2(aBar, bBar, cBar, a-1, b-1, c);
                                state2_index = state2.index();
                                index2 = this->index(amvAbar, amvBbar, amvCbar, amvA1, amvB1, amvC);
                            }
                            
                            std::size_t state3_index = 0;
                            int index3 = 0;
                            if (amvB2.isExist() == true) {
                                const OvpState state3(aBar, bBar, cBar, a, b-2, c);
                                state3_index = state3.index();
                                index3 = this->index(amvAbar, amvBbar, amvCbar, amvA, amvB2, amvC);
                            }
                            
                            std::size_t state4_index = 0;
                            int index4 = 0;
                            if (amvC1.isExist() == true) {
                                const OvpState state4(aBar, bBar, cBar, a, b-1, c -1);
                                state4_index = state4.index();
                                index4 = this->index(amvAbar, amvBbar, amvCbar, amvA, amvB1, amvC1);
                            }
                ...
                for (int amvC_index = 0; amvC_index < numOfAmvsC; ++amvC_index) {
                    ...
                    for (int amvA_index = 0; amvA_index < numOfAmvsA; ++amvA_index) {
                        ...
                        for (int amvB_index = 0; amvB_index < numOfAmvsB; ++amvB_index) {
                            ...
                            // batch -------------------------------------------------------
                            const OvpState state1(aBar, bBar, cBar, a, b, c-1);
                            std::size_t state1_index = state1.index();
                            const int index1 = this->index(amvAbar, amvBbar, amvCbar, amvA, amvB, amvC1);
                            
                            std::size_t state2_index = 0;
                            int index2 = 0;
                            if (amvA1.isExist() == true) {
                                const OvpState state2(aBar, bBar, cBar, a-1, b, c-1);
                                state2_index = state2.index();
                                index2 = this->index(amvAbar, amvBbar, amvCbar, amvA1, amvB, amvC1);
                            }
                            
                            std::size_t state3_index = 0;
                            int index3 = 0;
                            if (amvB1.isExist() == true) {
                                const OvpState state3(aBar, bBar, cBar, a, b-1, c-1);
                                state3_index = state3.index();
                                index3 = this->index(amvAbar, amvBbar, amvCbar, amvA, amvB1, amvC1);
                            }
                            
                            std::size_t state4_index = 0;
                            int index4 = 0;
                            if (amvC2.isExist() == true) {
                                const OvpState state4(aBar, bBar, cBar, a, b, c -2);
                                state4_index = state4.index();
                                index4 = this->index(amvAbar, amvBbar, amvCbar, amvA, amvB, amvC2);
                            }
                ...
                for (int amvA_index = 0; amvA_index < numOfAmvsA; ++amvA_index) {
                    ...
                    for (int amvB_index = 0; amvB_index < numOfAmvsB; ++amvB_index) {
                        ...
                        for (int amvC_index = 0; amvC_index < numOfAmvsC; ++amvC_index) {
                            ...
                            // batch -------------------------------------------------------
                            const OvpState state1(aBar-1, bBar, cBar, a+1, b, c);
                            const std::size_t state1_index = state1.index();
                            const int index1 = this->index(amvAbar1, amvBbar, amvCbar, amvA1p, amvB, amvC);

                            std::size_t state2_index = 0;
                            int index2 = 0;
                            if (amvA1.isExist() == true) {
                                const OvpState state2(aBar-1, bBar, cBar, a-1, b, c);
                                state2_index = state2.index();
                                index2 = this->index(amvAbar1, amvBbar, amvCbar, amvA1, amvB, amvC);
                            }
                ...
                for (int amvA_index = 0; amvA_index < numOfAmvsA; ++amvA_index) {
                    ...
                    for (int amvB_index = 0; amvB_index < numOfAmvsB; ++amvB_index) {
                        ...
                        for (int amvC_index = 0; amvC_index < numOfAmvsC; ++amvC_index) {
                            ...
                            // batch -------------------------------------------------------
                            const OvpState state1(aBar, bBar-1, cBar, a, b+1, c);
                            const std::size_t state1_index = state1.index();
                            const int index1 = this->index(amvAbar, amvBbar1, amvCbar, amvA, amvB1p, amvC);

                            std::size_t state2_index = 0;
                            int index2 = 0;
                            if (amvB1.isExist() == true) {
                                const OvpState state2(aBar, bBar-1, cBar, a, b-1, c);
                                state2_index = state2.index();
                                index2 = this->index(amvAbar, amvBbar1, amvCbar, amvA, amvB1, amvC);
                            }
                ...
                for (int amvA_index = 0; amvA_index < numOfAmvsA; ++amvA_index) {
                    ...
                    for (int amvB_index = 0; amvB_index < numOfAmvsB; ++amvB_index) {
                        ...
                        for (int amvC_index = 0; amvC_index < numOfAmvsC; ++amvC_index) {
                            ...
                            // batch -------------------------------------------------------
                            const OvpState state1(aBar, bBar, cBar-1, a, b, c+1);
                            const std::size_t state1_index = state1.index();
                            const int index1 = this->index(amvAbar, amvBbar, amvCbar1, amvA, amvB, amvC1p);

                            std::size_t state2_index = 0;
                            int index2 = 0;
                            if (amvC1.isExist() == true) {
                                const OvpState state2(aBar, bBar, cBar-1, a, b, c-1);
                                state2_index = state2.index();
                                index2 = this->index(amvAbar, amvBbar, amvCbar1, amvA, amvB, amvC1);
                            }
