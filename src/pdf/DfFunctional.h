...
struct FunctionalSets {
    FunctionalSets(int numOfTerms, TlMatrix::index_type dim)
        : FA_termR(numOfTerms, dim), FA_termX(numOfTerms, dim),
          FB_termR(numOfTerms, dim), FB_termX(numOfTerms, dim) {
    };
...
struct DerivativeFunctionalSets {
    DerivativeFunctionalSets(int numOfTerms, TlMatrix::index_type dim)
        : rFrRhoA_R(numOfTerms, dim), rFrRhoA_X(numOfTerms, dim),
          rFrRhoB_R(numOfTerms, dim), rFrRhoB_X(numOfTerms, dim),
          rFrGAA_R(numOfTerms, dim),  rFrGAA_X(numOfTerms, dim),
          rFrGAB_R(numOfTerms, dim),  rFrGAB_X(numOfTerms, dim), 
          rFrGBB_R(numOfTerms, dim),  rFrGBB_X(numOfTerms, dim) {
    };
...
public:
    ...
    // for Grid-Free ===================================================
public:
    FunctionalSets getFunctional_GF(const TlVector& rhoAs, 
                                    const TlVector& rhoBs);

    DerivativeFunctionalSets getDerivativeFunctional_GF(const TlVector& rhoAs, 
                                                        const TlVector& rhoBs);

    int getNumOfFunctionalTerms() const {
        return this->numOfFunctionalTerms_;
    }
