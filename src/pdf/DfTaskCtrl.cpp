                           ...
                           bool initialize)
{
    ...
    if (initialize == true) {
        ...
        if (isCutoffByDistribution == true) {
            dct = this->makeDistributedCutoffTable(orbitalInfo1,
                                                   orbitalInfo2);

            // setup progress status
            progress = 0;
            progress_all = this->getTotalCalcAmount2(orbitalInfo1,
                                                     orbitalInfo2,
                                                     shellArrayTable1,
                                                     shellArrayTable2,
                                                     dct);
                    ...
                    for ( ; shellTypeS >= 0; ) {
                        ...
                        for ( ; shellArrayIndexQ < shellArraySizeQ; ) {
                            ...
                            for ( ; shellArrayIndexS < shellArraySizeS; ) {
                                ...
                                // schwarz cutoff
                                const int shellQuartetType = this->getShellQuartetType(orbitalInfo, orbitalInfo,
                           ...
                           bool initialize)
{
    ...
    if (initialize == true) {
        ...
        //                                                    shellArrayTableRS);
        {
            ...
            for (int shellTypeP = maxShellType_PQ -1; shellTypeP >= 0; --shellTypeP) {
                const ShellArray& shellArrayP = shellArrayTable_PQ[shellTypeP];
                ShellArray::const_iterator pItEnd = shellArrayP.end();

                for (int shellTypeR = maxShellType_RS -1; shellTypeR >= 0; --shellTypeR) {
        ...
        // setup progress status
        progress = 0;
        progress_all = this->getTotalCalcAmount4(orbitalInfo_PQ,
                                                 orbitalInfo_RS,
                                                 shellPairArrayTable,
                                                 dct_PQ, dct_RS);
        progress_stage = 0;
                    ...
                    for ( ; shellTypeS >= 0; ) {
                        ...
                        for ( ; shellArrayIndexQ < shellArraySizeQ; ) {
                            ...
                            for ( ; shellArrayIndexS < shellArraySizeS; ) {
                                ...
                                // schwarz cutoff
                                const int shellQuartetType = this->getShellQuartetType(orbitalInfo_PQ,
                                                                                       orbitalInfo_RS,
                                                                                       shellTypeP, shellTypeQ,
                                                                                       shellTypeR, shellTypeS);
                                const bool isAlive = this->isAliveBySchwarzCutoff(shellIndexP, shellIndexQ,
...
// eq.32
DfTaskCtrl::DistributedCutoffTable
DfTaskCtrl::makeDistributedCutoffTable(const TlOrbitalInfoObject& orbitalInfo1,
                                       const TlOrbitalInfoObject& orbitalInfo2)
{
    static const double INV_EQ32_COEF = 1.0 / (std::pow(2.0 * TlMath::PI(), 0.25) * TlMath::PI());
    const double threshold = this->cutoffEpsilon_distribution_ * INV_EQ32_COEF;
    ...
    const std::vector<index_type> orbList2 = orbitalInfo2.getStartIndexArrayOfShellGroup();
...
DfTaskCtrl::ShellPairArrayTable DfTaskCtrl::getShellPairArrayTable(const TlOrbitalInfoObject& orbitalInfo1,
    ...
    const TlOrbitalInfoObject& orbitalInfo2)
{
    ...
    for (int shellPairType = 0; shellPairType < maxShellPairType; ++shellPairType) {
        ...
        for (std::size_t shellPairIndex = 0; shellPairIndex < shellPairArraySize; ++shellPairIndex) {
            ...
            const int numOfContractionsA = orbitalInfo1.getCgtoContraction(shellIndexA);
            const TlPosition posB = orbitalInfo2.getPosition(shellIndexB);
            const int numOfContractionsB = orbitalInfo2.getCgtoContraction(shellIndexB);
            const double AB2 = posB.squareDistanceFrom(posA);
        
            double judge = 0.0;
            for (int pgtoIndexA = 0; pgtoIndexA < numOfContractionsA; ++pgtoIndexA) {
                const double coefA = orbitalInfo1.getCoefficient(shellIndexA, pgtoIndexA);
                const double zetaA = orbitalInfo1.getExponent(shellIndexA, pgtoIndexA);
                
                for (int pgtoIndexB = 0; pgtoIndexB < numOfContractionsB; ++pgtoIndexB) {
                    const double coefB = orbitalInfo2.getCoefficient(shellIndexB, pgtoIndexB);
                    const double zetaB = orbitalInfo2.getExponent(shellIndexB, pgtoIndexB);
...
//         ((shellTypeP * maxShellType + shellTypeQ) * maxShellType + shellTypeR) * maxShellType + shellTypeS;
                                    ...
                                    const int shellTypeS)
{
    ...
    // schwarz cutoff
    const int shellQuartetType =
        ((shellTypeP * maxShellType_PQ + shellTypeQ) * maxShellType_PQ + shellTypeR) * maxShellType_RS + shellTypeS;
...
std::size_t DfTaskCtrl::getTotalCalcAmount2(const TlOrbitalInfoObject& orbitalInfo1,
                                            const TlOrbitalInfoObject& orbitalInfo2,
                                            const ShellArrayTable& shellArrayTable1,
                                            const ShellArrayTable& shellArrayTable2)
{
...
std::size_t DfTaskCtrl::getTotalCalcAmount4(const TlOrbitalInfoObject& orbitalInfo_PQ,
                                            const TlOrbitalInfoObject& orbitalInfo_RS,
                                            const ShellPairArrayTable& shellPairArrayTable,
        ...
        for (int shellTypeR = 0 ; shellTypeR < maxShellType_RS; ++shellTypeR) {
            ...
            for (std::size_t prIndex = 0; prIndex < numOfShellPairArray_PR; ++prIndex) {
                ...
                for (int shellTypeQ = 0; shellTypeQ < maxShellType_PQ; ++shellTypeQ) {
                    ...
                    //const ShellArray shellArrayQ = dct[shellIndexP][shellTypeQ];
                    DistributedCutoffTable::const_iterator dctItP = dct_PQ.find(shellIndexP);
                    ...
                    for (int shellTypeS = 0; shellTypeS < maxShellType_RS; ++shellTypeS) {
                        ...
                        //const ShellArray shellArrayS = dct[shellIndexR][shellTypeS];
                        DistributedCutoffTable::const_iterator dctItR = dct_RS.find(shellIndexR);
