...
protected:
    ...
    template<class SymmetricMatrixType>
    void checkMatrix(const RUN_TYPE runType, int iteration,
                     double* pRMS, double* pMax);

    /// convergence check in DensityMatrix
    template<class SymmetricMatrixType>
    void checkDensityMatrix(const RUN_TYPE runType, int iteration,
                            double* pRMS, double* pMax);
    
    /// convergence check in Fock
    template<class SymmetricMatrixType>
    void checkFockMatrix(const RUN_TYPE runType, int iteration,
                         double* pRMS, double* pMax);
    ...
    case METHOD_RKS:
        {
            ...
            this->checkMatrix<SymmetricMatrixType>(RUN_RKS, iteration,
                                                   &rmsMatrix, &maxMatrix);
                              ...
                              double* pRMS, double *pMax)
{
    ...
    case CONV_TARGET_DENSITY:
        this->checkDensityMatrix<SymmetricMatrixType>(runType, iteration,
                                                      pRMS, pMax);
        break;

    case CONV_TARGET_FOCK: 
        this->checkFockMatrix<SymmetricMatrixType>(runType, iteration,
                                                   pRMS, pMax);
        break;
...
template<class SymmetricMatrixType>
void DfConvcheck::checkDensityMatrix(const RUN_TYPE runType, const int iteration,
                                     double* pRMS, double *pMax)
{
    assert(iteration > 1);
    assert(pRMS != NULL);
    ...
    SymmetricMatrixType deltaP = DfObject::getPpqMatrix<SymmetricMatrixType>(runType, iteration);
                                  ...
                                  double* pRMS, double* pMax)
{
    ...
    SymmetricMatrixType deltaF = DfObject::getFpqMatrix<SymmetricMatrixType>(runType, iteration);
