                  ...
                  const std::vector<TlAtom>& Xs)
{
    ...
    this->addResultsToOutputBuffer<KinState, KinDataType>(this->KIN_, this->WORK_KIN);
    ...
    for (int i = 0; i < numOfDummyAtoms; ++i) {
        ...
        this->calcNucSS();
        this->calcNuc(this->a_bar_, this->b_bar_, this->a_, this->b_, 0);
        this->addResultsToOutputBuffer<NucState, NucDataType>(this->NUC_, this->WORK_NUCX);
    }
...
void DfHpqEngine::calcNuclearAttractionPart(const Query& query,
                                            const TlPosition& A,
                                            const TlPosition& B,
                                            const PGTOs& PGTOsA,
                                            const PGTOs& PGTOsB,
...
int DfHpqEngine::index(const TlAngularMomentumVector& amvAbar,
    ...
    const int index = ((amvAbar.index()*bBars +amvBbar.index())*as +amvA.index())*bs +amvB.index();
    return index;
}
...
void DfHpqEngine::transform6Dto5D_i(const int I_, const int J_,
                                    const int J,
...
void DfHpqEngine::calcOvp(const int a, const int b)
{
    ...
    if (a > 0) {
        ...
        std::cerr << TlUtils::format("Program Error: @DfHpqEngine::calcOvp() a=%d, b=%d",
                                     a, b)
                  << std::endl;
        abort();
    }
...
void DfHpqEngine::calcKin(const int a_bar, const int b_bar,
                     const int a, const int b)
{
    ...
    if (a_bar > 0) { 
        ...
        std::cerr << TlUtils::format("Program Error: @DfHpqEngine::calcKin() a=%d, b=%d",
                                     a, b)
                  << std::endl;
        abort();
    }
                          ...
                          const int a, const int b, const int m)
{
    ...
    if (a_bar > 0) {
        ...
        std::cerr << TlUtils::format("Program Error: @DfHpqEngine::calcNuc() a=%d, b=%d, m=%d",
                                     a, b, m)
                  << std::endl;
        abort();
    }
...
//         std::cerr << TlUtils::format("OvpSS = % f", value) << std::endl;
    }
    this->isCalcdOvp_[stateIndex] = true;
}
...
void DfHpqEngine::calcKinSS()
{
    ...
    for (int batch = 0; batch < numOfBatches; ++batch) {
        ...
        this->KIN_[stateIndex][index][batch] = coef * this->OVP_[ovpStateIndex][ovpIndex][batch];
    }
...
void DfHpqEngine::calcNucSS()
{
    ...
    const int sumOfAngularMomentums = this->a_bar_ + this->b_bar_ + this->a_ + this->b_;
    ...
    for (int batch = 0; batch < numOfBatches; ++batch) {
        ...
        const double ovpSS = this->OVP_[ovpStateIndex][ovpIndex][batch]; // 軌道係数はここに含まれる
...
//         std::cerr << TlUtils::format("calcNucSS() coef=% f, charge=% f, sqrt_zeta=% f, ovp=% f",
//                                      coef, charge, sqrt_zeta, ovpSS)
//                   << std::endl;
//         std::cerr << TlUtils::format("calcNucSS() zeta=% f, P(% f, %f, %f) C(% f, % f, % f)",
//                                      zeta, P.x(), P.y(), P.z(), this->C_.x(), this->C_.y(), this->C_.z())
//                   << std::endl;
        for (int m = 0; m <= sumOfAngularMomentums; ++m) {
            const std::size_t stateIndex = stateIndexPool[m];
            const double value = coef * fmtBuf[m];
//             std::cerr << TlUtils::format("calcNucSS() value=% f, m=%d, coef=% f, fmt=% f",
//                                          value, m, coef, fmtBuf[m])
//                       << std::endl;
    ...
    for (int amvA_index = 0; amvA_index < numOfAmvsA; ++amvA_index) {
        ...
        for (int amvB_index = 0; amvB_index < numOfAmvsB; ++amvB_index) {
            ...
            for (int batch = 0; batch < numOfBatches; ++batch) {
                ...
                // 1st term
                {
                    answer += (this->P_[batch][i] - this->A_[i]) * this->OVP_[state1_index][index1][batch];
                }
                
                // 2nd term
                if (amvA2.isExist() == true) {
                    answer += invZeta2 * amvA1.get(i) * this->OVP_[state2_index][index2][batch];
                }
                
                // 3rd term
                if (amvB1.isExist() == true) {
                    answer += invZeta2 * amvB.get(i) * this->OVP_[state3_index][index3][batch];
                }
    ...
    for (int amvB_index = 0; amvB_index < numOfAmvsB; ++amvB_index) {
        ...
        for (int amvA_index = 0; amvA_index < numOfAmvsA; ++amvA_index) {
            ...
            for (int batch = 0; batch < numOfBatches; ++batch) {
                ...
                // 1st term
                {
                    answer += (this->P_[batch][i] - this->B_[i]) * this->OVP_[state1_index][index1][batch];
                }
                
                // 2nd term
                if (amvA1.isExist() == true) {
                    answer += invZeta2 * amvA.get(i) * this->OVP_[state2_index][index2][batch];
                }
                
                // 3rd term
                if (amvB2.isExist() == true) {
                    answer += invZeta2 * amvB1.get(i) * this->OVP_[state3_index][index3][batch];
                }
...
void DfHpqEngine::Kin_EqA12_A(const KinState& state)
{
    ...
    const int numOfAmvsA = amvsA.size();
    const int numOfAmvsB = amvsB.size();
    this->KIN_[stateIndex].resize(numOfAmvsAbar * numOfAmvsBbar * numOfAmvsA * numOfAmvsB);
        ...
        for (int amvBbar_index = 0; amvBbar_index < numOfAmvsBbar; ++amvBbar_index) {
            ...
            for (int amvA_index = 0; amvA_index < numOfAmvsA; ++amvA_index) {
                ...
                for (int amvB_index = 0; amvB_index < numOfAmvsB; ++amvB_index) {
                    ...
                    const std::size_t state1_index = state1.index();
                    const int index1 = this->index(amvAbar, amvBbar, amvA1, amvB);
                    ...
                    const int index4 = this->index(amvAbar, amvBbar, amvA, amvB);
                    ...
                    for (int batch = 0; batch < numOfBatches; ++batch) {
                        ...
                        // 1st term
                        {
                            answer += (this->P_[batch][i] - this->A_[i]) * this->KIN_[state1_index][index1][batch];
                        }
                        
                        // 2nd term
                        if (amvA2.isExist() == true) {
                            answer += invZeta2 * amvA1.get(i) * this->KIN_[state2_index][index2][batch];
                        }
                        
                        // 3rd term
                        if (amvB1.isExist() == true) {
                            answer += invZeta2 * amvB.get(i) * this->KIN_[state3_index][index3][batch];
                        }
                        
                        // 4th term
                        {
                            answer += 2.0 * eta * this->OVP_[state4_index][index4][batch];
                        }
                        
                        // 5th term
                        if (amvA2.isExist() == true) {
                            answer -= (eta / zetaA) * amvA1.get(i) * this->OVP_[state5_index][index5][batch];
                        }
        ...
        for (int amvBbar_index = 0; amvBbar_index < numOfAmvsBbar; ++amvBbar_index) {
            ...
            for (int amvB_index = 0; amvB_index < numOfAmvsB; ++amvB_index) {
                ...
                for (int amvA_index = 0; amvA_index < numOfAmvsA; ++amvA_index) {
                    ...
                    const std::size_t state1_index = state1.index();
                    const int index1 = this->index(amvAbar, amvBbar, amvA, amvB1);
                    ...
                    for (int batch = 0; batch < numOfBatches; ++batch) {
                        ...
                        // 1st term
                        {
                            answer += (this->P_[batch][i] - this->B_[i]) * this->KIN_[state1_index][index1][batch];
                        }
                        
                        // 2nd term
                        if (amvA1.isExist() == true) {
                            answer += invZeta2 * amvA.get(i) * this->KIN_[state2_index][index2][batch];
                        }
                        
                        // 3rd term
                        if (amvB2.isExist() == true) {
                            answer += invZeta2 * amvB1.get(i) * this->KIN_[state3_index][index3][batch];
                        }
                        ...
                        // 5th term
                        if (amvB2.isExist() == true) {
                            answer -= (eta / zetaB) * amvB1.get(i) * this->OVP_[state5_index][index5][batch];
                        }
...
void DfHpqEngine::Nuc_EqA19_A(const NucState& state)
{
    ...
    const int numOfAmvsA = amvsA.size();
    const int numOfAmvsB = amvsB.size();
    this->NUC_[stateIndex].resize(numOfAmvsAbar * numOfAmvsBbar * numOfAmvsA * numOfAmvsB);
            ...
            for (int amvA_index = 0; amvA_index < numOfAmvsA; ++amvA_index) {
                ...
                for (int amvB_index = 0; amvB_index < numOfAmvsB; ++amvB_index) {
                    ...
                    for (int batch = 0; batch < numOfBatches; ++batch) {
                        ...
                        // 1st term
                        {
                            const double term1 = (this->P_[batch][i] - this->A_[i]) * this->NUC_[state1_index][index1][batch];
                            answer += term1;
                        }
                        
                        // 2nd term
                        {
                            const double term2 = (this->P_[batch][i] - this->C_[i]) * this->NUC_[state2_index][index2][batch];
                            answer -= term2;
                        }
                        ...
                        if (amvA2.isExist() == true) {
                            ...
                            const double term3 = coef * (this->NUC_[state31_index][index3][batch] - this->NUC_[state32_index][index3][batch]);
                            answer += term3;
                        }
                        ...
                        if (amvB1.isExist() == true) {
                            ...
                            answer += coef * (this->NUC_[state41_index][index4][batch] - this->NUC_[state42_index][index4][batch]);
                        }
            ...
            for (int amvB_index = 0; amvB_index < numOfAmvsB; ++amvB_index) {
                ...
                for (int amvA_index = 0; amvA_index < numOfAmvsA; ++amvA_index) {
                    ...
                    for (int batch = 0; batch < numOfBatches; ++batch) {
                        ...
                        // 1st term
                        {
                            answer += (this->P_[batch][i] - this->B_[i]) * this->NUC_[state1_index][index1][batch];
                        }
                        
                        // 2nd term
                        {
                            answer -= (this->P_[batch][i] - this->C_[i]) * this->NUC_[state2_index][index2][batch];
                        }
                        
                        // 3rd term
                        if (amvA1.isExist() == true) {
                            const double coef = invZeta2 * amvA.get(i);
                            answer += coef * (this->NUC_[state31_index][index3][batch] - this->NUC_[state32_index][index3][batch]);
                        }
        ...
        for (int amvBbar_index = 0; amvBbar_index < numOfAmvsBbar; ++amvBbar_index) {
            ...
            for (int amvA_index = 0; amvA_index < numOfAmvsA; ++amvA_index) {
                ...
                for (int amvB_index = 0; amvB_index < numOfAmvsB; ++amvB_index) {
                    ...
                    const std::size_t state1_index = state1.index();
                    const int index1 = this->index(amvAbar1, amvBbar, amvA1p, amvB);
                    ...
                    for (int batch = 0; batch < numOfBatches; ++batch) {
                        ...
                        // 1st term
                        answer += 2.0 * this->zetaA_[batch] * this->KIN_[state1_index][index1][batch];
                        
                        // 2nd term
                        if (amvA1.isExist() == true) {
                            answer -= amvA.get(i) * this->KIN_[state2_index][index2][batch];
                        }
        ...
        for (int amvBbar_index = 0; amvBbar_index < numOfAmvsBbar; ++amvBbar_index) {
            ...
            for (int amvA_index = 0; amvA_index < numOfAmvsA; ++amvA_index) {
                ...
                for (int amvB_index = 0; amvB_index < numOfAmvsB; ++amvB_index) {
                    ...
                    const std::size_t state1_index = state1.index();
                    const int index1 = this->index(amvAbar, amvBbar1, amvA, amvB1p);
                    ...
                    for (int batch = 0; batch < numOfBatches; ++batch) {
                        ...
                        // 1st term
                        answer += 2.0 * this->zetaB_[batch] * this->KIN_[state1_index][index1][batch];
                        
                        // 2nd term
                        if (amvB1.isExist() == true) {
                            answer -= amvB.get(i) * this->KIN_[state2_index][index2][batch];
                        }
            ...
            for (int amvA_index = 0; amvA_index < numOfAmvsA; ++amvA_index) {
                ...
                for (int amvB_index = 0; amvB_index < numOfAmvsB; ++amvB_index) {
                    ...
                    for (int batch = 0; batch < numOfBatches; ++batch) {
                        ...
                        // 1st term
                        answer += 2.0 * this->zetaA_[batch] * this->NUC_[state1_index][index1][batch];
                        
                        // 2nd term
                        if (amvA1.isExist() == true) {
                            answer -= amvA.get(i) * this->NUC_[state2_index][index2][batch];
                        }
            ...
            for (int amvA_index = 0; amvA_index < numOfAmvsA; ++amvA_index) {
                ...
                for (int amvB_index = 0; amvB_index < numOfAmvsB; ++amvB_index) {
                    ...
                    for (int batch = 0; batch < numOfBatches; ++batch) {
                        ...
                        // 1st term
                        answer += 2.0 * this->zetaB_[batch] * this->NUC_[state1_index][index1][batch];
                        
                        // 2nd term
                        if (amvB1.isExist() == true) {
                            answer -= amvB.get(i) * this->NUC_[state2_index][index2][batch];
                        }
