...
TlDistributeVector::TlDistributeVector(const size_type nSize)
    : m_nContext(0), m_nRows(nSize), m_nCols(1), m_nBlockSize(TlDistributeVector::systemBlockSize_)
{
    this->initialize();
}

TlDistributeVector::TlDistributeVector(const TlDistributeVector& rhs)
...
TlDistributeVector::TlDistributeVector(const std::vector<double>& rhs,
                                       const int globalSize)
    : m_nContext(0), m_nRows(globalSize), m_nCols(1), m_nBlockSize(TlDistributeVector::systemBlockSize_)
{
    this->initialize();
...
TlDistributeVector::TlDistributeVector(const TlVector& rhs)
    : m_nContext(0), m_nRows(rhs.getSize()), m_nCols(1), m_nBlockSize(TlDistributeVector::systemBlockSize_)
{
    this->initialize();
...
TlDistributeVector::~TlDistributeVector()
{
}


void TlDistributeVector::setSystemBlockSize(int blockSize)
{
    TlDistributeVector::systemBlockSize_ = blockSize;
}


void TlDistributeVector::initialize()
{
    ...
    // 行方向のglobal_index v.s. local_indexのリストを作成
    {
        ...
        for (int r = 0; r < nMyRows; ++r) {
            ...
            this->m_RowIndexTable[r] = nBlockIndex +(nIncrementBlockIndex*d.quot) +d.rem;
        }
    }
    ...
    // 列方向のglobal_index v.s. local_indexのリストを作成
    {
        ...
        for (int c = 0; c < nMyCols; ++c) {
            ...
            this->m_ColIndexTable[c] = nBlockIndex +(nIncrementBlockIndex*d.quot) +d.rem;
        }
    }
}


void TlDistributeVector::resize(const size_type size)
{
    assert(size > 0);

    TlDistributeVector tmp = *this;
    this->m_nRows = size;
    this->m_nCols = 1;
    this->initialize();

    const std::size_t copySize = std::min(size, tmp.getSize());
...
TlDistributeVector& TlDistributeVector::operator=(const TlDistributeVector& rhs)
{
    if (&rhs != this) {
...
TlDistributeVector& TlDistributeVector::operator=(const TlVector& rhs)
{
...
TlDistributeVector::size_type TlDistributeVector::getSize() const
{
    return this->m_nRows;
}


int TlDistributeVector::getIndex(int nGlobalRow, int nGlobalCol) const
{
    int nAnswer = -1;
    ...
    int nLocalProcRow = 0;
    int nLocalProcCol = 0;
    infog2l_(&nGlobalRow, &nGlobalCol, this->m_pDESC,
             &(this->m_nProcGridRow), &(this->m_nProcGridCol),
             &(this->m_nMyProcRow), &(this->m_nMyProcCol),
             &nLocalRow, &nLocalCol, &nLocalProcRow, &nLocalProcCol);
...
void TlDistributeVector::set(const size_type i, const double dValue)
{
    const int index = this->getIndex(i, 0);
    if (index != -1) {
...
double TlDistributeVector::get(const size_type index) const
{
    ...
    pdelget_("A", " ", &dAnswer, &(dataTmp[0]), &nGlobalRow, &nGlobalCol, this->m_pDESC);
...
double& TlDistributeVector::operator[](const size_type index)
{
    const int nGlobalRow = index +1;
    const int nGlobalCol = 0 +1;
...
int TlDistributeVector::getProcIdForIndex(const index_type globalIndex) const
{
...
TlDistributeVector& TlDistributeVector::operator*=(const double dCoef)
{
    this->data_ *= dCoef;
...
TlDistributeVector operator+(const TlDistributeVector& X, const TlDistributeVector& Y)
{
    TlDistributeVector Z(X);
    Z += Y;
...
bool TlDistributeVector::load(const std::string& sFilePath)
{
    ...
    if (bIsFail) {
        std::cerr << "[error] TlDistributeVector::load(): could not open file. " << sFilePath << std::endl;
        abort();
    }
...
bool TlDistributeVector::load(std::ifstream& ifs)
{
    ...
    if (rComm.isMaster() == true) {
        ...
        std::vector<std::vector<index_type> > indexLists(numOfProcs, std::vector<index_type>());
        std::vector<std::vector<double> > valueLists(numOfProcs, std::vector<double>());
        ...
        while (isFinished == false) {
            ...
            // データを送信
            std::map<int, std::vector<index_type> >::const_iterator itEnd = tmpIndexLists.end();
            for (std::map<int, std::vector<index_type> >::const_iterator it = tmpIndexLists.begin(); it != itEnd; ++it) {
                const int proc = it->first;
                const int numOfContents = it->second.size();
                ...
                rComm.iSendDataX(&(indexLists[proc][0]), sizeLists[proc], proc, TAG_LOAD_ROWCOLS);
...
bool TlDistributeVector::save(const std::string& sFilePath) const
{
    TlCommunicate& rComm = TlCommunicate::getInstance();
...
bool TlDistributeVector::save(std::ofstream& ofs) const
{
    // this const_cast is requiered for PGI compiler
    // "error: expression must be an lvalue or a function designator"
    DataType& dataTmp = const_cast<DataType&>(this->data_);
    ...
    if (rComm.isMaster() == true) {
        ...
        for (int r = 0; r < nRows; ++r) {
            ...
            for (int c = 0; c < nCols; ++c) {
                ...
                ofs.write(reinterpret_cast<const char*>(&(dataTmp[index])), sizeof(double));
            }
        }
    }
...
std::vector<double> TlDistributeVector::getVector() const
{
    // this const_cast is requiered for PGI compiler
    // "error: expression must be an lvalue or a function designator"
    DataType& dataTmp = const_cast<DataType&>(this->data_);
...
double operator*(const TlDistributeVector& X, const TlDistributeVector& Y)
{
    // this const_cast is requiered for PGI compiler
    // "error: expression must be an lvalue or a function designator"
    TlDistributeVector& Xtmp = const_cast<TlDistributeVector&>(X);
    TlDistributeVector& Ytmp = const_cast<TlDistributeVector&>(Y);
...
TlDistributeVector operator*(const TlDistributeVector& X, const double& Y)
{
    TlDistributeVector Z(X);
    Z *= Y;
...
TlDistributeVector operator*(const double& Y, const TlDistributeVector& X)
{
    return (X * Y);
}
