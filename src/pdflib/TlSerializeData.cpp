...
void TlSerializeData::copyChildren(const TlSerializeData& rhs)
{
    ...
    if (rhs.map_.empty() != true) {
        for (MapContainerType::const_iterator p = rhs.map_.begin(); p != rhs.map_.end(); ++p) {
            ...
            this->map_.insert(std::pair<TlSerializeData*, TlSerializeData*>(pKey, pValue));
        }
    }
}
...
void TlSerializeData::set(const char* pStr)
{
    this->set(std::string(pStr));
}
...
void TlSerializeData::setAt(const std::size_t index, const TlSerializeData& value)
{
    this->type_ = ARRAY;
    if ((index +1) > this->array_.size()) {
        this->resize(index +1);
...
const TlSerializeData& TlSerializeData::operator[](const TlSerializeData& key) const
{
    if (this->getType() == MAP) {
...
TlSerializeData::MapContainerType::iterator TlSerializeData::find(const TlSerializeData& key)
{
...
TlSerializeData::MapContainerType::const_iterator TlSerializeData::find(const TlSerializeData& key) const
{
