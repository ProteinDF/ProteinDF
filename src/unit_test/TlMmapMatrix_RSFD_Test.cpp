...
TEST(TlMmapMatrix_RSFD, construct)
{
    const int row = 100;
    const int col = 200;
    {
        cleanup();
        TlMmapMatrix_RSFD m(matPath, row, col);
    }
...
TEST(TlMmapMatrix_RSFD, set_matrix)
{
    cleanup();
    const int row = 100;
    const int col = 200;
    ...
    {
        TlMmapMatrix_RSFD m(matPath, ref.getNumOfRows(), ref.getNumOfCols());
...
TEST(TlMmapMatrix_RSFD, get_matrix)
{
    cleanup();
    const int row = 200;
    const int col = 100;
