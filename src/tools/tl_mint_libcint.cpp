...
void TlMint_Libcint::calc_ovp(int shell_p, int shell_q) {
  ...
  if (0 != cint1e_ovlp_sph(&(buf[0]), shls, &(this->atomTable_[0]), numOfAtoms,
                           &(this->basissetTable_[0]), numOfAOs,
                           &(this->values_[0]))) {
    for (int q = 0; q < dj; ++q) {
      for (int p = 0; p < di; ++p) {
        std::cout << TlUtils::format("(%d %d) = % f", shell_p + p, shell_q + q,
                                     buf[p + q * dj])
                  << std::endl;
      }
    }
  ...
  if (0 != cint2e_sph(&(buf[0]), shls, &(this->atomTable_[0]), numOfAtoms,
    ...
    for (int s = 0; s < dl; ++s) {
      for (int r = 0; r < dk; ++r) {
        for (int q = 0; q < dj; ++q) {
          for (int p = 0; p < di; ++p) {
            // printf("(%d %d|%d %d) = % f\n", p, q, r, s, buf[((p*dj +q)*dk
            // +r)*dl +s]);
            std::cout << TlUtils::format("(%d %d|%d %d) = % f", shell_p + p,
...
void TlMint_Libcint::makeBasissetTable(const TlOrbitalInfo &orbInfo) {
  ...
  for (int aoIndex = 0; aoIndex < numOfAOs; ++aoIndex) {
    ...
    for (int pgtoIndex = 0; pgtoIndex < numOfPGTOs; ++pgtoIndex) {
        ...
        double coef2 = CINTgto_norm(orbInfo.getShellType(aoIndex), orbInfo.getExponent(aoIndex, pgtoIndex));
...
void TlMint_Libcint::showAtomTable() const {
  ...
  for (int i = 0; i < atomTableSize; i += 6) {
    ...
    std::cout << TlUtils::format("(% f, % f, % f)", this->values_[atom_offset],
