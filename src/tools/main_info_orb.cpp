...
void printOrbInfo(const TlOrbitalInfo& tlOrbInfo, std::size_t orb) {
    const TlPosition pos = tlOrbInfo.getPosition(orb);
    std::cout << TlUtils::format("%d th orbital information:", orb)
              << std::endl;
    std::cout
        << TlUtils::format(
               " atom: index=%d, symbol=%s, position(% 8.3f, % 8.3f, % 8.3f)",
               tlOrbInfo.getAtomIndex(orb), tlOrbInfo.getAtomName(orb).c_str(),
    ...
    std::cout << TlUtils::format("          contraction=%d", contractions)
              << std::endl;
...
void showHelp() {
    ...
    std::cout
        << " index:        Specify orbital indeces which begin from '0' to "
           "output."
        << std::endl;
    std::cout << "               Output all orbitals if no entry." << std::endl;
    std::cout
        << " -a:           Atom index mode: index means atom serial number."
        << std::endl;
    std::cout
        << " -p PDF_param: ProteinDF parameter file (default: pdfparam.mpac)"
        << std::endl;
        ...
        if (saveMpacPath.empty()) {
            ...
            for (int i = 0; i < numOfQueries; ++i) {
                ...
                if (orb < numOfAOs) {
                    ...
                    entry["shell_type"] = TlUtils::format(
                        "%c", shellTypes[orbInfo.getShellType(orb)]);
                    entry["basis_type_id"] = orbInfo.getBasisType(orb);
                    entry["basis_type"] = std::string(
                        angularMomentumTypes[orbInfo.getBasisType(orb)]);
