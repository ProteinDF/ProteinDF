...
void DfLocalize::makeJobList() {
  ...
  const std::size_t maxJobIndex =
      orbQATableSize * (orbQATableSize - 1) / 2;  // '-1'は対角項が必要無いため
  this->jobList_.resize(maxJobIndex);

  std::size_t index = 0;
  JobItem item;
  std::vector<Orb_QA_Item>::const_iterator itrEnd = this->orb_QA_table_.end();
  for (std::vector<Orb_QA_Item>::const_iterator p = this->orb_QA_table_.begin();
       p != itrEnd; ++p) {
    item.orb_i = p->orb;
