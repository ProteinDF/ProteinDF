...
double DfLocalize::localize(TlDenseGeneralMatrix_Lapack* pC) {
    ...
    while (this->getJobItem(&jobItem) != 0) {
        ...
        if (std::fabs(deltaG) > 1.0E-16) {
            ...
            // assert(std::fabs(det - 1.0) < 1.0E-5);
            this->rotateVectors(&Cpi, &Cpj, rot);

            // for (index_type ao = 0; ao < numOfAOs; ++ao) {
            //     pC->set(ao, orb_i, Cpi.get(ao));
            //     pC->set(ao, orb_j, Cpj.get(ao));
            // }
...
// void DfLocalize::localize(const std::string& inputCMatrixPath, const int restartIteration) {
...
//     std::cerr << TlUtils::format("start iteration=%d", startIteration + 1) << std::endl;
...
//             double A_ij, B_ij;
//             this->calcQA(this->C_, orb_i, orb_j, &A_ij, &B_ij);
...
#pragma omp parallel for schedule(runtime) reduction(+ : sumQAii2)
    for (index_type i = 0; i < size; ++i) {
        // #ifdef _OPENMP
        //         this->log_.info(TlUtils::format("maktQATable: %d/%d @%d", i, size, omp_get_thread_num()));
        // #endif  // _OPENMP

        const index_type orb = startOrb + i;
        const double QAii2 = this->calcQA_ii(C, orb);
        // std::cout << TlUtils::format("Q_Ai^2 = %16.8e", QAii2) << std::endl;
        sumQAii2 += QAii2;
...
// void DfLocalize::rotateCmatrix(TlDenseGeneralMatrix_Lapack* pC, std::size_t orb_i, std::size_t orb_j,
...
//         A.set(0, i, pC->get(i, orb_i));
//         A.set(1, i, pC->get(i, orb_j));
//     }

//     const TlDenseGeneralMatrix_Lapack B = rot * A;  // rotate A to form B

//     for (std::size_t i = 0; i < numOfAOs; ++i) {
//         pC->set(i, orb_i, B.get(0, i));
//         pC->set(i, orb_j, B.get(1, i));
//     }
// }
...
void DfLocalize::rotateVectors(TlDenseVector_Lapack* pCpi, TlDenseVector_Lapack* pCpj,
    ...
    //     A.set(0, i, pCpi->get(i));
    //     A.set(1, i, pCpj->get(i));
    // }
...
// void DfLocalize::calcQA_ij(const TlDenseGeneralMatrix_Lapack& C, const std::size_t orb_i, const std::size_t orb_j,
//                            double* pA_ij, double* pB_ij) {
//     assert(pA_ij != NULL);
//     assert(pB_ij != NULL);
//     assert(C.getNumOfRows() == this->m_nNumOfAOs);
//     assert(C.getNumOfCols() == this->m_nNumOfMOs);
...
//     C.block(0, orb_i, numOfAOs, 1, &Cpi);
//     TlDenseGeneralMatrix_Lapack Cpj(numOfAOs, 1);
//     C.block(0, orb_j, numOfAOs, 1, &Cpj);

//     const TlDenseGeneralMatrix_Lapack SCpi = this->S_ * Cpi;
//     const TlDenseGeneralMatrix_Lapack SCpj = this->S_ * Cpj;
...
//         const std::vector<index_type>& groupAOs = this->group_[grp];
//         const index_type numOfGroupAOs = groupAOs.size();
//         if (numOfGroupAOs == 0) {
//             continue;
//         }

//         TlDenseGeneralMatrix_Lapack tCqi(numOfAOs, 1);
//         TlDenseGeneralMatrix_Lapack tCqj(numOfAOs, 1);
//         for (index_type k = 0; k < numOfGroupAOs; ++k) {
//             const index_type ao = groupAOs[k];
//             tCqi.set(0, ao, Cpi.get(ao, 0));
//             tCqj.set(0, ao, Cpj.get(ao, 0));
//         }

//         const TlDenseGeneralMatrix_Lapack QAiiMat = tCqi * SCpi;
//         assert(QAiiMat.getNumOfRows() == 1);
//         assert(QAiiMat.getNumOfCols() == 1);
//         const double QAii = QAiiMat.get(0, 0);

//         const TlDenseGeneralMatrix_Lapack QAjjMat = tCqj * SCpj;
//         assert(QAjjMat.getNumOfRows() == 1);
//         assert(QAjjMat.getNumOfCols() == 1);
//         const double QAjj = QAjjMat.get(0, 0);

//         const TlDenseGeneralMatrix_Lapack QAijMat = tCqj * SCpi + tCqi * SCpj;
//         assert(QAijMat.getNumOfRows() == 1);
//         assert(QAijMat.getNumOfCols() == 1);
//         const double QAij = 0.5 * QAijMat.get(0, 0);
...
#pragma omp parallel for schedule(runtime) reduction(+ : sumAij, sumBij)
    for (index_type grp = 0; grp < numOfGrps; ++grp) {
        ...
        // TlDenseVector_Lapack tCqi(numOfAOs);
        // TlDenseVector_Lapack tCqj(numOfAOs);
        // for (index_type k = 0; k < numOfGroupAOs; ++k) {
        //     const index_type ao = groupAOs[k];
        //     assert(0 <= ao);
        //     assert(ao < numOfAOs);
        //     tCqi.set(ao, Cpi.get(ao));
        //     tCqj.set(ao, Cpj.get(ao));
        // }
        ...
        // const double QAij1 = tCqj * SCpi;
        // const double QAij2 = tCqi * SCpj;
        const double QAij = 0.5 * (tCqj * SCpi + tCqi * SCpj);

        const double QAii_QAjj = QAii - QAjj;
        // std::cout << TlUtils::format("QAii=%f, QAjj=%f, QAij=%f, QAij1=%f, QAij2=%f", QAii, QAjj, QAij, QAij1, QAij2)
        // << std::endl;
        sumAij += QAij * QAij - 0.25 * QAii_QAjj * QAii_QAjj;
        sumBij += QAij * QAii_QAjj;
    }
...
double DfLocalize::calcQA_ii(const TlDenseGeneralMatrix_Lapack& C, const index_type orb_i) {
    ...
    for (index_type grp = 0; grp < numOfGrps; ++grp) {
        ...
        // TlDenseGeneralMatrix_Lapack tCqi(1, numOfAOs);
        // for (index_type k = 0; k < numOfGroupAOs; ++k) {
        //     const index_type ao = groupAOs[k];
        //     tCqi.set(0, ao, Cpi.get(ao, 0));
        // }
...
void DfLocalize::makeJobList() {
    ...
    // for (std::vector<Orb_QA_Item>::const_iterator p = this->orb_QA_table_.begin(); p != itrEnd; ++p) {
    //     item.orb_i = p->orb;

    //     for (std::vector<Orb_QA_Item>::const_iterator q = p + 1; q != itrEnd; ++q) {
    //         item.orb_j = q->orb;
    //         this->jobList_.push_back(item);
    //         ++index;
    //     }
