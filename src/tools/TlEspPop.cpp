...
TlEspPop::TlEspPop(const TlSerializeData& param)
    : param_(param), flGeom_(param["coordinates"]),
      resp_restriction_(TlEspPop::REST_NONE),
      param_a_(0.0005), param_b_(0.1),
      itr_(1), maxItr_(100), maxErrorThreshold_(1.0E-3), rmsErrorThreshold_(1.0E-2),
      verbose_(false) {
    // initialize --------------------------------------------------------------
    // set real atoms
...
void TlEspPop::saveInvDesignMatrixPath(const std::string& path)
{
    this->saveInvDesignMatPath_ = path;
}
                    ...
                    std::string PMatrixFilePath)
{
    ...
    while (this->itr_ < this->maxItr_) {
        ...
        case REST_QUADRIC:
            this->makeDesignMatrix_quadric(&designMat, &predicted);
            break;
...
// radii: atomic unit
std::vector<TlPosition>
TlEspPop::getMKGridsOnAtom(const TlPosition& center, const double radii)
{
...
void TlEspPop::makeDesignMatrix_quadric(TlSymmetricMatrix* pDesignMat,
                                        TlVector* pPredicted)
{
    assert(pDesignMat != NULL);
    assert(pPredicted != NULL);
    ...
    for (int a = 0; a < numOfRealAtoms; ++a) {
        ...
        // a == b
        {
            TlVector r_a2 = r_a;
            r_a2.dot(r_a);
            pDesignMat->set(a, a, r_a2.sum() + 2.0 * param_a);
        }
...
void TlEspPop::makeDesignMatrix_hyperbolic(TlSymmetricMatrix* pDesignMat,
                                           TlVector* pPredicted)
{
    assert(pDesignMat != NULL);
    assert(pPredicted != NULL);
    ...
    for (int a = 0; a < numOfRealAtoms; ++a) {
        ...
        // a == b
        {
            ...
            const double ya = pPredicted->get(a);
            const double rest = this->param_a_ * (1.0 / std::sqrt(ya * ya + param_b2));
            pDesignMat->set(a, a, r_a2.sum() + rest);
        }
...
bool TlEspPop::convCheck(const TlVector& modelCoef)
{
    ...
    if (this->itr_ > 1) {
        ...
        std::cerr << TlUtils::format("#%3d: MAX: % e RMS: % e", this->itr_, maxErr, rmsErr)
                  << std::endl;
...
void TlEspPop::output(const TlVector& modelCoef)
{
    ...
    for (int i = 0; i < numOfRealAtoms; ++i) {
        ...
        totalCharge += charge;
        std::cout << TlUtils::format("[%4d] % 8.3f", i, charge)
                  << std::endl;
    }
