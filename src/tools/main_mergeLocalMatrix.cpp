...
void showHelp(const std::string& progname) {
  std::cout << TlUtils::format("USAGE: %s file... path", progname.c_str())
            << std::endl;
...
int main(int argc, char* argv[]) {
  ...
  for (std::vector<std::string>::const_iterator it = readMatrixPaths.begin();
       it != itEnd; ++it) {
    ...
    if (TlFile::isExist(readFilePath) == false) {
      std::cerr << TlUtils::format("not found: %s.", readFilePath.c_str())
                << std::endl;
      return EXIT_FAILURE;
    }
    ...
    switch (lmh.type) {
      ...
      default:
        // unknown
        std::cerr << TlUtils::format("unknown file type: %s.",
                                     readFilePath.c_str())
                  << std::endl;
        return EXIT_FAILURE;
        break;
    ...
    if (TlFile::isExist(outputMatrixPath) == false) {
      if (isVerbose == true) {
        std::cerr << TlUtils::format("create output matrix: %s.",
                                     outputMatrixPath.c_str())
                  << std::endl;
      }

      if (isSymmetricMatrix == true) {
        pOutputMatrix =
            new TlFileSymmetricMatrix(outputMatrixPath, lmh.globalRow);
      } else {
        pOutputMatrix =
            new TlFileMatrix(outputMatrixPath, lmh.globalRow, lmh.globalCol);
      }
    } else {
      if (isVerbose == true) {
        std::cerr << TlUtils::format("loading output matrix: %s.",
                                     outputMatrixPath.c_str())
                  << std::endl;
      }
    ...
    if ((pOutputMatrix->getNumOfRows() != lmh.globalRow) ||
      ...
      std::cerr
          << "ERROR: output matrix size is not consistent with input matrix."
          << std::endl;
      return EXIT_FAILURE;
    }
