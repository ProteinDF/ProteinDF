...
void calc_eri(DfEriEngine* pEngine, const int shellIndexP,
              const int shellIndexQ, const int shellIndexR,
              const int shellIndexS, const TlOrbitalInfo& orbitalInfo);

void output(int indexP, int indexQ, int indexR, int indexS,
            const TlOrbitalInfoObject& orbitalInfo, const double* WORK);
...
int main(int argc, char* argv[]) {
  ...
  if (opt.getCount() == 2) {
    ...
    for (int i = 0; i < size; ++i) {
      ...
      calc_eri(&engine, indexP, indexQ, indexR, indexS, orbitalInfo);
    }
    ...
    if (!(0 <= indexP) && (indexP < numOfAOs)) {
      std::cerr << TlUtils::format("illegal input parameter1. %d / %d", indexP,
                                   numOfAOs)
                << std::endl;
      std::exit(1);
    }
    if (!(0 <= indexQ) && (indexQ < numOfAOs)) {
      std::cerr << TlUtils::format("illegal input parameter2. %d / %d", indexQ,
                                   numOfAOs)
                << std::endl;
      std::exit(1);
    }
    if (!(0 <= indexR) && (indexR < numOfAOs)) {
      std::cerr << TlUtils::format("illegal input parameter3. %d / %d", indexR,
                                   numOfAOs)
                << std::endl;
      std::exit(1);
    }
    if (!(0 <= indexS) && (indexS < numOfAOs)) {
      std::cerr << TlUtils::format("illegal input parameter4. %d / %d", indexS,
                                   numOfAOs)
                << std::endl;
      std::exit(1);
    }
...
void calc_eri(DfEriEngine* pEngine, const int indexP, const int indexQ,
  ...
  output(indexP, indexQ, indexR, indexS, orbitalInfo, pEngine->WORK);
}
...
void output(int indexP, int indexQ, int indexR, int indexS,
      ...
      for (int j = 0; j < maxStepsQ; ++j) {
        ...
        for (int k = 0; k < maxStepsR; ++k) {
          ...
          for (int l = 0; l < maxStepsS; ++l) {
            ...
            std::cout << TlUtils::format("[%d %d|%d %d] -> % f", P, Q, R, S,
                                         value)
                      << std::endl;
            ++index;
          }
  ...
  const int index =
      ...
      basisTypeS;
  const double value = WORK[index];
  std::cout << TlUtils::format("(%2d %2d|%2d %2d)=%18.10f", indexP, indexQ,
                               indexR, indexS, value)
            << std::endl;
}
