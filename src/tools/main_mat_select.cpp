...
{
    std::cout << TlUtils::format("%s [options] input_path output_path", name.c_str()) << std::endl;
...
int main(int argc, char* argv[])
{
    ...
    if (TlMatrix::isLoadable(inputMatrixPath)) {
        ...
        std::cerr << TlUtils::format("cannot load: %s", inputMatrixPath.c_str()) << std::endl;
        return EXIT_FAILURE;
    }
    ...
    if (! opt["t"].empty()) {
        ...
        if ((bottom < 0) || (A.getNumOfRows() <= bottom)) {
            std::cerr << TlUtils::format("bottom: out of range(0 - %d): %d",
                                         A.getNumOfRows() -1, bottom) << std::endl;
            return EXIT_FAILURE;
        }
    }

    if (top >= bottom) {
        std::cerr << TlUtils::format("no left row lines: top: %d bottom: %d",
                                     top, bottom) << std::endl;
        return EXIT_FAILURE;
    }
    ...
    if (! opt["l"].empty()) {
        ...
        if ((left < 0) || (A.getNumOfCols() <= left)) {
            std::cerr << TlUtils::format("left: out of range(0 - %d): %d",
                                         A.getNumOfCols() -1, left) << std::endl;
            return EXIT_FAILURE;
        }
    }
    ...
    if (! opt["t"].empty()) {
        ...
        if ((right < 0) || (A.getNumOfCols() <= right)) {
            std::cerr << TlUtils::format("right: out of range(0 - %d): %d",
                                         A.getNumOfCols() -1, right) << std::endl;
            return EXIT_FAILURE;
        }
    }

    if (left >= right) {
        std::cerr << TlUtils::format("no left col lines: left: %d right: %d",
                                     left, right) << std::endl;
        return EXIT_FAILURE;
    }
