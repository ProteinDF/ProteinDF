    ...
    if (pMat != NULL) {
        if (pMat->load(filePath) == true) {
            ...
            if (fileType == FT_SYMMAT) {
                for (TlMatrixObject::index_type row = 0; row < numOfRows;
                     ++row) {
                    for (TlMatrixObject::index_type col = 0; col <= row;
                         ++col) {
                        const ElementInfo ei(row, col, pMat->get(row, col));
                        std::vector<ElementInfo>::iterator it;
                        if (abs_mode == true) {
                            it = std::lower_bound(answer.begin(), answer.end(),
                                                  ei, CompElementInfo());
                        } else {
                            it = std::lower_bound(answer.begin(), answer.end(),
                                                  ei, CompElementInfo_ABS());
                        }
                        answer.insert(it, ei);
                ...
                for (TlMatrixObject::index_type row = 0; row < numOfRows;
                     ++row) {
                    for (TlMatrixObject::index_type col = 0; col < numOfCols;
                         ++col) {
                        const ElementInfo ei(row, col, pMat->get(row, col));
                        std::vector<ElementInfo>::iterator it;
                        if (abs_mode == true) {
