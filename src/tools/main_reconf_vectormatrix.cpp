...
void showHelp(const std::string& progname) {
    std::cout << TlUtils::format(
                     "%s [options] INPUT_BASENAME OUTPUT_BASENAME NUM_SUBUNITS",
                     progname.c_str())
              << std::endl;
...
int main(int argc, char* argv[]) {
    ...
    if (output_numOfSubunits < 1) {
        std::cerr << TlUtils::format("output # of subunits is too small: %d",
    ...
    int input_numOfSubunits = 1;
    {
        ...
        if (bVerbose == true) {
            std::cerr << TlUtils::format("#vectors: %d", numOfVectors)
                      << std::endl;
            std::cerr << TlUtils::format("#size: %d", sizeOfVector)
                      << std::endl;
            std::cerr << TlUtils::format("#subunits: %d", input_numOfSubunits)
                      << std::endl;
        }
    }
    ...
    for (int i = 0; i < output_numOfSubunits; ++i) {
        outputs[i] = new TlDenseGeneralMatrix_arrays_RowOriented(
            numOfVectors, sizeOfVector, output_numOfSubunits, i,
            isUsingMemManager);
    }
