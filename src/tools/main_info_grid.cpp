...
void debugMode() {
  ...
  for (std::vector<int>::const_iterator it = supportedGrids.begin();
    ...
    for (int i = 0; i < numOfGrids; ++i) {
      std::cout << numOfGrids << "\t" << i << "\t" << grids[i].x() << "\t"
                << grids[i].y() << "\t" << grids[i].z() << "\t" << weights[i]
                << std::endl;
    }
  }
};
...
int main(int argc, char* argv[]) {
  ...
  for (int i = 0; i < numOfArgs; ++i) {
    ...
    std::vector<int>::const_iterator it =
        std::find(supportedGrids.begin(), supportedGrids.end(), gridSize);
    if (it != supportedGrids.end()) {
      ...
      for (int i = 0; i < gridSize; ++i) {
        std::cout << TlUtils::format(
                         "point(% 12.8e, % 12.8e, % 12.8e) weight=% 12.8e",
                         grids[i].x(), grids[i].y(), grids[i].z(), weights[i])
                  << std::endl;
      }
    } else {
      std::cerr << TlUtils::format("# grid = %d, is not defined.", gridSize)
                << std::endl;
    }
  }
