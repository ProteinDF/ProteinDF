...
{
    std::cout << TlUtils::format("%s [options] INPUT OUTPUT", name.c_str()) << std::endl;
...
int main(int argc, char* argv[])
{
    ...
    if (TlMatrix::isLoadable(input_path) != true) {
        std::cerr << TlUtils::format("type mismatch: %s", input_path.c_str()) << std::endl;
        return EXIT_FAILURE;
    } 
    TlMatrix in;
    in.load(input_path);
    ...
    if (in.getNumOfRows() != in.getNumOfCols()) {
        std::cerr << TlUtils::format("wrong matrix size: %d x %d", in.getNumOfRows(), in.getNumOfCols()) << std::endl;
        return EXIT_FAILURE;
    }
