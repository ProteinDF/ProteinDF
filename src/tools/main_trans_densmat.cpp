...
void usage(const std::string& progname) {
  std::cout << TlUtils::format(
                   "usage: %s \"old matrix file\" \"new matrix file\"",
                   progname.c_str())
            << std::endl;
  std::cout << " transform matrix into the different basis." << std::endl;
  std::cout
      << " default old matrix file is old.mtx, and new matrix file is new.mtx."
      << std::endl;
  std::cout << std::endl;
  std::cout << "options:" << std::endl;
  std::cout
      << "-o FILE:    old ProteinDF parameter file (default: pdfparam.mpac.old)"
      << std::endl;
  std::cout
      << "-n FILE:    new ProteinDF parameter file (default: pdfparam.mpac)"
      << std::endl;
  std::cout << "-s FILE:    overlap matrix (default: fl_Work/fl_Mtr_Spq.matrix)"
            << std::endl;
}
