...
int main(int argc, char* argv[])
{
    ...
    if (isSymmetric == true) {
        std::cout << TlUtils::format("[sym] mem_size=%8.2fMB ",
                                     double(dim *(dim +1) * sizeof(double)) / (2.0 * 1024.0 * 1024.0));
    } else {
        std::cout << TlUtils::format("[---] mem_size=%8.2fMB ",
    ...
    double elapse = 0.0;
    if (isMMap == true) {
        std::size_t needMemSize = dim * dim * sizeof(double) + 100; // 100は念のため
    ...
    std::cout << "ratio = " << (ratio / (1024.0 * 1024.0)) << " MB/sec" << std::endl;
