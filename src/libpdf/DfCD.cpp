...
DfCD::DfCD(TlSerializeData* pPdfParam) : DfObject(pPdfParam), pEngines_(NULL) {
  ...
  if (this->K_engine_ == DfObject::K_ENGINE_CD) {
    ...
    if ((*pPdfParam)["debug/DfCD/FastCDK_mode"].getStr().empty() != true) {
      const std::string fastCDK_mode_str =
          TlUtils::toUpper((*pPdfParam)["debug/DfCD/FastCDK_mode"].getStr());
  ...
  if ((*pPdfParam)["debug/DfCD/build_supermatrix"].getStr().empty() != true) {
    this->debugBuildSuperMatrix_ =
        (*pPdfParam)["debug/DfCD/build_supermatrix"].getBoolean();
  }
...
void DfCD::calcCholeskyVectorsForJK() {
  ...
  if (this->debugBuildSuperMatrix_) {
    ...
    TlDenseSymmetricMatrix_BLAS_Old V;
    {
      PQ_PairArray I2PQ;
    ...
    TlDenseGeneralMatrix_BLAS_old L = this->calcCholeskyVectors(V);

    // check CD
    if (this->debugCheckCD_) {
      TlDenseGeneralMatrix_BLAS_old tL = L;
      tL.transposeInPlace();
      TlDenseGeneralMatrix_BLAS_old LL = L * tL;
...
void DfCD::calcCholeskyVectorsForK() {
  ...
  switch (this->fastCDK_mode_) {
    case FASTCDK_DEBUG_FULL_SUPERMATRIX: {
      this->log_.info(
          "fast CDK routine (using debug FULL supermatrix) is valided.");
    ...
    case FASTCDK_PRODUCTIVE_FULL: {
      ...
      const TlDenseGeneralMatrix_arrays_RowOriented Lk =
          this->calcCholeskyVectorsOnTheFlyS_new(
              orbInfo, this->getI2prVtrPath(), this->epsilon_K_,
      ...
      if (this->debugCheckCD_) {
        // check CD
        TlDenseGeneralMatrix_BLAS_old mLk = Lk.getTlMatrixObject();
        TlDenseGeneralMatrix_BLAS_old tmLk = mLk;
        tmLk.transposeInPlace();
        TlDenseGeneralMatrix_BLAS_old LL = mLk * tmLk;
        LL.save("fl_Work/debug_LL_K.mat");
      }
  ...
  if (this->isDedicatedBasisForGridFree_) {
    ...
    if (this->debugBuildSuperMatrix_) {
      ...
      const TlDenseGeneralMatrix_arrays_RowOriented Lxc =
          this->calcCholeskyVectorsOnTheFly<DfOverlapEngine>(
              orbInfo_p, orbInfo_q, this->getI2pqVtrXCPath());
      this->saveLxc(Lxc);
    }
  } else {
    if (this->debugBuildSuperMatrix_) {
      ...
      const TlDenseGeneralMatrix_arrays_RowOriented Lxc =
          this->calcCholeskyVectorsOnTheFlyS_new(
              orbInfo_p, this->getI2pqVtrXCPath(), this->epsilon_,
              &DfCD::calcDiagonals, &DfCD::getSuperMatrixElements);
    ...
    //     TlDenseGeneralMatrix_BLAS_old L = this->getLxc();
    //     TlDenseGeneralMatrix_BLAS_old tL = L;
    //     tL.transposeInPlace();
    //     TlDenseGeneralMatrix_BLAS_old LL = L * tL;
    //     LL.save("fl_Work/debug_LL.mat");
...
void DfCD::debugOutLjk(const TlDenseGeneralMatrix_BLAS_old& Ljk) {
  const std::string path =
      TlUtils::format("%s.debug", DfObject::getLjkMatrixPath().c_str());
  Ljk.save(path);
}

void DfCD::debugOutLk(const TlDenseGeneralMatrix_BLAS_old& Lk) {
  const std::string path =
      TlUtils::format("%s.debug", DfObject::getLkMatrixPath().c_str());
  Lk.save(path);
}

void DfCD::debugOutLxc(const TlDenseGeneralMatrix_BLAS_old& Lxc) {
  const std::string path =
      TlUtils::format("%s.debug", DfObject::getLxcMatrixPath().c_str());
  Lxc.save(path);
}

TlDenseGeneralMatrix_arrays_ColOriented DfCD::getLjk() {
  TlDenseGeneralMatrix_arrays_ColOriented Ljk =
      DfObject::getLjkMatrix<TlDenseGeneralMatrix_arrays_ColOriented>();
  return Ljk;
}

TlDenseGeneralMatrix_arrays_ColOriented DfCD::getLk() {
  TlDenseGeneralMatrix_arrays_ColOriented Lk =
      DfObject::getLkMatrix<TlDenseGeneralMatrix_arrays_ColOriented>();
  return Lk;
}

TlDenseGeneralMatrix_arrays_ColOriented DfCD::getLxc() {
  TlDenseGeneralMatrix_arrays_ColOriented Lxc =
      DfObject::getLxcMatrix<TlDenseGeneralMatrix_arrays_ColOriented>();
  return Lxc;
}
...
void DfCD::finalize(TlDenseSymmetricMatrix_BLAS_Old* pMat) {
  // do nothing
}
...
void DfCD::finalize(TlDenseGeneralMatrix_BLAS_old* pMat) {
  // do nothing
}
...
TlDenseSymmetricMatrix_BLAS_Old DfCD::getCholeskyVector(const TlVector_BLAS& L_col,
...
TlDenseGeneralMatrix_BLAS_old DfCD::getCholeskyVectorA(
  ...
  const index_type numOfItilde = L_col.getSize();
  TlDenseGeneralMatrix_BLAS_old answer(numOfOrbs_p, numOfOrbs_q);
  for (index_type i = 0; i < numOfItilde; ++i) {
    answer.set(I2PQ[i].index1(), I2PQ[i].index2(), L_col[i]);
  }
...
void DfCD::getJ(TlDenseSymmetricMatrix_BLAS_Old* pJ) {
  if (this->useMmapMatrix_) {
    this->getJ_S_mmap(pJ);
  } else {
    this->getJ_S(pJ);
...
void DfCD::getJ_S(TlDenseSymmetricMatrix_BLAS_Old* pJ) {
...
void DfCD::getJ_S_mmap(TlDenseSymmetricMatrix_BLAS_Old* pJ) {
  this->log_.info("calc J by CD method (serial).");
...
TlVector_BLAS DfCD::getScreenedDensityMatrix(const PQ_PairArray& I2PQ) {
  const TlDenseSymmetricMatrix_BLAS_Old P = this->getPMatrix();
...
TlVector_BLAS DfCD::getScreenedDensityMatrix(const RUN_TYPE runType,
  ...
  switch (runType) {
    ...
    case RUN_UKS_ALPHA:
    case RUN_UKS_BETA:
      P = this->getPpqMatrix<TlDenseSymmetricMatrix_BLAS_Old>(
...
void DfCD::expandJMatrix(const TlVector_BLAS& vJ, const PQ_PairArray& I2PQ,
...
void DfCD::expandKMatrix(const TlVector_BLAS& vK, const PQ_PairArray& I2PR,
...
void DfCD::getJ_A(TlDenseSymmetricMatrix_BLAS_Old* pJ) {
  const TlOrbitalInfo orbInfo_p((*this->pPdfParam_)["coordinates"],
                                (*this->pPdfParam_)["basis_set"]);
  const TlOrbitalInfo orbInfo_q((*this->pPdfParam_)["coordinates"],
                                (*this->pPdfParam_)["basis_set"]);
  ...
  this->divideCholeskyBasis(numOfCBs, &start_CholeskyBasis, &end_CholeskyBasis);
  for (index_type I = start_CholeskyBasis; I < end_CholeskyBasis; ++I) {
    TlDenseGeneralMatrix_BLAS_old LI =
        this->getCholeskyVectorA(orbInfo_p, orbInfo_q, L.getColVector(I), I2PQ);
    // LI.save(TlUtils::format("fl_Work/debug_LI.%d.mat", I));

    TlDenseGeneralMatrix_BLAS_old QI = LI;
    QI.dotInPlace(P);
...
void DfCD::divideCholeskyBasis(const index_type numOfCBs, index_type* pStart,
                               index_type* pEnd) {
  *pStart = 0;
  *pEnd = numOfCBs;
}

void DfCD::getK(const RUN_TYPE runType, TlDenseSymmetricMatrix_BLAS_Old* pK) {
  switch (this->fastCDK_mode_) {
    case FASTCDK_NONE:
      if (this->useMmapMatrix_) {
...
TlDenseSymmetricMatrix_BLAS_Old DfCD::getPMatrix(const RUN_TYPE runType, int itr) {
  TlDenseSymmetricMatrix_BLAS_Old P;
  {
    this->log_.info("use density matrix.");
    switch (runType) {
      case RUN_RKS:
        P = 0.5 * this->getPpqMatrix<TlDenseSymmetricMatrix_BLAS_Old>(RUN_RKS, itr);
        break;

      case RUN_UKS_ALPHA:
      case RUN_UKS_BETA:
        P = this->getPpqMatrix<TlDenseSymmetricMatrix_BLAS_Old>(runType, itr);
        break;

      case RUN_ROKS_ALPHA: {
        P = 0.5 *
            this->getPpqMatrix<TlDenseSymmetricMatrix_BLAS_Old>(RUN_ROKS_CLOSED,
                                                            itr);
        P +=
            this->getPpqMatrix<TlDenseSymmetricMatrix_BLAS_Old>(RUN_ROKS_OPEN, itr);
      } break;
...
void DfCD::getK_S_woCD(const RUN_TYPE runType,
  ...
  this->divideCholeskyBasis(numOfCBs, &start_CholeskyBasis, &end_CholeskyBasis);
  for (index_type I = start_CholeskyBasis; I < end_CholeskyBasis; ++I) {
    const TlDenseSymmetricMatrix_BLAS_Old l =
        this->getCholeskyVector(L.getColVector(I), I2PQ);
...
void DfCD::getK_A(const RUN_TYPE runType, TlDenseSymmetricMatrix_BLAS_Old* pK) {
  const TlOrbitalInfo orbInfo_p((*this->pPdfParam_)["coordinates"],
                                (*this->pPdfParam_)["basis_set"]);
  const TlOrbitalInfo orbInfo_q((*this->pPdfParam_)["coordinates"],
                                (*this->pPdfParam_)["basis_set"]);
  ...
  TlDenseSymmetricMatrix_BLAS_Old P = 0.5 * this->getPMatrix();  // RKS
...
void DfCD::getM(const TlDenseSymmetricMatrix_BLAS_Old& P,
...
void DfCD::getM_S(const TlDenseSymmetricMatrix_BLAS_Old& P,
                  TlDenseSymmetricMatrix_BLAS_Old* pM) {
  ...
  this->divideCholeskyBasis(numOfCBs, &start_CholeskyBasis, &end_CholeskyBasis);
  for (index_type I = start_CholeskyBasis; I < end_CholeskyBasis; ++I) {
    TlDenseSymmetricMatrix_BLAS_Old LI =
        this->getCholeskyVector(L.getColVector(I), I2PQ);
...
void DfCD::getM_A(const TlDenseSymmetricMatrix_BLAS_Old& P,
                  TlDenseSymmetricMatrix_BLAS_Old* pM) {
  ...
  const TlDenseGeneralMatrix_arrays_ColOriented L = this->getLxc();
  this->log_.info(
      TlUtils::format("L(xc; A): %d x %d", L.getNumOfRows(), L.getNumOfCols()));
  const index_type numOfCBs = L.getNumOfCols();
...
TlDenseSymmetricMatrix_BLAS_Old DfCD::getPMatrix() {
  TlDenseSymmetricMatrix_BLAS_Old P;
...
TlDenseGeneralMatrix_arrays_RowOriented DfCD::calcCholeskyVectorsOnTheFlyA(
  ...
  const TlVector_BLAS::size_type N = I2PQ.size();

  TlDenseGeneralMatrix_arrays_RowOriented L(N, 1, 1, 0);
  // TlDenseGeneralMatrix_BLAS_old tmpL(N, N);
  const double threshold = this->epsilon_;
  this->log_.info(
      TlUtils::format("Cholesky Decomposition: epsilon=%e", this->epsilon_));

  double error = d.getMaxAbsoluteElement();
  // double error = d.sum();
  std::vector<TlVector_BLAS::size_type> pivot(N);
  for (TlVector_BLAS::size_type i = 0; i < N; ++i) {
    pivot[i] = i;
  }
  ...
  while (error > threshold) {
#ifdef DEBUG_CD
    this->log_.debug(
        TlUtils::format("CD progress: %12d/%12d: err=% 16.10e", m, N, error));
#endif  // DEBUG_CD
    ...
    const index_type numOf_G_cols = N - (m + 1);
    {
      ...
      G_pm = this->getSuperMatrixElementsA(orbInfo_p, orbInfo_q, pivot_m,
                                           G_col_list, I2PQ, schwartzTable);
    }
    assert(static_cast<index_type>(G_pm.size()) == numOf_G_cols);

    // CD calc
    const TlVector_BLAS L_pm = L.getVector(pivot_m);
...
void DfCD::calcDiagonals(const TlOrbitalInfoObject& orbInfo,
                         PQ_PairArray* pI2PQ, TlVector_BLAS* pDiagonals) {
  const index_type numOfAOs = orbInfo.getNumOfOrbitals();
  ...
  this->log_.info(TlUtils::format("primitive GTO quartet threshold: %e",
                                  this->cutoffThreshold_primitive_));
  ...
  bool hasTask =
      pDfTaskCtrl->getQueue2(orbInfo, true, this->grainSize_, &taskList, true);
  while (hasTask == true) {
    this->calcDiagonals_kernel(orbInfo, taskList, &diagonalMat, pI2PQ);
    hasTask =
        pDfTaskCtrl->getQueue2(orbInfo, true, this->grainSize_, &taskList);
  }
  delete pDfTaskCtrl;
  pDfTaskCtrl = NULL;
...
void DfCD::calcDiagonals_K_full(const TlOrbitalInfoObject& orbInfo,
  ...
  DfTaskCtrl* pDfTaskCtrl = this->getDfTaskCtrlObject();
  std::vector<DfTaskCtrl::Task2> taskList;
  bool hasTask = pDfTaskCtrl->getQueue2(orbInfo, orbInfo, true,
                                        this->grainSize_, &taskList, true);
  while (hasTask == true) {
    this->calcDiagonals_K_full_kernel(orbInfo, taskList, &diagonalMat, pI2PR);
    hasTask = pDfTaskCtrl->getQueue2(orbInfo, orbInfo, true, this->grainSize_,
                                     &taskList);
  }
  delete pDfTaskCtrl;
  pDfTaskCtrl = NULL;
...
void DfCD::calcDiagonalsA(const TlOrbitalInfoObject& orbInfo_p,
                          ...
                          PQ_PairArray* pI2PQ, TlSparseMatrix* pSchwartzTable,
                          TlVector_BLAS* pDiagonals) {
  const double tau = this->CDAM_tau_;
  this->log_.info(TlUtils::format("CDAM tau: %e", tau));
  this->log_.info(TlUtils::format("primitive GTO quartet threshold: %e",
  ...
  DfTaskCtrl* pDfTaskCtrl = this->getDfTaskCtrlObject();
  std::vector<DfTaskCtrl::Task2> taskList;
  bool hasTask = pDfTaskCtrl->getQueue2(orbInfo_p, orbInfo_q, true,
                                        this->grainSize_, &taskList, true);
  while (hasTask == true) {
    this->calcDiagonalsA_kernel(orbInfo_p, orbInfo_q, taskList, pI2PQ,
                                pSchwartzTable, &diagonalMat);
    hasTask = pDfTaskCtrl->getQueue2(orbInfo_p, orbInfo_q, true,
...
#pragma omp parallel
  {
    ...
    this->pEngines_[threadID]->setPrimitiveLevelThreshold(
        this->cutoffThreshold_primitive_);
...
#pragma omp for schedule(runtime)
    for (int i = 0; i < taskListSize; ++i) {
      ...
      this->pEngines_[threadID]->calc(0, orbInfo, shellIndexP, 0, orbInfo,
                                      shellIndexQ, 0, orbInfo, shellIndexP, 0,
                                      orbInfo, shellIndexQ);
          ...
          if ((shellIndexP != shellIndexQ) || (indexP >= indexQ)) {
            ...
            if (std::fabs(value) > tau) {
              if (value > 0) {
                ...
                this->log_.warn(
                    TlUtils::format("pqpq_value: (%d %d)=% e is spoiled.",
                                    indexP, indexQ, value));
              }
            }
          }
...
void DfCD::calcDiagonals_K_full_kernel(
    const TlOrbitalInfoObject& orbInfo,
    const std::vector<DfTaskCtrl::Task2>& taskList,
...
#pragma omp for schedule(runtime)
    for (int i = 0; i < taskListSize; ++i) {
      ...
      assert(this->pEngines_[threadID] != NULL);
      this->pEngines_[threadID]->calc(0, orbInfo, shellIndexP, 0, orbInfo,
                                      shellIndexP, 0, orbInfo, shellIndexR, 0,
                                      orbInfo, shellIndexR);
        ...
        for (int r = 0; r < maxStepsR; ++r) {
          ...
          if (std::fabs(value) > tau) {
            if (value > 0) {
              ...
              this->log_.warn(
                  TlUtils::format("pprr_value: (%d %d)=% e is spoiled.", indexP,
                                  indexR, value));
            }
          }
        }
...
void DfCD::calcDiagonals_K_half_kernel(
    const TlOrbitalInfoObject& orbInfo,
    const std::vector<DfTaskCtrl::Task2>& taskList,
...
#pragma omp for schedule(runtime)
    for (int i = 0; i < taskListSize; ++i) {
        ...
        for (int p = 0; p < maxStepsP; ++p) {
          ...
          for (int r = 0; r < maxStepsR; ++r) {
            ...
            if (indexP >= indexR) {
              ...
              const double value =
                  coef * this->pEngines_[threadID]->value(pprr_index);
...

      {
        assert(this->pEngines_[threadID] != NULL);
        this->pEngines_[threadID]->calc(0, orbInfo, shellIndexP, 0, orbInfo,
                                        shellIndexR, 0, orbInfo, shellIndexP, 0,
                                        orbInfo, shellIndexR);
        ...
        for (int p = 0; p < maxStepsP; ++p) {
          ...
          for (int r = 0; r < maxStepsR; ++r) {
            ...
            if (indexP > indexR) {  // indexP != indexR
              const int prpr_index =
                  ((p * maxStepsR + r) * maxStepsP + p) * maxStepsR + r;

              const double coef = (indexP != indexR) ? 2.0 : 1.0;
              const double value =
                  coef * this->pEngines_[threadID]->value(prpr_index);
...
#pragma omp for schedule(runtime)
    for (int i = 0; i < taskListSize; ++i) {
      ...
      assert(this->pEngines_[threadID] != NULL);
      this->pEngines_[threadID]->calc(0, orbInfo_p, shellIndexP, 0, orbInfo_q,
                                      shellIndexQ, 0, orbInfo_p, shellIndexP, 0,
                                      orbInfo_q, shellIndexQ);
        ...
        for (int q = 0; q < maxStepsQ; ++q) {
          ...
          if (std::fabs(value) > tau) {
            if (value > 0) {
              ...
              this->log_.warn(
                  TlUtils::format("pqpq_value: (%d %d)=% e is spoiled.", indexP,
                                  indexQ, value));
            }
          }
        }
      ...
      for (int shellTypeB = 0; shellTypeB < maxShellType; ++shellTypeB) {
        ...
        for (int shellTypeC = 0; shellTypeC < maxShellType; ++shellTypeC) {
          ...
          for (int shellTypeD = 0; shellTypeD < maxShellType; ++shellTypeD) {
            ...
            if (this->cutoffAll_schwartz_[shellTypeABCD] > 0) {
              const double ratio =
                  (double)this->cutoffAlive_schwartz_[shellTypeABCD] /
...
std::vector<double> DfCD::getSuperMatrixElements(
  ...
  const std::vector<IndexPair4S> calcList =
      this->getCalcList(orbInfo, G_row, G_col_list, start, end, I2PQ);
  this->calcERIs(orbInfo, calcList);
  const std::vector<double> answer =
      this->setERIs(orbInfo, G_row, G_col_list, start, end, I2PQ);

  return answer;
}

std::vector<double> DfCD::getSuperMatrixElementsA(
    const TlOrbitalInfoObject& orbInfo_p, const TlOrbitalInfoObject& orbInfo_q,
  ...
  const std::vector<IndexPair4A> calcList =
      this->getCalcListA(orbInfo_p, orbInfo_q, G_row, G_col_list, I2PQ);
  this->calcERIsA(orbInfo_p, orbInfo_q, calcList, schwartzTable);
  const std::vector<double> answer =
      this->setERIsA(orbInfo_p, orbInfo_q, G_row, G_col_list, I2PQ);

  return answer;
}

std::vector<DfCD::IndexPair4S> DfCD::getCalcList(
    const TlOrbitalInfoObject& orbInfo, const index_type G_row,
...
std::vector<DfCD::IndexPair4A> DfCD::getCalcListA(
    const TlOrbitalInfoObject& orbInfo_p, const TlOrbitalInfoObject& orbInfo_q,
    const index_type G_row, const std::vector<index_type>& G_col_list,
    const PQ_PairArray& I2PQ) {
...
#pragma omp for schedule(runtime)
    for (int i = 0; i < numOfList; ++i) {
...

      {
        ...
        this->pEngines_[threadID]->calc(0, orbInfo, shellIndexP, 0, orbInfo,
                                        shellIndexQ, 0, orbInfo, shellIndexR, 0,
                                        orbInfo, shellIndexS);
...
#pragma omp for schedule(runtime)
    for (int i = 0; i < numOfList; ++i) {
      ...
      if (isAlive == true) {
        ...
        const int maxStepsS = 2 * shellTypeS + 1;
        this->pEngines_[threadID]->calc(0, orbInfo_p, shellIndexP, 0, orbInfo_q,
                                        shellIndexQ, 0, orbInfo_p, shellIndexR,
...
std::vector<double> DfCD::setERIs(const TlOrbitalInfoObject& orbInfo,
...
#pragma omp parallel for
  for (index_type i = start; i < end; ++i) {
    ...
    double value = 0.0;
    if (this->getCachedValue(orbInfo, indexP, indexQ, indexR, indexS,
                             this->ERI_cache_, &value)) {
      answer[i] = value;
    } else {
      CnErr.abort(TlUtils::format("%s: %d: not found value in cache.", __FILE__,
                                  __LINE__));
    }
  }
...
std::vector<double> DfCD::setERIsA(const TlOrbitalInfoObject& orbInfo_p,
                                   const TlOrbitalInfoObject& orbInfo_q,
                                   const index_type G_row,
                                   const std::vector<index_type> G_col_list,
                                   const PQ_PairArray& I2PQ) {
...
#pragma omp critical(DfCD__setERIs)
    ...
    assert(values.empty() != true);
    {
      ...
      const int index =
          ((basisTypeP * maxStepsQ + basisTypeQ) * maxStepsR + basisTypeR) *
              maxStepsS +
          basisTypeS;
...
//
////////////////////////////////////////////////////////////////////////////////
TlDenseSymmetricMatrix_BLAS_Old DfCD::getSuperMatrix(
  ...
  TlDenseSymmetricMatrix_BLAS_Old V(N);
...
TlDenseSymmetricMatrix_BLAS_Old DfCD::getSuperMatrix_K_full(
  ...
  TlDenseSymmetricMatrix_BLAS_Old V2(v2size);
...
#pragma omp for schedule(runtime)
    for (std::size_t i = 0; i < N; ++i) {
      ...
      for (std::size_t j = 0; j < N; ++j) {
        ...
        {
          this->pEngines_[threadID]->calc(0, orbInfo, shellIndexP, 0, orbInfo,
                                          shellIndexQ, 0, orbInfo, shellIndexR,
...
TlDenseSymmetricMatrix_BLAS_Old DfCD::getSuperMatrix_K_half(
    const TlOrbitalInfoObject& orbInfo, PQ_PairArray* pI2PR) {
  // const index_type numOfOrbs = orbInfo.getNumOfOrbitals();
...
#pragma omp for schedule(runtime)
    for (std::size_t i = 0; i < N; ++i) {
      ...
      for (std::size_t j = 0; j < i; ++j) {
        ...
        if (indexQ != indexS) {
          this->pEngines_[threadID]->calc(0, orbInfo, shellIndexP, 0, orbInfo,
                                          shellIndexS, 0, orbInfo, shellIndexR,
                                          0, orbInfo, shellIndexQ);
          const int index =
              ((basisTypeP * maxStepsS + basisTypeS) * maxStepsR + basisTypeR) *
                  maxStepsQ +
              basisTypeQ;
      ...
      {
        {
          ...
          const int index =
              ((basisTypeP * maxStepsP + basisTypeP) * maxStepsR + basisTypeR) *
                  maxStepsR +
              basisTypeR;
        ...
        if (indexP != indexR) {
          this->pEngines_[threadID]->calc(0, orbInfo, shellIndexP, 0, orbInfo,
                                          shellIndexR, 0, orbInfo, shellIndexP,
                                          0, orbInfo, shellIndexR);
          const int index =
              ((basisTypeP * maxStepsR + basisTypeR) * maxStepsP + basisTypeP) *
                  maxStepsR +
              basisTypeR;
...
TlDenseGeneralMatrix_BLAS_old DfCD::calcCholeskyVectors(
    const TlDenseSymmetricMatrix_BLAS_Old& V) {
  const index_type N = V.getNumOfRows();
  TlVector_BLAS d(N);  // 対角成分
  ...
  while (error > threshold) {
    ...
    // CD calc
    const TlVector_BLAS L_pm = L.getRowVector(pivot_m);
    std::vector<double> L_xm(numOf_G_cols);
#pragma omp parallel for schedule(runtime)
    for (index_type i = 0; i < numOf_G_cols; ++i) {
      const index_type pivot_i = pivot[m + 1 + i];  // from (m+1) to N
      TlVector_BLAS L_pi = L.getRowVector(pivot_i);
      const double sum_ll = (L_pi.dotInPlace(L_pm)).sum();
      // const double l_m_pi = (G_pm[i] - sum_ll) * inv_l_m_pm;
      const double l_m_pi =
          (V.get(pivot_m, pivot[m + 1 + i]) - sum_ll) * inv_l_m_pm;
...
// K full --------------------------------------------------------------
std::vector<double> DfCD::getSuperMatrixElements_K_full(
    const TlOrbitalInfoObject& orbInfo, const index_type G_row,
    const std::vector<index_type>& G_col_list, const PQ_PairArray& I2PQ) {
  this->ERI_cache_.clear();
  ...
  const std::vector<IndexPair4S> calcList =
      this->getCalcList_K_full(orbInfo, G_row, G_col_list, start, end, I2PQ);
  this->calcERIs_K(orbInfo, calcList);
  const std::vector<double> answer =
      this->setERIs_K_full(orbInfo, G_row, G_col_list, start, end, I2PQ);

  return answer;
}

std::vector<DfCD::IndexPair4S> DfCD::getCalcList_K_full(
    const TlOrbitalInfoObject& orbInfo, const index_type G_row,
    const std::vector<index_type>& G_col_list, const index_type start,
...
std::vector<double> DfCD::setERIs_K_full(
    const TlOrbitalInfoObject& orbInfo, const index_type G_row,
    const std::vector<index_type> G_col_list, const index_type start,
    const index_type end, const PQ_PairArray& I2PR) {
  assert(0 <= start);
...
// K half --------------------------------------------------------------
std::vector<double> DfCD::getSuperMatrixElements_K_half(
    const TlOrbitalInfoObject& orbInfo, const index_type G_row,
  ...
  const std::vector<IndexPair4S> calcList =
      this->getCalcList_K_half(orbInfo, G_row, G_col_list, start, end, I2PR);
  this->calcERIs_K(orbInfo, calcList);
  const std::vector<double> answer =
      this->setERIs_K_half(orbInfo, G_row, G_col_list, start, end, I2PR);

  return answer;
}

std::vector<DfCD::IndexPair4S> DfCD::getCalcList_K_half(
    const TlOrbitalInfoObject& orbInfo, const index_type G_row,
    const std::vector<index_type>& G_col_list, const index_type start,
    const index_type end, const PQ_PairArray& I2PR) {
  assert(0 <= start);
...
std::vector<double> DfCD::setERIs_K_half(
    const TlOrbitalInfoObject& orbInfo, const index_type G_row,
    const std::vector<index_type> G_col_list, const index_type start,
    const index_type end, const PQ_PairArray& I2PQ) {
  assert(0 <= start);
...
#pragma omp parallel for
  for (index_type i = start; i < end; ++i) {
    ...
    if (G_row == G_col) {
      // 対角項
      {
        if (this->getCachedValue(orbInfo, indexP, indexP, indexR, indexR,
                                 this->ERI_cache_, &tmp)) {
          const double coef = (indexP != indexR) ? 2.0 : 1.0;
#pragma omp critical(DfCD__setERIs_set_answer)
          { value += coef * tmp; }
      ...
      if (indexP != indexR) {
        if (this->getCachedValue(orbInfo, indexP, indexR, indexP, indexR,
                                 this->ERI_cache_, &tmp)) {
          // const double coef = (indexP != indexR) ? 2.0 : 1.0;
...
#pragma omp critical(DfCD__setERIs_set_answer)
      ...
      if (indexQ != indexS) {
        if (this->getCachedValue(orbInfo, indexP, indexS, indexR, indexQ,
                                 this->ERI_cache_, &tmp)) {
          const double coef = (indexP != indexR) ? 2.0 : 1.0;
#pragma omp critical(DfCD__setERIs_set_answer)
          { value += coef * tmp; }
    ...
    //         this->log_.info(TlUtils::format("(%d, %d|%d, %d) I=%d, J=%d: c=%
    //         e, v= %e",
...
bool DfCD::getCachedValue(const TlOrbitalInfoObject& orbInfo, index_type indexP,
                          index_type indexQ, index_type indexR,
                          index_type indexS, const ERI_CacheType& cache,
                          double* pValue) {
  assert(pValue != NULL);
  bool answer = false;
  ...
  ERI_CacheType::const_iterator it = cache.find(
      IndexPair4S(shellIndexP, shellIndexQ, shellIndexR, shellIndexS));
