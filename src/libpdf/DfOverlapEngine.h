...
///
#define OVP_STATE_MAX                                                          \
  (OVP_A_BAR_MAX * OVP_B_BAR_MAX * OVP_C_BAR_MAX * OVP_D_BAR_MAX * OVP_A_MAX * \
   OVP_B_MAX * OVP_C_MAX * OVP_D_MAX)
...
class DfOverlapEngine : public DfEngineObject {
 public:
  ...
  struct Query {
   public:
    Query(int a_bar, int b_bar, int c_bar, int d_bar, int a, int b, int c,
          int d)
        : shellTypeA_bar(a_bar),
          shellTypeB_bar(b_bar),
          shellTypeC_bar(c_bar),
          shellTypeD_bar(d_bar),
          shellTypeA(a),
          shellTypeB(b),
          shellTypeC(c),
          shellTypeD(d) {}
  ...
  struct OvpState {
    OvpState(int inAbar, int inBbar, int inCbar, int inDbar, int inA, int inB,
             int inC, int inD)
        : a_bar(inAbar),
          b_bar(inBbar),
          c_bar(inCbar),
    ...
    int index() const {
      const int answer =
          ((((((a_bar * OVP_B_BAR_MAX + b_bar) * OVP_C_BAR_MAX + c_bar) *
                  OVP_D_BAR_MAX +
              d_bar) *
                 OVP_A_MAX +
             a) *
 ...
 private:
  PGTOs getPGTOs(const TlOrbitalInfoObject& orbitalInfo, const int shellIndex);

  void calc0(const Query& query, const TlPosition& A, const TlPosition& B,
             const TlPosition& C, const TlPosition& D, const PGTOs& PGTOs_A,
             const PGTOs& PGTOs_B, const PGTOs& PGTOs_C, const PGTOs& PGTOs_D);
