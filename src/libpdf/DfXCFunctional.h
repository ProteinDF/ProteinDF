...
class DfXCFunctional : public DfObject {
   ...
   protected:
    ...
    // double getFockExchangeEnergy(const TlDenseSymmetricMatrix_Lapack& P,
    // const TlDenseSymmetricMatrix_Lapack& Ex); double
    // getFockExchangeEnergy(const TlDenseSymmetricMatrix_Lapack& PA, const
    // TlDenseSymmetricMatrix_Lapack& PB,
...
void DfXCFunctional::getFxc(const SymmetricMatrixType& P1,
    ...
    switch (this->m_nXCFunctional) {
        ...
        default:
            this->log_.critical(TlUtils::format("XC functional: %s",
                                                this->m_sXCFunctional.c_str()));
            CnErr.abort(
                "unknown XC functional. STOP. @DfXCFunctional::getFxc()");
    }
