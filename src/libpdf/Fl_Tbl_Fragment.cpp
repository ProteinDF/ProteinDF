...
Fl_Tbl_Fragment::Fl_Tbl_Fragment(const Fl_Geometry& flGeom) : FlFrag(flGeom) {
...
void Fl_Tbl_Fragment::makeTable() {
    ...
    for (int i = 0; i < number_qclo; i++) {
        fo << std::setw(8) << FragTbl[i].qclo << std::setw(6)
           << FragTbl[i].fragment_alpha << std::setw(6)
           << FragTbl[i].fragmentqclo_alpha << std::setw(6)
           << FragTbl[i].fragment_beta << std::setw(6)
           << FragTbl[i].fragmentqclo_beta << std::endl;
    }
    fo.close();
}
...
void Fl_Tbl_Fragment::setData() {
    ...
    if (!fi) {
        log.error("Cannot open " + TblFile);
        CnErr.abort("Fl_Tbl_Fragment", "", "setData",
                    "Cannot open FragmentTable");
    }
    for (int i = 0; i < number_fragment; i++) {
        number_fragmentqclo[i] = 0;
    }
    for (int i = 0; i < number_qclo; i++) {
        fi >> FragTbl[i].qclo >> FragTbl[i].fragment_alpha >>
            FragTbl[i].fragmentqclo_alpha >> FragTbl[i].fragment_beta >>
