...
void DfTotalEnergy::exec_template() {
  ...
  switch (this->m_nMethodType) {
    case METHOD_RKS:
      Ppq = DfObject::getPpqMatrix<SymmetricMatrixType>(RUN_RKS,
                                                        this->m_nIteration);
      break;

    case METHOD_UKS:
      PpqA = DfObject::getPpqMatrix<SymmetricMatrixType>(RUN_UKS_ALPHA,
                                                         this->m_nIteration);
      PpqB = DfObject::getPpqMatrix<SymmetricMatrixType>(RUN_UKS_BETA,
                                                         this->m_nIteration);
      Ppq = PpqA + PpqB;
      break;
    ...
    case J_ENGINE_RI_J: {
      ...
      if (!((this->m_bMemorySave == false) &&
        ...
        this->m_dE_J_Rho_RhoTilde =
            this->calcJRhoRhoTilde_DIRECT<DfEriType, SymmetricMatrixType,
                                          VectorType>(Ppq);
      }
    } break;
      ...
      case METHOD_RKS: {
        if (dfXCFunctional.getXcType() != DfXCFunctional::HF) {
          if (this->m_bIsXCFitting == true) {
            ...
            this->m_dExc =
                this->calcExc_DIRECT<DfOverlapType, SymmetricMatrixType,
                                     VectorType>(Ppq, Eps);
          } else {
      ...
      case METHOD_UKS: {
        if (dfXCFunctional.getXcType() != DfXCFunctional::HF) {
          if (this->m_bIsXCFitting == true) {
            ...
            this->m_dExc =
                this->calcExc_DIRECT<DfOverlapType, SymmetricMatrixType,
                                     VectorType>(PpqA, EpsA);
            this->m_dExc +=
                this->calcExc_DIRECT<DfOverlapType, SymmetricMatrixType,
                                     VectorType>(PpqB, EpsB);
          } else {
            if (this->XC_engine_ != XC_ENGINE_GRID) {
...
SymmetricMatrixType DfTotalEnergy::getPpq(const METHOD_TYPE methodType) {
  ...
  switch (methodType) {
    ...
    case METHOD_UKS: {
      ...
      Ppq = DfObject::getPpqMatrix<SymmetricMatrixType>(RUN_UKS_ALPHA,
                                                        this->m_nIteration);
      if (Ppq.getNumOfRows() != this->m_nNumOfAOs ||
          Ppq.getNumOfCols() != this->m_nNumOfAOs) {
        CnErr.abort("DfTotalEnergy", "getPpq", "", "program error");
      ...
      Ppq_b = DfObject::getPpqMatrix<SymmetricMatrixType>(RUN_UKS_BETA,
                                                          this->m_nIteration);
    ...
    case METHOD_ROKS: {
      ...
      {
        Ppq = DfObject::getPpqMatrix<SymmetricMatrixType>(RUN_ROKS_CLOSED,
                                                          this->m_nIteration);
        if (Ppq.getNumOfRows() != this->m_nNumOfAOs ||
            Ppq.getNumOfCols() != this->m_nNumOfAOs) {
      ...
      {
        P2pq = DfObject::getPpqMatrix<SymmetricMatrixType>(RUN_ROKS_OPEN,
                                                           this->m_nIteration);
  ...
  switch (runType) {
    case RUN_RKS:
      ...
      if (static_cast<TlVectorAbstract::size_type>(this->numOfAuxXC_) !=
        ...
        this->logger(
            "DfTotalEnergy dimension is not consistency, but continue\n");
      }
      break;
...
double DfTotalEnergy::calcEnergyFromDummy() {
  ...
  this->logger(TlUtils::format(
      " total energy derived from dummy atom = %18.16lf\n", energy_from_dummy));
