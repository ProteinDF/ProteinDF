...
class DfTotalEnergy : public DfObject {
   ...
   protected:
    ...
    template <typename DfEriType, typename SymmetricMatrixType,
              typename VectorType>
    double calcJRhoRhoTilde_DIRECT(const SymmetricMatrixType& D);

    /// 交換相関ポテンシャル部分の計算を行う
    template <typename DfOverlapType, typename SymmetricMatrixType,
              typename VectorType>
    double calcExc_DIRECT(const SymmetricMatrixType& D, const VectorType& E);

    template <typename SymmetricMatrixType>
    double calcExc(const RUN_TYPE runType, const SymmetricMatrixType& P);
    ...
    switch (this->J_engine_) {
        case J_ENGINE_RI_J: {
            ...
            const VectorType rho =
                this->getRho<VectorType>(this->m_nMethodType);
            this->m_dE_J_RhoTilde_RhoTilde =
                this->calcJRhoTildeRhoTilde<SymmetricMatrixType, VectorType>(
                    rho);

            if (!((this->m_bMemorySave == false) &&
                  (this->m_bDiskUtilization == false))) {
                // NOT use Threeindexintegrals
                this->m_dE_J_Rho_RhoTilde = this->calcJRhoRhoTilde_DIRECT<
                    DfEriType, SymmetricMatrixType, VectorType>(Ppq);
            }
        } break;
    ...
    if ((this->m_bMemorySave == false) && (this->m_bDiskUtilization == false)) {
        ...
        switch (this->m_nMethodType) {
                ...
                if (dfXCFunctional.getXcType() != DfXCFunctional::HF) {
                    if (this->m_bIsXCFitting == true) {
                        const VectorType Eps =
                            this->getEps<VectorType>(RUN_RKS);
                        this->m_dExc = this->calcExc_DIRECT<
                            DfOverlapType, SymmetricMatrixType, VectorType>(
                            Ppq, Eps);
                    } else {
                        if (this->XC_engine_ != XC_ENGINE_GRID) {
                            this->m_dExc =
                                this->calcExc(RUN_RKS, 0.5 * Ppq) * 2.0;
                        } else {
                            DfXCFunctional dfXCFunctional(this->pPdfParam_);
            ...
            case METHOD_UKS: {
                if (dfXCFunctional.getXcType() != DfXCFunctional::HF) {
                    if (this->m_bIsXCFitting == true) {
                        const VectorType EpsA =
                            this->getEps<VectorType>(RUN_UKS_ALPHA);
                        const VectorType EpsB =
                            this->getEps<VectorType>(RUN_UKS_BETA);
                        this->m_dExc = this->calcExc_DIRECT<
                            DfOverlapType, SymmetricMatrixType, VectorType>(
                            PpqA, EpsA);
                        this->m_dExc += this->calcExc_DIRECT<
            ...
            case METHOD_ROKS: {
                if (dfXCFunctional.getXcType() != DfXCFunctional::HF) {
                    if (this->m_bIsXCFitting == true) {
                        const VectorType Eps_closed =
                            0.5 * this->getEps<VectorType>(RUN_ROKS_CLOSED);
                        const VectorType Eps_open =
                            this->getEps<VectorType>(RUN_ROKS_CLOSED);
    ...
    switch (methodType) {
        ...
        case METHOD_ROKS: {
            SymmetricMatrixType P2pq;
            {
                ...
                if (Ppq.getNumOfRows() != this->m_nNumOfAOs ||
                    Ppq.getNumOfCols() != this->m_nNumOfAOs) {
                    CnErr.abort("DfTotalEnergy", "DfTotalEnergyMain", "",
                                "program error");
                }
            }
            {
    ...
    switch (runType) {
        case RUN_RKS:
            ...
            if (static_cast<TlVectorAbstract::size_type>(this->numOfAuxXC_) !=
                E.getSize()) {
                this->logger(
                    TlUtils::format("dimension_eps = %d\n", E.getSize()));
                this->logger(
                    TlUtils::format("numOfAuxXC_ = %d\n", this->numOfAuxXC_));
                this->logger(
                    "DfTotalEnergy dimension is not consistency, but "
                    "continue\n");
            }
            break;
        ...
        case RUN_UKS_ALPHA:
            ...
            if (static_cast<TlVectorAbstract::size_type>(this->numOfAuxXC_) !=
                E.getSize()) {
                this->logger(
                    TlUtils::format("dimension of epsa = %d\n", E.getSize()));
                this->logger(
                    TlUtils::format("numOfAuxXC_   = %d\n", this->numOfAuxXC_));
                this->logger(
                    "DfTotalEnergy dimension is not consistency, but "
        ...
        case RUN_UKS_BETA:
            ...
            if (static_cast<TlVectorAbstract::size_type>(this->numOfAuxXC_) !=
                E.getSize()) {
                this->logger(
                    TlUtils::format("dimension of epsb = %d\n", E.getSize()));
                this->logger(
                    TlUtils::format("numOfAuxXC_   = %d\n", this->numOfAuxXC_));
                this->logger(
                    "DfTotalEnergy dimension is not consistency, but "
...
double DfTotalEnergy::calculate_E_WITH_DIRECT(const SymmetricMatrixType& D) {
    ...
    if (this->m_nIteration > 1) {
        const std::string fname =
            "fl_Work/fl_Mtr_Epqtmp" + TlUtils::xtos(this->m_nIteration - 1);
        tmpEpq.load(fname);
    } else {
        tmpEpq.load(this->getHpqMatrixPath());
        if (tmpEpq.getNumOfRows() != this->m_nNumOfAOs ||
            tmpEpq.getNumOfCols() != this->m_nNumOfAOs) {
            CnErr.abort("DfTotalEnergy", "calculate_oep+JRR+Exc", "",
                        "program error");
        }
    }
    ...
    if (this->m_nNumOfDummyAtoms > 0) {
        ...
        if (((this->m_nIteration == 1) && (chgextra_number == 0)) ||
            ((this->m_nIteration != 1) &&
             (this->m_nIteration <= (chgextra_number + 1)))) {
...
template <typename DfOverlapType, typename SymmetricMatrixType,
          typename VectorType>
double DfTotalEnergy::calcExc_DIRECT(const SymmetricMatrixType& D,
                                     const VectorType& E) {
    // put eps*[pqs] into B
...
double DfTotalEnergy::calcExc(const RUN_TYPE runType,
    ...
    SymmetricMatrixType Exc = DfObject::getExcMatrix<SymmetricMatrixType>(
...
void DfTotalEnergy::calcRealEnergy() {
    ...
    const double current_energy =
        (*this->pPdfParam_)["TEs"][this->m_nIteration].getDouble();
    ...
    this->logger(TlUtils::format(
        " total energy excluding dummy charge = %18.16lf\n", real_energy));
    return;
}
...
double DfTotalEnergy::calcEnergyFromDummy() {
    ...
    const int start_number_dummy =
        this->m_nNumOfAtoms - this->m_nNumOfDummyAtoms;
    ...
    this->logger(TlUtils::format(
        " energy derived from nuclear part = %18.16lf\n", energy_from_nuclear));

    // sum
    const double energy_from_dummy = energy_from_Hpq2 + energy_from_nuclear;

    this->logger(
        TlUtils::format(" total energy derived from dummy atom = %18.16lf\n",
                        energy_from_dummy));
