...
TlSerializeData PdfUserInput::getSerializeData() const { return this->data_; }
...
void PdfUserInput::load_conventional() {
    ...
    std::string sKeyword = "";
    bool bNextValue =
        false;  // keywordを読み込み後、"="を読み込んだらtrue。それ以外false
    int nNumOfLine = 0;
    ...
    while (!ifs.eof()) {
        ...
        while (!sLine.empty()) {
            ...
            if (sKeyword == "") {
                ...
                // キーワード行
                // "=" とホワイトスペースの前まで取得
                std::string::size_type nKeyword =
                    sLine.find_first_of("= \f\n\r\t\v");
                ...
                if (bNextValue == false) {
                    ...
                    if (sLine[0] == '=') {
                        ...
                        // error
                        std::cerr << "could not found '='. line =" << nNumOfLine
                                  << ". stop." << std::endl;
                    ...
                    switch (bracketType) {
                        case '[': {
                            // "[" で始まるvalue は"]"
                            // まで格納する(改行を含まない)。
                            const std::string::size_type nEndBracket =
                                sLine.find_first_of(']');
                        ...
                        case '{': {
                            ...
                            // "{" で始まるvalue は"}"もしくは"}end"
                            // まで格納する(改行を含む)。
                            bool bFoundEndBracket = false;
                            do {
                                std::string::size_type nEndBracket =
                                    sLine.find_first_of("}");
                                if (nEndBracket != std::string::npos) {
                                    // 終端記号を発見した
                                    sValue = sLine.substr(1, nEndBracket - 1);
    ...
    if (this->data_["geometry/cartesian/input"].getStr() != "") {
        const std::string str =
            this->data_["geometry/cartesian/input"].getStr();
    ...
    if (this->data_["basis-set/density-auxiliary"].getStr() != "") {
        const std::string str =
            this->data_["basis-set/density-auxiliary"].getStr();
    ...
    if (this->data_["basis-set/exchange-auxiliary"].getStr() != "") {
        const std::string str =
            this->data_["basis-set/exchange-auxiliary"].getStr();
    ...
    while (in) {
        ...
        if ((tmp == "X") || (TlAtom::getElementNumber(tmp) != 0)) {
            ...
            this->log_.critical(TlUtils::format(
                "atom \"%s\" is not defined. stop.", tmp.c_str()));
            abort();
        }
        ...
        // 電荷
        if ((tmp.compare(0, 2, "//") == 0) || (tmp.compare(0, 1, "#") == 0)) {
            tmp = "";
        }
        if (!tmp.empty()) {
    ...
    while (in) {
        ...
        if (sLine[0] == '"') {
            ...
            if (nNameEnd != std::string::npos) {
                ...
                sLine = (nNameEnd + 1 < sLine.length())
                            ? sLine.substr(nNameEnd + 1)
                            : "";
            } else {
                std::cerr << "double quotation is not closed. stop."
                          << std::endl;
                abort();
            }
        } else {
...
void PdfUserInput::moleculeBasisSetDensityAuxiliary(const std::string& str) {
    ...
    while (in) {
        ...
        for (int i = 0; i < numOfCGTOs; ++i) {
            TlSerializeData cGTO;
            cGTO["shell_type"] =
                TlUtils::format("%c", flDbBasis.getrouShell(i));
...
void PdfUserInput::moleculeBasisSetExchangeAuxiliary(const std::string& str) {
    ...
    while (in) {
        ...
        for (int i = 0; i < numOfCGTOs; ++i) {
            TlSerializeData cGTO;
            cGTO["shell_type"] =
                TlUtils::format("%c", flDbBasis.getmyuShell(i));
            cGTO["scale_factor"] = 1.0;
...
bool PdfUserInput::check() {
    ...
    // xc-poteintial
    {
        ...
        if (sTilde == "") {
            if ((TlUtils::toUpper(this->data_["scf-memory-saving"].getStr()) ==
                 "NO")) {
                std::cout << " 'scf-memory-saving = yes' overridded."
                          << std::endl;
            }
