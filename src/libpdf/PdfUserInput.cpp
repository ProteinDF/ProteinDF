  ...
  while (!ifs.eof()) {
    ...
    while (!sLine.empty()) {
      ...
      if ((sLine.compare(0, 2, "//") == 0) ||
          (sLine.compare(0, 2, "--") == 0) || (sLine.compare(0, 1, "#") == 0)) {
        sLine = "";
        break;
      }
      ...
      if (sKeyword == "") {
        ...
        if (bNextValue == false) {
          ...
          switch (bracketType) {
            ...
            case '{': {
              ...
              if (bFoundEndBracket == false) {
                ...
                std::cerr << "could not found '}'. line =" << nStartBracketLine
                          << ". stop." << std::endl;
                abort();
              }
            } break;
          ...
          // store value
          // std::cerr << "g = " << sGroup << ", k = " << sKeyword << ", v = "
          // << sValue << std::endl;
...
void PdfUserInput::molecule_geometry_cartesian_input(const std::string& str) {
  ...
  while (in) {
    ...
    // コメント行を除く
    if ((sLine.compare(0, 2, "//") == 0) || (sLine.compare(0, 1, "#") == 0)) {
      continue;
    }
    ...
    if ((tmp == "X") || (TlAtom::getElementNumber(tmp) != 0)) {
      ...
      this->log_.critical(
          TlUtils::format("atom \"%s\" is not defined. stop.", tmp.c_str()));
      abort();
    }
  ...
  while (in) {
    ...
    if (sLine[0] == '"') {
      ...
      if (nNameEnd != std::string::npos) {
        ...
        sLine =
            (nNameEnd + 1 < sLine.length()) ? sLine.substr(nNameEnd + 1) : "";
      } else {
