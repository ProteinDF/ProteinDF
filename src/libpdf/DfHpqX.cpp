...
void DfHpqX::getForce(const TlSymmetricMatrix& P, TlMatrix* pForce,
  ...
  for (int shellTypeP = maxShellType - 1; shellTypeP >= 0; --shellTypeP) {
    ...
    for (int shellTypeQ = maxShellType - 1; shellTypeQ >= 0; --shellTypeQ) {
      ...
      for (std::size_t p = 0; p < shellArraySizeP; ++p) {
        ...
        this->getForce_partProc(this->orbitalInfo_, shellTypeP, shellTypeQ,
                                shellIndexP, shellArrayQ, P, pForce,
                                pForce_Xonly);
      }
    }
  }
...
void DfHpqX::getForce_partProc(const TlOrbitalInfoObject& orbitalInfo,
                               ...
                               const TlMatrixObject& P, TlMatrix* pForce_woX,
                               TlMatrix* pForce_Xonly) {
...
std::vector<double> DfHpqX::getESP(const TlMatrixObject& P,
  ...
  while (hasTask == true) {
    ...
    hasTask = pTaskCtrl->getQueue2(this->orbitalInfo_, isCutoffByDistribution,
        ...
        for (int stepP = 0; stepP < maxStepsP; ++stepP) {
          ...
          for (int stepQ = 0; stepQ < maxStepsQ; ++stepQ) {
            ...
            if ((shellIndexP != shellIndexQ) ||
                (globalShellIndexP >= globalShellIndexQ)) {
              const double coef =
                  (globalShellIndexP != globalShellIndexQ) ? 2.0 : 1.0;
              esp += coef * P.get(globalShellIndexP, globalShellIndexQ) *
                     this->pEngines_[threadID].WORK_NUC[index];
              // std::cout << TlUtils::format("(%d, %d): % f * % f * % f => %
              // f",
              //                              globalShellIndexP,
              //                              globalShellIndexQ, coef,
              //                              this->pEngines_[threadID].WORK_NUC[index],
              //                              P.get(globalShellIndexP,
              //                              globalShellIndexQ), esp)
              //           << std::endl;
            }
...
#pragma omp critical(DfHpqX__getESP)
      ...
      //                 for (std::size_t r = 0; r < numOfGrids; ++r) {
      //                     const TlPosition& posR = grids[r];
      //                     this->pEngines_[threadID].calc(query, posP, posQ,
      //                     pgtosP, pgtosQ, posR);
      //
      //                     double esp = 0.0;
      //                     int index = 0;
      ...
      //                         shellIndexP + stepP; for (int stepQ = 0; stepQ
      //                         < maxStepsQ; ++stepQ) {
      ...
      //                                 globalShellIndexQ) ? 2.0 : 1.0; esp +=
      //                                 coef * P.get(globalShellIndexP,
      //                                 globalShellIndexQ) *
      //                                 this->pEngines_[threadID].WORK_NUC[index];
      //                             }
      //                             ++index;
