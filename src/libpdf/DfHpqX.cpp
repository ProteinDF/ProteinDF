...
void DfHpqX::getHpq(TlDenseSymmetricMatrix_Lapack* pHpq,
    ...
    while (hasTask == true) {
        ...
        hasTask = pTaskCtrl->getQueue2(this->orbitalInfo_, true,
...
void DfHpqX::getHpq_part(const TlOrbitalInfoObject& orbitalInfo,
                         ...
                         const std::vector<TlAtom>& Xs, TlMatrixObject* pHpq,
...
#pragma omp for schedule(runtime)
        for (int i = 0; i < taskListSize; ++i) {
            ...
            this->pEngines_[threadID].calc(query, posP, posQ, pgtosP, pgtosQ,
                                           Cs, Xs);
            ...
            for (int stepP = 0; stepP < maxStepsP; ++stepP) {
                ...
                for (int stepQ = 0; stepQ < maxStepsQ; ++stepQ) {
                    ...
                    if ((shellIndexP != shellIndexQ) ||
                        ...
                        const double value =
                            coef * (this->pEngines_[threadID].WORK_KIN[index] +
                                    this->pEngines_[threadID].WORK_NUC[index]);
                        pHpq->add(globalShellIndexP, globalShellIndexQ, value);

                        const double valueFromDummyCharge =
                            coef * this->pEngines_[threadID].WORK_NUCX[index];
                        pHpq2->add(globalShellIndexP, globalShellIndexQ,
                                   valueFromDummyCharge);
                    }
                    ++index;
                }
            }
    ...
    for (int shellTypeP = maxShellType - 1; shellTypeP >= 0; --shellTypeP) {
        ...
        for (int shellTypeQ = maxShellType - 1; shellTypeQ >= 0; --shellTypeQ) {
            ...
            for (std::size_t p = 0; p < shellArraySizeP; ++p) {
                ...
                this->getForce_partProc(this->orbitalInfo_, shellTypeP,
                                        shellTypeQ, shellIndexP, shellArrayQ, P,
                                        pForce, pForce_Xonly);
            }
        }
    }
...
#pragma omp for schedule(runtime)
        for (std::size_t q = 0; q < shellArraySizeQ; ++q) {
            ...
            // 運動エネルギー
            this->pEngines_[threadID].calcKineticPart(queryPQ10, posP, posQ,
                                                      pgtosP, pgtosQ);
            {
                int index = 0;
                for (int stepP = 0; stepP < maxStepsP; ++stepP) {
                    ...
                    for (int stepQ = 0; stepQ < maxStepsQ; ++stepQ) {
                        ...
                        const double dKin_dA =
                            this->pEngines_[threadID].WORK_KIN[index];
...
DfHpqEngine::PGTOs DfHpqX::getPGTOs(const index_type shellIndex) {
    const int numOfContractions =
        this->orbitalInfo_.getCgtoContraction(shellIndex);
    DfHpqEngine::PGTOs pgtos(numOfContractions);

    for (int i = 0; i < numOfContractions; ++i) {
        const DfHpqEngine::PGTO pgto(
            this->orbitalInfo_.getCoefficient(shellIndex, i),
...
std::vector<double> DfHpqX::getESP(const TlMatrixObject& P,
    ...
    std::vector<DfTaskCtrl::Task2> taskList;
    bool hasTask =
        pTaskCtrl->getQueue2(this->orbitalInfo_, isCutoffByDistribution,
                             this->grainSize_, &taskList, true);
    while (hasTask == true) {
        this->getESP_part(this->orbitalInfo_, taskList, P, grids, &values);
...
#pragma omp for schedule(runtime)
        for (int i = 0; i < taskListSize; ++i) {
            ...
            for (std::size_t r = 0; r < numOfGrids; ++r) {
                ...
                this->pEngines_[threadID].calc(query, posP, posQ, pgtosP,
                                               pgtosQ, posR);
                ...
                for (int stepP = 0; stepP < maxStepsP; ++stepP) {
                    ...
                    for (int stepQ = 0; stepQ < maxStepsQ; ++stepQ) {
                        ...
                        if ((shellIndexP != shellIndexQ) ||
                            (globalShellIndexP >= globalShellIndexQ)) {
                            const double coef =
                                (globalShellIndexP != globalShellIndexQ) ? 2.0
                                                                         : 1.0;
                            esp += coef *
                                   P.get(globalShellIndexP, globalShellIndexQ) *
                                   this->pEngines_[threadID].WORK_NUC[index];
                            // std::cout << TlUtils::format("(%d, %d): % f * % f
                            // * % f => % f",
                            //                              globalShellIndexP,
                            //                              globalShellIndexQ,
                            //                              coef,
                            //                              this->pEngines_[threadID].WORK_NUC[index],
                            //                              P.get(globalShellIndexP,
                            //                              globalShellIndexQ),
                            //                              esp)
                            //           << std::endl;
                        }
...
#pragma omp critical(DfHpqX__getESP)
            ...
            //                 for (std::size_t r = 0; r < numOfGrids; ++r) {
            //                     const TlPosition& posR = grids[r];
            //                     this->pEngines_[threadID].calc(query, posP,
            //                     posQ, pgtosP, pgtosQ, posR);
            //
            //                     double esp = 0.0;
            //                     int index = 0;
            //                     for (int stepP = 0; stepP < maxStepsP;
            //                     ++stepP) {
            //                         const index_type globalShellIndexP =
            //                         shellIndexP + stepP; for (int stepQ = 0;
            //                         stepQ < maxStepsQ; ++stepQ) {
            //                             const index_type globalShellIndexQ =
            //                             shellIndexQ + stepQ;
            //
            //                             esp += 2.0 * P.get(globalShellIndexP,
            //                             globalShellIndexQ) *
            //                             this->pEngines_[threadID].WORK_NUC[index];
            //                             ++index;
            //                         }
            ...
            //                             shellIndexQ + stepQ;
            //
            //                             if (globalShellIndexP >=
            //                             globalShellIndexQ)
            //                             {
            //                                 double coef = (globalShellIndexP
            //                                 != globalShellIndexQ) ? 2.0
            //                                 : 1.0; esp += coef *
            //                                 P.get(globalShellIndexP,
            //                                 globalShellIndexQ) *
            //                                 this->pEngines_[threadID].WORK_NUC[index];
