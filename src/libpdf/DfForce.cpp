...
void DfForce::calcForce() {
  ...
  TlSymmetricMatrix P =
      this->getPpqMatrix<TlSymmetricMatrix>(runType, iteration);
...
void DfForce::calcForceFromPureXC(const RUN_TYPE runType) {
  ...
  // for RKS
  const TlSymmetricMatrix P =
      0.5 * this->getPpqMatrix<TlSymmetricMatrix>(runType, iteration);
  ...
  if (chk < TOO_SMALL) {
    if (rot.get(0, 0) > TOO_SMALL) {
      if (rot.get(1, 1) > TOO_SMALL) {
        // x, y != 0
        const double det =
            rot.get(0, 0) * rot.get(1, 1) - rot.get(0, 1) * rot.get(1, 0);
        ...
        // x, z != 0
        const double det =
            rot.get(0, 0) * rot.get(2, 2) - rot.get(0, 2) * rot.get(2, 0);
        const double inv_det = 1.0 / det;
      ...
      if (rot.get(2, 2) > TOO_SMALL) {
        // y, z != 0
        const double det =
            rot.get(2, 2) * rot.get(1, 1) - rot.get(2, 1) * rot.get(1, 2);
        const double inv_det = 1.0 / det;
        const double trp = rot.get(2, 2);
    ...
    for (int JP = 0; JP <= IP; ++JP) {
      ...
      for (int IC = 0; IC < 3; ++IC) {
        ...
        for (int JC = 0; JC < JEND; ++JC) {
            ...
            for (int IB = 0; IB < 3; ++IB) {
              ...
              for (int JA = 0; JA < 3; ++JA) {
                for (int JB = 0; JB < 3; ++JB) {
                  ...
                  sum += TENS[IA][IB][IC] * TENS[JA][JB][JC] * rot.get(IA, JA) *
                         X[IP][IB] * X[JP][JB];
                  // this->log_.info(TlUtils::format("rot(%d,%d)=% e,
                  // X1(%d,%d)=% e, X2(%d,%d)=% e",
                  //                                 IA,JA,rot.get(IA, JA),
                  //                                 IP,IB,X[IP][IB],
                  //                                 JP,JB,X[JP][JB]));
          ...
          // this->log_.info(TlUtils::format("IP=%d, JP=%d, IC=%d, JC=%d, SUM=%
          // e",
          //                                 IP, JP, IC, JC, sum));
