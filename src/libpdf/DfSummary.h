...
void DfSummary::exec() {
    ...
    switch (this->m_nMethodType) {
        case METHOD_RKS: {
            ...
            const MatrixType C = this->getCMatrix<MatrixType>(
            ...
            if (this->m_bIsXCFitting == true) {
                myu.load(
                    this->getMyuPath(DfObject::RUN_RKS, this->m_nIteration));
            }

            TlDenseVector_Lapack nyu;
            if (this->m_sXCFunctional == "xalpha") {
                nyu.load(
                    this->getNyuPath(DfObject::RUN_RKS, this->m_nIteration));
            }
        ...
        case METHOD_ROKS: {
            ...
            // this->loggerStartTitle("output guess.lcao.roks and
            // guess.occ.roks");
            ...
            if (this->m_bIsXCFitting == true) {
                myu.load(
                    this->getMyuPath(DfObject::RUN_ROKS, this->m_nIteration));
            }

            TlDenseVector_Lapack nyu;
            if (this->m_sXCFunctional == "xalpha") {
                nyu.load(
                    this->getNyuPath(DfObject::RUN_ROKS, this->m_nIteration));
            }
        ...
        case METHOD_UKS: {
            ...
            const MatrixType CB = this->getCMatrix<MatrixType>(
            ...
            const TlDenseVector_Lapack rhoA =
                this->getRho<TlDenseVector_Lapack>(DfObject::RUN_UKS_ALPHA,
                                                   this->m_nIteration);

            TlDenseVector_Lapack myuA;
            if (this->m_bIsXCFitting == true) {
                myuA.load(this->getMyuPath(DfObject::RUN_UKS_ALPHA,
                                           this->m_nIteration));
            }

            TlDenseVector_Lapack nyuA;
            if (this->m_sXCFunctional == "xalpha") {
                nyuA.load(this->getNyuPath(DfObject::RUN_UKS_ALPHA,
                                           this->m_nIteration));
            }

            const TlDenseVector_Lapack rhoB =
                this->getRho<TlDenseVector_Lapack>(DfObject::RUN_UKS_BETA,
                                                   this->m_nIteration);

            TlDenseVector_Lapack myuB;
            if (this->m_bIsXCFitting == true) {
                myuB.load(this->getMyuPath(DfObject::RUN_UKS_BETA,
                                           this->m_nIteration));
            }

            TlDenseVector_Lapack nyuB;
            if (this->m_sXCFunctional == "xalpha") {
                nyuB.load(this->getNyuPath(DfObject::RUN_UKS_BETA,
                                           this->m_nIteration));
            }
    ...
    for (int orderMO = 0; orderMO < numOfMOs; orderMO += 10) {
        ...
        for (int i = 0; i < numOfAOs; ++i) {
            this->logger(TlUtils::format(
                ...
                TlOrbitalInfoObject::basisTypeNameTbl_[orbInfo.getBasisType(
                    i)]));
