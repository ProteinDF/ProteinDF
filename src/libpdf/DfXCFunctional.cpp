...
void DfXCFunctional::buildXcMatrix() {
  ...
  switch (this->m_nMethodType) {
    ...
    case METHOD_UKS: {
      ...
      this->saveFxcMatrix<TlSymmetricMatrix>(RUN_UKS_ALPHA, this->m_nIteration,
                                             FxcA);
    ...
    case METHOD_ROKS: {
      ...
      TlSymmetricMatrix PBpq;
      if (this->m_bIsUpdateXC == true) {
        PApq = 0.5 * this->getDiffDensityMatrix<TlSymmetricMatrix>(
                         RUN_ROKS_CLOSED, this->m_nIteration);
        PBpq = PApq;
        PApq += this->getDiffDensityMatrix<TlSymmetricMatrix>(
            RUN_ROKS_OPEN, this->m_nIteration);
      } else {
        PApq = 0.5 * this->getPpqMatrix<TlSymmetricMatrix>(
                         RUN_ROKS_CLOSED, this->m_nIteration - 1);
        PBpq = PApq;
        PApq += this->getPpqMatrix<TlSymmetricMatrix>(RUN_ROKS_OPEN,
                                                      this->m_nIteration - 1);
      }
      ...
      this->saveFxcMatrix<TlSymmetricMatrix>(RUN_ROKS_ALPHA, this->m_nIteration,
                                             FxcA);
      this->saveFxcMatrix<TlSymmetricMatrix>(RUN_ROKS_BETA, this->m_nIteration,
                                             FxcB);
    } break;
...
void DfXCFunctional::checkGridAccuracy() {
  if (this->m_nXCFunctional != HF) {
    ...
    if (this->m_nMethodType == METHOD_RKS) {
      ...
      this->log_.info(
          TlUtils::format(" number of electrons(alpha) = % 16.10f (input: %d)",
                          rhoA, this->m_nNumOfAlphaElectrons));
      this->log_.info(
          TlUtils::format(" number of electrons(beta ) = % 16.10f (input: %d)",
...
double DfXCFunctional::getGrimmeDispersionEnergy() {
  if (DfXCFunctional::isCalcd_E_disp_ != true) {
    ...
    for (int i = 0; i < numOfAtoms; ++i) {
      ...
      if ((C6.find(symbolA) == C6.end()) || (R0.find(symbolA) == R0.end())) {
        this->logger(TlUtils::format(
            " Grimme dispersion parameters in atom(%s) is not regiestered.\n",
            symbolA.c_str()));
        continue;
      }
