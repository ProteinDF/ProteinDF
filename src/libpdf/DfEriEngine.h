...
///   [a = (a_x, a_y, a_z); angular momentum vector] (a_x + a_y + a_z; angular
///   momentum) [A; position vector] [alpha; exponent]
///
/// 制限値:
/// - angular momentum L
...
/// 基数が0であるため+1が必要-> (L+1)
#define ERI_NUM_OF_R_KINDS \
  (((ERI_L_MAX + 1) + 1) * ((ERI_L_MAX + 1) + 2) * ((ERI_L_MAX + 1) + 3) / 6)
...
#define ERI_NUM_OF_ERI_STATES                                    \
  ((ERI_A_BAR_MAX + 1) * (ERI_B_BAR_MAX + 1) * (ERI_A_MAX + 1) * \
   (ERI_B_MAX + 1) * (ERI_P_MAX + 1) * (ERI_A_PRIME_MAX + 1) *   \
   (ERI_B_PRIME_MAX + 1) * (ERI_P_PRIME_MAX + 1))
...
class DfEriEngine : public DfEngineObject {
 public:
  ...
  struct AngularMomentum2 {
    ...
    int index() const {
      return ((a_bar * (ERI_B_BAR_MAX + 1) + b_bar) * (ERI_A_MAX + 1) + a) *
                 (ERI_B_MAX + 1) +
             b;
    }

    static int maxIndex() {
      return (ERI_A_BAR_MAX + 1) * (ERI_B_BAR_MAX + 1) * (ERI_A_MAX + 1) *
             (ERI_B_MAX + 1);
    }
  ...
  class PGTO_Pair {
   public:
    ...
    PGTO_Pair(const int a, const double coefA, const double expA, const int b,
              const double coefB, const double expB, const TlPosition& A,
              const TlPosition& B)
 ...
 private:
  struct E4 {
   public:
    ...
    E4(const TlPosition& P, const TlPosition& Q, double sigma_P, double sigma_Q,
 ...
 private:
  struct ContractScale {
   public:
    ...
    int index() const {
      const int v =
          ((a_prime * ERI_B_PRIME_MAX) + b_prime) * ERI_P_PRIME_MAX + p_prime;
      return v;
    }
  ...
  struct ContractScale_cmp {
    bool operator()(const ContractScale& rhs1,
                    const ContractScale& rhs2) const {
  ...
  struct ContractState {
   public:
    explicit ContractState(
        ...
        const int ap = 0, const int bp = 0, const int pp = 0, const int cp = 0,
        const int dp = 0, const int qp = 0)
        : r_index(R.index()),
          r(R.angularMomentum()),
          a_prime(ap),
          b_prime(bp),
   ...
   public:
    ...
    std::string debugOut() const {
      return TlUtils::format("[%u(%u); (%u %u %u|%u %u %u)]", r, r_index,
                             a_prime, b_prime, p_prime, c_prime, d_prime,
                             q_prime);
    }
    ...
    int index() const {
      if (this->isCached_index_ != true) {
        this->cached_index_ =
            ((((((this->a_prime) * (ERI_B_PRIME_MAX + 1) + this->b_prime) *
                    (ERI_P_PRIME_MAX + 1) +
                this->p_prime) *
                   (ERI_A_PRIME_MAX + 1) +
               this->c_prime) *
 ...
 private:
  struct ERI_State {
   public:
    ...
    // for debug
    std::string debugOut() const {
      return TlUtils::format("(%d %d, %d %d %d, %d %d %d|", a_bar, b_bar, a, b,
                             p, a_prime, b_prime, p_prime);
    }

    int index() const {
      const int index =
          (((((((this->a_bar) * (ERI_B_BAR_MAX + 1) + this->b_bar) *
                   (ERI_A_MAX + 1) +
               this->a) *
                  (ERI_B_MAX + 1) +
              this->b) *
 ...
 private:
  ...
  void copyResultsToOutputBuffer(const AngularMomentum2& qAB,
                                 const AngularMomentum2& qCD, double* pOutput);
  ...
  ContractScalesVector choice(const AngularMomentum2& AB1,
                              const AngularMomentum2& AB2);

  void choice(const int a_bar, const int b_bar, const int a, const int b,
              const int p, const int a_prime, const int b_prime,
              const int p_prime, ContractScalesSet* pContractList);
  // int index_contract(const int a_prime, const int b_prime, const int p_prime)
  // const;

  ContractScalesVector transContractScales_SetToVector(
      const ContractScalesSet& contractScales);

  void contract(const AngularMomentum2& qAB, const AngularMomentum2& qCD,
  ...
  void contract_bra(const AngularMomentum2& qAB,
                    const TlAngularMomentumVector& r, const int a_prime,
                    const int b_prime, const int p_prime,
                    const int nR_dash_index);
  // void contract_ket(const AngularMomentum2& qCD,
  //                   const ContractState& cs, const std::vector<double>&
  //                   KQ_values);
  void get_contract_ket_coef_numerators(const int c_prime, const int d_prime,
                                        const int zeta_exp,
  ...
  // calc PQ
  void calcPQ(const AngularMomentum2& qAB, const AngularMomentum2& qCD,
              const ContractScalesVector& bra_contractScales,
              const ContractScalesVector& ket_contractScales);
  ...
  std::vector<int> get_csindex_for_calcPQ(
      const int angularMomentumP, const AngularMomentum2& qAB,
      const int angularMomentumQ, const AngularMomentum2& qCD,
      const ContractScalesVector& bra_contractScales,
      const ContractScalesVector& ket_contractScales);

  void transpose(const AngularMomentum2& qAB, const AngularMomentum2& qCD,
                 const ContractScalesVector& ket_contractScales);

  // ERI ---------------------------------------------------------------------
  void calcERI(const int a_bar, const int b_bar, const int a, const int b,
               const int p, const int a_prime, const int b_prime,
               const int p_prime, EriDataType* pERI);

  int index(const TlAngularMomentumVector& a_bar,
  ...
  // transform 6D to 5D
  void transform6Dto5D(const AngularMomentum2& qAB, const AngularMomentum2& qCD,
                       double* pOutput);
  ...
  void transform10Fto7F_i(const int I_, const int J_, const int K_,
                          const int L_, const int J, const int K, const int L,
                          const double* pInput, double* pOutput);
  void transform10Fto7F_j(const int I_, const int J_, const int K_,
                          const int L_, const int I, const int K, const int L,
                          const double* pInput, double* pOutput);
  void transform10Fto7F_k(const int I_, const int J_, const int K_,
                          const int L_, const int I, const int J, const int L,
                          const double* pInput, double* pOutput);
  void transform10Fto7F_l(const int I_, const int J_, const int K_,
                          const int L_, const int I, const int J, const int K,
                          const double* pInput, double* pOutput);
