...
void DfHpqEngine::calcOvp(const int a, const int b) {
  ...
  if (a > 0) {
    ...
    std::cerr << TlUtils::format(
                     "Program Error: @DfHpqEngine::calcOvp() a=%d, b=%d", a, b)
              << std::endl;
    abort();
  }
...
void DfHpqEngine::calcKin(const int a_bar, const int b_bar, const int a,
  ...
  if (a_bar > 0) {
    ...
    std::cerr << TlUtils::format(
                     "Program Error: @DfHpqEngine::calcKin() a=%d, b=%d", a, b)
              << std::endl;
    abort();
  }
...
void DfHpqEngine::calcNuc(const int a_bar, const int b_bar, const int a,
  ...
  if (a_bar > 0) {
    ...
    std::cerr << TlUtils::format(
                     "Program Error: @DfHpqEngine::calcNuc() a=%d, b=%d, m=%d",
                     a, b, m)
              << std::endl;
    abort();
  }
...
void DfHpqEngine::calcNucSS() {
  ...
  for (int batch = 0; batch < numOfBatches; ++batch) {
    ...
    const double ovpSS =
        this->OVP_[ovpStateIndex][ovpIndex][batch];  // 軌道係数はここに含まれる
    ...
    //         std::cerr << TlUtils::format("calcNucSS() coef=% f, charge=% f,
    //         sqrt_zeta=% f, ovp=% f",
    //                                      coef, charge, sqrt_zeta, ovpSS)
    //                   << std::endl;
    //         std::cerr << TlUtils::format("calcNucSS() zeta=% f, P(% f, %f,
    //         %f) C(% f, % f, % f)",
    //                                      zeta, P.x(), P.y(), P.z(),
    //                                      this->C_.x(), this->C_.y(),
    //                                      this->C_.z())
    ...
    for (int m = 0; m <= sumOfAngularMomentums; ++m) {
      ...
      //             std::cerr << TlUtils::format("calcNucSS() value=% f, m=%d,
      //             coef=% f, fmt=% f",
  ...
  for (int amvA_index = 0; amvA_index < numOfAmvsA; ++amvA_index) {
    ...
    for (int amvB_index = 0; amvB_index < numOfAmvsB; ++amvB_index) {
      ...
      for (int batch = 0; batch < numOfBatches; ++batch) {
        ...
        if (amvA2.isExist() == true) {
          answer +=
              invZeta2 * amvA1.get(i) * this->OVP_[state2_index][index2][batch];
        }

        // 3rd term
        if (amvB1.isExist() == true) {
          answer +=
              invZeta2 * amvB.get(i) * this->OVP_[state3_index][index3][batch];
        }
  ...
  for (int amvB_index = 0; amvB_index < numOfAmvsB; ++amvB_index) {
    ...
    for (int amvA_index = 0; amvA_index < numOfAmvsA; ++amvA_index) {
      ...
      for (int batch = 0; batch < numOfBatches; ++batch) {
        ...
        if (amvA1.isExist() == true) {
          answer +=
              invZeta2 * amvA.get(i) * this->OVP_[state2_index][index2][batch];
        }

        // 3rd term
        if (amvB2.isExist() == true) {
          answer +=
              invZeta2 * amvB1.get(i) * this->OVP_[state3_index][index3][batch];
        }
      ...
      for (int amvA_index = 0; amvA_index < numOfAmvsA; ++amvA_index) {
        ...
        for (int amvB_index = 0; amvB_index < numOfAmvsB; ++amvB_index) {
          ...
          for (int batch = 0; batch < numOfBatches; ++batch) {
            ...
            // 1st term
            {
              const double term1 = (this->P_[batch][i] - this->A_[i]) *
            ...
            // 2nd term
            {
              const double term2 = (this->P_[batch][i] - this->C_[i]) *
            ...
            if (amvA2.isExist() == true) {
              ...
              const double term3 =
                  coef * (this->NUC_[state31_index][index3][batch] -
            ...
            if (amvB1.isExist() == true) {
              ...
              answer += coef * (this->NUC_[state41_index][index4][batch] -
        ...
        for (int amvA_index = 0; amvA_index < numOfAmvsA; ++amvA_index) {
          ...
          for (int batch = 0; batch < numOfBatches; ++batch) {
            ...
            if (amvA1.isExist() == true) {
              ...
              answer += coef * (this->NUC_[state31_index][index3][batch] -
                                this->NUC_[state32_index][index3][batch]);
            }
