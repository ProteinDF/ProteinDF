...
void DfOverlapEngine::copyResultsToOutputBuffer() {
    ...
    for (int index_a_bar = 0; index_a_bar < amvsAbar.size(); ++index_a_bar) {
        ...
        for (int index_b_bar = 0; index_b_bar < amvsBbar.size();
            ...
            for (int index_c_bar = 0; index_c_bar < amvsCbar.size();
                 ++index_c_bar) {
                const TlAngularMomentumVector amvCbar =
                    amvsCbar.get(index_c_bar);

                for (int index_d_bar = 0; index_d_bar < amvsDbar.size();
                     ++index_d_bar) {
                    const TlAngularMomentumVector amvDbar =
                        amvsDbar.get(index_d_bar);

                    for (int index_a = 0; index_a < amvsA.size(); ++index_a) {
                        const TlAngularMomentumVector amvA = amvsA.get(index_a);

                        for (int index_b = 0; index_b < amvsB.size();
                             ++index_b) {
                            const TlAngularMomentumVector amvB =
                                amvsB.get(index_b);

                            for (int index_c = 0; index_c < amvsC.size();
                                 ++index_c) {
                                const TlAngularMomentumVector amvC =
                                    amvsC.get(index_c);

                                for (int index_d = 0; index_d < amvsD.size();
                                     ++index_d) {
                                    const TlAngularMomentumVector amvD =
                                        amvsD.get(index_d);

                                    const int stateAmvsIndex = this->index(
                                        amvAbar, amvBbar, amvCbar, amvDbar,
                                        amvA, amvB, amvC, amvD);
                                    assert(0 <= stateAmvsIndex);
                                    assert(stateAmvsIndex <
                                           static_cast<int>(
                                               this->OVP_[stateIndex].size()));
                                    assert(static_cast<int>(
                                               this->OVP_[stateIndex]
                                                         [stateAmvsIndex]
                                                             .size()) <= batch);

                                    double value = 0.0;
                                    for (int i = 0; i < batch; ++i) {
                                        value += this->OVP_[stateIndex]
                                                           [stateAmvsIndex][i];
                                    }
...
void DfOverlapEngine::EQ20A(const OvpState& state) {
    ...
    this->OVP_[stateIndex].resize(numOfAmvsAbar * numOfAmvsBbar *
                                  numOfAmvsCbar * numOfAmvsDbar * numOfAmvsA *
                                  numOfAmvsB * numOfAmvsC * numOfAmvsD);
    ...
    for (int amvAbar_index = 0; amvAbar_index < numOfAmvsAbar;
        ...
        for (int amvBbar_index = 0; amvBbar_index < numOfAmvsBbar;
            ...
            for (int amvCbar_index = 0; amvCbar_index < numOfAmvsCbar;
                 ++amvCbar_index) {
                const TlAngularMomentumVector amvCbar =
                    amvsCbar.get(amvCbar_index);

                for (int amvDbar_index = 0; amvDbar_index < numOfAmvsDbar;
                     ++amvDbar_index) {
                    const TlAngularMomentumVector amvDbar =
                        amvsDbar.get(amvDbar_index);

                    for (int amvA_index = 0; amvA_index < numOfAmvsA;
                         ++amvA_index) {
                        const TlAngularMomentumVector amvA =
                            amvsA.get(amvA_index);

                        const int i = this->initiative(amvA);
                        const TlAngularMomentumVector amvA1 =
                            amvA - this->E1_[i];
                        const TlAngularMomentumVector amvA2 =
                            amvA - this->E2_[i];
                        ...
                        for (int amvB_index = 0; amvB_index < numOfAmvsB;
                             ++amvB_index) {
                            const TlAngularMomentumVector amvB =
                                amvsB.get(amvB_index);
                            const TlAngularMomentumVector amvB1 =
                                amvB - this->E1_[i];

                            for (int amvC_index = 0; amvC_index < numOfAmvsC;
                                 ++amvC_index) {
                                const TlAngularMomentumVector amvC =
                                    amvsC.get(amvC_index);
                                const TlAngularMomentumVector amvC1 =
                                    amvC - this->E1_[i];

                                for (int amvD_index = 0;
                                     amvD_index < numOfAmvsD; ++amvD_index) {
                                    const TlAngularMomentumVector amvD =
                                        amvsD.get(amvD_index);
                                    const TlAngularMomentumVector amvD1 =
                                        amvD - this->E1_[i];

                                    // batch
                                    // -------------------------------------------------------
                                    const OvpState state1(aBar, bBar, cBar,
                                                          dBar, a - 1, b, c, d);
                                    const std::size_t state1_index =
                                        state1.index();
                                    const int index1 = this->index(
                                        amvAbar, amvBbar, amvCbar, amvDbar,
                                    ...
                                    if (amvA2.isExist() == true) {
                                        const OvpState state2(aBar, bBar, cBar,
                                                              dBar, a - 2, b, c,
                                                              d);
                                    ...
                                    if (amvB1.isExist() == true) {
                                        const OvpState state3(aBar, bBar, cBar,
                                                              dBar, a - 1,
                                                              b - 1, c, d);
                                    ...
                                    if (amvC1.isExist() == true) {
                                        const OvpState state4(aBar, bBar, cBar,
                                                              dBar, a - 1, b,
                                                              c - 1, d);
                                    ...
                                    if (amvD1.isExist() == true) {
                                        const OvpState state5(aBar, bBar, cBar,
                                                              dBar, a - 1, b, c,
                                                              d - 1);
                                    ...
                                    const int index = this->index(
                                        amvAbar, amvBbar, amvCbar, amvDbar,
                                        amvA, amvB, amvC, amvD);
                                    this->OVP_[stateIndex][index].resize(
                                        numOfBatches);
                                    for (int batch = 0; batch < numOfBatches;
                                         ++batch) {
                                        double answer = 0.0;
                                        ...
                                        // 1st term
                                        {
                                            answer += (this->G_[batch][i] -
                                                       this->A_[i]) *
                                                      this->OVP_[state1_index]
                                                                [index1][batch];
                                        }
                                        ...
                                        if (amvD1.isExist() == true) {
                                            answer += coef * amvD.get(i) *
                                                      this->OVP_[state5_index]
                                                                [index5][batch];
                                        }

                                        this->OVP_[stateIndex][index][batch] =
                                            answer;
                                    }
                                }
                            }
            ...
            for (int amvCbar_index = 0; amvCbar_index < numOfAmvsCbar;
                ...
                for (int amvDbar_index = 0; amvDbar_index < numOfAmvsDbar;
                    ...
                    for (int amvB_index = 0; amvB_index < numOfAmvsB;
                        ...
                        const TlAngularMomentumVector amvB1 =
                            amvB - this->E1_[i];
                        const TlAngularMomentumVector amvB2 =
                            amvB - this->E2_[i];
            ...
            for (int amvCbar_index = 0; amvCbar_index < numOfAmvsCbar;
                ...
                for (int amvDbar_index = 0; amvDbar_index < numOfAmvsDbar;
                    ...
                    for (int amvC_index = 0; amvC_index < numOfAmvsC;
                        ...
                        const TlAngularMomentumVector amvC1 =
                            amvC - this->E1_[i];
                        const TlAngularMomentumVector amvC2 =
                            amvC - this->E2_[i];
            ...
            for (int amvCbar_index = 0; amvCbar_index < numOfAmvsCbar;
                ...
                for (int amvDbar_index = 0; amvDbar_index < numOfAmvsDbar;
                    ...
                    for (int amvD_index = 0; amvD_index < numOfAmvsD;
                        ...
                        const TlAngularMomentumVector amvD1 =
                            amvD - this->E1_[i];
                        const TlAngularMomentumVector amvD2 =
                            amvD - this->E2_[i];
                        ...
                        for (int amvA_index = 0; amvA_index < numOfAmvsA;
                            ...
                            for (int amvB_index = 0; amvB_index < numOfAmvsB;
                                ...
                                for (int amvC_index = 0;
                                     amvC_index < numOfAmvsC; ++amvC_index) {
                                    const TlAngularMomentumVector amvC =
                                        amvsC.get(amvC_index);
                                    const TlAngularMomentumVector amvC1 =
                                        amvC - this->E1_[i];
            ...
            for (int amvCbar_index = 0; amvCbar_index < numOfAmvsCbar;
                ...
                for (int amvDbar_index = 0; amvDbar_index < numOfAmvsDbar;
                    ...
                    for (int amvA_index = 0; amvA_index < numOfAmvsA;
                        ...
                        const TlAngularMomentumVector amvA1 =
                            amvA - this->E1_[i];
                        const TlAngularMomentumVector amvA1p =
                            amvA + this->E1_[i];
                        ...
                        for (int amvB_index = 0; amvB_index < numOfAmvsB;
                            ...
                            for (int amvC_index = 0; amvC_index < numOfAmvsC;
                                ...
                                for (int amvD_index = 0;
                                    ...
                                    // batch
                                    // -------------------------------------------------------
                                    const OvpState state1(aBar - 1, bBar, cBar,
                                                          dBar, a + 1, b, c, d);
                                    const std::size_t state1_index =
                                        state1.index();
                                    const int index1 = this->index(
                                        amvAbar1, amvBbar, amvCbar, amvDbar,
                                        amvA1p, amvB, amvC, amvD);

                                    std::size_t state2_index = 0;
                                    int index2 = 0;
                                    if (amvA1.isExist() == true) {
                                        const OvpState state2(aBar - 1, bBar,
                                                              cBar, dBar, a - 1,
                                                              b, c, d);
                                        state2_index = state2.index();
                                        index2 = this->index(
                ...
                for (int amvDbar_index = 0; amvDbar_index < numOfAmvsDbar;
                    ...
                    for (int amvA_index = 0; amvA_index < numOfAmvsA;
                        ...
                        for (int amvB_index = 0; amvB_index < numOfAmvsB;
                            ...
                            const TlAngularMomentumVector amvB1 =
                                amvB - this->E1_[i];
                            const TlAngularMomentumVector amvB1p =
                                amvB + this->E1_[i];
                            ...
                            for (int amvC_index = 0; amvC_index < numOfAmvsC;
                                ...
                                for (int amvD_index = 0;
                                    ...
                                    // batch
                                    // -------------------------------------------------------
                                    const OvpState state1(aBar, bBar - 1, cBar,
                                                          dBar, a, b + 1, c, d);
                                    const std::size_t state1_index =
                                        state1.index();
                                    const int index1 = this->index(
                                        amvAbar, amvBbar1, amvCbar, amvDbar,
                                        amvA, amvB1p, amvC, amvD);

                                    std::size_t state2_index = 0;
                                    int index2 = 0;
                                    if (amvB1.isExist() == true) {
                                        const OvpState state2(aBar, bBar - 1,
                                                              cBar, dBar, a,
                                                              b - 1, c, d);
                                        state2_index = state2.index();
                                        index2 = this->index(
                    ...
                    for (int amvA_index = 0; amvA_index < numOfAmvsA;
                        ...
                        for (int amvB_index = 0; amvB_index < numOfAmvsB;
                            ...
                            for (int amvC_index = 0; amvC_index < numOfAmvsC;
                                ...
                                const TlAngularMomentumVector amvC1 =
                                    amvC - this->E1_[i];
                                const TlAngularMomentumVector amvC1p =
                                    amvC + this->E1_[i];
                                ...
                                for (int amvD_index = 0;
                                    ...
                                    // batch
                                    // -------------------------------------------------------
                                    const OvpState state1(aBar, bBar, cBar - 1,
                                                          dBar, a, b, c + 1, d);
                                    const std::size_t state1_index =
                                        state1.index();
                                    const int index1 = this->index(
                                        amvAbar, amvBbar, amvCbar1, amvDbar,
                                        amvA, amvB, amvC1p, amvD);

                                    std::size_t state2_index = 0;
                                    int index2 = 0;
                                    if (amvC1.isExist() == true) {
                                        const OvpState state2(aBar, bBar,
                                                              cBar - 1, dBar, a,
                                                              b, c - 1, d);
                                        state2_index = state2.index();
                                        index2 = this->index(
        ...
        for (int amvBbar_index = 0; amvBbar_index < numOfAmvsBbar;
            ...
            for (int amvCbar_index = 0; amvCbar_index < numOfAmvsCbar;
                ...
                for (int amvDbar_index = 0; amvDbar_index < numOfAmvsDbar;
                    ...
                    const TlAngularMomentumVector amvDbar1 =
                        amvDbar - this->E1_[i];
                        ...
                        for (int amvB_index = 0; amvB_index < numOfAmvsB;
                            ...
                            for (int amvC_index = 0; amvC_index < numOfAmvsC;
                                ...
                                for (int amvD_index = 0;
                                    ...
                                    const TlAngularMomentumVector amvD1 =
                                        amvD - this->E1_[i];
                                    const TlAngularMomentumVector amvD1p =
                                        amvD + this->E1_[i];
                                    ...
                                    const std::size_t state1_index =
                                        state1.index();
                                    const int index1 = this->index(
                                        amvAbar, amvBbar, amvCbar, amvDbar1,
                                        amvA, amvB, amvC, amvD1p);
