...
const double DfOverlapEngine::MAX_EXPONENT =
    std::log(std::numeric_limits<double>::max());

/// 結果出力用のサイズ
/// f軌道の10Fから7Fへの変換領域にも利用するため、
/// f軌道のサイズを10Fベースとして確保している。
/// = 3 * 3 * 3 * 3 * 10(f) * 10 * 10 * 10
const int DfOverlapEngine::OUTPUT_BUFFER_SIZE =
    3 * 3 * 3 * 3 * 10 * 10 * 10 * 10 + 1;
...
void DfOverlapEngine::calc(const int diff1, const TlOrbitalInfoObject& orbInfo1,
  ...
  const Query query(diff1, diff2, diff3, diff4, shellType1, shellType2,
                    shellType3, shellType4);
  this->calc0(query, pos1, pos2, pos3, pos4, pgtos1, pgtos2, pgtos3, pgtos4);
}

void DfOverlapEngine::calc0(const Query& query, const TlPosition& A,
                            const TlPosition& B, const TlPosition& C,
                            const TlPosition& D, const PGTOs& PGTOsA,
                            const PGTOs& PGTOsB, const PGTOs& PGTOsC,
                            const PGTOs& PGTOsD) {
  ...
  const int numOfBatches =
      numOfPGTOsA * numOfPGTOsB * numOfPGTOsC * numOfPGTOsD;
      ...
      for (int indexC = 0; indexC < numOfPGTOsC; ++indexC) {
        ...
        for (int indexD = 0; indexD < numOfPGTOsD; ++indexD) {
          ...
          if (kappa_exp < DfOverlapEngine::MAX_EXPONENT) {
            ...
            const double sss =
                coefA * coefB * coefC * coefD * base * std::sqrt(base) * kappa;
  ...
  this->calc(this->a_bar_, this->b_bar_, this->c_bar_, this->d_bar_, this->a_,
             this->b_, this->c_, this->d_);

  this->copyResultsToOutputBuffer();
  this->transform6Dto5D();
}

DfOverlapEngine::PGTOs DfOverlapEngine::getPGTOs(
    const TlOrbitalInfoObject& orbitalInfo, const int shellIndex) {
...
void DfOverlapEngine::calc(const int a_bar, const int b_bar, const int c_bar,
                           const int d_bar, const int a, const int b,
                           const int c, const int d) {
  // std::cerr << TlUtils::format("DfOverlapEngine::calc() (%d %d %d, %d, %d,
  // %d)",
  //                              a_bar, b_bar, c_bar,
  //                              a, b, c)
  //           << std::endl;
  if ((a_bar < 0) || (b_bar < 0) || (c_bar < 0) || (d_bar < 0) || (a < 0) ||
      (b < 0) || (c < 0) || (d < 0)) {
    return;
  }
  ...
  if (a_bar > 0) {
    ...
    std::cerr << TlUtils::format(
                     "Program Error: @DfOverlapEngine::calc() a=%d, b=%d, c=%d",
                     a, b, c)
              << std::endl;
    abort();
  }
...
int DfOverlapEngine::index(const TlAngularMomentumVector& amvAbar,
  ...
  const int index = ((((((amvAbar.index() * bBars + amvBbar.index()) * cBars +
                         amvCbar.index()) *
                            dBars +
                        amvDbar.index()) *
                           as +
  ...
  for (int index_a_bar = 0; index_a_bar < amvsAbar.size(); ++index_a_bar) {
    ...
    for (int index_b_bar = 0; index_b_bar < amvsBbar.size(); ++index_b_bar) {
      ...
      for (int index_c_bar = 0; index_c_bar < amvsCbar.size(); ++index_c_bar) {
        ...
        for (int index_d_bar = 0; index_d_bar < amvsDbar.size();
             ++index_d_bar) {
              ...
              for (int index_c = 0; index_c < amvsC.size(); ++index_c) {
                ...
                for (int index_d = 0; index_d < amvsD.size(); ++index_d) {
                  ...
                  const int stateAmvsIndex =
                      this->index(amvAbar, amvBbar, amvCbar, amvDbar, amvA,
                                  amvB, amvC, amvD);
                  assert(0 <= stateAmvsIndex);
                  assert(stateAmvsIndex <
                         static_cast<int>(this->OVP_[stateIndex].size()));
                  assert(static_cast<int>(
                             this->OVP_[stateIndex][stateAmvsIndex].size()) <=
                         batch);
...
void DfOverlapEngine::transform6Dto5D() {
  ...
  if (a == 2) {
    this->transform6Dto5D_i(I_, J_, K_, L_, J, K, L, this->WORK,
                            this->pTransformBuf_);
    I = 5;
    const std::size_t end = I_ * J_ * K_ * L_ * I * J * K * L;
    assert(end <= static_cast<std::size_t>(OUTPUT_BUFFER_SIZE));
    std::copy(this->pTransformBuf_, this->pTransformBuf_ + end, this->WORK);
  }
  if (b == 2) {
    this->transform6Dto5D_j(I_, J_, K_, L_, I, K, L, this->WORK,
                            this->pTransformBuf_);
    J = 5;
    const std::size_t end = I_ * J_ * K_ * L_ * I * J * K * L;
    assert(end <= static_cast<std::size_t>(OUTPUT_BUFFER_SIZE));
  ...
  if (c == 2) {
    this->transform6Dto5D_k(I_, J_, K_, L_, I, J, L, this->WORK,
                            this->pTransformBuf_);
    K = 5;
    const std::size_t end = I_ * J_ * K_ * L_ * I * J * K * L;
    assert(end <= static_cast<std::size_t>(OUTPUT_BUFFER_SIZE));
  ...
  if (d == 2) {
    this->transform6Dto5D_l(I_, J_, K_, L_, I, J, K, this->WORK,
                            this->pTransformBuf_);
    L = 5;
    const std::size_t end = I_ * J_ * K_ * L_ * I * J * K * L;
    assert(end <= static_cast<std::size_t>(OUTPUT_BUFFER_SIZE));
  ...
  if (a == 3) {
    this->transform10Fto7F_i(I_, J_, K_, L_, J, K, L, this->WORK,
                             this->pTransformBuf_);
    I = 7;
    const std::size_t end = I_ * J_ * K_ * L_ * I * J * K * L;
    assert(end <= static_cast<std::size_t>(OUTPUT_BUFFER_SIZE));
  ...
  if (b == 3) {
    this->transform10Fto7F_j(I_, J_, K_, L_, I, K, L, this->WORK,
                             this->pTransformBuf_);
    J = 7;
    const std::size_t end = I_ * J_ * K_ * L_ * I * J * K * L;
    assert(end <= static_cast<std::size_t>(OUTPUT_BUFFER_SIZE));
  ...
  if (c == 3) {
    this->transform10Fto7F_k(I_, J_, K_, L_, I, J, L, this->WORK,
                             this->pTransformBuf_);
    K = 7;
    const std::size_t end = I_ * J_ * K_ * L_ * I * J * K * L;
    assert(end <= static_cast<std::size_t>(OUTPUT_BUFFER_SIZE));
  ...
  if (d == 3) {
    this->transform10Fto7F_l(I_, J_, K_, L_, I, J, K, this->WORK,
                             this->pTransformBuf_);
    L = 7;
    const std::size_t end = I_ * J_ * K_ * L_ * I * J * K * L;
    assert(end <= static_cast<std::size_t>(OUTPUT_BUFFER_SIZE));
...
void DfOverlapEngine::transform6Dto5D_i(const int I_, const int J_,
                                        const int K_, const int L_, const int J,
...
void DfOverlapEngine::transform6Dto5D_j(const int I_, const int J_,
                                        const int K_, const int L_, const int I,
                                        const int K, const int L,
                                        const double* pInput, double* pOutput) {
  for (int i_ = 0; i_ < I_; ++i_) {
    for (int j_ = 0; j_ < J_; ++j_) {
...
void DfOverlapEngine::transform6Dto5D_k(const int I_, const int J_,
                                        const int K_, const int L_, const int I,
...
void DfOverlapEngine::transform6Dto5D_l(const int I_, const int J_,
                                        const int K_, const int L_, const int I,
...
void DfOverlapEngine::transform10Fto7F_i(const int I_, const int J_,
                                         const int K_, const int L_,
...
void DfOverlapEngine::transform10Fto7F_j(const int I_, const int J_,
                                         const int K_, const int L_,
...
void DfOverlapEngine::transform10Fto7F_k(const int I_, const int J_,
                                         const int K_, const int L_,
...
void DfOverlapEngine::transform10Fto7F_l(const int I_, const int J_,
                                         const int K_, const int L_,
...
void DfOverlapEngine::EQ20A(const OvpState& state) {
  ...
  this->OVP_[stateIndex].resize(numOfAmvsAbar * numOfAmvsBbar * numOfAmvsCbar *
                                numOfAmvsDbar * numOfAmvsA * numOfAmvsB *
                                numOfAmvsC * numOfAmvsD);
  ...
  for (int amvAbar_index = 0; amvAbar_index < numOfAmvsAbar; ++amvAbar_index) {
    ...
    for (int amvBbar_index = 0; amvBbar_index < numOfAmvsBbar;
      ...
      for (int amvCbar_index = 0; amvCbar_index < numOfAmvsCbar;
           ++amvCbar_index) {
        const TlAngularMomentumVector amvCbar = amvsCbar.get(amvCbar_index);

        for (int amvDbar_index = 0; amvDbar_index < numOfAmvsDbar;
             ++amvDbar_index) {
          ...
          for (int amvA_index = 0; amvA_index < numOfAmvsA; ++amvA_index) {
            ...
            for (int amvB_index = 0; amvB_index < numOfAmvsB; ++amvB_index) {
              ...
              for (int amvC_index = 0; amvC_index < numOfAmvsC; ++amvC_index) {
                ...
                for (int amvD_index = 0; amvD_index < numOfAmvsD;
                     ++amvD_index) {
                  ...
                  const int index1 =
                      this->index(amvAbar, amvBbar, amvCbar, amvDbar, amvA1,
                                  amvB, amvC, amvD);

                  std::size_t state2_index = 0;
                  int index2 = 0;
                  if (amvA2.isExist() == true) {
                    const OvpState state2(aBar, bBar, cBar, dBar, a - 2, b, c,
                                          d);
                    state2_index = state2.index();
                    index2 = this->index(amvAbar, amvBbar, amvCbar, amvDbar,
                                         amvA2, amvB, amvC, amvD);
                  }

                  std::size_t state3_index = 0;
                  int index3 = 0;
                  if (amvB1.isExist() == true) {
                    const OvpState state3(aBar, bBar, cBar, dBar, a - 1, b - 1,
                                          c, d);
                    state3_index = state3.index();
                    index3 = this->index(amvAbar, amvBbar, amvCbar, amvDbar,
                                         amvA1, amvB1, amvC, amvD);
                  }

                  std::size_t state4_index = 0;
                  int index4 = 0;
                  if (amvC1.isExist() == true) {
                    const OvpState state4(aBar, bBar, cBar, dBar, a - 1, b,
                                          c - 1, d);
                    state4_index = state4.index();
                    index4 = this->index(amvAbar, amvBbar, amvCbar, amvDbar,
                                         amvA1, amvB, amvC1, amvD);
                  }

                  std::size_t state5_index = 0;
                  int index5 = 0;
                  if (amvD1.isExist() == true) {
                    const OvpState state5(aBar, bBar, cBar, dBar, a - 1, b, c,
                                          d - 1);
                    state5_index = state5.index();
                    index5 = this->index(amvAbar, amvBbar, amvCbar, amvDbar,
                                         amvA1, amvB, amvC, amvD1);
                  }
              ...
              for (int amvC_index = 0; amvC_index < numOfAmvsC; ++amvC_index) {
                ...
                for (int amvD_index = 0; amvD_index < numOfAmvsD;
                  ...
                  if (amvA1.isExist() == true) {
                    const OvpState state2(aBar, bBar, cBar, dBar, a - 1, b - 1,
                                          c, d);
                    state2_index = state2.index();
                    index2 = this->index(amvAbar, amvBbar, amvCbar, amvDbar,
                                         amvA1, amvB1, amvC, amvD);
                  ...
                  if (amvB2.isExist() == true) {
                    const OvpState state3(aBar, bBar, cBar, dBar, a, b - 2, c,
                                          d);
                    state3_index = state3.index();
                    index3 = this->index(amvAbar, amvBbar, amvCbar, amvDbar,
                                         amvA, amvB2, amvC, amvD);
                  }

                  std::size_t state4_index = 0;
                  int index4 = 0;
                  if (amvC1.isExist() == true) {
                    const OvpState state4(aBar, bBar, cBar, dBar, a, b - 1,
                                          c - 1, d);
                    state4_index = state4.index();
                    index4 = this->index(amvAbar, amvBbar, amvCbar, amvDbar,
                                         amvA, amvB1, amvC1, amvD);
                  }

                  std::size_t state5_index = 0;
                  int index5 = 0;
                  if (amvD1.isExist() == true) {
                    const OvpState state5(aBar, bBar, cBar, dBar, a, b - 1, c,
                                          d - 1);
                    state5_index = state5.index();
                    index5 = this->index(amvAbar, amvBbar, amvCbar, amvDbar,
                                         amvA, amvB1, amvC, amvD1);
                  }
                  ...
                  for (int batch = 0; batch < numOfBatches; ++batch) {
                    ...
                    // 1st term
                    {
                      answer += (this->G_[batch][i] - this->B_[i]) *
                                this->OVP_[state1_index][index1][batch];
                    }

                    // 2nd term
                    if (amvA1.isExist() == true) {
                      answer += coef * amvA.get(i) *
                                this->OVP_[state2_index][index2][batch];
                    }

                    // 3rd term
                    if (amvB2.isExist() == true) {
                      answer += coef * amvB1.get(i) *
                                this->OVP_[state3_index][index3][batch];
                    }
            ...
            for (int amvA_index = 0; amvA_index < numOfAmvsA; ++amvA_index) {
              ...
              for (int amvB_index = 0; amvB_index < numOfAmvsB; ++amvB_index) {
                ...
                for (int amvD_index = 0; amvD_index < numOfAmvsD;
                  ...
                  int index2 = 0;
                  if (amvA1.isExist() == true) {
                    const OvpState state2(aBar, bBar, cBar, dBar, a - 1, b,
                                          c - 1, d);
                    state2_index = state2.index();
                    index2 = this->index(amvAbar, amvBbar, amvCbar, amvDbar,
                                         amvA1, amvB, amvC1, amvD);
                  ...
                  int index3 = 0;
                  if (amvB1.isExist() == true) {
                    const OvpState state3(aBar, bBar, cBar, dBar, a, b - 1,
                                          c - 1, d);
                    state3_index = state3.index();
                    index3 = this->index(amvAbar, amvBbar, amvCbar, amvDbar,
                                         amvA, amvB1, amvC1, amvD);
                  ...
                  if (amvC2.isExist() == true) {
                    const OvpState state4(aBar, bBar, cBar, dBar, a, b, c - 2,
                                          d);
                    state4_index = state4.index();
                    index4 = this->index(amvAbar, amvBbar, amvCbar, amvDbar,
                                         amvA, amvB, amvC2, amvD);
                  }

                  std::size_t state5_index = 0;
                  int index5 = 0;
                  if (amvD1.isExist() == true) {
                    const OvpState state5(aBar, bBar, cBar, dBar, a, b, c - 1,
                                          d - 1);
                    state5_index = state5.index();
                    index5 = this->index(amvAbar, amvBbar, amvCbar, amvDbar,
                                         amvA, amvB, amvC1, amvD1);
                  }
                  ...
                  for (int batch = 0; batch < numOfBatches; ++batch) {
                    ...
                    // 1st term
                    {
                      answer += (this->G_[batch][i] - this->C_[i]) *
                                this->OVP_[state1_index][index1][batch];
                    }
          ...
          for (int amvD_index = 0; amvD_index < numOfAmvsD; ++amvD_index) {
            ...
            for (int amvA_index = 0; amvA_index < numOfAmvsA; ++amvA_index) {
              ...
              for (int amvB_index = 0; amvB_index < numOfAmvsB; ++amvB_index) {
                ...
                for (int amvC_index = 0; amvC_index < numOfAmvsC;
                     ++amvC_index) {
                  const TlAngularMomentumVector amvC = amvsC.get(amvC_index);
                  const TlAngularMomentumVector amvC1 = amvC - this->E1_[i];
                  ...
                  int index2 = 0;
                  if (amvA1.isExist() == true) {
                    const OvpState state2(aBar, bBar, cBar, dBar, a - 1, b, c,
                                          d - 1);
                    state2_index = state2.index();
                    index2 = this->index(amvAbar, amvBbar, amvCbar, amvDbar,
                                         amvA1, amvB, amvC, amvD1);
                  ...
                  int index3 = 0;
                  if (amvB1.isExist() == true) {
                    const OvpState state3(aBar, bBar, cBar, dBar, a, b - 1, c,
                                          d - 1);
                    state3_index = state3.index();
                    index3 = this->index(amvAbar, amvBbar, amvCbar, amvDbar,
                                         amvA, amvB1, amvC, amvD1);
                  ...
                  int index4 = 0;
                  if (amvC1.isExist() == true) {
                    const OvpState state4(aBar, bBar, cBar, dBar, a, b, c - 1,
                                          d - 1);
                    state4_index = state4.index();
                    index4 = this->index(amvAbar, amvBbar, amvCbar, amvDbar,
                                         amvA, amvB, amvC1, amvD1);
                  ...
                  if (amvD2.isExist() == true) {
                    const OvpState state5(aBar, bBar, cBar, dBar, a, b, c,
                                          d - 2);
                    state5_index = state5.index();
                    index5 = this->index(amvAbar, amvBbar, amvCbar, amvDbar,
                                         amvA, amvB, amvC, amvD2);
                  }
                  ...
                  for (int batch = 0; batch < numOfBatches; ++batch) {
                    ...
                    // 1st term
                    {
                      answer += (this->G_[batch][i] - this->D_[i]) *
                                this->OVP_[state1_index][index1][batch];
                    }
            ...
            for (int amvB_index = 0; amvB_index < numOfAmvsB; ++amvB_index) {
              ...
              for (int amvC_index = 0; amvC_index < numOfAmvsC; ++amvC_index) {
                ...
                for (int amvD_index = 0; amvD_index < numOfAmvsD;
                  ...
                  // batch
                  // -------------------------------------------------------
                  const OvpState state1(aBar - 1, bBar, cBar, dBar, a + 1, b, c,
                                        d);
                  const std::size_t state1_index = state1.index();
                  const int index1 =
                      this->index(amvAbar1, amvBbar, amvCbar, amvDbar, amvA1p,
                                  amvB, amvC, amvD);

                  std::size_t state2_index = 0;
                  int index2 = 0;
                  if (amvA1.isExist() == true) {
                    const OvpState state2(aBar - 1, bBar, cBar, dBar, a - 1, b,
                                          c, d);
                    state2_index = state2.index();
                    index2 = this->index(amvAbar1, amvBbar, amvCbar, amvDbar,
                                         amvA1, amvB, amvC, amvD);
                  }
                  ...
                  for (int batch = 0; batch < numOfBatches; ++batch) {
                    ...
                    if (amvA1.isExist() == true) {
                      answer -=
                          amvA.get(i) * this->OVP_[state2_index][index2][batch];
                    }
            ...
            for (int amvB_index = 0; amvB_index < numOfAmvsB; ++amvB_index) {
              ...
              for (int amvC_index = 0; amvC_index < numOfAmvsC; ++amvC_index) {
                ...
                for (int amvD_index = 0; amvD_index < numOfAmvsD;
                  ...
                  // batch
                  // -------------------------------------------------------
                  const OvpState state1(aBar, bBar - 1, cBar, dBar, a, b + 1, c,
                                        d);
                  const std::size_t state1_index = state1.index();
                  const int index1 =
                      this->index(amvAbar, amvBbar1, amvCbar, amvDbar, amvA,
                                  amvB1p, amvC, amvD);

                  std::size_t state2_index = 0;
                  int index2 = 0;
                  if (amvB1.isExist() == true) {
                    const OvpState state2(aBar, bBar - 1, cBar, dBar, a, b - 1,
                                          c, d);
                    state2_index = state2.index();
                    index2 = this->index(amvAbar, amvBbar1, amvCbar, amvDbar,
                                         amvA, amvB1, amvC, amvD);
                  }
                  ...
                  for (int batch = 0; batch < numOfBatches; ++batch) {
                    ...
                    if (amvB1.isExist() == true) {
                      answer -=
                          amvB.get(i) * this->OVP_[state2_index][index2][batch];
                    }
            ...
            for (int amvB_index = 0; amvB_index < numOfAmvsB; ++amvB_index) {
              ...
              for (int amvC_index = 0; amvC_index < numOfAmvsC; ++amvC_index) {
                ...
                for (int amvD_index = 0; amvD_index < numOfAmvsD;
                  ...
                  // batch
                  // -------------------------------------------------------
                  const OvpState state1(aBar, bBar, cBar - 1, dBar, a, b, c + 1,
                                        d);
                  const std::size_t state1_index = state1.index();
                  const int index1 =
                      this->index(amvAbar, amvBbar, amvCbar1, amvDbar, amvA,
                                  amvB, amvC1p, amvD);

                  std::size_t state2_index = 0;
                  int index2 = 0;
                  if (amvC1.isExist() == true) {
                    const OvpState state2(aBar, bBar, cBar - 1, dBar, a, b,
                                          c - 1, d);
                    state2_index = state2.index();
                    index2 = this->index(amvAbar, amvBbar, amvCbar1, amvDbar,
                                         amvA, amvB, amvC1, amvD);
                  }
                  ...
                  for (int batch = 0; batch < numOfBatches; ++batch) {
                    ...
                    // 1st term
                    answer += 2.0 * this->zetaC_[batch] *
                              this->OVP_[state1_index][index1][batch];

                    // 2nd term
                    if (amvC1.isExist() == true) {
                      answer -=
                          amvC.get(i) * this->OVP_[state2_index][index2][batch];
                    }
            ...
            for (int amvB_index = 0; amvB_index < numOfAmvsB; ++amvB_index) {
              ...
              for (int amvC_index = 0; amvC_index < numOfAmvsC; ++amvC_index) {
                ...
                for (int amvD_index = 0; amvD_index < numOfAmvsD;
                  ...
                  // batch
                  // -------------------------------------------------------
                  const OvpState state1(aBar, bBar, cBar, dBar - 1, a, b, c,
                                        d + 1);
                  const std::size_t state1_index = state1.index();
                  const int index1 =
                      this->index(amvAbar, amvBbar, amvCbar, amvDbar1, amvA,
                                  amvB, amvC, amvD1p);

                  std::size_t state2_index = 0;
                  int index2 = 0;
                  if (amvD1.isExist() == true) {
                    const OvpState state2(aBar, bBar, cBar, dBar - 1, a, b, c,
                                          d - 1);
                    state2_index = state2.index();
                    index2 = this->index(amvAbar, amvBbar, amvCbar, amvDbar1,
                                         amvA, amvB, amvC, amvD1);
                  }
                  ...
                  for (int batch = 0; batch < numOfBatches; ++batch) {
                    ...
                    // 1st term
                    answer += 2.0 * this->zetaD_[batch] *
                              this->OVP_[state1_index][index1][batch];

                    // 2nd term
                    if (amvD1.isExist() == true) {
                      answer -=
                          amvD.get(i) * this->OVP_[state2_index][index2][batch];
                    }
