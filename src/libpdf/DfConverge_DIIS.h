...
class DfConverge_DIIS : public DfConverge_Damping {
 ...
 protected:
  ...
  template <class MatrixType, class SymmetricMatrixType>
  TlDenseGeneralMatrix_BLAS_old buildBMatrix(const RUN_TYPE runType, const int itr,
                                         int last);

  template <class MatrixType, class SymmetricMatrixType>
  TlDenseGeneralMatrix_BLAS_old getCoef(const RUN_TYPE runType, const int itr,
                                    int last);
...
MatrixType DfConverge_DIIS::getResidual(const RUN_TYPE runType, const int itr) {
...
/// B行列を作成する
template <class MatrixType, class SymmetricMatrixType>
TlDenseGeneralMatrix_BLAS_old DfConverge_DIIS::buildBMatrix(const RUN_TYPE runType,
                                                        const int startItr,
                                                        const int cycles) {
  TlDenseGeneralMatrix_BLAS_old B(cycles + 1, cycles + 1);
  for (int i = 0; i < cycles; ++i) {
    ...
    for (int j = 0; j < i; ++j) {
      const MatrixType r_j = this->getResidual<MatrixType, SymmetricMatrixType>(
          runType, startItr + j);
...
/// 係数cを求める
template <class MatrixType, class SymmetricMatrixType>
TlDenseGeneralMatrix_BLAS_old DfConverge_DIIS::getCoef(const RUN_TYPE runType,
                                                   const int startItr,
                                                   int cycles) {
  const TlDenseGeneralMatrix_BLAS_old B =
      this->buildBMatrix<MatrixType, SymmetricMatrixType>(runType, startItr,
                                                          cycles);
  assert(B.getNumOfCols() == cycles + 1);

  TlDenseGeneralMatrix_BLAS_old y(cycles + 1, 1);
  for (int i = 0; i < cycles; ++i) {
    y.set(i, 0, 0.0);
  }
  y.set(cycles, 0, -1.0);
...
//         // 残差ベクトルを求める
//         MatrixType r = this->getResidual<MatrixType,
//         SymmetricMatrixType>(runType, itr -1); // 一つ前
...
//         if (maxError < 0.1) {
//             int diis_e_max_pass_itr =
//             (*this->pPdfParam_)["DIIS_e_max_pass_itr"].getInt(); if
//             (diis_e_max_pass_itr == -1) {
//                 (*this->pPdfParam_)["DIIS_e_max_pass_itr"] = itr;
//                 diis_e_max_pass_itr = itr;
//             }
//             this->log_.info(TlUtils::format("e_max pass itr = %d",
//             diis_e_max_pass_itr));

//             int last = this->numOfLastItems_;
//             const int startItr = std::max(diis_e_max_pass_itr, itr - last
//             +1); // 現itrを追加すると+1 last = std::min(last, itr - startItr
//             +1); this->log_.info(TlUtils::format("itr=%d, start=%d, last=%d",
//                                             itr, startItr, last));

//             // 係数行列の取得
//             const TlDenseGeneralMatrix_BLAS_old c = this->getCoef<MatrixType,
...
//                 const std::string inFilePath = DfObject::getRhoPath(runType,
//                 startItr +i); assert(TlFile::isExist(inFilePath));
...
void DfConverge_DIIS::convergeKSMatrix(const DfObject::RUN_TYPE runType) {
  this->log_.info(TlUtils::format("converge KS matrix using DIIS. #%d",
                                  this->m_nIteration));
  const int itr = this->m_nIteration;
  ...
  // backup
  {
    const std::string path =
        this->getFpqMatrixPath(runType, this->m_nIteration);
  ...
  if (itr >= this->startIterationOfDIIS_) {
    // 残差ベクトルを求める
    MatrixType r =
        this->getResidual<MatrixType, SymmetricMatrixType>(runType, itr);
    ...
    if (maxError < 0.1) {
      int diis_e_max_pass_itr =
          (*this->pPdfParam_)["DIIS_e_max_pass_itr"].getInt();
      if (diis_e_max_pass_itr == -1) {
        (*this->pPdfParam_)["DIIS_e_max_pass_itr"] = itr;
        diis_e_max_pass_itr = itr;
      }
      this->log_.info(
          TlUtils::format("e_max pass itr = %d", diis_e_max_pass_itr));
      ...
      for (int i = 0; i < last; ++i) {
        const std::string inFilePath =
            DfObject::getFpqMatrixPath(runType, startItr + i) + ".diis";
        assert(TlFile::isExistFile(inFilePath));
        Fi.load(inFilePath);

        const double coef = c.get(i, 0);
        this->log_.info(
            TlUtils::format("%d-th coef=% 8.3f", startItr + i, coef));
        newF += coef * Fi;
      }
...
void DfConverge_DIIS::convergePMatrix(const DfObject::RUN_TYPE runType) {
  this->log_.info(TlUtils::format("converge dennsity matrix using DIIS. #%d/%d",
                                  this->m_nIteration,
  ...
  if (itr >= this->startIterationOfDIIS_) {
    // 残差ベクトルを求める
    MatrixType r =
        this->getResidual<MatrixType, SymmetricMatrixType>(runType, itr - 1);
    ...
    if (maxError < 0.1) {
      ...
      for (int i = 0; i < last; ++i) {
        const std::string inFilePath =
            DfObject::getPpqMatrixPath(runType, startItr + i - 1);
        assert(TlFile::isExistFile(inFilePath));
        Pi.load(inFilePath);

        const double coef = c.get(i, 0);
        this->log_.info(
            TlUtils::format("%d-th coef=% 8.3f", startItr + i - 1, coef));
        newP += coef * Pi;
      }
    ...
    this->log_.info(
        TlUtils::format("the iteration, %d, is lower than start number, %d.",
                        itr, this->startIterationOfDIIS_));
  }
