...
Fl_Db_Basis::Fl_Db_Basis(const std::string& sBasisName)
    : m_sBasisName(sBasisName) {
    this->setData();
}
...
void Fl_Db_Basis::setData() {
    ...
    if (isNormalTerminate == false) {
        CnErr.abort(
            TlUtils::format("The orbital basis set \'%s\' is not found. Please "
                            "check Basis set name.",
                            this->m_sBasisName.c_str()));
    }
}

void Fl_Db_Basis::read_cGTOs(const int numOfCGTOs, const std::string& shellType,
    ...
    for (int i = 0; i < numOfCGTOs; ++i) {
        ...
        for (int j = 0; j < GtoNum; ++j) {
            ...
            if (parts.size() >= 2) {
                tmp_cgto.contraction[j].coefficient =
