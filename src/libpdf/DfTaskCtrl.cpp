...
bool DfTaskCtrl::getQueue(const std::size_t maxIndeces,
                          ...
                          std::vector<std::size_t>* pTasks, bool initialize) {
    assert(pTasks != NULL);
    ...
    this->log_.info(
        TlUtils::format("progress: %ld/%ld %.1f%%", currentIndex, maxIndeces,
                        100.0 * (double(currentIndex) / double(maxIndeces))));
...
bool DfTaskCtrl::getQueue2(const TlOrbitalInfoObject& orbitalInfo,
    ...
    if (initialize == true) {
        ...
        if (isCutoffByDistribution == true) {
            ...
            progress = 0;
            progress_all =
                this->getTotalCalcAmount2(orbitalInfo, shellArrayTable);
            progress_stage = 0;
            division = progress_all * 0.1;
        }
                    ...
                    if (grainSize >= maxGrainSize) {
                        if (progress >= (progress_stage * division)) {
                            ...
                            this->log_.info(TlUtils::format(
                                "progress: %3.0f%% done.", progressLevel));
...
bool DfTaskCtrl::getQueue2(const TlOrbitalInfoObject& orbitalInfo1,
    ...
    if (initialize == true) {
        ...
        if (isCutoffByDistribution == true) {
            ...
            // setup progress status
            progress = 0;
            progress_all = this->getTotalCalcAmount2(orbitalInfo1, orbitalInfo2,
                                                     shellArrayTable1,
                                                     shellArrayTable2, dct);
            progress_stage = 0;
            division = progress_all * 0.1;
            // pre-screening report
            this->prescreeningReport();
        ...
        for (; shellTypeR >= 0;) {
            ...
            for (; prIndex < numOfShellPairArray_PR;) {
                const index_type shellIndexP =
                    shellPairArray_PR[prIndex].shellIndex1;
                const index_type shellIndexR =
                    shellPairArray_PR[prIndex].shellIndex2;
                task.shellIndex1 = shellIndexP;
                    ...
                    for (; shellTypeS >= 0;) {
                        ...
                        for (; shellArrayIndexQ < shellArraySizeQ;) {
                            ...
                            const index_type maxShellIndexS =
                                (shellIndexP == shellIndexR) ? shellIndexQ
                                                             : shellIndexR;
                            ShellArray::const_iterator sItEnd =
                                std::upper_bound(shellArrayS.begin(),
                                                 shellArrayS.end(),
                                                 maxShellIndexS);
                            ...
                            for (; shellArrayIndexS < shellArraySizeS;) {
                                const index_type shellIndexS =
                                    shellArrayS[shellArrayIndexS];
                                task.shellIndex4 = shellIndexS;

                                // schwarz cutoff
                                const int shellQuartetType =
                                    this->getShellQuartetType(
                                        orbitalInfo, orbitalInfo, shellTypeP,
                                        shellTypeQ, shellTypeR, shellTypeS);
                                const bool isAlive =
                                    this->isAliveBySchwarzCutoff(
                                        shellIndexP, shellIndexQ, shellIndexR,
                                        shellIndexS, shellQuartetType,
                                        schwarzTable, this->cutoffThreshold_);
                                ++shellArrayIndexS;
                                progress += maxStepsP * maxStepsQ * maxStepsR *
                                            maxStepsS;
                                if (isAlive == true) {
                                    pTaskList->push_back(task);
                                    ++grainSize;
                                    ...
                                    if (grainSize >= maxGrainSize) {
                                        if (progress >=
                                            (progress_stage * division)) {
                                            const double progressLevel =
                                                (double)progress /
                                                (double)progress_all * 100.0;
                                            this->log_.info(TlUtils::format(
                                                "progress: %3.0f%% done.",
                                                progressLevel));
...
bool DfTaskCtrl::getQueue4(const TlOrbitalInfoObject& orbitalInfo_PQ,
    ...
    if (initialize == true) {
        ...
        progress = 0;
        progress_all =
            this->getTotalCalcAmount4(orbitalInfo_PQ, orbitalInfo_RS,
                                      shellPairArrayTable, dct_PQ, dct_RS);
        progress_stage = 0;
        division = progress_all * 0.1;
                    ...
                    for (; shellTypeS >= 0;) {
                        ...
                        for (; shellArrayIndexQ < shellArraySizeQ;) {
                            ...
                            for (; shellArrayIndexS < shellArraySizeS;) {
                                ...
                                const int shellQuartetType =
                                    this->getShellQuartetType(
                                        orbitalInfo_PQ, orbitalInfo_RS,
                                        shellTypeP, shellTypeQ, shellTypeR,
                                        shellTypeS);
                                const bool isAlive =
                                    this->isAliveBySchwarzCutoff(
                                        shellIndexP, shellIndexQ, shellIndexR,
    ...
    for (index_type index_p = 0; index_p < numOfRowIndexes; ++index_p) {
        ...
        for (index_type index_r = 0; index_r < numOfColIndexes; ++index_r) {
            ...
            const double max_P_pr = std::max<double>(
                1.0E-20, this->getMaxValue(P, shellIndexP, maxStepsP,
            ...
            for (index_type index_q = 0; index_q < numOfRowIndexes; ++index_q) {
                ...
                for (index_type index_s = 0; index_s < numOfColIndexes;
                     ++index_s) {
        ...
        for (; shellTypeR >= 0;) {
            ...
            for (; prIndex < numOfShellPairArray_PR;) {
                ...
                for (; shellTypeQ >= 0;) {
                    const int maxStepsQ = 2 * shellTypeQ + 1;
                    const ShellArray shellArrayQ =
                        this->selectShellArrayByDistribution(
                            shellArrayTable[shellTypeQ], shellIndexP,
                            orbitalInfo);
                    ShellArray::const_iterator qItEnd = std::upper_bound(
                        shellArrayQ.begin(), shellArrayQ.end(), shellIndexP);
                    const std::size_t shellArraySizeQ =
                    ...
                    for (; shellTypeS >= 0;) {
                        ...
                        const ShellArray shellArrayS =
                            this->selectShellArrayByDistribution(
                                shellArrayTable[shellTypeS], shellIndexR,
                                orbitalInfo);
                        ...
                        for (; shellArrayIndexQ < shellArraySizeQ;) {
                            ...
                            // if (std::binary_search(rowIndexes.begin(),
                            // rowIndexes.end(), shellIndexQ) != true) {
                            //     continue;
                            // }
                            const double max_P_qr =
                                this->getMaxValue(P, shellIndexQ, maxStepsQ,
                                                  shellIndexR, maxStepsR);
                            const double max_P_pqr =
                                std::max(max_P_pr, max_P_qr);
                            ...
                            for (; shellArrayIndexS < shellArraySizeS;) {
                                ...
                                // if (std::binary_search(colIndexes.begin(),
                                // colIndexes.end(), shellIndexS) != true) {
                                //     continue;
                                // }
                                const double max_P_ps =
                                    this->getMaxValue(P, shellIndexP, maxStepsP,
                                                      shellIndexS, maxStepsS);
                                const double max_P_qs =
                                    this->getMaxValue(P, shellIndexQ, maxStepsQ,
                                                      shellIndexS, maxStepsS);

                                // schwarz cutoff
                                const double max_P_pqs =
                                    std::max(max_P_ps, max_P_qs);
                                const double max_P_pqrs =
                                    std::max(max_P_pqr, max_P_pqs);
                                const double cutoffThreshold =
                                    this->cutoffThreshold_ / max_P_pqrs;
                                const int shellQuartetType =
                                    this->getShellQuartetType(
                    ...
                    for (; shellTypeS >= 0;) {
                        ...
                        for (; shellArrayIndexQ < shellArraySizeQ;) {
                            ...
                            for (; shellArrayIndexS < shellArraySizeS;) {
                                ...
                                const bool isAlive =
                                    this->isAliveBySchwarzCutoff(
                                        shellIndexP, shellIndexQ, shellIndexP,
                                        shellIndexQ, shellQuartetType,
                                        schwarzTable, this->cutoffThreshold_);
                                ++shellArrayIndexS;
                                if (isAlive == true) {
                                    pTaskList->push_back(task);
...
DfTaskCtrl::ShellPairArrayTable DfTaskCtrl::getShellPairArrayTable(
    ...
    for (int shellTypeP = maxShellType1 - 1; shellTypeP >= 0; --shellTypeP) {
        ...
        for (int shellTypeR = maxShellType2 - 1; shellTypeR >= 0;
             --shellTypeR) {
            const ShellArray& shellArrayR = shellArrayTable2[shellTypeR];
            ShellArray::const_iterator rItEnd = shellArrayR.end();

            const int shellPairType_PR =
                shellTypeP * maxShellType1 + shellTypeR;
            for (ShellArray::const_iterator pIt = shellArrayP.begin();
                 pIt != pItEnd; ++pIt) {
                const index_type indexP = *pIt;
...
DfTaskCtrl::ShellPairArrayTable DfTaskCtrl::selectShellPairArrayTableByDensity(
    ...
    for (int shellPairType = 0; shellPairType < maxShellPairType;
        ...
        const ShellPairArray& shellPairArray =
            inShellPairArrayTable[shellPairType];
        const std::size_t shellPairArraySize = shellPairArray.size();
        ShellPairArray tmp;
        tmp.reserve(shellPairArraySize);

        for (std::size_t shellPairIndex = 0;
             shellPairIndex < shellPairArraySize; ++shellPairIndex) {
            const index_type shellIndexA =
                shellPairArray[shellPairIndex].shellIndex1;
            const index_type shellIndexB =
                shellPairArray[shellPairIndex].shellIndex2;
            ...
            for (int pgtoIndexA = 0; pgtoIndexA < numOfContractionsA;
                ...
                for (int pgtoIndexB = 0; pgtoIndexB < numOfContractionsB;
                    ...
                    const double zetaB =
                        orbitalInfo.getExponent(shellIndexB, pgtoIndexB);
                    ...
                    for (int i = 0; i < 6; ++i) {
                        ...
                        const double term1 =
                            CONTRIBUTE_COEF / (std::sqrt(zetaA_B) * param);
                        const double term2 =
                            std::exp(-zetaAB * gamma[i] * AB2 / param);
                        const double T =
                            zetaAB * zetaAB * AB2 / (zetaA_B * param);
                        double term3 = 0.0;
    ...
    for (int shellPairType = 0; shellPairType < maxShellPairType;
        ...
        for (std::size_t shellPairIndex = 0;
            ...
            for (int pgtoIndexA = 0; pgtoIndexA < numOfContractionsA;
                ...
                const double zetaA =
                    orbitalInfo1.getExponent(shellIndexA, pgtoIndexA);
...
TlSparseSymmetricMatrix DfTaskCtrl::makeSchwarzTable(
    ...
    for (index_type shellIndexP = 0; shellIndexP < maxShellIndex;) {
        ...
        for (index_type shellIndexQ = 0; shellIndexQ < maxShellIndex;) {
            ...
            pEngine->calc(0, orbitalInfo, shellIndexP, 0, orbitalInfo,
...
void DfTaskCtrl::prescreeningReport() {
    ...
    if (hasCutoff_density == true) {
        ...
        this->log_.info(TlUtils::format("epsilon(density): %e",
                                        this->cutoffEpsilon_density_));
...
void DfTaskCtrl::cutoffReport() {
    ...
    if (hasCutoffSchwarz == true) {
        ...
        this->log_.info(
            TlUtils::format("threshold: %e", this->cutoffThreshold_));
        this->log_.info("type: alive / all (ratio)");
        for (int shellTypeA = 0; shellTypeA < maxShellType; ++shellTypeA) {
            for (int shellTypeB = 0; shellTypeB < maxShellType; ++shellTypeB) {
                const int shellTypeAB = shellTypeA * maxShellType + shellTypeB;
                for (int shellTypeC = 0; shellTypeC < maxShellType;
                    ...
                    for (int shellTypeD = 0; shellTypeD < maxShellType;
                        ...
                        if (this->cutoffAll_schwarz_[shellTypeABCD] > 0) {
                            const double ratio =
                                (double)this
                                    ->cutoffAlive_schwarz_[shellTypeABCD] /
                                (double)this
                                    ->cutoffAll_schwarz_[shellTypeABCD] *
                                100.0;
                            this->log_.info(TlUtils::format(
                                " %4s: %12ld / %12ld (%6.2f%%)",
...
std::size_t DfTaskCtrl::getTotalCalcAmount2(
    ...
    for (int shellTypeP = 0; shellTypeP < maxShellType; ++shellTypeP) {
        ...
        for (int shellTypeQ = 0; shellTypeQ < maxShellType; ++shellTypeQ) {
            ...
            for (std::size_t shellArrayIndexP = 0;
                 shellArrayIndexP < shellArraySizeP; ++shellArrayIndexP) {
                const index_type shellIndexP = shellArrayP[shellArrayIndexP];
    ...
    for (int shellTypeP = 0; shellTypeP < maxShellType; ++shellTypeP) {
        ...
        for (int shellTypeQ = 0; shellTypeQ < maxShellType; ++shellTypeQ) {
            ...
            for (std::size_t shellArrayIndexP = 0;
                ...
                DistributedCutoffTable::const_iterator dctItQ =
                    dct.find(shellIndexP);
    ...
    for (int shellTypeP = 0; shellTypeP < maxShellType; ++shellTypeP) {
        ...
        for (int shellTypeR = 0; shellTypeR < maxShellType; ++shellTypeR) {
            ...
            for (std::size_t prIndex = 0; prIndex < numOfShellPairArray_PR;
                ...
                for (int shellTypeQ = 0; shellTypeQ < maxShellType;
                     ++shellTypeQ) {
                    const int maxStepsQ = 2 * shellTypeQ + 1;

                    // const ShellArray shellArrayQ =
                    // dct[shellIndexP][shellTypeQ];
                    ...
                    for (int shellTypeS = 0; shellTypeS < maxShellType;
                         ++shellTypeS) {
                        const int maxStepsS = 2 * shellTypeS + 1;
                        ...
                        const ShellArray shellArrayS =
                            dctItR->second[shellTypeS];

                        for (std::size_t shellArrayIndexQ = 0;
                             shellArrayIndexQ < shellArraySizeQ;
                             ++shellArrayIndexQ) {
                            const index_type shellIndexQ =
                                shellArrayQ[shellArrayIndexQ];
                            ...
                            const std::size_t shellArraySizeS =
                                std::distance(shellArrayS.begin(), sItEnd);
                            answer += shellArraySizeS * maxStepsP * maxStepsQ *
                                      maxStepsR * maxStepsS;
                        }
                    }
                }
    ...
    for (int shellTypeP = 0; shellTypeP < maxShellType_PQ; ++shellTypeP) {
        ...
        for (int shellTypeR = 0; shellTypeR < maxShellType_RS; ++shellTypeR) {
            ...
            for (std::size_t prIndex = 0; prIndex < numOfShellPairArray_PR;
                ...
                for (int shellTypeQ = 0; shellTypeQ < maxShellType_PQ;
                     ++shellTypeQ) {
                    const int maxStepsQ = 2 * shellTypeQ + 1;
                    ...
                    for (int shellTypeS = 0; shellTypeS < maxShellType_RS;
                         ++shellTypeS) {
                        const int maxStepsS = 2 * shellTypeS + 1;
