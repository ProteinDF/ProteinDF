...
Fl_Geometry::Fl_Geometry(const Fl_Geometry& rhs)
    : filePath_(rhs.filePath_),
      isUpdate_(false),
      atoms_(rhs.atoms_),
      isCalcdTotalCharge_(false) {}

Fl_Geometry::~Fl_Geometry() {
    if ((this->filePath_ != "") && (this->isUpdate_ == true)) {
        this->save();
    }
}
...
void Fl_Geometry::save() const {
    ...
    if (fs.is_open() != true) {
        CnErr.abort(TlUtils::format(
            "could not open file \"%s\" at Fl_Geometry::write()",
    ...
    for (int i = 0; i < size; ++i) {
        ...
        fs << TlUtils::format("%-2s %+16.10e [%s] %+16.10e %+16.10e %+16.10e\n",
                              symbol.c_str(), charge, label.c_str(), x, y, z);
    }
...
void Fl_Geometry::setup(const TlSerializeData& geomData) {
    if (geomData.hasKey("groups")) {
        TlSerializeData::MapConstIterator subGroupEnd =
            geomData["groups"].endMap();
        for (TlSerializeData::MapConstIterator subGroup =
                 geomData["groups"].beginMap();
             subGroup != subGroupEnd; ++subGroup) {
            this->setup(subGroup->second);
        }
    }

    if (geomData.hasKey("atoms")) {
        TlSerializeData::ArrayConstIterator atomEnd =
            geomData["atoms"].endArray();
        for (TlSerializeData::ArrayConstIterator atom =
                 geomData["atoms"].beginArray();
             atom != atomEnd; ++atom) {
            AtomData ad;
...
int Fl_Geometry::getAtomKindNumber() const {
    ...
    for (std::vector<AtomData>::const_iterator p = this->atoms_.begin();
         p != pEnd; ++p) {
...
int Fl_Geometry::getNumOfAtoms() const { return this->atoms_.size(); }
