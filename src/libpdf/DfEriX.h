...
class DfEriX : public DfObject {
   ...
   public:
    ...
    void getJ(const TlDenseVector_Lapack& rho,
              TlDenseSymmetricMatrix_Lapack* pP);
    ...
    class IntegralAggregater {
       ...
       public:
        ...
        int countUp(index_type i, index_type j, index_type k, index_type l,
            ...
            const std::size_t ijKey =
                i + (2 * this->maxIndex_ - (j + 1)) * j / 2;
            const std::size_t klKey =
                k + (2 * this->maxIndex_ - (l + 1)) * l / 2;
            ...
            //                              i, j, k, l,
            //                              this->value_[ijKey][klKey])
            //           << std::endl;
            return this->value_[ijKey][klKey];
        }

        int getCount(index_type i, index_type j, index_type k,
                     index_type l) const {
            assert((0 <= i) && (i < this->maxIndex_));
            assert((0 <= j) && (j < this->maxIndex_));
            assert((0 <= k) && (k < this->maxIndex_));
