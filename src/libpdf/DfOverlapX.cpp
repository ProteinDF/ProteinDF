...
#pragma omp for schedule(runtime)
        for (int i = 0; i < taskListSize; ++i) {
            ...
            // DfOverlapEngine::getPGTOs(orbitalInfo2, shellIndexQ); const
            // DfOverlapEngine::Query query(0, 0, 0, 0, shellTypeP, shellTypeQ,
            // 0, 0);
...
#pragma omp for schedule(runtime)
        for (int i = 0; i < taskListSize; ++i) {
            ...
            for (index_type shellIndexR = 0; shellIndexR < numOfAuxXC;) {
                ...
                // orbitalInfo_XC.getPosition(shellIndexR); const
                ...
                // this->pEngines_[threadID].calc0(query, posP, posQ, posR,
                // posS, pgtosP, pgtosQ, pgtosR, pgtosS);
                this->pEngines_[threadID].calc(
...
#pragma omp for schedule(runtime)
        for (int i = 0; i < taskListSize; ++i) {
            ...
            // DfOverlapEngine::Query query(1, 0, 0, 0, shellTypeP, shellTypeQ,
            // 0, 0);
