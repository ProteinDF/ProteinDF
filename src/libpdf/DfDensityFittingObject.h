...
void DfDensityFittingTmpl<SymmetricMatrix, Vector, DfERI_Class>::calc() {
    ...
    switch (this->m_nMethodType) {
        case METHOD_RKS: {
            ...
            this->log_.info(TlUtils::format(" the number of electrons = %5.2lf",
                                            dNumOfElec));
            ...
            this->log_.info(
                " following parameter must be equal to the number of "
                "electrons.");
        ...
        case METHOD_UKS: {
            ...
            this->log_.info(TlUtils::format(
                " the number of alpha elctrons = %5.2lf", dNumOfElecA));
            this->log_.info(TlUtils::format(
                " the number of beta  elctrons = %5.2lf", dNumOfElecB));
            ...
            this->log_.info(
                ...
                "electrons.");
            this->log_.info(
                TlUtils::format(" Sum RouAlphaA*nAlpha = %f", SumA));
            this->log_.info(
                TlUtils::format(" Sum RouAlphaB*nAlpha = %f", SumB));
        } break;
...
void DfDensityFittingTmpl<SymmetricMatrix, Vector, DfERI_Class>::getTalpha_ROKS(
    ...
    if (this->m_bDiskUtilization == false) {
        {
            const SymmetricMatrix prevP2 =
                this->getP2pq(this->m_nIteration - 2);
            PA -= prevP2;
        }
        {
            const SymmetricMatrix prevP1 =
                this->getP1pq(this->m_nIteration - 2);
            PA -= prevP1;
            PB -= prevP1;
        }
