    ...
    struct MatrixInfo {
       ...
       public:
        ...
        std::size_t getSize() const {
            return this->pMatrix_->getNumOfRows() *
                   this->pMatrix_->getNumOfCols() * sizeof(double);
        }
...
bool TlMatrixCache::set(const std::string& path, const MatrixType& matrix,
    ...
    const std::size_t bufSize =
        matrix.getNumOfRows() * matrix.getNumOfCols() * sizeof(double);
    ...
    if (isPrepareMatrix == false) {
        ...
        if (TlFile::isExistFile(path) == true) {
            if (matrix.load(path) == true) {
                if (this->isDebugOut_ == true) {
                    std::cerr
                        << TlUtils::format("TlMatrixCache::get() from disk: %s",
                                           path.c_str())
                        << std::endl;
                }
            ...
            log.critical(TlUtils::format(
                "TlMatrixCache::get(): file not found: %s", path.c_str()));
#ifndef NDEBUG
            abort();
#endif  // NDEBUG
        }
