...
TlSerializeData PdfKeyword::getDefault() const {
    ...
    for (KeywordListType::const_iterator p = this->kwdList_.begin(); p != pEnd;
         ++p) {
...
void PdfKeyword::initialize() {
    ...
    if (mpac.load(kwdPath) == true) {
        ...
        for (TlSerializeData::ArrayConstIterator it = kwds.beginArray();
             it != itEnd; ++it) {
            KeywordInfo item;
...
void PdfKeyword::setupAliasList() {
    ...
    if (PdfHome != NULL) {
        aliasKwdPath =
            TlUtils::format("%s/data/%s", PdfHome, aliasKwdPath.c_str());
    }
        ...
        if (it != this->kwdAlias_.end()) {
            ...
            if (pData->hasKey(newKeyword) == true) {
                log.warn(TlUtils::format(" kwd[%s] overwrite by alias[%s]:",
                                         newKeyword.c_str(), keyword.c_str()));
...
std::string PdfKeyword::getCSV(bool showHiddenItem) const {
    ...
    for (int i = 0; i < numOfItems; ++i) {
        ...
        if (((item.type & KWD_HIDDEN) != 0) && (showHiddenItem == false)) {
            continue;
        }
        output +=
            TlUtils::format("\"%s\", \"%s\", \"%s\", \"%s\"\n",
                            item.keyword.c_str(), item.explanation.c_str(),
                            item.defaultValue.c_str(), item.syntax.c_str());
    }
...
std::string PdfKeyword::getCSV_jp(bool showHiddenItem) const {
    ...
    for (int i = 0; i < numOfItems; ++i) {
        ...
        output +=
            TlUtils::format("\"%s\", \"%s\", \"%s\", \"%s\"\n",
                            item.keyword.c_str(), item.desc_jp.c_str(),
                            item.defaultValue.c_str(), item.syntax.c_str());
    }
...
TlSerializeData PdfKeyword::getSerializeData() const {
    ...
    for (KeywordListType::const_iterator it = this->kwdList_.begin();
         it != itEnd; ++it) {
...
void PdfKeyword::makeDB() const {
    ...
    if (this->kwdDb_.size() != numOfKwds) {
        this->log_.debug(
            TlUtils::format("[WARN] size mismatch: file: %s, line: %d.",
