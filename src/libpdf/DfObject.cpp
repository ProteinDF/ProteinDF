...
void DfObject::setParam(const TlSerializeData& data) {
    ...
    if (!data["use_mmap"].getStr().empty()) {
        this->useMmap_ = data["use_mmap"].getBoolean();
    }
    ...
    // for memory ==============================================================
    // this->isUseCache_ =
    // (*(this->pPdfParam_))["use_matrix_cache"].getBoolean(); if
    // (this->isUseCache_ == true) {
    ...
    if (paramFileBaseName["Lxc_errors_vtr"].getStr().empty() == true) {
        paramFileBaseName["Lxc_errors_vtr"] = "Lxc_errors.vtr";
    }
...
std::string DfObject::getLkMatrixPath(const std::string& dir) {
    return this->makeFilePath("Lk_matrix", "", dir);
}
...
std::string DfObject::getLxcErrorsVtrPath() {
    return this->makeFilePath("Lxc_errors_vtr");
}
