...
void DfLevelshift::main(const RUN_TYPE runType, int iteration,
  ...
  const double delta_group_closed =
      pdfParam["level_shift/delta_group_closed"].getDouble();
  const double delta_group_open =
      pdfParam["level_shift/delta_group_open"].getDouble();
  const double delta_group_virtual =
      pdfParam["level_shift/delta_group_virtual"].getDouble();

  this->log_.info("construct Level Shift Operator.");

  this->log_.info("RKS,UKS calculation");
  this->log_.info(
      TlUtils::format(" level shift for closed  MO = %8.2lf", ls_closed_mo));
  this->log_.info(
      TlUtils::format(" level shift for open    MO = %8.2lf", ls_open_mo));
  this->log_.info(
      TlUtils::format(" level shift for virtual MO = %8.2lf\n", ls_virtual_mo));
  this->log_.info(TlUtils::format(" level shift between closed  MO = %8.2lf",
                                  delta_group_closed));
  this->log_.info(TlUtils::format("    level shift between open    MO = %8.2lf",
                                  delta_group_open));
  this->log_.info(TlUtils::format("    level shift between virtual MO = %8.2lf",
                                  delta_group_virtual));
  ...
  TlMatrix Cprime(this->m_nNumOfMOs, this->m_nNumOfMOs);
  {
    if (iteration == 1 && ((this->initialGuessType_ != GUESS_LCAO) &&
      ...
      this->log_.info(
          "construct level shift operator with fukue's Rou at iteration==1");
      this->log_.info(
          "level shift value is simply add to F' matrix (guess Rou is solution "
          "of DFT)");
      ...
      if (TlFile::isExistFile(
        ...
        const TlMatrix C =
            DfObject::getCMatrix<TlMatrix>(runType, iteration - 1);
      ...
      if (Cprime.getNumOfRows() != this->m_nNumOfMOs ||
        ...
        this->log_.warn(TlUtils::format("rowDim of previous C' matrix = %d",
                                        Cprime.getNumOfRows()));
        this->log_.warn(TlUtils::format("colDIm of previous C' matrix = %d",
                                        Cprime.getNumOfCols()));
        this->log_.warn(
            TlUtils::format("number_mo_basis = %d", this->m_nNumOfMOs));
        this->log_.warn(
            "DfLevelshift dimension is not consistency, but continue");
      }
    }
  }
  ...
  TlSymmetricMatrix Fprime;
  {
    ...
    if (Fprime.getNumOfRows() != this->m_nNumOfMOs ||
      ...
      this->log_.warn(
          TlUtils::format("rowDim of F' matrix = %d", Fprime.getNumOfRows()));
      this->log_.warn(
          TlUtils::format("colDIm of F' matrix = %d", Fprime.getNumOfCols()));
      this->log_.warn(
          TlUtils::format("number_mo_basis = %d", this->m_nNumOfMOs));
      this->log_.warn(
          "DfLevelshift dimension is not consistency, but continue");
