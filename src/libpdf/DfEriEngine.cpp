...
DfEriEngine::DfEriEngine()
    ...
    // abp(r)cdq
    const int P_PRIME_MAX3 =
        ERI_P_PRIME_MAX * ERI_P_PRIME_MAX * ERI_P_PRIME_MAX;
    const int max_abpRcdq_index =
        P_PRIME_MAX3 * P_PRIME_MAX3 * ERI_NUM_OF_R_KINDS;
    ...
    this->ERI_ket_.resize(ERI_NUM_OF_ERI_STATES);
    {
        ...
        for (int q_a_bar = 0; q_a_bar < (ERI_A_BAR_MAX + 1); ++q_a_bar) {
            for (int q_b_bar = 0; q_b_bar < (ERI_B_BAR_MAX + 1); ++q_b_bar) {
                for (int q_a = 0; q_a < (ERI_A_MAX + 1); ++q_a) {
                    for (int q_b = 0; q_b < (ERI_B_MAX + 1); ++q_b) {
                        AngularMomentum2 q(q_a_bar, q_b_bar, q_a, q_b);
                        const int angularMomentumP =
                            q.a + q.b + q.a_bar + q.b_bar;
                        const ContractScalesVector bra_contractScales =
                            this->choice(q);
                        const int max_bra_cs_index = bra_contractScales.size();
                        for (int bra_cs_index = 0;
                             bra_cs_index < max_bra_cs_index; ++bra_cs_index) {
                            const int a_prime =
                                bra_contractScales[bra_cs_index].a_prime;
                            const int b_prime =
                                bra_contractScales[bra_cs_index].b_prime;
                            const int p_prime =
                                bra_contractScales[bra_cs_index].p_prime;
                            for (int p = 0; p <= angularMomentumP; ++p) {
                                const ERI_State bra_state(a_bar, b_bar, a, b, p,
                                                          a_prime, b_prime,
                                                          p_prime);
                                const int braStateIndex = bra_state.index();

                                // const TlAngularMomentumVectorSet amvs_p(p);
                                // const int numOf_amvs_p = amvs_p.size(); //
                                // amvs_p の数だけで十分
                                this->ERI_bra_[braStateIndex].resize(
                                    numOf_amvs_p_max);
                                this->ERI_ket_[braStateIndex].resize(
                                    numOf_amvs_p_max);
                            }
                        }
                    }
    ...
    this->isCalcdERI_.resize(ERI_NUM_OF_ERI_STATES);
...
DfEriEngine::~DfEriEngine() {
    ...
    std::cerr << TlUtils::format("max sumOfAngularMomentums = %ld",
                                 this->maxSizeOf_sumOfAngularMomentums_)
              << std::endl;
    std::cerr << TlUtils::format(
                     "max (a~, b~, a, b, p, a', b', p') = (%d, %d, %d, %d, %d, "
                     "%d, %d, %d)",
                     this->max_a_bar_, this->max_b_bar_, this->max_a_,
                     this->max_b_, this->max_p_, this->max_a_prime_,
                     this->max_b_prime_, this->max_p_prime_)
              << std::endl;
...
DfEriEngine::CGTO_Pair DfEriEngine::getCGTO_pair(
    const TlOrbitalInfoObject& orbInfo, const index_type shellIndexP,
    const index_type shellIndexQ, const double cutoffThreshold) {
    ...
    // Pの1番目(最大)とQを比較すれば良い。
    // static const double INV_EQ32_COEF = 1.0 / (std::pow(2.0 * TlMath::PI(),
    // 0.25) * TlMath::PI()); const double threshold = cutoffThreshold *
    // INV_EQ32_COEF; const double distance2 = PQ.squareDistanceFrom(); if
    // (shellIndexQ >= 0) {
    ...
    //         const double Kab = std::fabs(inv_zeta * std::exp(- exponentP *
    //         exponentQ * inv_zeta * distance2)); if (Kab < threshold) {
    ...
    for (int pP = 0; pP < numOfContractionsP; ++pP) {
        ...
        for (int pQ = 0; pQ < numOfContractionsQ; ++pQ) {
            ...
            PGTO_Pair pgtoPair(shellTypeP, coefP, expP, shellTypeQ, coefQ, expQ,
                               P, Q);
...
DfEriEngine::CGTO_Pair DfEriEngine::getCGTO_pair(
    const TlOrbitalInfoObject& orbInfo1, const TlOrbitalInfoObject& orbInfo2,
    const index_type shellIndex1, const index_type shellIndex2,
...
void DfEriEngine::calc(const int diff1, const TlOrbitalInfoObject& orbInfo1,
    ...
    const AngularMomentum2 qPQ(diff1, diff2, shellType1, shellType2);
    const AngularMomentum2 qRS(diff3, diff4, shellType3, shellType4);

    const CGTO_Pair PQ = this->getCGTO_pair(orbInfo1, orbInfo2, shell1, shell2);
    const CGTO_Pair RS = this->getCGTO_pair(orbInfo3, orbInfo4, shell3, shell4);

    this->calc0(qPQ, qRS, PQ, RS);
}

void DfEriEngine::calc0(const AngularMomentum2& qAB,
                        const AngularMomentum2& qCD, const CGTO_Pair& IJ,
                        const CGTO_Pair& KL) {
...
#ifdef CHECK_MAX_COUNT
    this->maxSizeOf_sumOfAngularMomentums_ = std::max(
        this->maxSizeOf_sumOfAngularMomentums_, this->sumOfAngularMomentums_);
#endif  // CHECK_MAX_COUNT
    ...
    // for debug
    {
        const std::string AB_str = TlUtils::format("[%f %f %f]", this->AB_[0],
                                                   this->AB_[1], this->AB_[2]);
        const std::string CD_str = TlUtils::format("[%f %f %f]", this->CD_[0],
                                                   this->CD_[1], this->CD_[2]);
        std::cerr
            << TlUtils::format(
                   ">>>> BEGIN\n(%d %d, %d %d |%d %d, %d %d) AB=%s, CD=%s",
                   qAB.a_bar, qAB.b_bar, qAB.a, qAB.b, qCD.a_bar, qCD.b_bar,
                   qCD.a, qCD.b, AB_str.c_str(), CD_str.c_str())
            << std::endl;
    }
#endif  // DEBUG_HGP
    ...
    for (int block = 0; block < blocks.quot; ++block) {
        ...
        std::copy(KL.PS.begin() + base, KL.PS.begin() + base + block_KL,
                  ket.begin());
        this->ket_ = ket;

#ifdef DEBUG_HGP
        // for debug
        std::cerr << TlUtils::format("block1: KP=%d, KQ=%d", this->bra_.size(),
                                     this->ket_.size())
                  << std::endl;
#endif  // DEBUG_HGP
...

    {
        ...
        std::copy(KL.PS.begin() + base, KL.PS.begin() + base + blocks.rem,
                  ket.begin());
        this->ket_ = ket;

#ifdef DEBUG_HGP
        // for debug
        std::cerr << TlUtils::format("block2: KP=%d, KQ=%d", this->bra_.size(),
                                     this->ket_.size())
                  << std::endl;
#endif  // DEBUG_HGP
    ...
    // for debug
    // {
    //     std::cerr << TlUtils::format(">>>> 6D >>>> (%d %d %d %d | %d %d %d
    //     %d)",
...
void DfEriEngine::calcGrad(const AngularMomentum2& in_qAB,
                           const AngularMomentum2& in_qCD, const CGTO_Pair& IJ,
                           const CGTO_Pair& KL) {
    // initialize
    this->sumOfAngularMomentums_ =
        in_qAB.a + in_qAB.b + in_qCD.a + in_qCD.b + 1;
    ...
    std::fill(this->WORK_D, this->WORK_D + OUTPUT_BUFFER_SIZE, 0.0);
#endif  // DEBUG_CHECK_WORK_D
...
#ifdef DEBUG_CHECK_WORK_D
        this->transform6Dto5D(qAB00, qCD01, this->WORK_D);
#endif  // DEBUG_CHECK_WORK_D
    }

    // calculate D' from difference of sum of A', B', and C'.
...
void DfEriEngine::copyResultsToOutputBuffer(const AngularMomentum2& qAB,
    ...
    const int ketStateIndex = ket_state.index();
                ...
                for (int iDbar = 0; iDbar < numOfAmvsDbar; ++iDbar) {
                    ...
                    for (int iA = 0; iA < numOfAmvsA; ++iA) {
                        ...
                        for (int iB = 0; iB < numOfAmvsB; ++iB) {
                            ...
                            // amvsB.get(iB);
                            const int bra_index =
                                ((iAbar * numOfAmvsBbar + iBbar) * numOfAmvsA +
                                 iA) *
                                    numOfAmvsB +
                                iB;
                            ...
                            for (int iC = 0; iC < numOfAmvsC; ++iC) {
                                ...
                                for (int iD = 0; iD < numOfAmvsD; ++iD) {
                                    const TlAngularMomentumVector amvD =
                                        amvsD.get(iD);
                                    const int ket_index = this->index(
                                        amvCbar, amvDbar, amvC, amvD, amvQ);
                                    ...
                                    assert(
                                        ...
                                        this->ERI_ket_[ketStateIndex][ket_index]
                                            .size());
                                    ...
                                    // for debug
                                    {
                                        double v = eri[ket_state][ket_index]
                                                      [bra_index];
                                        std::cerr
                                            << TlUtils::format(
                                                   "output[%3d] (%s %s, %s "
...
void DfEriEngine::transform6Dto5D(const AngularMomentum2& qAB,
    ...
    std::cerr
        << TlUtils::format(
               "DfEriEngine::transform6Dto5D() in: a=%d, b=%d, c=%d, d=%d", a,
               b, c, d)
        << std::endl;
    ...
    if (a == 2) {
        this->transform6Dto5D_i(I_, J_, K_, L_, J, K, L, pOutput,
                                this->pTransformBuf_);
        I = 5;
        const int end = I_ * J_ * K_ * L_ * I * J * K * L;
        std::copy(this->pTransformBuf_, this->pTransformBuf_ + end, pOutput);
    }
    if (b == 2) {
        this->transform6Dto5D_j(I_, J_, K_, L_, I, K, L, pOutput,
                                this->pTransformBuf_);
        J = 5;
        const int end = I_ * J_ * K_ * L_ * I * J * K * L;
        std::copy(this->pTransformBuf_, this->pTransformBuf_ + end, pOutput);
    ...
    if (c == 2) {
        this->transform6Dto5D_k(I_, J_, K_, L_, I, J, L, pOutput,
                                this->pTransformBuf_);
        K = 5;
        const int end = I_ * J_ * K_ * L_ * I * J * K * L;
        std::copy(this->pTransformBuf_, this->pTransformBuf_ + end, pOutput);
    ...
    if (d == 2) {
        this->transform6Dto5D_l(I_, J_, K_, L_, I, J, K, pOutput,
                                this->pTransformBuf_);
        L = 5;
        const int end = I_ * J_ * K_ * L_ * I * J * K * L;
        std::copy(this->pTransformBuf_, this->pTransformBuf_ + end, pOutput);
    ...
    if (a == 3) {
        this->transform10Fto7F_i(I_, J_, K_, L_, J, K, L, pOutput,
                                 this->pTransformBuf_);
        I = 7;
        const int end = I_ * J_ * K_ * L_ * I * J * K * L;
        std::copy(this->pTransformBuf_, this->pTransformBuf_ + end, pOutput);
    ...
    if (b == 3) {
        this->transform10Fto7F_j(I_, J_, K_, L_, I, K, L, pOutput,
                                 this->pTransformBuf_);
        J = 7;
        const int end = I_ * J_ * K_ * L_ * I * J * K * L;
        std::copy(this->pTransformBuf_, this->pTransformBuf_ + end, pOutput);
    ...
    if (c == 3) {
        this->transform10Fto7F_k(I_, J_, K_, L_, I, J, L, pOutput,
                                 this->pTransformBuf_);
        K = 7;
        const int end = I_ * J_ * K_ * L_ * I * J * K * L;
        std::copy(this->pTransformBuf_, this->pTransformBuf_ + end, pOutput);
    ...
    if (d == 3) {
        this->transform10Fto7F_l(I_, J_, K_, L_, I, J, K, pOutput,
                                 this->pTransformBuf_);
        L = 7;
        const int end = I_ * J_ * K_ * L_ * I * J * K * L;
        std::copy(this->pTransformBuf_, this->pTransformBuf_ + end, pOutput);
...
void DfEriEngine::transform6Dto5D_i(const int I_, const int J_, const int K_,
                                    const int L_, const int J, const int K,
                                    const int L, const double* pInput,
                                    double* pOutput) {
#ifdef DEBUG_TRANSFORM_6D_TO_5D
    std::cerr
        << TlUtils::format(
               "DfEriEngine::transform6Dto5D_i() in: I^=%d, J^=%d, K^=%d, "
               "L^=%d, J=%d, K=%d, L=%d",
               I_, J_, K_, L_, J, K, L)
        << std::endl;
#endif  // DEBUG_TRANSFORM_6D_TO_5D
...
void DfEriEngine::transform6Dto5D_j(const int I_, const int J_, const int K_,
                                    const int L_, const int I, const int K,
                                    const int L, const double* pInput,
                                    double* pOutput) {
#ifdef DEBUG_TRANSFORM_6D_TO_5D
    std::cerr
        << TlUtils::format(
               "DfEriEngine::transform6Dto5D_j() in: I^=%d, J^=%d, K^=%d, "
               "L^=%d, I=%d, K=%d, L=%d",
               I_, J_, K_, L_, I, K, L)
        << std::endl;
#endif  // DEBUG_TRANSFORM_6D_TO_5D
...
void DfEriEngine::transform6Dto5D_k(const int I_, const int J_, const int K_,
                                    const int L_, const int I, const int J,
                                    const int L, const double* pInput,
                                    double* pOutput) {
#ifdef DEBUG_TRANSFORM_6D_TO_5D
    std::cerr
...
void DfEriEngine::transform6Dto5D_l(const int I_, const int J_, const int K_,
                                    const int L_, const int I, const int J,
                                    const int K, const double* pInput,
                                    double* pOutput) {
#ifdef DEBUG_TRANSFORM_6D_TO_5D
    std::cerr
        << TlUtils::format(
               "DfEriEngine::transform6Dto5D_l() in: I^=%d, J^=%d, K^=%d, "
               "L^=%d, I=%d, J=%d, K=%d",
               I_, J_, K_, L_, I, J, K)
        << std::endl;
#endif  // DEBUG_TRANSFORM_6D_TO_5D
...
void DfEriEngine::transform10Fto7F_i(const int I_, const int J_, const int K_,
                                     const int L_, const int J, const int K,
                                     const int L, const double* pInput,
                                     double* pOutput) {
                        ...
                        for (int k = 0; k < K; ++k) {
                            for (int l = 0; l < L; ++l) {
                                ...
                                pOutput[z3_7f] =
                                    0.5 * route_1_15 *
                                    (5 * zzz - 3 * (xxz + yyz + zzz));
                                pOutput[xz2_7f] = 0.25 * route_25 *
                                                  (5 * xzz - (xxx + xyy + xzz));
                                pOutput[yz2_7f] = 0.25 * route_25 *
                                                  (5 * yzz - (xxy + yyy + yzz));
                                pOutput[x2y_y3_7f] =
                                    0.25 * route_23 * (3 * xxy - yyy);
                                pOutput[x3_xy2_7f] =
                                    0.25 * route_23 * (xxx - 3 * xyy);
                                pOutput[xyz_7f] = xyz;
...
void DfEriEngine::transform10Fto7F_j(const int I_, const int J_, const int K_,
                                     const int L_, const int I, const int K,
                                     const int L, const double* pInput,
                                     double* pOutput) {
    const double route_23 = std::sqrt(2.0 / 3.0);
...
void DfEriEngine::transform10Fto7F_k(const int I_, const int J_, const int K_,
                                     const int L_, const int I, const int J,
                                     const int L, const double* pInput,
                                     double* pOutput) {
    const double route_23 = std::sqrt(2.0 / 3.0);
...
void DfEriEngine::transform10Fto7F_l(const int I_, const int J_, const int K_,
                                     const int L_, const int I, const int J,
                                     const int K, const double* pInput,
                                     double* pOutput) {
    const double route_23 = std::sqrt(2.0 / 3.0);
...
void DfEriEngine::compD(const AngularMomentum2& qAB,
    ...
    for (int i = 0; i < maxSize; ++i) {
        const double value_D =
            -(this->WORK_A[i] + this->WORK_B[i] + this->WORK_C[i]);
#ifdef DEBUG_CHECK_WORK_D
        {
            const double calcd_D = this->WORK_D[i];
            const double diff = calcd_D - value_D;
            if (std::fabs(diff) > 1.0E-5) {
                this->log_.debug(TlUtils::format(
                    "DfEriEngine::compD(): wrong D' value %4d-th "
                    "calcd:% f, val:% f, diff:% f",
                    i, calcd_D, value_D, diff));
            }
        }
#endif  // DEBUG_CHECK_WORK_D

        this->WORK_D[i] = value_D;
    }
}

void DfEriEngine::calc(const AngularMomentum2& qAB,
                       const AngularMomentum2& qCD) {
    // this->log_.debug(TlUtils::format("DfEriEngine::calc(): a~=%d, b~=%d,
    // a=%d, b=%d,",
    //                                  qAB.a_bar, qAB.b_bar, qAB.a, qAB.b));
    // this->log_.debug(TlUtils::format("                     c~=%d, d~=%d,
    // c=%d, d=%d,",
    ...
    {
        int count = 0;
        for (ContractScalesVector::const_iterator p =
                 bra_contractScales_vtr.begin();
             p != bra_contractScales_vtr.end(); ++p) {
            std::cerr << TlUtils::format(
                             "choice[%d]: (a', b', p')=(%d, %d, %d)", count,
                             p->a_prime, p->b_prime, p->p_prime)
                      << std::endl;
            ++count;
        }
        count = 0;
        for (ContractScalesVector::const_iterator p =
                 ket_contractScales_vtr.begin();
             p != ket_contractScales_vtr.end(); ++p) {
            std::cerr << TlUtils::format(
                             "choice[%d]: (c', d', q')=(%d, %d, %d)", count,
                             p->a_prime, p->b_prime, p->p_prime)
                      << std::endl;
            ++count;
        }
    ...
    // contract
    this->contract(qAB, qCD, bra_contractScales_vtr, ket_contractScales_vtr);

    // calc PQ
    this->calcPQ(qAB, qCD, bra_contractScales_vtr, ket_contractScales_vtr);

    // calc ERI for bra-
    this->calcERI(qAB.a_bar, qAB.b_bar, qAB.a, qAB.b, 0, 0, 0, 0,
                  &(this->ERI_bra_));
    ...
    for (ContractScalesType::const_iterator it =
             this->bra_contractScales_.begin();
         it != itEnd; ++it) {
        const int a_prime = it->pack.a_prime;
        const int b_prime = it->pack.b_prime;
        const int p_prime = it->pack.p_prime;

        for (int index_a_bar = 0; index_a_bar < amvs_a_bar.size();
             ++index_a_bar) {
            const TlAngularMomentumVector amv_a_bar =
                amvs_a_bar.get(index_a_bar);

            for (int index_b_bar = 0; index_b_bar < amvs_b_bar.size();
                 ++index_b_bar) {
                const TlAngularMomentumVector amv_b_bar =
                    amvs_b_bar.get(index_b_bar);
                ...
                for (int index_a = 0; index_a < amvs_a.size(); ++index_a) {
                    ...
                    for (int index_b = 0; index_b < amvs_b.size(); ++index_b) {
                        const TlAngularMomentumVector amv_b =
                            amvs_b.get(index_b);

                        const int bra_index = this->index(amv_a_bar, amv_b_bar,
                                                          amv_a, amv_b, amv_p);
                        const std::string str = TlUtils::format(
                            "BRA_ERI: (%s %s, %s %s %s, %d %d %d|",
                            amv_a_bar.debugOut().c_str(),
                            amv_b_bar.debugOut().c_str(),
                            amv_a.debugOut().c_str(), amv_b.debugOut().c_str(),
                            amv_p.debugOut().c_str(), a_prime, b_prime,
                            p_prime);
                        const std::vector<double> values =
                            eri[bra_state][bra_index];
                        for (int i = 0; i < values.size(); ++i) {
                            std::cerr
                                << TlUtils::format("%s [%3d] = % f",
    ...
    // calc ERI for ket-
    this->calcERI(qCD.a_bar, qCD.b_bar, qCD.a, qCD.b, 0, 0, 0, 0,
                  &(this->ERI_ket_));
    std::swap(this->AB_,
              this->CD_);  // AB, CDを元に戻す(batchブロック再計算のため)
}
...
void DfEriEngine::calcGrad(const AngularMomentum2& qAB,
    ...
    const AngularMomentum2 qCD01(0, 1, qCD.a, qCD.b);
    ...
    const ContractScalesVector bra_contractScales_vtr =
        this->choice(qAB10, qAB01);
    const ContractScalesVector ket_contractScales_vtr = this->choice(qCD00);

    // contract
    this->contract(qAB10, qCD00, bra_contractScales_vtr,
                   ket_contractScales_vtr);

    // A ----------------------------------------------------
    {
        // calc PQ
        this->calcPQ(qAB10, qCD00, bra_contractScales_vtr,
                     ket_contractScales_vtr);

        // calc ERI for bra-
        this->calcERI(1, 0, qAB.a, qAB.b, 0, 0, 0, 0, &(this->ERI_bra_));

        // swap
        this->transpose(qAB10, qCD00, ket_contractScales_vtr);
        std::swap(this->AB_, this->CD_);

        // calc ERI for ket-
        this->calcERI(0, 0, qCD.a, qCD.b, 0, 0, 0, 0, &(this->ERI_ket_));
        std::swap(this->AB_,
                  this->CD_);  // AB, CDを元に戻す(batchブロック再計算のため)
    }
    ...
    {
        // // calc PQ
        this->calcPQ(qAB01, qCD00, bra_contractScales_vtr,
                     ket_contractScales_vtr);

        // calc ERI for bra-
        this->calcERI(0, 1, qAB.a, qAB.b, 0, 0, 0, 0, &(this->ERI_bra_));

        // swap
        this->transpose(qAB01, qCD00, ket_contractScales_vtr);
        std::swap(this->AB_, this->CD_);
...
void DfEriEngine::calcE4CQ() {
    ...
    //     const double thresholdForQ = this->primitiveLevelThreshold_ /
    //     std::fabs(maxU_P); for (int i = 0; i < KQ; ++i) {
    ...
    //     const double thresholdForP = this->primitiveLevelThreshold_ /
    //     std::fabs(maxU_Q); for (int i = 0; i < KP; ++i) {
    ...
    // for debug
    {
        ...
        for (int m = 0; m <= M; ++m) {
            ...
            for (int i = 0; i < KPKQ; ++i) {
                std::cerr << TlUtils::format("calcR0(in): [0](%d) [%d] = %f", m,
                                             i, this->pRM_[index][i])
                          << std::endl;
            }
        }
        ...
        for (int i = 0; i < size; ++i) {
            ...
            for (int kpkq = 0; kpkq < KPKQ; ++kpkq) {
                std::cerr << TlUtils::format("calcR(out): %s(0) [%d] = %f",
                                             amv.debugOut().c_str(), kpkq,
                                             this->pRM_[index][kpkq])
                          << std::endl;
            }
        }
    }
...
DfEriEngine::ContractScalesVector DfEriEngine::choice(
    ...
    if (this->choice_tbl_.find(index) == this->choice_tbl_.end()) {
        ...
        this->choice(qAB.a_bar, qAB.b_bar, qAB.a, qAB.b, 0, 0, 0, 0,
                     &contractList);
        this->choice_tbl_[index] =
            this->transContractScales_SetToVector(contractList);
    }
...
DfEriEngine::ContractScalesVector DfEriEngine::choice(
    const AngularMomentum2& qAB1, const AngularMomentum2& qAB2) {
    ContractScalesSet contractList;
    this->choice(qAB1.a_bar, qAB1.b_bar, qAB1.a, qAB1.b, 0, 0, 0, 0,
                 &contractList);
    this->choice(qAB2.a_bar, qAB2.b_bar, qAB2.a, qAB2.b, 0, 0, 0, 0,
                 &contractList);
    return this->transContractScales_SetToVector(contractList);
}

// pContractList に計算に必要な状態が登録される
void DfEriEngine::choice(const int a_bar, const int b_bar, const int a,
                         const int b, const int p, const int a_prime,
                         const int b_prime, const int p_prime,
                         ContractScalesSet* pContractList) {
    assert(pContractList != NULL);

    if ((a_bar < 0) || (b_bar < 0) || (a < 0) || (b < 0) || (p < 0) ||
    ...
    if ((a_bar == 0) && (b_bar == 0) && (a == 0) && (b == 0)) {
        ...
        // use eq.47
        this->choice(a_bar, b_bar - 1, a, b, p + 1, a_prime, b_prime, p_prime,
                     pContractList);
        this->choice(a_bar + 1, b_bar - 1, a, b, p, a_prime, b_prime, p_prime,
                     pContractList);
        this->choice(a_bar, b_bar - 1, a - 1, b, p, a_prime, b_prime, p_prime,
                     pContractList);
        this->choice(a_bar, b_bar - 1, a, b - 1, p, a_prime, b_prime, p_prime,
                     pContractList);
    } else if (a_bar > 0) {
        // use eq.43
        this->choice(a_bar - 1, b_bar, a + 1, b, p, a_prime + 1, b_prime,
                     p_prime, pContractList);
        this->choice(a_bar - 1, b_bar, a - 1, b, p, a_prime, b_prime, p_prime,
                     pContractList);
        this->choice(a_bar - 1, b_bar, a, b, p - 1, a_prime + 1, b_prime,
                     p_prime, pContractList);
    } else if (b > 0) {
        // use eq.44
        this->choice(a_bar, b_bar, a + 1, b - 1, p, a_prime, b_prime, p_prime,
                     pContractList);
        this->choice(a_bar, b_bar, a, b - 1, p, a_prime, b_prime, p_prime,
                     pContractList);
        this->choice(a_bar - 1, b_bar, a, b - 1, p, a_prime, b_prime, p_prime,
                     pContractList);
        this->choice(a_bar, b_bar - 1, a, b - 1, p, a_prime, b_prime, p_prime,
                     pContractList);
    } else if (a > 0) {
        // use eq.45 (p -> a)
        this->choice(a_bar, b_bar, a - 1, b, p - 1, a_prime, b_prime, p_prime,
                     pContractList);
        this->choice(a_bar, b_bar, a - 1, b, p + 1, a_prime, b_prime,
                     p_prime + 1, pContractList);
        this->choice(a_bar, b_bar, a - 1, b, p, a_prime, b_prime + 1,
                     p_prime + 1, pContractList);
        this->choice(a_bar - 1, b_bar, a - 1, b, p, a_prime, b_prime + 1,
                     p_prime + 1, pContractList);
        this->choice(a_bar, b_bar - 1, a - 1, b, p, a_prime, b_prime + 1,
                     p_prime + 1, pContractList);
    } else {
        // something wrong
        std::cerr
            << TlUtils::format(
                   "program error. @ DfEriEngine::choice() [%d, %d, %d, %d, "
                   "%d, %d, %d, %d]",
...
DfEriEngine::ContractScalesVector DfEriEngine::transContractScales_SetToVector(
    const ContractScalesSet& contractScales) {
    const int size = contractScales.size();
    ContractScalesVector answer(size);

    int index = 0;
    ContractScalesSet::const_iterator pEnd = contractScales.end();
    for (ContractScalesSet::const_iterator p = contractScales.begin();
         p != pEnd; ++p) {
        answer[index] = *p;
        ++index;
    }
...
void DfEriEngine::contract(const AngularMomentum2& qAB,
    ...
    int nR_dash_index = 0;
    const int max_bra_cs_index = bra_contractScales.size();
    for (int bra_cs_index = 0; bra_cs_index < max_bra_cs_index;
         ++bra_cs_index) {
        ...
        for (int r = 0; r <= R; ++r) {
            //             std::cerr << TlUtils::format("contract() for bra-
            //             a'=%d b'=%d p'=%d r=%d",
            //                                          a_prime, b_prime,
            //                                          p_prime, r)
            //                                          << std::endl;
            ...
            for (int amv_index = 0; amv_index < numOfAmv; ++amv_index) {
                ...
                this->contract_bra(qAB, amv, a_prime, b_prime, p_prime,
                                   nR_dash_index);
                ++nR_dash_index;
            }
        }
...
// for profile
#ifdef CHECK_MAX_COUNT
    this->maxSizeOf_nR_dash_ =
        std::max(this->maxSizeOf_nR_dash_, nR_dash_index);
...
// for debug
    ...
    for (n_R_dash_Type::const_iterator p = this->n_R_dash_.begin();
        ...
        std::string v_string = "[";
        for (std::vector<double>::const_iterator q = values.begin();
             q != values.end(); ++q) {
            v_string += TlUtils::format("%e ", *q);
        }
        v_string += "]";
        std::cerr << TlUtils::format("(R') %s = %s", cs.debugOut().c_str(),
                                     v_string.c_str())
                  << std::endl;
    }
#endif  // DEBUG_R_DASH
    ...
    for (int ket_cs_index = 0; ket_cs_index < max_ket_cs_index;
         ++ket_cs_index) {
...
void DfEriEngine::contract_bra(const AngularMomentum2& qAB,
                               ...
                               const int a_prime, const int b_prime,
                               const int p_prime, const int nR_dash_index) {
    const int a = qAB.a;
    const int b = qAB.b;
    ...
    for (int KQ_index = 0; KQ_index < KQ; ++KQ_index) {
        ...
        std::cerr << TlUtils::format(
                         "contract_bra[%2d]: alpha=% f, beta=% f, zeta=% f, "
                         "exp_zeta=%d, coef=% f, r0=% f",
...
// eq.36
void DfEriEngine::get_contract_ket_coef_numerators(
    const int c_prime, const int d_prime, const int zeta_exp,
...
//                                const ContractState& cs, const
//                                std::vector<double>& KQ_values)
// {
//     const int c = qCD.a;
//     const int d = qCD.b;
...
void DfEriEngine::calcPQ(const AngularMomentum2& qAB,
                         ...
                         const ContractScalesVector& bra_contractScales,
                         const ContractScalesVector& ket_contractScales) {
    std::fill(this->isCalcdERI_.begin(), this->isCalcdERI_.end(), 0);

    const int angularMomentumP = qAB.a + qAB.b + qAB.a_bar + qAB.b_bar;
    const int angularMomentumQ = qCD.a + qCD.b + qCD.a_bar + qCD.b_bar;
...
#ifdef DEBUG_HGP
    std::cerr << "angularMomentumP=" << angularMomentumP
              << ", angularMomentumQ=" << angularMomentumQ << std::endl;
    std::cerr << "calcPQ: max_ket_index=" << max_ket_index << std::endl;
#endif  // DEBUG_HGP

    // ContractState cs;
    const std::vector<int> cs_indeces = this->get_csindex_for_calcPQ(
        angularMomentumP, qAB, angularMomentumQ, qCD, bra_contractScales,
        ket_contractScales);
    ...
    for (int bra_cs_index = 0; bra_cs_index < max_bra_cs_index;
        ...
        for (int p = 0; p <= angularMomentumP; ++p) {
            const ERI_State bra_state(a_bar, b_bar, a, b, p, a_prime, b_prime,
                                      p_prime);
            const int braStateIndex = bra_state.index();
            this->isCalcdERI_[braStateIndex] = 1;
            ...
            assert(static_cast<int>(this->ERI_bra_[braStateIndex].size()) >
                   numOf_amvs_p);
            ...
            for (int i = 0; i < numOf_amvs_p; ++i) {
                ...
                const int bra_index =
                    this->index(amv_a_bar, amv_b_bar, amv_a, amv_b, amv_p);
                assert(bra_index < numOf_amvs_p);

                // ket ---------------------------------------------------------
                std::vector<double>& ket =
                    this->ERI_bra_[braStateIndex][bra_index];
                // ket.resize(ERI_MAX_BATCH);
                ket.resize(max_ket_index);
                int ket_index = 0;
                ...
                for (int ket_cs_index = 0; ket_cs_index < max_ket_cs_index;
                     ++ket_cs_index) {
                    // const int c_prime =
                    // ket_contractScales[ket_cs_index].a_prime; const int
                    // d_prime = ket_contractScales[ket_cs_index].b_prime; const
                    // int q_prime = ket_contractScales[ket_cs_index].p_prime;
                    // cs.setCDQ(c_prime, d_prime, q_prime);
                    ...
                    for (int q = 0; q <= angularMomentumQ; ++q) {
                        ...
                        for (int j = 0; j < numOf_amvs_q; ++j) {
                            // const TlAngularMomentumVector amv_q =
                            // amvs_q.get(j); const TlAngularMomentumVector r =
                            // amv_p + amv_q; cs.setR(r);

                            // const int cs_index = cs.index();
                            // if (cs_indeces[cs_index_order] != cs_index) {
                            //     std::cerr << TlUtils::format("err! %d != %d",
                            //                                  cs_indeces[cs_index_order],
                            //                                  cs_index)
                            //               << std::endl;
                            // }
                            ...
                            std::cerr << TlUtils::format(
                                             ...
                                             ket_index, r.debugOut().c_str(),
                                             a_prime, b_prime, p_prime, c_prime,
                                             d_prime, q_prime, value, coef)
                                      << std::endl;
#endif  // DEBUG_CALC_PQ
...
std::vector<int> DfEriEngine::get_csindex_for_calcPQ(
    const int angularMomentumP, const AngularMomentum2& qAB,
    const int angularMomentumQ, const AngularMomentum2& qCD,
    const ContractScalesVector& bra_contractScales,
    const ContractScalesVector& ket_contractScales) {
    // const int index =
    //     ((angularMomentumP*AngularMomentum2::maxIndex() +
    //     qAB.index())*ERI_P_MAX +
    //     angularMomentumQ)*AngularMomentum2::maxIndex()
    //     + qCD.index();
    const int index = qAB.index() * AngularMomentum2::maxIndex() + qCD.index();
    if (this->csindeces_forCalcPQ_.find(index) ==
        this->csindeces_forCalcPQ_.end()) {
        ...
        for (int bra_cs_index = 0; bra_cs_index < max_bra_cs_index;
            ...
            for (int p = 0; p <= angularMomentumP; ++p) {
                const TlAngularMomentumVectorSet amvs_p(p);
                const int numOf_amvs_p =
                    amvs_p.size();  // amvs_p の数だけで十分
                for (int i = 0; i < numOf_amvs_p; ++i) {
                    const TlAngularMomentumVector amv_p = amvs_p.get(i);
                    ...
                    for (int ket_cs_index = 0; ket_cs_index < max_ket_cs_index;
                         ++ket_cs_index) {
                        const int c_prime =
                            ket_contractScales[ket_cs_index].a_prime;
                        const int d_prime =
                            ket_contractScales[ket_cs_index].b_prime;
                        const int q_prime =
                            ket_contractScales[ket_cs_index].p_prime;
                        cs.setCDQ(c_prime, d_prime, q_prime);
...
void DfEriEngine::transpose(const AngularMomentum2& qAB,
    ...
    const int max_bra_index = max_amvs_a_bar_index * max_amvs_b_bar_index *
                              max_amvs_a_index * max_amvs_b_index;
...
#ifdef DEBUG_HGP
    std::cerr << "DfEriEngine::transpose(): angularMomentumQ="
              << angularMomentumQ << std::endl;
#endif  // DEBUG_HGP

    // ket ---------------------------------------------------------------------
    int ket_index = 0;
    // ContractScalesVector::const_iterator it_ket_end =
    // ket_contractScales.end(); for (ContractScalesVector::const_iterator
    // it_ket = ket_contractScales.begin(); it_ket != it_ket_end; ++it_ket) {
    const int max_ket_cs_index = ket_contractScales.size();
    for (int ket_cs_index = 0; ket_cs_index < max_ket_cs_index;
         ++ket_cs_index) {
        const int c_prime = ket_contractScales[ket_cs_index].a_prime;
        ...
        for (int q = 0; q <= angularMomentumQ; ++q) {
            const ERI_State ket_state(c_bar, d_bar, c, d, q, c_prime, d_prime,
                                      q_prime);
            const int ketStateIndex = ket_state.index();
            this->isCalcdERI_[ketStateIndex] = 1;

#ifdef DEBUG_HGP
            std::cerr << TlUtils::format("DfEriEngine::transpose(): set %s",
                                         ket_state.debugOut().c_str())
                      << std::endl;
#endif  // DEBUG_HGP
            ...
            for (int j = 0; j < numOf_amvs_q; ++j) {
                ...
                this->ERI_ket_[ket_state.index()][amvs_q_index].resize(
                    ERI_MAX_BATCH);

                // bra ---------------------------------------------------------
                int bra_index = 0;
                for (int amvs_a_bar_index = 0;
                     amvs_a_bar_index < max_amvs_a_bar_index;
                     ++amvs_a_bar_index) {
                    const TlAngularMomentumVector amv_a_bar =
                        amvs_a_bar.get(amvs_a_bar_index);

                    for (int amvs_b_bar_index = 0;
                         amvs_b_bar_index < max_amvs_b_bar_index;
                         ++amvs_b_bar_index) {
                        const TlAngularMomentumVector amv_b_bar =
                            amvs_b_bar.get(amvs_b_bar_index);

                        for (int amvs_a_index = 0;
                             amvs_a_index < max_amvs_a_index; ++amvs_a_index) {
                            const TlAngularMomentumVector amv_a =
                                amvs_a.get(amvs_a_index);

                            for (int amvs_b_index = 0;
                                 amvs_b_index < max_amvs_b_index;
                                 ++amvs_b_index) {
                                const TlAngularMomentumVector amv_b =
                                    amvs_b.get(amvs_b_index);

                                const int amvs_bra_index = this->index(
                                    amv_a_bar, amv_b_bar, amv_a, amv_b, amv_p);

                                //                                 std::cerr <<
                                //                                 TlUtils::format("transpose()
                                //                                 copy from [%s
                                //                                 %d %d] to [%s
                                //                                 %d %d], ",
                                ...
                                const double value =
                                    this->ERI_bra_[braStateIndex]
                                                  [amvs_bra_index][ket_index];

                                //                                 std::cerr <<
                                //                                 "value = " <<
                                //                                 value <<
                                //                                 std::endl;
                                this->ERI_ket_[ketStateIndex][amvs_q_index]
                                              [bra_index] = value;
    ...
    std::cerr << "transpose() end:: isCalcdERI_.size()="
              << this->isCalcdERI_.size() << std::endl;
#endif  // DEBUG_HGP
}

void DfEriEngine::calcERI(const int a_bar, const int b_bar, const int a,
                          const int b, const int p, const int a_prime,
                          const int b_prime, const int p_prime,
                          EriDataType* pERI) {
    if ((a_bar < 0) || (b_bar < 0) || (a < 0) || (b < 0) || (p < 0) ||
    ...
    if (this->isCalcdERI_[eriState.index()] == 0) {
        // recursive
        if (b_bar > 0) {
            // use eq.47
            this->calcERI(a_bar, b_bar - 1, a, b, p + 1, a_prime, b_prime,
                          p_prime, pERI);
            this->calcERI(a_bar + 1, b_bar - 1, a, b, p, a_prime, b_prime,
                          p_prime, pERI);
            this->calcERI(a_bar, b_bar - 1, a - 1, b, p, a_prime, b_prime,
                          p_prime, pERI);
            this->calcERI(a_bar, b_bar - 1, a, b - 1, p, a_prime, b_prime,
                          p_prime, pERI);
            this->ERI_EQ47(eriState, pERI);
        } else if (a_bar > 0) {
            // use eq.43
            this->calcERI(a_bar - 1, b_bar, a + 1, b, p, a_prime + 1, b_prime,
                          p_prime, pERI);
            this->calcERI(a_bar - 1, b_bar, a - 1, b, p, a_prime, b_prime,
                          p_prime, pERI);
            this->calcERI(a_bar - 1, b_bar, a, b, p - 1, a_prime + 1, b_prime,
                          p_prime, pERI);
            this->ERI_EQ43(eriState, pERI);
        } else if (b > 0) {
            // use eq.44
            this->calcERI(a_bar, b_bar, a + 1, b - 1, p, a_prime, b_prime,
                          p_prime, pERI);
            this->calcERI(a_bar, b_bar, a, b - 1, p, a_prime, b_prime, p_prime,
                          pERI);
            this->calcERI(a_bar - 1, b_bar, a, b - 1, p, a_prime, b_prime,
                          p_prime, pERI);
            this->calcERI(a_bar, b_bar - 1, a, b - 1, p, a_prime, b_prime,
                          p_prime, pERI);
            ...
            // use eq.45
            this->calcERI(a_bar, b_bar, a - 1, b, p - 1, a_prime, b_prime,
                          p_prime, pERI);
            this->calcERI(a_bar, b_bar, a - 1, b, p + 1, a_prime, b_prime,
                          p_prime + 1, pERI);
            this->calcERI(a_bar, b_bar, a - 1, b, p, a_prime, b_prime + 1,
                          p_prime + 1, pERI);
            this->calcERI(a_bar - 1, b_bar, a - 1, b, p, a_prime, b_prime + 1,
                          p_prime + 1, pERI);
            this->calcERI(a_bar, b_bar - 1, a - 1, b, p + 1, a_prime, b_prime,
                          p_prime + 1, pERI);
            ...
            this->log_.critical(TlUtils::format(
                " [a~=%d, b~=%d, a=%d, b=%d, p=%d, a'=%d, b='%d, p='%d]", a_bar,
                b_bar, a, b, p, a_prime, b_prime, p_prime));
            abort();
        }

        this->isCalcdERI_[eriState.index()] = 1;
    }
}

int DfEriEngine::index(const TlAngularMomentumVector& a_bar,
                       const TlAngularMomentumVector& b_bar,
                       const TlAngularMomentumVector& a,
                       const TlAngularMomentumVector& b,
    ...
    const int index =
        ((((a_bar.index()) * b_bars + b_bar.index()) * as + a.index()) * bs +
         b.index()) *
            ps +
        p.index();
...
void DfEriEngine::ERI_EQ44(const ERI_State eriState, EriDataType* pERI) {
    ...
    const int numOfElements =
        numOfAmv_a_bar * numOfAmv_b_bar * numOfAmv_a * numOfAmv_b * numOfAmv_p;
    const int stateIndex = eriState.index();
    ...
    for (int amv_b_index = 0; amv_b_index < numOfAmv_b; ++amv_b_index) {
        ...
        for (int amv_a_bar_index = 0; amv_a_bar_index < numOfAmv_a_bar;
             ++amv_a_bar_index) {
            const TlAngularMomentumVector amv_a_bar =
                AMVS_a_bar.get(amv_a_bar_index);
            const TlAngularMomentumVector amv_a_bar1 = amv_a_bar - this->E1_[i];

            for (int amv_b_bar_index = 0; amv_b_bar_index < numOfAmv_b_bar;
                 ++amv_b_bar_index) {
                const TlAngularMomentumVector amv_b_bar =
                    AMVS_b_bar.get(amv_b_bar_index);
                const TlAngularMomentumVector amv_b_bar1 =
                    amv_b_bar - this->E1_[i];

                for (int amv_a_index = 0; amv_a_index < numOfAmv_a;
                     ++amv_a_index) {
                    const TlAngularMomentumVector amv_a =
                        AMVS_a.get(amv_a_index);
                    const TlAngularMomentumVector amv_a1p =
                        amv_a + this->E1_[i];

                    for (int amv_p_index = 0; amv_p_index < numOfAmv_p;
                         ++amv_p_index) {
                        const TlAngularMomentumVector amv_p =
                            AMVS_p.get(amv_p_index);

                        const int index = this->index(amv_a_bar, amv_b_bar,
                                                      amv_a, amv_b, amv_p);
                        ...
                        const int eriState1_index = eriState1.index();
                        const int index1 = this->index(amv_a_bar, amv_b_bar,
                                                       amv_a1p, amv_b1, amv_p);

                        const ERI_State eriState2(a_bar, b_bar, a, b - 1, p,
                                                  a_prime, b_prime, p_prime);
                        const int eriState2_index = eriState2.index();
                        const int index2 = this->index(amv_a_bar, amv_b_bar,
                                                       amv_a, amv_b1, amv_p);

                        int eriState3_index = 0;
                        int index3 = 0;
                        if (amv_a_bar1.isExist() == true) {
                            const ERI_State eriState3(a_bar - 1, b_bar, a,
                                                      b - 1, p, a_prime,
                                                      b_prime, p_prime);
                            eriState3_index = eriState3.index();
                            index3 = this->index(amv_a_bar1, amv_b_bar, amv_a,
                                                 amv_b1, amv_p);
                        }

                        int eriState4_index = 0;
                        int index4 = 0;
                        if (amv_b_bar1.isExist() == true) {
                            const ERI_State eriState4(a_bar, b_bar - 1, a,
                                                      b - 1, p, a_prime,
                                                      b_prime, p_prime);
                            eriState4_index = eriState4.index();
                            index4 = this->index(amv_a_bar, amv_b_bar1, amv_a,
                                                 amv_b1, amv_p);
                        }

                        const int batchSize = this->ERI_batch_;
                        assert(batchSize < ERI_MAX_BATCH);
                        for (int batch = 0; batch < batchSize; ++batch) {
                            ...
                            {
                                const double value1 =
                                    (*pERI)[eriState1_index][index1][batch];
#ifdef DEBUG_EQ44
                                std::cerr
                                    << TlUtils::format(
                                           "EQ44(1st): (%s %s, %s %s %s, %d %d "
                                           "%d| "
                                           "batch[%d] = % f",
                                           amv_a_bar.debugOut().c_str(),
                                           amv_b_bar.debugOut().c_str(),
                            ...
                            {
                                const double value2 =
                                    (*pERI)[eriState2_index][index2][batch];
#ifdef DEBUG_EQ44
                                std::cerr
                                    << TlUtils::format(
                                           "EQ44(2nd): (%s %s, %s %s %s, %d %d "
                                           "%d| "
                                           "batch[%d] = % f",
                                           amv_a_bar.debugOut().c_str(),
                                           amv_b_bar.debugOut().c_str(),
        ...
        // answer -= this->AB_[i] * value2;
                            ...
                            if (amv_a_bar1.isExist() == true) {
                                answer +=
                                    amv_a_bar.get(i) *
                                    (*pERI)[eriState3_index][index3][batch];
                            }
                            ...
                            if (amv_b_bar1.isExist() == true) {
                                answer -=
                                    amv_b_bar.get(i) *
                                    (*pERI)[eriState4_index][index4][batch];
                            }
...
void DfEriEngine::ERI_EQ44_00xxx(const ERI_State eriState, EriDataType* pERI) {
    ...
    const ERI_State eriState1(a_bar, b_bar, a + 1, b - 1, p, a_prime, b_prime,
                              p_prime);
    const int eriState1_index = eriState1.index();
    const ERI_State eriState2(a_bar, b_bar, a, b - 1, p, a_prime, b_prime,
                              p_prime);
    const int eriState2_index = eriState2.index();
    const int batchSize = this->ERI_batch_;
    for (int amv_b_index = 0; amv_b_index < numOfAmv_b; ++amv_b_index) {
        ...
        for (int amv_a_index = 0; amv_a_index < numOfAmv_a; ++amv_a_index) {
            ...
            for (int amv_p_index = 0; amv_p_index < numOfAmv_p; ++amv_p_index) {
                ...
                // batch -----------------------------------------------
                const int index1 =
                    this->index(amv_a_bar, amv_b_bar, amv_a1p, amv_b1, amv_p);
                const int index2 =
                    this->index(amv_a_bar, amv_b_bar, amv_a, amv_b1, amv_p);
    ...
    for (int amv_a_index = 0; amv_a_index < numOfAmv_a; ++amv_a_index) {
        ...
        for (int amv_a_bar_index = 0; amv_a_bar_index < numOfAmv_a_bar;
            ...
            for (int amv_b_bar_index = 0; amv_b_bar_index < numOfAmv_b_bar;
                ...
                for (int amv_b_index = 0; amv_b_index < numOfAmv_b;
                     ++amv_b_index) {
                    const TlAngularMomentumVector amv_b =
                        AMVS_b.get(amv_b_index);

                    for (int amv_p_index = 0; amv_p_index < numOfAmv_p;
                         ++amv_p_index) {
                        const TlAngularMomentumVector amv_p =
                            AMVS_p.get(amv_p_index);
                        const TlAngularMomentumVector amv_p1 =
                            amv_p - this->E1_[i];
                        const TlAngularMomentumVector amv_p1p =
                            amv_p + this->E1_[i];
                        ...
                        // betch -----------------------------------------------
                        int eriState1_index = 0;
                        int index1 = 0;
                        if (amv_p1.isExist() == true) {
                            const ERI_State eriState1(a_bar, b_bar, a - 1, b,
                                                      p - 1, a_prime, b_prime,
                                                      p_prime);
                            eriState1_index = eriState1.index();
                            index1 = this->index(amv_a_bar, amv_b_bar, amv_a1,
                                                 amv_b, amv_p1);
                        }
                        ...
                        const ERI_State eriState3(a_bar, b_bar, a - 1, b, p,
                                                  a_prime, b_prime + 1,
                                                  p_prime + 1);
                        const int eriState3_index = eriState3.index();
                        const int index3 = this->index(amv_a_bar, amv_b_bar,
                                                       amv_a1, amv_b, amv_p);

                        int eriState4_index = 0;
                        int index4 = 0;
                        if (amv_a_bar1.isExist() == true) {
                            const ERI_State eriState4(a_bar - 1, b_bar, a - 1,
                                                      b, p, a_prime,
                                                      b_prime + 1, p_prime + 1);
                            eriState4_index = eriState4.index();
                            index4 = this->index(amv_a_bar1, amv_b_bar, amv_a1,
                                                 amv_b, amv_p);
                        }

                        int eriState5_index = 0;
                        int index5 = 0;
                        if (amv_b_bar1.isExist() == true) {
                            const ERI_State eriState5(a_bar, b_bar - 1, a - 1,
                                                      b, p, a_prime,
                                                      b_prime + 1, p_prime + 1);
                            eriState5_index = eriState5.index();
                            index5 = this->index(amv_a_bar, amv_b_bar1, amv_a1,
                                                 amv_b, amv_p);
                        }
                            ...
                            // 2nd term
                            {
                                ...
                                std::cerr
                                    << TlUtils::format(
                                           ...
                                           amv_a1.debugOut().c_str(),
                                           amv_b.debugOut().c_str(),
                                           amv_p1p.debugOut().c_str(), a_prime,
                                           b_prime, p_prime + 1, batch, value2)
                                    << std::endl;
#endif  // DEBUG_EQ45
                                answer += value2;
                            }
        ...
        // answer += this->AB_[i] * value3;
                            ...
                            if (amv_b_bar1.isExist() == true) {
                                answer +=
                                    amv_b_bar.get(i) *
                                    (*pERI)[eriState5_index][index5][batch];
                            }
...
void DfEriEngine::ERI_EQ45_00x0x(const ERI_State eriState, EriDataType* pERI) {
    ...
    const ERI_State eriState2(a_bar, b_bar, a - 1, b, p + 1, a_prime, b_prime,
                              p_prime + 1);
    const int eriState2_index = eriState2.index();
    const ERI_State eriState3(a_bar, b_bar, a - 1, b, p, a_prime, b_prime + 1,
                              p_prime + 1);
    const int eriState3_index = eriState3.index();
    const int batchSize = this->ERI_batch_;
    for (int amv_a_index = 0; amv_a_index < numOfAmv_a; ++amv_a_index) {
        ...
        for (int amv_p_index = 0; amv_p_index < numOfAmv_p; ++amv_p_index) {
            ...
            // betch -----------------------------------------------
            const int index2 =
                this->index(amv_a_bar, amv_b_bar, amv_a1, amv_b, amv_p1p);
            const int index3 =
                this->index(amv_a_bar, amv_b_bar, amv_a1, amv_b, amv_p);
            double* const answers = &((*pERI)[stateIndex][index][0]);
            ...
            if (amv_p1.isExist() == true) {
                ...
                const int eriState1_index = eriState1.index();
                const int index1 =
                    this->index(amv_a_bar, amv_b_bar, amv_a1, amv_b, amv_p1);
                const double p_i = amv_p.get(i);
                ...
                for (int amv_b_index = 0; amv_b_index < numOfAmv_b;
                    ...
                    for (int amv_p_index = 0; amv_p_index < numOfAmv_p;
                        ...
                        for (int batch = 0; batch < batchSize; ++batch) {
                            ...
                            const TlAngularMomentumVector amv_a_bar1 =
                                amv_a_bar - this->E1_[i];
                            ...
                            // 1st term
                            {
                                const TlAngularMomentumVector amv_p1p =
                                    amv_p + this->E1_[i];
                                const ERI_State eriState1(a_bar - 1, b_bar, a,
                                                          b, p + 1, a_prime + 1,
                                                          b_prime, p_prime + 1);
                                const int index1 =
                                    this->index(amv_a_bar1, amv_b_bar, amv_a,
                                                amv_b, amv_p1p);
                                // const double value1 =
                                // (*pERI)[eriState1][index1][batch];
                                const double value1 =
                                    (*pERI)[eriState1.index()][index1][batch];
#ifdef DEBUG_EQ46
                                std::cerr << TlUtils::format(
                                                 "EQ46(1st): (%s %s, %s %s %s, "
                                                 "%d %d %d| "
                            ...
                            // 2nd term
                            const TlAngularMomentumVector amv_a1 =
                                amv_a - this->E1_[i];
                            if (amv_a1.isExist() == true) {
                                const ERI_State eriState2(a_bar - 1, b_bar,
                                                          a - 1, b, p, a_prime,
                                                          b_prime, p_prime);
                                const int index2 =
                                    this->index(amv_a_bar1, amv_b_bar, amv_a1,
                                                amv_b, amv_p);
                                // const double value2 =
                                // (*pERI)[eriState2][index2][batch];
                                const double value2 =
                                    (*pERI)[eriState2.index()][index2][batch];
#ifdef DEBUG_EQ46
                                std::cerr
                                    << TlUtils::format(
                                           "EQ46(2nd): (%s %s, %s %s %s, %d %d "
                                           "%d| "
                                           "batch[%d] = % f",
                                           amv_a_bar1.debugOut().c_str(),
                                           amv_b_bar.debugOut().c_str(),
                            ...
                            // 3rd term
                            {
                                const ERI_State eriState3(
                                    a_bar - 1, b_bar, a, b, p, a_prime + 1,
                                    b_prime + 1, p_prime + 1);
                                const int index3 = this->index(
                                    amv_a_bar1, amv_b_bar, amv_a, amv_b, amv_p);
                                // const double value3 =
                                // (*pERI)[eriState3][index3][batch];
                                const double value3 =
                                    (*pERI)[eriState3.index()][index3][batch];
#ifdef DEBUG_EQ46
                                std::cerr << TlUtils::format(
                                                 "EQ46(3rd): (%s %s, %s %s %s, "
                                                 "%d %d %d| "
                            ...
                            // 4th term
                            const TlAngularMomentumVector amv_a_bar2 =
                                amv_a_bar - this->E2_[i];
                            if (amv_a_bar2.isExist() == true) {
                                const ERI_State eriState4(
                                    a_bar - 2, b_bar, a, b, p, a_prime + 1,
                                    b_prime + 1, p_prime + 1);
                                const int index4 = this->index(
                                    amv_a_bar2, amv_b_bar, amv_a, amv_b, amv_p);
                                // answer -= (amv_a_bar.get(i) -1.0) *
                                // (*pERI)[eriState4][index4][batch];
                                answer -=
                                    (amv_a_bar.get(i) - 1.0) *
                                    (*pERI)[eriState4.index()][index4][batch];
                            }
                            ...
                            const TlAngularMomentumVector amv_b_bar1 =
                                amv_b_bar - this->E1_[i];
                            if (amv_b_bar1.isExist() == true) {
                                const ERI_State eriState5(
                                    a_bar - 1, b_bar - 1, a, b, p, a_prime + 1,
                                    b_prime + 1, p_prime + 1);
                                const int index5 =
                                    this->index(amv_a_bar1, amv_b_bar1, amv_a,
                                                amv_b, amv_p);
                                // answer += amv_b_bar.get(i) *
                                // (*pERI)[eriState5][index5][batch];
                                answer +=
                                    amv_b_bar.get(i) *
                                    (*pERI)[eriState5.index()][index5][batch];
                            }
                    ...
                    for (int amv_p_index = 0; amv_p_index < numOfAmv_p;
                        ...
                        for (int batch = 0; batch < batchSize; ++batch) {
                            ...
                            // 1st term
                            {
                                const TlAngularMomentumVector amv_a1p =
                                    amv_a + this->E1_[i];
                                const ERI_State eriState1(
                                    a_bar - 1, b_bar, a + 1, b, p, a_prime + 1,
                                    b_prime, p_prime);
                                const int index1 =
                                    this->index(amv_a_bar1, amv_b_bar, amv_a1p,
                                                amv_b, amv_p);
                                // const double value1 =
                                // (*pERI)[eriState1][index1][batch];
                                const double value1 =
                    ...
                    for (int amv_p_index = 0; amv_p_index < numOfAmv_p;
                        ...
                        for (int batch = 0; batch < batchSize; ++batch) {
                            ...
                            // 1st term
                            {
                                const TlAngularMomentumVector amv_p1p =
                                    amv_p + this->E1_[i];
                                const ERI_State eriState1(a_bar, b_bar - 1, a,
                                                          b, p + 1, a_prime,
                                                          b_prime, p_prime);
                                const int index1 =
                                    this->index(amv_a_bar, amv_b_bar1, amv_a,
                                                amv_b, amv_p1p);
                                const double value1 =
                                    (*pERI)[eriState1.index()][index1][batch];
#ifdef DEBUG_EQ47
                            ...
                            // 2nd term
                            const TlAngularMomentumVector amv_a_bar1p =
                                amv_a_bar + this->E1_[i];
                            {
                                const ERI_State eriState2(a_bar + 1, b_bar - 1,
                                                          a, b, p, a_prime,
                                                          b_prime, p_prime);
                                const int index2 =
                                    this->index(amv_a_bar1p, amv_b_bar1, amv_a,
                                                amv_b, amv_p);
                                const double value2 =
                                    (*pERI)[eriState2.index()][index2][batch];
#ifdef DEBUG_EQ47
                            ...
                            const TlAngularMomentumVector amv_a1 =
                                amv_a - this->E1_[i];
                            if (amv_a1.isExist() == true) {
                                const ERI_State eriState3(a_bar, b_bar - 1,
                                                          a - 1, b, p, a_prime,
                                                          b_prime, p_prime);
                                const int index3 =
                                    this->index(amv_a_bar, amv_b_bar1, amv_a1,
                                                amv_b, amv_p);
                                const double value3 =
                                    (*pERI)[eriState3.index()][index3][batch];
#ifdef DEBUG_EQ47
                            ...
                            // 4th term
                            const TlAngularMomentumVector amv_b1 =
                                amv_b - this->E1_[i];
