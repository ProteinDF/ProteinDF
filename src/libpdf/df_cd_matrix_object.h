...
Vector DfCdkMatrix::getScreenedDensityMatrix(const RUN_TYPE runType,
    ...
    switch (runType) {
        case RUN_RKS:
            P = 0.5 *
                this->getPInMatrix<SymmetricMatrix>(RUN_RKS, this->m_nIteration - 1);
            break;

        case RUN_UKS_ALPHA:
        case RUN_UKS_BETA:
            P = this->getPInMatrix<SymmetricMatrix>(runType, this->m_nIteration - 1);
            break;

        case RUN_ROKS_ALPHA: {
            P = 0.5 *
                this->getPInMatrix<SymmetricMatrix>(RUN_ROKS_CLOSED,
                                                    this->m_nIteration - 1);
            P += this->getInqMatrix<TlDenseSymmetricMatrix_Lapack>(
