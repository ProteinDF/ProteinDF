...
void TlLibcint::calc_ovp(const int shell_id_i, const int shell_id_j) {
    ...
    if (0 != cint1e_ovlp_sph(&(buf[0]), shls, &(this->atomTable_[0]),
        ...
        for (int q = 0; q < dj; ++q) {
            for (int p = 0; p < di; ++p) {
                std::cout << TlUtils::format("(%d %d) = % f", shell_id_i + p,
                                             shell_id_j + q, buf[p + q * dj])
                          << std::endl;
            }
        }
    } else {
                ...
                for (int q = 0; q < dj; ++q) {
                    for (int p = 0; p < di; ++p) {
                        ...
                        std::cout << TlUtils::format(
                                         "(%d %d|%d %d) = % f", shell_id_i + p,
                                         shell_id_j + q, shell_id_k + r,
                                         shell_id_l + s,
...
void TlLibcint::makeBasissetTable(const TlOrbitalInfo &orbInfo) {
    ...
    // bas[i*8+5]: 原始GTO exp のオフセット(env[bas[i*8+5]+0],
    // env[bas[i*8+5]+1], ...) bas[i*8+6]: 原始GTO coef のオフセットenv offset
    // to save column-major contraction coefficients.
...
int TlLibcint::getValuesOffset() const { return this->valuesOffset_; }
