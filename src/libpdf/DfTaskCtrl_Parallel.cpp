...
bool DfTaskCtrl_Parallel::getQueue2(const TlOrbitalInfoObject& orbitalInfo,
  ...
  bool answer = false;
  if (this->isMasterSlave_ == true) {
    answer = this->getQueue2_MS(orbitalInfo, isCutoffDistribution, maxGrainSize,
                                pTask, initialize);
  } else {
    answer = this->getQueue2_DC(orbitalInfo, isCutoffDistribution, maxGrainSize,
                                pTask, initialize);
  }
...
bool DfTaskCtrl_Parallel::getQueue2_MS_master(
  ...
  for (int i = 0; i < maxContinuousSessionRequests; ++i) {
    ...
    if ((isRecvSessionID == true) && (rComm.test(sessionID, &rank) == true)) {
      rComm.wait(sessionID);
      isRecvSessionID = false;
      // std::cerr << TlUtils::format("session accept: rank=%d, session=%d",
      // rank, sessionID)
      //           << std::endl;
  ...
  for (RequestListType::iterator it = requestList.begin(); it != itEnd; ++it) {
    ...
    if (it->state[Request::SEND_ANSWER] != true) {
      ...
      if (it->state[Request::HAS_TASK] == true) {
        ...
        rComm.iSendDataX((index_type*)&(it->unpackTaskList[0]), it->taskSize,
                         rank, DFTC_TASK + sessionID);
        // std::cerr << TlUtils::format("send_task: rank=%d, session=%d", rank,
        // sessionID)
        //           << std::endl;
      } else {
        it->finish = 9999;
        rComm.iSendData(it->finish, rank, DFTC_FINISH + sessionID);
        // std::cerr << TlUtils::format("send_finish: rank=%d, session=%d",
        // rank, sessionID)
        //           << std::endl;
      }
...
bool DfTaskCtrl_Parallel::getQueue2_MS_slave(
  ...
  for (int i = 0; i < numOfSessions; ++i) {
    ...
    if ((session.state[Session::WAIT_SIZE] == true) &&
      ...
      session.unpackTaskList.resize(session.size);
      rComm.iReceiveDataX(&(session.unpackTaskList[0]), session.size, master,
                          DFTC_TASK + i);
      session.state[Session::RECV_TASK_LIST] = true;
    }
...
bool DfTaskCtrl_Parallel::getQueue2_MS_master(
  ...
  for (RequestListType::iterator it = requestList.begin(); it != itEnd; ++it) {
    if (it->state[Request::TASK_ASSIGNED] != true) {
      std::vector<Task2> tmpTaskList;
      const bool hasTask = DfTaskCtrl::getQueue2(orbitalInfo1, orbitalInfo2,
...
bool DfTaskCtrl_Parallel::getQueue4_DC(
  ...
  bool answer = false;
  answer = DfTaskCtrl::getQueue4(orbitalInfo, schwarzTable, globalMaxGrainSize,
...
bool DfTaskCtrl_Parallel::getQueue4_MS(
  ...
  TlCommunicate& rComm = TlCommunicate::getInstance();
  if (rComm.isMaster() == true) {
    answer = this->getQueue4_MS_master(orbitalInfo, schwarzTable, maxGrainSize,
                                       pTaskList, initialize);
  } else {
    answer = this->getQueue4_MS_slave(orbitalInfo, schwarzTable, maxGrainSize,
                                      pTaskList, initialize);
  }
...
bool DfTaskCtrl_Parallel::getQueue_Force4(
  ...
  bool answer = false;
  if (this->isMasterSlave_ == true) {
    answer = this->getQueue_Force4_MS(orbitalInfo, schwarzTable, maxGrainSize,
                                      pTaskList, initialize);
  } else {
    answer = this->getQueue_Force4_DC(orbitalInfo, schwarzTable, maxGrainSize,
                                      pTaskList, initialize);
  }
