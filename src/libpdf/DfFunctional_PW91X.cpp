...
double DfFunctional_PW91X::getFunctional(const double dRhoA,
...
void DfFunctional_PW91X::getDerivativeFunctional(const double dRhoA,
                                                 const double dGammaAA,
...
void DfFunctional_PW91X::getDerivativeFunctional(
...
double DfFunctional_PW91X::pw91x(const double r, const double s) {
  ...
  const double term2 = sCC *
                       (F - exp(-100.0 * sCC / (M_2_2_3 * r_8_3 * AA)) * G) /
                       (M_2_2_3 * r_8_3 * AA);
...
double DfFunctional_PW91X::pw91x_roundRho(const double r, const double s) {
  ...
  const double term7 =
      400.0 * sCC * sCC * term2_e / (3.0 * M_2_1_3 * r_19_3 * AAAA);
...
double DfFunctional_PW91X::pw91x_roundGamma(const double r, const double s) {
  ...
  const double ans1 =
      term1 * (term2 + term3 + term4 + term5) / (term6 + term7 + 1.0);
  ...
  const double ans2 = term8 * (term9 + term10 + 1.0) *
                      (term11 + term12 + term13) / (ans2_b * ans2_b);
