...
class DfConverge_Anderson : public DfConverge_Damping {
 ...
 protected:
  ...
  template <class SymmetricMatrixType, class VectorType>
  void writeKSMatrixFromVector(const DfObject::RUN_TYPE runType,
                               const int nIteration, const VectorType& v) const;

  template <class SymmetricMatrixType, class VectorType>
  VectorType getVectorOfPMatrix(const DfObject::RUN_TYPE runType,
                                const int nIteration) const;

  template <class SymmetricMatrixType, class VectorType>
  void writePMatrixFromVector(const DfObject::RUN_TYPE runType,
                              const int nIteration, const VectorType& v) const;
...
void DfConverge_Anderson::convergeRhoTilde(const DfObject::RUN_TYPE runType) {
  ...
  this->log_.info(
      TlUtils::format(" start-number = %2d", nAndersonStartIterationNumber));
...
void DfConverge_Anderson::convergeKSMatrix(const DfObject::RUN_TYPE runType) {
  ...
  if (this->m_nIteration >= (nAndersonStartIterationNumber - 1)) {
    VectorType nextY1 =
        this->getVectorOfKSMatrix<SymmetricMatrixType, VectorType>(runType,
                                                                   nIteration);
    nextY1.save("fl_Work/fl_Vct_anderson." + TlUtils::xtos(nIteration));
  }
  ...
  if (this->m_nIteration >= nAndersonStartIterationNumber) {
    ...
    VectorType X1 = this->getVectorOfKSMatrix<SymmetricMatrixType, VectorType>(
        runType, nIteration - 1);

    VectorType Y1;
    Y1.load("fl_Work/fl_Vct_anderson." + TlUtils::xtos(nIteration - 1));

    VectorType X2 = this->getVectorOfKSMatrix<SymmetricMatrixType, VectorType>(
        runType, nIteration - 2);

    VectorType X0 = this->anderson(X1, X2, Y0, Y1);
    this->writeKSMatrixFromVector<SymmetricMatrixType, VectorType>(
        runType, nIteration, X0);
  } else {
    // do damping
...
void DfConverge_Anderson::convergePMatrix(const DfObject::RUN_TYPE runType) {
  ...
  if (this->m_nIteration >= (nAndersonStartIterationNumber - 1)) {
    VectorType nextY1 =
        this->getVectorOfPMatrix<SymmetricMatrixType, VectorType>(
            runType, this->m_nIteration - 1);
    nextY1.save("fl_Work/fl_Vct_anderson." +
  ...
  if (this->m_nIteration >= nAndersonStartIterationNumber) {
    ...
    VectorType X1 = this->getVectorOfPMatrix<SymmetricMatrixType, VectorType>(
        runType, this->m_nIteration - 2);
    X1.save("fl_Work/fl_Vct_andersonX1." +
            TlUtils::xtos(this->m_nIteration - 2));

    VectorType Y1;
    Y1.load("fl_Work/fl_Vct_anderson." + TlUtils::xtos(this->m_nIteration - 2));

    VectorType X2 = this->getVectorOfPMatrix<SymmetricMatrixType, VectorType>(
        runType, this->m_nIteration - 3);
    X2.save("fl_Work/fl_Vct_andersonX2." +
            TlUtils::xtos(this->m_nIteration - 3));

    VectorType X0 = this->anderson(X1, X2, Y0, Y1);
    this->writePMatrixFromVector<SymmetricMatrixType, VectorType>(
        runType, this->m_nIteration - 1, X0);
  } else {
    // do damping
...
// 変数末尾の数字の大きさに注意すること
template <class VectorType>
VectorType DfConverge_Anderson::anderson(const VectorType& X1,
                                         const VectorType& X2,
                                         const VectorType& Y0,
                                         const VectorType& Y1) {
  const double beta =
      this->m_dDampingFactorOfAnderson;  // damping factor of Anderson's method
...
template <class SymmetricMatrixType, class VectorType>
VectorType DfConverge_Anderson::getVectorOfKSMatrix(
...
template <class SymmetricMatrixType, class VectorType>
VectorType DfConverge_Anderson::getVectorOfPMatrix(
    const DfObject::RUN_TYPE runType, const int nIteration) const {
