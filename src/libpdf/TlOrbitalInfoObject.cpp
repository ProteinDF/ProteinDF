...
void TlOrbitalInfoObject::setCGTO(const Fl_Gto& flGto) {
    ...
    for (int cgto = 0; cgto < numOfCgto; ++cgto) {
        ...
        switch (shell) {
            ...
            default:
                this->log_.critical(TlUtils::format(
                    "Now we support 's' to 'g': input %d.", shell));
        ...
        double factor = 0.0;
        const double normalizedFactor =
            flGto.getNormalizedfactor(cgto, l, m, n);
        for (int pgto = 0; pgto < cgtoContraction; ++pgto) {
            const double coef = flGto.getCoefficient(cgto, pgto) *
                                flGto.getNormalized(cgto, pgto, l, m, n) *
                                normalizedFactor;
...
void TlOrbitalInfoObject::setCGTO_coulomb(const Fl_Gto& flGto) {
    ...
    for (int cgto = 0; cgto < numOfCgto; ++cgto) {
        ...
        switch (shell) {
            ...
            default:
                this->log_.critical(TlUtils::format(
                    "Now we support 's' to 'd': input %d.", shell));
                this->log_.critical(
                    "stop at TlOrbitalInfoObject::setCGTO_coulomb().");
                exit(1);
        }
...
void TlOrbitalInfoObject::makeOrbitalTable() {
    ...
    for (int atom = 0; atom < numOfAtoms; ++atom) {
        ...
        for (int cgto = 0; cgto < numOfCgtos; ++cgto) {
            this->log_.debug(
                TlUtils::format("current: cgto=%d symbol=%s label=\"%s\"", cgto,
                                this->cgtos_[cgto].atomSymbol.c_str(),
                                this->cgtos_[cgto].label.c_str()));
            if ((this->cgtos_[cgto].atomSymbol == symbol) &&
                (this->cgtos_[cgto].label == label)) {
                ...
                switch (shell) {
                    ...
                    default:
                        std::cerr
                            << TlUtils::format("make table error. @ %s:%d\n",
                                               __FILE__, __LINE__)
                            << std::endl;
                        break;
                }
                ...
                for (int basisType = 0; basisType < maxBasisType; ++basisType) {
                    tmpOrbital.basisType =
                        shellType2 + basisType;  // s=0, px=1, py=2, ...
        ...
        if (isFound == false) {
            this->log_.critical(
                TlUtils::format("the basis is not found: atom index = %d, "
                                "symbol=\"%s\", label=\"%s\"",
...
TlOrbitalInfoObject::getStartIndexArrayOfShellGroup() const {
...
void TlOrbitalInfoObject::getPrefactor_PX(const TlPosition& pos,
                                          double* pValue) {
    assert(pValue != NULL);
    *pValue = pos.x();
}

void TlOrbitalInfoObject::getPrefactor_PY(const TlPosition& pos,
                                          double* pValue) {
    assert(pValue != NULL);
    *pValue = pos.y();
}

void TlOrbitalInfoObject::getPrefactor_PZ(const TlPosition& pos,
                                          double* pValue) {
    assert(pValue != NULL);
    *pValue = pos.z();
}

void TlOrbitalInfoObject::getPrefactor_DXY(const TlPosition& pos,
                                           double* pValue) {
    assert(pValue != NULL);
    *pValue = pos.x() * pos.y();
}

void TlOrbitalInfoObject::getPrefactor_DXZ(const TlPosition& pos,
                                           double* pValue) {
    assert(pValue != NULL);
    *pValue = pos.x() * pos.z();
}

void TlOrbitalInfoObject::getPrefactor_DYZ(const TlPosition& pos,
                                           double* pValue) {
    assert(pValue != NULL);
    *pValue = pos.y() * pos.z();
}

void TlOrbitalInfoObject::getPrefactor_DXX_YY(const TlPosition& pos,
                                              double* pValue) {
    assert(pValue != NULL);
    *pValue = 0.5 * (pos.x() * pos.x() - pos.y() * pos.y());
}

void TlOrbitalInfoObject::getPrefactor_DZZ(const TlPosition& pos,
                                           double* pValue) {
    assert(pValue != NULL);
    *pValue = INV_SQRT3 * (pos.z() * pos.z() -
                           0.5 * (pos.x() * pos.x() + pos.y() * pos.y()));
}

void TlOrbitalInfoObject::getGradPrefactor_S(const TlPosition& pos,
                                             const double alpha,
                                             double* pValueX, double* pValueY,
                                             double* pValueZ) {
    assert(pValueX != NULL);
    assert(pValueY != NULL);
    assert(pValueZ != NULL);
...
void TlOrbitalInfoObject::getGradPrefactor_PX(const TlPosition& pos,
                                              const double alpha,
                                              double* pValueX, double* pValueY,
                                              double* pValueZ) {
    assert(pValueX != NULL);
...
void TlOrbitalInfoObject::getGradPrefactor_PY(const TlPosition& pos,
                                              const double alpha,
                                              double* pValueX, double* pValueY,
                                              double* pValueZ) {
    assert(pValueX != NULL);
...
void TlOrbitalInfoObject::getGradPrefactor_PZ(const TlPosition& pos,
                                              const double alpha,
                                              double* pValueX, double* pValueY,
                                              double* pValueZ) {
    assert(pValueX != NULL);
...
void TlOrbitalInfoObject::getGradPrefactor_DXY(const TlPosition& pos,
                                               const double alpha,
                                               double* pValueX, double* pValueY,
                                               double* pValueZ) {
    assert(pValueX != NULL);
...
void TlOrbitalInfoObject::getGradPrefactor_DXZ(const TlPosition& pos,
                                               const double alpha,
                                               double* pValueX, double* pValueY,
                                               double* pValueZ) {
    assert(pValueX != NULL);
...
void TlOrbitalInfoObject::getGradPrefactor_DYZ(const TlPosition& pos,
                                               const double alpha,
                                               double* pValueX, double* pValueY,
                                               double* pValueZ) {
    assert(pValueX != NULL);
...
void TlOrbitalInfoObject::getGradPrefactor_DXX_YY(const TlPosition& pos,
                                                  const double alpha,
                                                  double* pValueX,
                                                  double* pValueY,
                                                  double* pValueZ) {
...
void TlOrbitalInfoObject::getGradPrefactor_DZZ(const TlPosition& pos,
                                               const double alpha,
                                               double* pValueX, double* pValueY,
                                               double* pValueZ) {
    assert(pValueX != NULL);
