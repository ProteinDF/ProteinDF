   ...
   public:
    ...
    Cgto(const Cgto& rhs)
        : Snum(rhs.Snum),
          Pnum(rhs.Pnum),
          Dnum(rhs.Dnum),
          basisName(rhs.basisName),
          atom(rhs.atom),
          label(rhs.label),
          shellname(rhs.shellname),
          shell(rhs.shell),
          scalefactor(rhs.scalefactor),
          pgto(rhs.pgto) {}

    int contraction() const { return this->pgto.size(); }
   ...
   public:
    struct Pgto {
     public:
      ...
      Pgto(const Pgto& rhs)
          : exponent(rhs.exponent), coefficient(rhs.coefficient) {}
    ...
    std::string label;      // label2 for atom to which CGTO belongs. this label
                            // starts in '@' character.
