...
const double DfFunctional_VWN::EC_COEF =
    4.0 / (9.0 * (pow(2.0, 1.0 / 3.0) - 1.0));  // epsilon_c の係数
...
void DfFunctional_VWN::getDerivativeFunctional(const double dRhoA,
                                               const double dRhoB,
...
double DfFunctional_VWN::epsilonC(const double x, const double zeta) {
    ...
    const double dArg2Coef =
        1.0 + (EC_COEF * (dEc_f - dEc_p) / dEc_a - 1.0) * zeta4;

    const double dAnswer = dEc_p + dEc_a * g * dArg2Coef;

    // std::cout << "zeta=" << zeta << ", ECp=" << dEc_p << ", ECa=" << dEc_a <<
    // ", g=" << g << ", coef=" << dArg2Coef << std::endl;
...
double DfFunctional_VWN::epsilonC(const double x) {
    ...
    // const double dArg2Coef = 1.0 + (EC_COEF * (dEc_f - dEc_p) / dEc_a - 1.0)
    // * zeta4;
...
double DfFunctional_VWN::epsilonC_PARA(const double x) {
    return this->epsilonC(VWN5_A_PARA, VWN5_B_PARA, VWN5_C_PARA, VWN5_X0_PARA,
                          x);
}

double DfFunctional_VWN::epsilonC_FERR(const double x) {
    return this->epsilonC(VWN5_A_FERR, VWN5_B_FERR, VWN5_C_FERR, VWN5_X0_FERR,
                          x);
}
...
// (A13)
double DfFunctional_VWN::epsilonC(const double A, const double b,
...
// h(x)
// (A15)
double DfFunctional_VWN::h(const double dEc_p, const double dEc_f,
                           const double dEc_a) {
    return EC_COEF * ((dEc_f - dEc_p) / dEc_a) - 1.0;
}

// (A18)
double DfFunctional_VWN::epsilonCPrime(const double A, const double b,
                                       const double c, const double x0,
                                       const double x) {
    // X(x) = x^2 + bx + c (A14)
    const double X = x * (x + b) + c;
...
// (A19)
double DfFunctional_VWN::h_prime(const double dEc_p, const double dEc_f,
                                 const double dEc_a, const double dEc_p_prime,
                                 const double dEc_f_prime,
                                 const double dEc_a_prime) {
    const double dInvEc_a = 1.0 / dEc_a;

    const double dAnswer =
        EC_COEF * dInvEc_a *
        (dEc_f_prime - dEc_p_prime - (dEc_f - dEc_p) * dInvEc_a * dEc_a_prime);
...
void DfFunctional_VWN::roundVWN_roundRho(const double dRhoA, const double dRhoB,
    ...
    *pRoundF_roundRhoB = 0.0;
    ...
    const double h_prime = this->h_prime(dEc_p, dEc_f, dEc_a, dEc_p_prime,
                                         dEc_f_prime, dEc_a_prime);
    ...
    // const double term2coef = dEc_a * (g_prime * (1.0 + h * zeta) + 4.0 * g *
    // h
    // * zeta);
    ...
    //      << TlUtils::format(" RoundF_roundRhoA = %+e + %+e * %+e = %+e", EC,
    //      dRho, dRoundEC_roundRhoA, *pRoundF_roundRhoA)
    //      << "\n"
    ...
    //      << TlUtils::format(" RoundF_roundRhoB = %+e + %+e * %+e = %+e", EC,
    //      dRho, dRoundEC_roundRhoB, *pRoundF_roundRhoB) << std::endl;
}

// specialized for RKS
void DfFunctional_VWN::roundVWN_roundRho(const double dRhoA,
                                         double* pRoundF_roundRhoA) {
    assert(pRoundF_roundRhoA != NULL);
    ...
    *pRoundF_roundRhoA = 0.0;
    ...
    // const double h = this->h(dEc_p, dEc_f, dEc_a);
    // const double h_prime = this->h_prime(dEc_p, dEc_f, dEc_a, dEc_p_prime,
    // dEc_f_prime, dEc_a_prime);  const double zeta3 = zeta * zeta * zeta; //
    // zeta^3  const double zeta4 = zeta3 * zeta; // zeta^4
    ...
    // const double term2coef = dEc_a * (g_prime * (1.0 + h * zeta4) + 4.0 * g *
    // h
    // * zeta3); // =0 when RKS.
...
double DfFunctional_VWN::epsilonCPrime_PARA(const double x) {
    return this->epsilonCPrime(VWN5_A_PARA, VWN5_B_PARA, VWN5_C_PARA,
                               VWN5_X0_PARA, x);
}

double DfFunctional_VWN::epsilonCPrime_FERR(const double x) {
    return this->epsilonCPrime(VWN5_A_FERR, VWN5_B_FERR, VWN5_C_FERR,
                               VWN5_X0_FERR, x);
}

double DfFunctional_VWN::epsilonCPrime_ANTI(const double x) {
    return this->epsilonCPrime(VWN_A_ANTI, VWN_B_ANTI, VWN_C_ANTI, VWN_X0_ANTI,
                               x);
}
