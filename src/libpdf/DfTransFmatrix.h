...
void DfTransFmatrix::main(const RUN_TYPE runType, const std::string& fragname,
    ...
    // "read F matrix"
    SymmetricMatrixType symmF(DfObject::getFpqMatrix<SymmetricMatrixType>(
    ...
    if (bPdfQcloMode == false) {
        ...
        // PDF-QCLO method
        X = DfObject::getCMatrix<MatrixType>(runType, tmp_iteration - 1,
                                             fragname);
    }
    ...
    if (bPdfQcloMode != true) {
        ...
        fname += fragname + ".";
        fname +=
            (this->m_sRunTypeSuffix[runType] + TlUtils::xtos(tmp_iteration));
        Fprime.save(fname);
    }
}
