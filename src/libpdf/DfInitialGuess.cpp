...
DfInitialGuess::DfInitialGuess(TlSerializeData* pPdfParam)
  ...
  if ((*this->pPdfParam_)["guess/normalize_density_matrix"].getStr().empty() ==
      false) {
...
TlVector DfInitialGuess::createOccupation(const RUN_TYPE runType) {
  ...
  switch (runType) {
    ...
    case RUN_ROKS_CLOSED: {
      const std::vector<int> occLevel_c =
          this->getLevel(pdfParam["method/roks/closed_occlevel"].getStr());
