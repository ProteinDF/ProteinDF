  ...
  switch (nType) {
    ...
    case 7:  // x2-y2
      *pPrefactor = 0.5 * (pos.x() * pos.x() - pos.y() * pos.y());
      break;
    case 8:  // z2
      *pPrefactor = INV_SQRT3 * (pos.z() * pos.z() -
...
// {
//     this->getPhiTable(gridPosition, 0, this->m_tlOrbInfo.getNumOfOrbitals(),
//     aPhi);
...
//         const TlPosition pos = gridPosition -
//         this->m_tlOrbInfo.getPosition(nOrb); const double distance2 =
//         pos.squareDistanceFrom(); const int nBasisType =
//         this->m_tlOrbInfo.getBasisType(nOrb); double prefactor = 0.0;
...
//             if (shoulder <= TOOBIG) {
//                 const double gtmp = this->m_tlOrbInfo.getCoefficient(nOrb,
//                 nPGTO) * std::exp(-shoulder); dPhi += prefactor * gtmp;
//             }
//         }
...
//                 const double gtmp = this->m_tlOrbInfo.getCoefficient(nOrb,
//                 nPGTO) * std::exp(-shoulder); dPhi += prefactor * gtmp;
//                 this->getPrefactorForDerivative(nBasisType, alpha, pos,
//                 &prefactorX, &prefactorY, &prefactorZ); dGradPhiX +=
//                 prefactorX * gtmp; dGradPhiY += prefactorY * gtmp; dGradPhiZ
//                 += prefactorZ * gtmp;
//             }
//         }
...
//         const TlPosition pos = gridPosition -
//         this->m_tlOrbInfo.getPosition(AO); const double distance2 =
//         pos.squareDistanceFrom(); const int basisType =
//         this->m_tlOrbInfo.getBasisType(AO); double prefactor = 0.0;
...
void DfCalcGridX::getAOs_core(const TlPosition& gridPosition,
  ...
  for (int i = 0; i < numOfIndeces; ++i) {
    ...
    for (int PGTO = 0; PGTO < contraction; ++PGTO) {
      ...
      if (shoulder <= TOOBIG) {
        const double g = this->m_tlOrbInfo.getCoefficient(AO_index, PGTO) *
                         std::exp(-shoulder);
        double coef = 0.0;
...
void DfCalcGridX::getDAOs(const TlPosition& gridPosition,
  ...
  this->getDAOs_core(gridPosition, AO_indeces, p_dAO_dx_values, p_dAO_dy_values,
                     p_dAO_dz_values);
}
  ...
  for (int i = 0; i < numOfIndeces; ++i) {
    ...
    for (int PGTO = 0; PGTO < contraction; ++PGTO) {
      ...
      if (shoulder <= TOOBIG) {
        const double g = this->m_tlOrbInfo.getCoefficient(AO_index, PGTO) *
                         std::exp(-shoulder);
        this->getPrefactorForDerivative(basisType, alpha, pos, &pX, &pY, &pZ);
        dAO_dx += pX * g;
        dAO_dy += pY * g;
        dAO_dz += pZ * g;
      }
  ...
  for (int i = 0; i < numOfIndeces; ++i) {
    ...
    for (int PGTO = 0; PGTO < contraction; ++PGTO) {
      ...
      if (shoulder <= TOOBIG) {
        const double g = this->m_tlOrbInfo.getCoefficient(AO_index, PGTO) *
                         std::exp(-shoulder);
        this->getPrefactorForSecondDerivative(basisType, alpha, pos, &pXX, &pXY,
                                              &pXZ, &pYY, &pYZ, &pZZ);
        d2AO_dxdx += pXX * g;
        d2AO_dxdy += pXY * g;
        d2AO_dxdz += pXZ * g;
...
void DfCalcGridX::buildFock(std::vector<WFGrid>::const_iterator pBegin,
  ...
  for (std::vector<WFGrid>::const_iterator p = pBegin; p != pEnd; ++p) {
    ...
    // 対角要素以外
    std::vector<WFGrid>::const_iterator qEnd = std::upper_bound(
        p + 1, pEnd, WFGrid(0, cutoffValue / tmp1A), WFGrid_sort_functional());
      ...
      if (rhoA > densityCutOffValue) {
        ...
        for (int atomIndexB = 0; atomIndexB < numOfAtoms; ++atomIndexB) {
          if (atomIndexA != atomIndexB) {
            const TlVector nablaB_omegaA = dfGenGrid.JGP_nablaB_omegaA(
                atomIndexA, atomIndexB, gridPosition);
            const TlVector E_nablaB_omegaAi =
  ...
  for (int atomIndexA = startAtomIndex; atomIndexA < endAtomIndex;
    ...
    for (int gridIndex = 0; gridIndex < numOfGrids; ++gridIndex) {
      ...
      this->getD2AOs(gridPosition, &d2AO_dxdx_values, &d2AO_dxdy_values,
                     &d2AO_dxdz_values, &d2AO_dydy_values, &d2AO_dydz_values,
                     &d2AO_dzdz_values);
      ...
      this->getGradRhoAtGridPoint(P_A, AO_values, dAO_dx_values, dAO_dy_values,
                                  dAO_dz_values, &gradRhoAX, &gradRhoAY,
                                  &gradRhoAZ);
      ...
      if (rhoA > densityCutOffValue) {
        const double w = partitioningWeight * singleCenterWeight;
        const double gammaAA = gradRhoAX * gradRhoAX + gradRhoAY * gradRhoAY +
                               gradRhoAZ * gradRhoAZ;
        ...
        // calc energy
        const double ene =
            pFunctional->getFunctional(rhoA, rhoB, gammaAA, gammaAB, gammaBB);
        ene_xc += w * ene;
...
// experimental code -----------------------------------------------------------
// void DfCalcGridX::calcRhoVals_LDA(const std::vector<index_type>&
// P_rowIndexes,
//                                   const std::vector<index_type>&
//                                   P_colIndexes, const TlMatrix& P, TlMatrix*
//                                   pGridMatrix)
// {
...
// void DfCalcGridX::calcRhoVals_GGA(const std::vector<index_type>&
// P_rowIndexes,
//                                   const std::vector<index_type>&
//                                   P_colIndexes, const TlMatrix& P, TlMatrix*
//                                   pGridMatrix)
...
//         const TlPosition r = gridPosition -
//         this->m_tlOrbInfo.getPosition(orb); const double r2 =
//         r.squareDistanceFrom(); const int basisType =
//         this->m_tlOrbInfo.getBasisType(orb); double prefactor = 0.0;
...
//             const double alpha = this->m_tlOrbInfo.getExponent(orb,
//             pgtoIndex); const double exponent = alpha * r2; const double coef
//             = this->m_tlOrbInfo.getCoefficient(orb, pgtoIndex); value += coef
//             * std::exp(- exponent);
//         }
...
//         for (int pgtoIndex = 0; pgtoIndex < numOfPGTOs; ++pgtoIndex) {
//             const double alpha = this->m_tlOrbInfo.getExponent(orb,
//             pgtoIndex); const double exponent = alpha * r2; const double e =
//             std::exp(- exponent); const double coef =
//             this->m_tlOrbInfo.getCoefficient(orb, pgtoIndex);
...
//             const double gammaAA =  gradRhoX_A*gradRhoX_A +
//             gradRhoY_A*gradRhoY_A + gradRhoZ_A*gradRhoZ_A; energy += w *
//             pFunctional->getFunctional(rhoA, gammaAA); // RKS code
//         }
//     }
...
//             const double gammaAA =  gradRhoX_A*gradRhoX_A +
//             gradRhoY_A*gradRhoY_A + gradRhoZ_A*gradRhoZ_A;
//             // build K
//             double rF_rR_A;
...
//             const double coef2_AX = w * (rF_rG_AA2 * gradRhoX_A + rF_rG_AB *
//             gradRhoX_A); const double coef2_AY = w * (rF_rG_AA2 * gradRhoY_A
//             + rF_rG_AB * gradRhoY_A); const double coef2_AZ = w * (rF_rG_AA2
...
//             // energy
//             energy += w * pFunctional->getFunctional(rhoA, gammaAA); // RKS
//             code
//         }
//     }
...
#pragma omp parallel for schedule(runtime)
  for (index_type grid = 0; grid < numOfGrids; ++grid) {
    ...
    std::vector<double> dAO_dy_values;
    std::vector<double> dAO_dz_values;
    this->getDAOs(gridPosition, &dAO_dx_values, &dAO_dy_values, &dAO_dz_values);
    ...
    this->getRhoAtGridPoint(P_A, AO_values, &rhoA);
    this->getGradRhoAtGridPoint(P_A, AO_values, dAO_dx_values, dAO_dy_values,
                                dAO_dz_values, &gradRhoXA, &gradRhoYA,
                                &gradRhoZA);
...
#pragma omp parallel for schedule(runtime)
  for (index_type grid = 0; grid < numOfGrids; ++grid) {
    ...
    this->getGradRhoAtGridPoint(P_A, AO_values, dAO_dx_values, dAO_dy_values,
                                ...
                                &gradRhoZA);
    this->getRhoAtGridPoint(P_B, AO_values, &rhoB);
    this->getGradRhoAtGridPoint(P_B, AO_values, dAO_dx_values, dAO_dy_values,
                                dAO_dz_values, &gradRhoXB, &gradRhoYB,
                                &gradRhoZB);
...
#pragma omp parallel for schedule(runtime) reduction(+ : energy)
  for (index_type grid = 0; grid < numOfGrids; ++grid) {
    ...
    if (rhoA > densityCutOffValue) {
      ...
      // this->build_XC_Matrix(roundF_roundRhoA, phi, pFunctional, weight,
      // pF_A);
      this->build_XC_Matrix(roundF_roundRhoA, AO_values, pFunctional, weight,
                            pF_A);
...
#pragma omp parallel for schedule(runtime) reduction(+ : energy)
  for (index_type grid = 0; grid < numOfGrids; ++grid) {
    ...
    if ((rhoA > densityCutOffValue) || (rhoB > densityCutOffValue)) {
      ...
      // this->build_XC_Matrix(roundF_roundRhoA, phi, pFunctional, weight,
      // pF_A); this->build_XC_Matrix(roundF_roundRhoB, phi, pFunctional,
      // weight, pF_B);
      this->build_XC_Matrix(roundF_roundRhoA, AO_values, pFunctional, weight,
                            pF_A);
      this->build_XC_Matrix(roundF_roundRhoB, AO_values, pFunctional, weight,
                            pF_B);
  ...
  for (index_type grid = 0; grid < numOfGrids; ++grid) {
    ...
    if (rhoA > densityCutOffValue) {
      const double gammaAA =
          gradRhoXA * gradRhoXA + gradRhoYA * gradRhoYA + gradRhoZA * gradRhoZA;
      double roundF_roundRhoA;
      double roundF_roundGammaAA;
      double roundF_roundGammaAB;
      pFunctional->getDerivativeFunctional(rhoA, gammaAA, &roundF_roundRhoA,
                                           &roundF_roundGammaAA,
                                           &roundF_roundGammaAB);
      ...
      this->build_XC_Matrix(roundF_roundRhoA, roundF_roundGammaAA,
                            roundF_roundGammaAB, gradRhoXA, gradRhoYA,
...
#pragma omp parallel for schedule(runtime) reduction(+ : energy)
  for (index_type grid = 0; grid < numOfGrids; ++grid) {
    ...
    if ((rhoA > densityCutOffValue) || ((rhoB > densityCutOffValue))) {
      const double gammaAA =
          gradRhoXA * gradRhoXA + gradRhoYA * gradRhoYA + gradRhoZA * gradRhoZA;
      const double gammaAB =
          gradRhoXA * gradRhoXB + gradRhoYA * gradRhoYB + gradRhoZA * gradRhoZB;
      const double gammaBB =
          gradRhoXB * gradRhoXB + gradRhoYB * gradRhoYB + gradRhoZB * gradRhoZB;
      ...
      //                       phi, gradPhiX, gradPhiY, gradPhiZ,
      //                       pFunctional, weight, pF_A);
      // this->build_XC_Matrix(roundF_roundRhoB, roundF_roundGammaBB,
      // roundF_roundGammaAB,
      ...
      this->build_XC_Matrix(roundF_roundRhoA, roundF_roundGammaAA,
                            ...
                            dAO_dz_values, pFunctional, weight, pF_A);
      this->build_XC_Matrix(roundF_roundRhoB, roundF_roundGammaBB,
                            roundF_roundGammaAB, gradRhoXB, gradRhoYB,
...
//                                   DfFunctional_LDA* pFunctional, const double
//                                   weight, TlMatrixObject* pF_A)
// {
...
//                                  WFGrid(0,
//                                  std::sqrt(std::fabs(cutOffValue/coeff1_A))),
...
//                                   const double gradRhoAX, const double
//                                   gradRhoAY, const double gradRhoAZ, const
//                                   std::vector<WFGrid>& phi, const
//                                   std::vector<WFGrid>& gradPhiX, const
//                                   std::vector<WFGrid>& gradPhiY, const
//                                   std::vector<WFGrid>& gradPhiZ,
//                                   DfFunctional_GGA* pFunctional,
//                                   const double weight,
//                                   TlMatrixObject* pF_A)
...
//         * (roundF_roundGammaAA2 * gradRhoAX + roundF_roundGammaAB *
//         gradRhoAX);
//     const double coeff2_AY = weight
//         * (roundF_roundGammaAA2 * gradRhoAY + roundF_roundGammaAB *
//         gradRhoAY);
//     const double coeff2_AZ = weight
//         * (roundF_roundGammaAA2 * gradRhoAZ + roundF_roundGammaAB *
//         gradRhoAZ);
//     const double cutOffValue = this->m_inputedCutoffThreshold;
...
//                                                                             WFGrid_sort_functional());
//                 this->buildFock(phi.begin(), pEnd, coeff1_A, cutOffValue,
//                 pF_A);
//             }
//             // phi v.s. grad-phi(x)
//             if ((gradPhiX.size() > 0) &&
//                 (std::fabs(coeff2_AX * phi_0 * gradPhiX[0].value) >
//                 cutOffValue)) { std::vector<WFGrid>::const_iterator pEnd =
//                 std::upper_bound(phi.begin(), phi.end(),
//                                                                             WFGrid(0, cutOffValue / (coeff2_AX * gradPhiX[0].value)),
//                                                                             WFGrid_sort_functional());
//                 this->buildFock(phi.begin(), pEnd, gradPhiX.begin(),
//                 gradPhiX.end(),
//                                 coeff2_AX, cutOffValue, pF_A);
//                 this->buildFock(gradPhiX.begin(), gradPhiX.end(),
//                 phi.begin(), pEnd,
//                                 coeff2_AX, cutOffValue, pF_A);
//             }
//             // phi v.s. grad-phi(y)
//             if ((gradPhiY.size() > 0) &&
//                 (std::fabs(coeff2_AY * phi_0 * gradPhiY[0].value) >
//                 cutOffValue)) { std::vector<WFGrid>::const_iterator pEnd =
//                 std::upper_bound(phi.begin(), phi.end(),
//                                                                             WFGrid(0, cutOffValue / (coeff2_AY * gradPhiY[0].value)),
//                                                                             WFGrid_sort_functional());
//                 this->buildFock(phi.begin(), pEnd, gradPhiY.begin(),
//                 gradPhiY.end(),
//                                 coeff2_AY, cutOffValue, pF_A);
//                 this->buildFock(gradPhiY.begin(), gradPhiY.end(),
//                 phi.begin(), pEnd,
//                                 coeff2_AY, cutOffValue, pF_A);
//             }
//             // phi v.s. grad-phi(z)
//             if ((gradPhiZ.size() > 0) &&
//                 (std::fabs(coeff2_AZ * phi_0 * gradPhiZ[0].value) >
//                 cutOffValue)) { std::vector<WFGrid>::const_iterator pEnd =
//                 std::upper_bound(phi.begin(), phi.end(),
//                                                                             WFGrid(0, cutOffValue / (coeff2_AZ * gradPhiZ[0].value)),
//                                                                             WFGrid_sort_functional());
//                 this->buildFock(phi.begin(), pEnd, gradPhiZ.begin(),
//                 gradPhiZ.end(),
//                                 coeff2_AZ, cutOffValue, pF_A);
//                 this->buildFock(gradPhiZ.begin(), gradPhiZ.end(),
//                 phi.begin(), pEnd,
//                                 coeff2_AZ, cutOffValue, pF_A);
//             }
//         }
...
void DfCalcGridX::getWholeDensity(double* pRhoA, double* pRhoB) const {
  ...
  TlMatrix weightMat = gridMat.getBlockMatrix(0, GM_WEIGHT, numOfGrids, 1);
