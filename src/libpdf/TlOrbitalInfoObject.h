...
class TlOrbitalInfoObject {
 ...
 public:
  ...
  struct PGTO {
    ...
    bool operator!=(const PGTO& rhs) const { return !(this->operator==(rhs)); }
  };
  ...
  struct CGTO {
   ...
   public:
    bool operator==(const CGTO& rhs) const {
      bool answer = false;
      if ((this->atomSymbol == rhs.atomSymbol) && (this->shell == rhs.shell)) {
    ...
    bool operator!=(const CGTO& rhs) const { return !(this->operator==(rhs)); }
  };
