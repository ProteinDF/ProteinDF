...
public:
    ...
    struct PGTO {
        ...
        bool operator==(const PGTO& rhs) const {
            return (
                (std::fabs(this->dCoefficient - rhs.dCoefficient) < 1.0E-5) &&
    ...
    public:
        bool operator==(const Orbital& rhs) const {
            ...
            if ((this->shellType == rhs.shellType) &&
                ...
                (std::fabs(this->prefactor_gradx - rhs.prefactor_gradx) <
                 1.0E-5) &&
                (std::fabs(this->prefactor_grady - rhs.prefactor_grady) <
                 1.0E-5) &&
                (std::fabs(this->prefactor_gradz - rhs.prefactor_gradz) <
                 1.0E-5)) {
                answer = true;
            }
...
public:
    ...
    index_type getShellIndex(const index_type AO) const {
        static const int basisTypeBase[] = {0, 1, 4, 9,
                                            16};  // s, px, dxy, f, g
    ...
    std::string getBasisTypeName(const index_type AO) const {
        assert(AO < this->getNumOfOrbitals());
        return std::string(
            TlOrbitalInfoObject::basisTypeNameTbl_[this->getBasisType(AO)]);
    }
...
protected:
    ...
    static void getGradPrefactor_DYZ(const TlPosition& pos, const double alpha,
                                     double* pDX, double* pDY, double* pDZ);
    static void getGradPrefactor_DXX_YY(const TlPosition& pos,
                                        const double alpha, double* pDX,
