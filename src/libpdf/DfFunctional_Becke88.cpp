...
const double DfFunctional_Becke88::EPS =
    1.0E-16;  // std::numeric_limits<double>::epsilon();
const double DfFunctional_Becke88::INV_3 = 1.0 / 3.0;
const double DfFunctional_Becke88::M_4_3 = 4.0 / 3.0;
const double DfFunctional_Becke88::G_PARAM =
    -(3.0 / 2.0) * std::pow(3.0 / (4.0 * M_PI), (1.0 / 3.0));
...
double DfFunctional_Becke88::getFunctional(const double dRhoA,
                                           const double dRhoB,
...
void DfFunctional_Becke88::getDerivativeFunctional(
    ...
    *pRoundF_roundGammaAA = 0.0;
    if ((dRhoA > EPS) && (dGammaAA > EPS)) {
        ...
        // roundF_roundRho
        *pRoundF_roundRhoA =
            M_4_3 * std::pow(dRhoA, INV_3) * (gA - xA * g_primeA);
        // roundF_roundGamma
    ...
    *pRoundF_roundGammaBB = 0.0;
    if ((dRhoB > EPS) && (dGammaBB > EPS)) {
        ...
        *pRoundF_roundRhoB =
            M_4_3 * std::pow(dRhoB, INV_3) * (gB - xB * g_primeB);
...
// for RKS
void DfFunctional_Becke88::getDerivativeFunctional(
    const double dRhoA, const double dGammaAA, double* pRoundF_roundRhoA,
...
double DfFunctional_Becke88::g_prime(const double x) {
    ...
    const double arg1 =
        6.0 * bx * bx * (arcsinhx + x / std::sqrt(xx + 1.0)) * invM * invM;
...
TlDenseGeneralMatrix_Lapack DfFunctional_Becke88::getDerivativeFunctionalCore(
    ...
    //                  = [M_4_3 * std::pow(rhoA, INV_3)] * [(gA - xA *
    //                  g_primeA)]
    ...
    if ((rhoA > 1.0E-16) && (xA > 1.0E-16)) {
        ...
        const double roundF_roundGammaAA_termR =
            1.0 / std::pow(rhoA, 4.0 / 3.0);
    ...
    if ((rhoB > 1.0E-16) && (xB > 1.0E-16)) {
        ...
        const double roundF_roundGammaBB_termR =
            1.0 / std::pow(rhoB, 4.0 / 3.0);
