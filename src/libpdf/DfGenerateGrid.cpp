...
void DfGenerateGrid::generateGrid_atom(const TlDenseGeneralMatrix_Lapack& O,
    ...
    if (atomnum != 0) {
        ...
        for (int radvec = 0; radvec < radvec_max; ++radvec) {
            ...
            // The coordinates of the atom on which the present grid is centred
            // are put into the array "Ogridr"
            ...
            for (int i = 0; i < Ogrid; ++i) {
                // this->log_.info(TlUtils::format("%5d: % 12.8f % 12.8f
                // % 12.8f",
...
void DfGenerateGrid::calcMultiCenterWeight_Becke(
    ...
    for (int Omega = 0; Omega < numOfGrids; ++Omega) {
        ...
        for (int m = 0; m < numOfAtoms; ++m) {
            ...
            // TlAtom::getElementNumber(this->flGeometry_.getAtomSymbol(m));
            // const double R_m =
            // this->getCovalentRadiiForBecke(atomicNumber_m);
            ...
            //         double u_ij = (rr[m] - rr[n]) *
            //         this->invDistanceMatrix_(m, n);
