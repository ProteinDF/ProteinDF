...
DfGenerateGrid::DfGenerateGrid(TlSerializeData* pPdfParam)
  ...
  if (TlUtils::toUpper(sGridType) == "COARSE") {
    ...
    if (pdfParam["grid/radial_quadorature_method"].getStr() != "") {
      const std::string method =
          TlUtils::toUpper(pdfParam["grid/radial_quadorature_method"].getStr());
  ...
  for (int mc = 0; mc < dNumOfAtoms; ++mc) {
    ...
    for (int nc = 0; nc < mc; ++nc) {
      ...
      if (dist < 1E-10) {
        std::cerr << " distance < 1E-10: " << mc << " th atom and " << nc
                  << " th atom." << std::endl;
      }
  ...
  this->numOfColsOfGrdMat_ = 5;  // x, y, z, weight, atom_index
  {
    ...
    if (dfXcFunctional.getFunctionalType() == DfXCFunctional::LDA) {
      ...
      this->numOfColsOfGrdMat_ +=
          coef * 4;  // rho, gradRhoX, gradRhoY, gradRhoZ
    }
  }
...
void DfGenerateGrid::setCellPara() {
  ...
  } else if ((this->m_gridType == MEDIUM_FINE) || (this->m_gridType == FINE) ||
             (this->m_gridType == SG_1)) {
...
void DfGenerateGrid::generateGrid(const TlMatrix& O, const int iatom,
  ...
  if (atomicNumber > 0) {
    ...
    for (int radvec = 0; radvec < this->nrgrid; ++radvec) {
      ...
      const double dr0 =
          2.0 * rM / ((1.0 - this->xGL_[radvec]) * (1.0 - this->xGL_[radvec]));
      ...
      for (int Omega = 0; Omega < nOgrid; ++Omega) {
        ...
        for (int mc = 0; mc < nNumOfAtoms; ++mc) {
          ...
          const int atomnum1 =
              TlPrdctbl::getAtomicNumber(this->flGeometry_.getAtomSymbol(mc));
...
void DfGenerateGrid::generateGrid_SG1(const TlMatrix& O, const int iAtom,
                                      std::vector<double>* pCoordX,
                                      std::vector<double>* pCoordY,
                                      std::vector<double>* pCoordZ,
                                      std::vector<double>* pWeight) {
  ...
  if (atomnum != 0) {
    ...
    for (int radvec = 0; radvec < radvec_max; ++radvec) {
      ...
      switch (this->radialGridType_) {
        case RG_GaussChebyshev:
          this->getRadialAbscissaAndWeight_GaussChebyshev(rM, Nr, i, &ri, &wr);
          break;

        case RG_EularMaclaurin:
          this->getRadialAbscissaAndWeight_EulerMaclaurin(rM, Nr, i, &ri, &wr);
          break;
      ...
      if (this->m_gridType == SG_1) {
        ...
        Ogrid = this->getNumOfPrunedAnglarPoints(ri, this->nOgrid, atomnum);
      }

      // this->log_.info(TlUtils::format("radvec=%2d: %4d / %4d: r_i=%e",
      // radvec, Ogrid, numOfGrids, ri));

      // The coordinates of the atom on which the present grid is centred are
      // put into the array "Ogridr"
      std::vector<TlPosition> grid(Ogrid);
      std::vector<double> lebWeight(Ogrid);
      this->getSphericalGrids(Ogrid, ri, wr, this->coord_[iAtom], O, &grid,
                              &lebWeight);
...
void DfGenerateGrid::points2(const int nOgrid, const double r0,
                             ...
                             const TlMatrix& O, std::vector<TlPosition>& Ogrid,
                             std::vector<double>& w) {
  double bm[4];
  double bl[4];
...
void DfGenerateGrid::calcMultiCenterWeight_Becke(
  ...
  for (int Omega = 0; Omega < numOfGrids; ++Omega) {
    ...
    for (int m = 0; m < numOfAtoms; ++m) {
      ...
      // TlAtom::getElementNumber(this->flGeometry_.getAtomSymbol(m)); const
      // double R_m = this->getCovalentRadiiForBecke(atomicNumber_m);

      // for (int n = 0; n < numOfAtoms; ++n) {
      //     if (m != n) {
      //         const int atomicNumber_n =
      //         TlAtom::getElementNumber(this->flGeometry_.getAtomSymbol(n));
      //         const double R_n =
  ...
  for (int Omega = 0; Omega < numOfGrids; ++Omega) {
    ...
    for (int m = 0; m < numOfAtoms; ++m) {
      ...
      for (int n = 0; n < numOfAtoms; ++n) {
        if (m != n) {
          // const int atomicNumber_n =
          // TlAtom::getElementNumber(this->flGeometry_.getAtomSymbol(n)); const
          // double R_n = this->getCovalentRadiiForBecke(atomicNumber_n);
          ...
          if (u_ij < -a) {
            ...
            g = (1.0 / 16.0) *
                (35.0 * ua - 35.0 * ua3 + 21.0 * ua5 - 5.0 * ua5 * ua2);
          }
  ...
  for (int atomIndex_n = 0; atomIndex_n < numOfAtoms; ++atomIndex_n) {
    if (atomIndex_m != atomIndex_n) {
      ...
      if (atomicNumber_n > 0) {
        ...
        double u_ij =
            (rr_m - rr_n) * this->invDistanceMatrix_(atomIndex_m, atomIndex_n);
  ...
  if (atomicNumber != 0) {
    ...
    for (int radvec = 0; radvec < numOfRadialGrids; ++radvec) {
      ...
      switch (radialGridType) {
        case RG_GaussChebyshev:
          this->getRadialAbscissaAndWeight_GaussChebyshev(rM, numOfRadialGrids,
                                                          i, &ri, &wr);
          break;

        case RG_EularMaclaurin:
          this->getRadialAbscissaAndWeight_EulerMaclaurin(rM, numOfRadialGrids,
                                                          i, &ri, &wr);
          break;
      ...
      if (gridType == SG_1) {
        ...
        numOfAngularGrids =
            this->getNumOfPrunedAnglarPoints(ri, maxAngularGrids, atomicNumber);
      }
      ...
      this->getSphericalGrids(numOfAngularGrids, ri, wr,
                              this->coord_[atomIndex], this->O_, &angularGrids,
                              &lebWeights);
      ...
      switch (partitioningMethod) {
        case Paritioning_Becke:
          this->calcMultiCenterWeight_Becke(atomIndex, numOfAngularGrids,
...
// eq. B7
TlVector DfGenerateGrid::JGP_nablaB_omegaA(const int atomIndexA,
  ...
  if (PA > 1.0E-10) {
    ...
    for (int atomIndexB = 0; atomIndexB < numOfAtoms; ++atomIndexB) {
      if (atomIndexB != atomIndexA) {
        ...
        const double myu_AB =
            (rr_A - rr_B) * this->invDistanceMatrix_(atomIndexA, atomIndexB);
