...
DfCalcGrid::DfCalcGrid(TlSerializeData* pPdfParam, int num_iter)
    ...
    // MaxAuxNum; this->workelement   = lessauxnumber * ( lessauxnumber + 1 ) /
    // 2;
    this->nlsd_type = -1;
    ...
    if (m_sXCFunctional == "gxalpha~") {
        ...
        CnErr.abort(TlUtils::format(
            "unknown XC functional(%s). @DfCalcGrid constructer",
...
void DfCalcGrid::calcXCInteg(TlDenseVector_Lapack& tmpVectorA,
    ...
    TlDenseVector_Lapack RhoAlphaA, RhoAlphaB;
    {
        switch (this->m_nMethodType) {
            ...
            case METHOD_ROKS:
                RhoAlphaA.load(
                    this->getRhoPath(RUN_UKS_ALPHA, this->m_nIteration));
                RhoAlphaB.load(
                    this->getRhoPath(RUN_UKS_BETA, this->m_nIteration));
                break;
    ...
    for (index_type i = 0; i < numOfGrids; ++i) {
        ...
        for (int ialpha = 0; ialpha < this->m_nNumOfAux; ++ialpha) {
            ...
            if (shoulder <= TOOBIG) {
                ...
                const int basisType =
                    this->m_tlOrbInfoAuxCD_.getBasisType(ialpha);
    ...
    for (index_type i = 0; i < numOfGrids; ++i) {
        ...
        for (int ialpha = 0; ialpha < this->m_nNumOfAux; ++ialpha) {
            ...
            if (shoulder <= TOOBIG) {
                ...
                gridRhoA += RhoAlphaA.get(ialpha) * gAlpha;
                gridRhoB +=
                    RhoAlphaB.get(ialpha) * gAlpha;  // for SP and ROKS case
            }
        }
    ...
    for (index_type i = 0; i < numOfGrids; ++i) {
        ...
        for (int idelta = 0; idelta < this->numOfAuxXC_; ++idelta) {
            ...
            if (shoulder <= TOOBIG) {
                ...
                gDelta *=
                    prefactor * this->m_tlOrbInfoXC_.getCoefficient(idelta, 0);
                ...
                switch (xc) {
                    ...
                    case 8:
                        ...
                        RouDeltaEpsilonCP =
                            -0.0310907 * VWNPdrfunc(rs) * gDweight / 6.0;
                        break;
                    case 10:
                        // PZ
                        break;
                ...
                tmpVectorA.add(idelta, 4.0 * F13 * EpsilonXP + EpsilonCP +
                                           RouDeltaEpsilonCP);
            }
        }
    }
    ...
    for (index_type i = 0; i < numOfGrids; ++i) {
        ...
        for (int idelta = 0; idelta < this->numOfAuxXC_; ++idelta) {
            ...
            if (shoulder <= TOOBIG) {
                ...
                // this->getPrefactor(this->flVctMyuTable[idelta].basisType,
                // pos);
                double prefactor = this->getPrefactor(basisType, pos);
                switch (basisType) {
                    case 7:  // dxx-yy
                ...
                switch (xc) {
                    ...
                    case 2:
                        ...
                        RouDeltaEpsilonCF = 0.0127 * rs * HLdrfunc(rs / 75.0) *
                                            gDweight / 225.0;
                        break;
                    case 4:
                        // JMW
                        ...
                        RouDeltaEpsilonCF = 0.0178 * rs * HLdrfunc(rs / 26.46) *
                                            gDweight / 79.38;
                        break;
                    case 6:
                        // GL
                    ...
                    case 8:
                        ...
                        RouDeltaEpsilonCF =
                            -0.01554535 * VWNFdrfunc(rs) * gDweight / 6.0;
                        break;
                    case 10:
                        // PZ
                        break;
