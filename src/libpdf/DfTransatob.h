...
void DfTransatob::Cprime2C(const DfObject::RUN_TYPE runType,
    ...
    if (X.getNumOfRows() != this->m_nNumOfAOs || X.getNumOfCols() != numOfMOs) {
        ...
        this->log_.info(
            TlUtils::format("number_ao_basis = %d", this->m_nNumOfAOs));
        this->log_.info(
            TlUtils::format("number_independant_basis = %d", numOfMOs));
        this->log_.info(
            "DfTransatob dimension is not consistency, but continue\n");
    }
    ...
    if ((Cprime.getNumOfRows() != numOfMOs) ||
        ...
        this->log_.info(
            TlUtils::format("row of C' = %d", Cprime.getNumOfRows()));
        this->log_.info(
            TlUtils::format("col of C' = %d", Cprime.getNumOfRows()));
...
void DfTransatob::run_method(const TlMatrixObject::index_type numOfMOs,
    ...
    switch (this->m_nMethodType) {
        ...
        case METHOD_UKS:
            this->Cprime2C<GeneralMatrix>(RUN_UKS_ALPHA, numOfMOs,
                                          fragment);  // UKS alpha spin
            this->Cprime2C<GeneralMatrix>(RUN_UKS_BETA, numOfMOs,
                                          fragment);  // UKS beta spin
            break;
