...
void DfFunctional_LYP::getDerivativeFunctional(
    ...
    *pRoundF_roundGammaBB = 0.0;
    ...
    if ((std::fabs(dRhoA) > TOLERANCE) && (std::fabs(dRhoB) > TOLERANCE)) {
        ...
        const double dArg1_coef = -4.0 * LYP_PARAM_A /
                                  (1.0 + LYP_PARAM_D * dRhoToM1_3) * dRhoAB /
                                  dRho;
        const double dArg1A =
            dArg1_coef * (INV_3 * LYP_PARAM_D * dRhoToM4_3 /
        ...
        // for debug
        //   std::cout << "\n"
        //      << TlUtils::format("dRoundRoundLYP_roundRhoARoundGammaAA =
        //      %+e\n", dRoundRoundLYP_roundRhoARoundGammaAA)
        //      << TlUtils::format("dRoundRoundLYP_roundRhoARoundGammaAB =
        //      %+e\n", dRoundRoundLYP_roundRhoARoundGammaAB)
        //      << TlUtils::format("dRoundRoundLYP_roundRhoARoundGammaBB =
        //      %+e\n", dRoundRoundLYP_roundRhoARoundGammaBB)
        //      << TlUtils::format("dRoundRoundLYP_roundRhoBRoundGammaAA =
        //      %+e\n", dRoundRoundLYP_roundRhoBRoundGammaAA)
        //      << TlUtils::format("dRoundRoundLYP_roundRhoBRoundGammaAB =
        //      %+e\n", dRoundRoundLYP_roundRhoBRoundGammaAB)
        //      << TlUtils::format("dRoundRoundLYP_roundRhoBRoundGammaBB =
        //      %+e\n", dRoundRoundLYP_roundRhoBRoundGammaBB)
        //      << std::endl;
...
double DfFunctional_LYP::LYP(const double dRhoA, const double dRhoB,
    ...
    const double term1 = -4.0 * LYP_PARAM_A /
                         (1.0 + LYP_PARAM_D * dPow_rho_m1_3) * dRhoAB / dRho;
...
void DfFunctional_LYP::roundLYP_roundGamma(const double dRho,
    ...
    dRoundLYP_roundGammaAA =
        -ABOmega * (INV_9 * dRhoAB *
                        (1.0 - 3.0 * dDelta - (dDelta - 11.0) * dRhoA / dRho) -
                    dRhoA * dRhoA);
    // dRoundLYP_roundGammaBB = - ABOmega * (INV_9 * dRhoAB * (1.0 - 3.0 *
    // dDelta
...
void DfFunctional_LYP::roundLYP_roundRhoAB(const double dRhoA,
    ...
    // for debug
    //   std::cout << std::endl;
    //   std::cout << ">>>> DfFunctional_LYP::roundLYP_roundRhoAB()" <<
    //   std::endl; std::cout << TlUtils::format("dRhoA = %+e, dRhoB = %+e",
    //   dRhoA, dRhoB)
    //      << std::endl;
    ...
    const double dArg1_coef = -4.0 * LYP_PARAM_A /
                              (1.0 + LYP_PARAM_D * dPow_rho_mInv3) * dRhoAB /
                              dRho;
    const double dArg1A =
        dArg1_coef * (INV_3 * (LYP_PARAM_D * dPow_rho_m4_3) /
...
void DfFunctional_LYP::roundRoundLYP_roundRhoRoundGamma(
    ...
    // dRoundRoundLYP_roundRhoXRoundGammaAA
    {
        ...
        //     (dDelta - 11.0) * dRhoB * dInvRho); const double term2B_2 = -
...
TlDenseGeneralMatrix_Lapack DfFunctional_LYP::getFunctionalCore(
    ...
    if ((rhoA > 1.0E-16) && (xA > 1.0E-16) && (rhoB > 1.0E-16) &&
        ...
        double roundLYP_roundGammaAA, roundLYP_roundGammaAB,
        ...
        const double term1 = -4.0 * LYP_PARAM_A /
                             (1.0 + LYP_PARAM_D * pow_rho_m1_3) * rhoAB / rho;
...
TlDenseGeneralMatrix_Lapack DfFunctional_LYP::getDerivativeFunctionalCore(
    ...
    if ((rhoA > 1.0E-16) && (xA > 1.0E-16) && (rhoB > 1.0E-16) &&
        ...
        this->roundRoundLYP_roundRhoRoundGamma(
            rho, rhoA, rhoB, invRho, rhoAB, omega, omegaPrime, delta,
            deltaPrime, &roundRoundLYP_roundRhoARoundGammaAA,
            &roundRoundLYP_roundRhoBRoundGammaAA,
            &roundRoundLYP_roundRhoARoundGammaAB,
            &roundRoundLYP_roundRhoBRoundGammaAB,
        ...
        // alpha spin ======================================================
        {
            ...
            // roundF_roundGamma
            //*pRoundF_roundGammaAA = - ABOmega * (INV_9 * rhoAB * (1.0 - 3.0 *
            //delta
            //- (delta - 11.0) * rhoA / rho) - rhoB * rhoB);
            const double roundF_roundGammaAA_R0 =
                -ABOmega *
                (INV_9 * rhoAB *
                     (1.0 - 3.0 * delta - (delta - 11.0) * rhoA / rho) -
                 rhoB * rhoB);
        ...
        // beta spin =======================================================
        {
            ...
            // roundF_roundGamma
            //*pRoundF_roundGammaBB = - ABOmega * (INV_9 * rhoAB * (1.0 - 3.0 *
            //delta
            //- (delta - 11.0) * rhoB / rho) - rhoA * rhoA);
            const double roundF_roundGammaBB_R0 =
                -ABOmega *
                (INV_9 * rhoAB *
                     (1.0 - 3.0 * delta - (delta - 11.0) * rhoB / rho) -
                 rhoA * rhoA);
        ...
        // both ============================================================
        {
            ...
            const double roundF_roundGammaAB_R0 =
                -ABOmega *
                (INV_9 * rhoAB * (47.0 - 7.0 * delta) - M_4_3 * rho * rho);
