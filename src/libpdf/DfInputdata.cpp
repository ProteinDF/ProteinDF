...
TlSerializeData DfInputdata::main(const bool isReadUserInput) {
    ...
    if (isReadUserInput) {
        ...
        //
        {
            ...
            const TlOrbitalInfo orbInfo(param["coordinates"],
                                        param["basis_set"]);
            const TlOrbitalInfo_Density orbInfo_J(param["coordinates"],
                                                  param["basis_set_j"]);
            const TlOrbitalInfo_XC orbInfo_XC(param["coordinates"],
                                              param["basis_set_xc"]);
...
void DfInputdata::show(const TlSerializeData& data) const {
    ...
    if (data["show_coordinates"].getBoolean() == true) {
        ...
        for (int i = 0; i < numOfAtoms; ++i) {
            ...
            log.info(TlUtils::format(
                " %-2s %+8.3e (%+16.10e %+16.10e %+16.10e) [%s] \n",
                symbol.c_str(), charge, pos.x(), pos.y(), pos.z(),
                label.c_str()));
        }
    } else {
        log.info(" printing coordinates is rejected.");
        log.info(
            " to print the keywords, please input 'show_coordinates = yes'.");
    }
    log.info("\n");

    // print basis set
    {
        const std::string showOrbitalBasis =
            TlUtils::toUpper(data["show_orbital_basis"].getStr());
        ...
        if (showOrbitalBasis == "GAMESS") {
            ...
            log.info("\n");
            log.info(
                " >>>> Inputted Density Orbital Basis Set (GAMESS format) "
                "<<<<");
            log.info(orb_j.getStr_GAMESS());
            log.info("\n");
            log.info(
                " >>>> Inputted XC Orbital Basis Set (GAMESS format) <<<<");
            log.info(orb_k.getStr_GAMESS());
            log.info("\n");
            if (isUseGF) {
                log.info(
                    " >>>> Inputted GridFree Orbital Basis Set (GAMESS format) "
                    "<<<<");
