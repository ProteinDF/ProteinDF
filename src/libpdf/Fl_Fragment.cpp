...
std::string Fl_Fragment::getStr() const {
  ...
  for (std::map<int, Fragment>::const_iterator p = this->m_fragments.begin();
    ...
    str +=
        TlUtils::format("number_of_orbitals = %d\n", p->second.numOfOrbitals);
...
void Fl_Fragment::load() {
  ...
  while (!ifs.eof()) {
    ...
    if ((term1 == "Frame") && (term2 == "=")) {
      ...
      this->m_fragments[currentFragment].numOfElectrons =
          std::atoi(term3.c_str());
      continue;
    } else if ((term1 == "number_of_alpha_electrons") && (term2 == "=")) {
      this->m_fragments[currentFragment].numOfAlphaElectrons =
          std::atoi(term3.c_str());
      continue;
    } else if ((term1 == "number_of_beta_electrons") && (term2 == "=")) {
      this->m_fragments[currentFragment].numOfBetaElectrons =
          std::atoi(term3.c_str());
      continue;
    } else if ((term1 == "number_of_orbitals") && (term2 == "=")) {
      this->m_fragments[currentFragment].numOfOrbitals =
          std::atoi(term3.c_str());
      continue;
    } else if ((term1 == "number_of_occupied_orbitals") && (term2 == "=")) {
      this->m_fragments[currentFragment].numOfOccupiedOrbitals =
          std::atoi(term3.c_str());
      continue;
    } else if ((term1 == "number_of_occupied_alpha_orbitals") &&
               (term2 == "=")) {
      this->m_fragments[currentFragment].numOfOccupiedAlphaOrbitals =
          std::atoi(term3.c_str());
      continue;
    } else if ((term1 == "number_of_occupied_beta_orbitals") &&
               (term2 == "=")) {
      this->m_fragments[currentFragment].numOfOccupiedBetaOrbitals =
          std::atoi(term3.c_str());
      continue;
    } else if ((term1 == "number_of_unoccupied_orbitals") && (term2 == "=")) {
      this->m_fragments[currentFragment].numOfUnoccupiedOrbitals =
          std::atoi(term3.c_str());
      continue;
    } else if ((term1 == "number_of_unoccupied_alpha_orbitals") &&
               (term2 == "=")) {
      this->m_fragments[currentFragment].numOfUnoccupiedAlphaOrbitals =
          std::atoi(term3.c_str());
      continue;
    } else if ((term1 == "number_of_unoccupied_beta_orbitals") &&
               (term2 == "=")) {
      this->m_fragments[currentFragment].numOfUnoccupiedBetaOrbitals =
          std::atoi(term3.c_str());
      continue;
    } else {
...
int Fl_Fragment::getNumOfFragments() const { return this->m_fragments.size(); }
...
void Fl_Fragment::calcDefault(const std::vector<int>& norbcut) {
  ...
  for (std::map<int, Fragment>::iterator p = this->m_fragments.begin();
       p != pEnd; ++p) {
    ...
    if (p->second.numOfOrbitals == -1) {
      if ((p->second.numOfOccupiedOrbitals != -1) &&
        ...
        p->second.numOfOrbitals =
            p->second.numOfOccupiedOrbitals + p->second.numOfUnoccupiedOrbitals;
      } else {
        p->second.numOfOrbitals = norbcut[index];
      }
    }
    if (p->second.numOfOccupiedOrbitals == -1) {
      p->second.numOfOccupiedOrbitals = (p->second.numOfElectrons + 1) / 2;
    }
    if (p->second.numOfOccupiedAlphaOrbitals == -1) {
      p->second.numOfOccupiedAlphaOrbitals = p->second.numOfAlphaElectrons;
    }
    if (p->second.numOfOccupiedBetaOrbitals == -1) {
      p->second.numOfOccupiedBetaOrbitals = p->second.numOfBetaElectrons;
    }
