...
void DfXCFunctional_Parallel::buildXC_ScaLAPACK() {
  ...
  switch (this->m_nMethodType) {
    case METHOD_RKS: {
      ...
      if (this->m_bIsUpdateXC == true) {
        Ppq = 0.5 *
              this->getDiffDensityMatrix<TlDenseSymmetricMatrix_Scalapack>(
                  RUN_RKS, this->m_nIteration);
      } else {
        Ppq = 0.5 *
              this->getPpqMatrix<TlDenseSymmetricMatrix_Scalapack>(
                  RUN_RKS, this->m_nIteration - 1);
      }
    ...
    case METHOD_UKS: {
      ...
      if (this->m_bIsUpdateXC == true) {
        PApq = this->getDiffDensityMatrix<TlDenseSymmetricMatrix_Scalapack>(
            RUN_UKS_ALPHA, this->m_nIteration);
        PBpq = this->getDiffDensityMatrix<TlDenseSymmetricMatrix_Scalapack>(
            RUN_UKS_BETA, this->m_nIteration);
      } else {
