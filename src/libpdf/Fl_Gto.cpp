...
Fl_Gto::~Fl_Gto() {
    if ((this->m_sFileName != "") && (this->m_isUpdate == true)) {
        this->save();
    }
}
...
int Fl_Gto::getSnum(int i) const { return this->cgto[i].Snum; }

int Fl_Gto::getPnum(int i) const { return this->cgto[i].Pnum; }

int Fl_Gto::getDnum(int i) const { return this->cgto[i].Dnum; }

int Fl_Gto::getNumOfCGTOs() const { return this->cgto.size(); }

std::string Fl_Gto::getAtom(int s) const {
    if (s < 0) {
        // #ifndef NDEBUG
        //     std::cerr << "Assertion! (s>0) at Fl_Gto::getAtom(). s = " << s
        //     << std::endl;
        // #endif // NDEBUG
        return "";
    }
...
void Fl_Gto::save(const std::string& newSavePath) {
    ...
    if (fs.is_open() != true) {
        CnErr.abort(
            TlUtils::format("could not open file \"%s\" at Fl_Gto::write()",
    ...
    for (int i = 0; i < this->getNumOfCGTOs(); i++) {
        if (previous_atom != cgto[i].atom || previous_label != cgto[i].label) {
            ...
            ss << TlUtils::format("%5s %5s = NULL ;; %s\n",
                                  cgto[i].atom.c_str(), cgto[i].label.c_str(),
                                  cgto[i].basisName.c_str());
        ...
        for (int l = 0; l <= angular; l++) {
            for (int m = 0; m <= angular; m++) {
                for (int n = 0; n <= angular; n++) {
                    if (l + m + n == angular) {
                        ss << TlUtils::format("  (%1ld,%1ld,%1ld) = %18.8lE ",
                                              l, m, n,
            ...
            for (int l = 0; l <= angular; l++) {
                for (int m = 0; m <= angular; m++) {
                    for (int n = 0; n <= angular; n++) {
                        if (l + m + n == angular) {
                            ss << TlUtils::format(
                                "  (%1ld,%1ld,%1ld) = %18.8lE %18.8lE \n", l, m,
                                n, getNormalized(i, j, l, m, n),
...
void Fl_Gto::load() {
    ...
    if (numOfCGTOs == -1) {
        CnErr.abort(TlUtils::format("not found numbercgto line at %s",
                                    this->m_sFileName.c_str()));
    }
        ...
        while (std::getline(fs, line)) {
            ...
            if (isReadPGTO != true) {
                if (term1 == "Snum") {
                    ...
                    cgto.scalefactor =
                        std::atof(TlUtils::getPdfParam(term2).c_str());
                } else if (term1 == "contraction") {
                    contraction =
                        std::atoi(TlUtils::getPdfParam(term2).c_str());
                ...
                cgto.pgto[currentContraction].exponent =
                    std::atof(term1.c_str());
                cgto.pgto[currentContraction].coefficient =
                    std::atof(term2.c_str());
...
void Fl_Gto::setup(const TlSerializeData& basisSet) {
    ...
    for (TlSerializeData::MapConstIterator p = basisSet.beginMap(); p != pEnd;
         ++p) {
        ...
        std::string label = "";
        if ((pos != std::string::npos) && (pos + 1) <= atomLabel.size()) {
        ...
        for (TlSerializeData::ArrayConstIterator q =
                 contents["cGTOs"].beginArray();
             q != qEnd; ++q) {
            Cgto cgto;
            ...
            //             std::cerr << TlUtils::format("scale_factor=%e,
            //             shell=%c", cgto.scalefactor, cgto.shell)
            //                       << std::endl;

            const TlSerializeData& pGTOs = (*q)["pGTOs"];
            TlSerializeData::ArrayConstIterator rEnd = pGTOs.endArray();
            for (TlSerializeData::ArrayConstIterator r = pGTOs.beginArray();
                 r != rEnd; ++r) {
                ...
                //                 std::cerr << TlUtils::format("coef=%e,
                //                 exp=%e", coef, exp)
                //                           << std::endl;
            }
...
double Fl_Gto::getNormalizedfactor(int s, int l, int m, int n) const {
    ...
    ans *= Math.dbfact(2 * l - 1) * Math.dbfact(2 * m - 1) *
           Math.dbfact(2 * n - 1);
...
double Fl_Gto::getNormalized(const int s, const int p, const int l, const int m,
    ...
    switch (getShell(s)) {
        ...
        default:
            log.error(
                " *** Fl_Gto::getNormalized() not support over g angular "
                "momentum, "
                "sorry.");
            CnErr.abort();
            break;
    ...
    if (l + m + n > maxangular) {
        log.error(
            " *** Fl_Gto::getNormalized() illegal angular momentum, "
            "inconsistent "
            "with shell.");
        CnErr.abort();
    }
    ...
    ans *= std::pow(Math.dbfact(2 * l - 1) * Math.dbfact(2 * m - 1) *
                        Math.dbfact(2 * n - 1),
                    -1.0 / 2.0);
...
double Fl_Gto::getCoulombnormalized(const int s, const int p, const int l,
    ...
    switch (this->getShell(s)) {
        ...
        default:
            CnErr.abort("Fl_Gto", "",
                        "double getNormalized(int,int,int,int,int)",
    ...
    switch (angular) {
        ...
        case 2:
            if (l == 2 || m == 2 || n == 2) {
                ...
                ans *= std::pow(Math.PI() / getExponent(s, p), -5.0 / 4.0);
                ans *= std::pow(
                    60.0 * 4.0 * getExponent(s, p) * getExponent(s, p) / 49.0,
                    1.0 / 2.0);
            } else {
                ...
                ans *= std::pow(Math.PI() / getExponent(s, p), -5.0 / 4.0);
                ans *=
                    std::pow(4.0 * 20.0 * getExponent(s, p) * getExponent(s, p),
                             1.0 / 2.0);
            }
            break;
        default:
            CnErr.abort(
                "Fl_Gto", "", "double getNormalized(int,int,int,int,int)",
