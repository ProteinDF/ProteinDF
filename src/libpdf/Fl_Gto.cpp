...
void Fl_Gto::save(const std::string& newSavePath) {
  ...
  if (fs.is_open() != true) {
    CnErr.abort(TlUtils::format("could not open file \"%s\" at Fl_Gto::write()",
  ...
  for (int i = 0; i < this->getNumOfCGTOs(); i++) {
    if (previous_atom != cgto[i].atom || previous_label != cgto[i].label) {
      ...
      ss << TlUtils::format("%5s %5s = NULL ;; %s\n", cgto[i].atom.c_str(),
                            cgto[i].label.c_str(), cgto[i].basisName.c_str());
...
void Fl_Gto::setup(const TlSerializeData& basisSet) {
  ...
  for (TlSerializeData::MapConstIterator p = basisSet.beginMap(); p != pEnd;
    ...
    for (TlSerializeData::ArrayConstIterator q = contents["cGTOs"].beginArray();
         q != qEnd; ++q) {
      Cgto cgto;
      ...
      //             std::cerr << TlUtils::format("scale_factor=%e, shell=%c",
      //             cgto.scalefactor, cgto.shell)
      //                       << std::endl;
      ...
      for (TlSerializeData::ArrayConstIterator r = pGTOs.beginArray();
        ...
        //                 std::cerr << TlUtils::format("coef=%e, exp=%e", coef,
        //                 exp)
        //                           << std::endl;
      }
