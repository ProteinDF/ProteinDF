    ...
    switch (this->initialGuessType_) {
        ...
        case GUESS_HARRIS: {
            switch (this->m_nMethodType) {
                ...
                case METHOD_UKS:
                    this->logger(
                        "Sorry. harris method is not supported except RKS. "
                        "stop.\n");
                    CnErr.abort();
                    break;
...
void DfPreScf::createOccupation(const RUN_TYPE runType) {
    ...
    switch (runType) {
        ...
        case RUN_UKS_ALPHA: {
            std::vector<int> aoocLevel = this->getLevel(
                pdfParam["method/sp/alpha-spin-occlevel"].getStr());
        ...
        case RUN_UKS_BETA: {
            std::vector<int> boocLevel = this->getLevel(
                pdfParam["method/sp/beta-spin-occlevel"].getStr());
