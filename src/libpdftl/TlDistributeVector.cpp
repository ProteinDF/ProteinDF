...
const std::size_t TlDistributeVector::FILE_BUFFER_SIZE =
...
TlDistributeVector::TlDistributeVector(const size_type nSize)
    : m_nContext(0),
      m_nRows(nSize),
      m_nCols(1),
      m_nBlockSize(TlDistributeVector::systemBlockSize_) {
  this->initialize();
}
...
TlDistributeVector& TlDistributeVector::operator=(
    const TlDistributeVector& rhs) {
  if (&rhs != this) {
...
TlDistributeVector operator+(const TlDistributeVector& X,
...
bool TlDistributeVector::load(const std::string& sFilePath) {
  ...
  if (bIsFail) {
    std::cerr << "[error] TlDistributeVector::load(): could not open file. "
              << sFilePath << std::endl;
    abort();
  }
...
bool TlDistributeVector::save(std::ofstream& ofs) const {
  ...
  // const double dEndTime = rComm.getTime();
  // if (rComm.isMaster() == true){
  //  std::cout << "TlDistributeMatrix::save() time:" << (dEndTime - dStartTime)
  //  << std::endl;
  //}
