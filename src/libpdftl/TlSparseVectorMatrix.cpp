...
TlSparseVectorMatrix::TlSparseVectorMatrix(index_type row, index_type col,
    ...
    : TlMatrixObject(RSFD),
      numOfRows_(row),
      numOfCols_(col),
      cacheMemSize_(cacheMemSize) {
  this->data_.clear();
}

TlSparseVectorMatrix::TlSparseVectorMatrix(const TlSparseVectorMatrix& rhs)
    : TlMatrixObject(RSFD),
      numOfRows_(rhs.numOfRows_),
      numOfCols_(rhs.numOfCols_),
      data_(rhs.data_) {}
...
void TlSparseVectorMatrix::setRowVector(const index_type row,
                                        const TlVector& cols, bool isKeeped) {
  this->data_[row] = cols;
...
void TlSparseVectorMatrix::updateCache(const std::size_t row) {
  ...
  if (numOfCachingIndeces > MINIMUM_CACHING_INDECES) {
    const std::size_t currentMemSize =
        this->data_.size() * this->getNumOfCols() * sizeof(double);
