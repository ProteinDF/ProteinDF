...
TlDistributeSymmetricMatrix::TlDistributeSymmetricMatrix(
    const TlDistributeVector& rhs, const index_type dim)
    : TlDistributeMatrix(rhs, dim, dim) {}

TlDistributeSymmetricMatrix::~TlDistributeSymmetricMatrix() {}

TlMatrixObject::size_type TlDistributeSymmetricMatrix::getNumOfElements()
    const {
...
void TlDistributeSymmetricMatrix::resize(const index_type size) {
  ...
  // backup old object
  TlDistributeSymmetricMatrix tmp(*this);
...
TlDistributeSymmetricMatrix& TlDistributeSymmetricMatrix::operator=(
    const TlDistributeSymmetricMatrix& rhs) {
  if (&rhs != this) {
...
int TlDistributeSymmetricMatrix::getProcIdForIndex(index_type globalRow,
...
TlMatrixObject::size_type TlDistributeSymmetricMatrix::getIndex(
...
double TlDistributeSymmetricMatrix::operator()(index_type row,
...
void TlDistributeSymmetricMatrix::set(index_type row, index_type col,
                                      const double value) {
  if (row < col) {
    std::swap(row, col);
  }
...
void TlDistributeSymmetricMatrix::add(index_type row, index_type col,
                                      const double value) {
  if (row < col) {
    std::swap(row, col);
  }
...
double TlDistributeSymmetricMatrix::getLocal(index_type row,
                                             index_type col) const {
  if (row < col) {
    std::swap(row, col);
  }
...
TlVector TlDistributeSymmetricMatrix::getRowVector(
...
TlVector TlDistributeSymmetricMatrix::getColumnVector(
    const index_type col) const {
  return this->getRowVector(col);
}

TlVector TlDistributeSymmetricMatrix::getPartialMatrix(int* pStartRow,
                                                       int* pEndRow,
                                                       int* pStartCol,
                                                       int* pEndCol) const {
...
void TlDistributeSymmetricMatrix::mergeSparseMatrixAsync(
...
double TlDistributeSymmetricMatrix::getMaxAbsoluteElement(int* pOutRow,
                                                          int* pOutCol) const {
...
bool TlDistributeSymmetricMatrix::load(const std::string& sFilePath) {
...
#ifdef DEBUG
    std::cerr
        << "[error] TlDistributeSymmetricMatrix::load(): could not open file. "
        << sFilePath << std::endl;
#endif  // DEBUG
    return false;
  }
  ...
  if (bAnswer != true) {
    if (rComm.isMaster() == true) {
      std::cerr << "TlDistributeSymmetricMatrix::load() is not supported: "
                << sFilePath << std::endl;
      std::abort();
    }
  }
...
bool TlDistributeSymmetricMatrix::load(std::ifstream& ifs) {
  ...
  index_type dim;
  if (rComm.isMaster() == true) {
    TlSymmetricMatrix::getHeaderInfo(ifs, &matType, &dim);
  }
...
bool TlDistributeSymmetricMatrix::save(const std::string& sFilePath) const {
  ...
  if (rComm.isMaster() == true) {
    ...
    // store local matrix
    {
      ...
      TlDistributeMatrix::saveElements(&fm, buf);
    }
...
bool TlDistributeSymmetricMatrix::loadHdf5(const std::string& filepath,
...
bool TlDistributeSymmetricMatrix::saveLocal(const std::string& filePath) const {
  // std::cerr << "TlDistributeSymmetricMatrix::saveLocal() file=" << filePath
  // << std::endl;
...
bool TlDistributeSymmetricMatrix::loadLocal(const std::string& filePath) {
  // std::cerr << "TlDistributeSymmetricMatrix::loadLocal() file=" << filePath
  // << std::endl;
  bool answer = false;
...
TlDistributeSymmetricMatrix::getMatrixElementsInLocal() const {
...
// matrix operation
// =============================================================================
bool TlDistributeSymmetricMatrix::diagonal(
    TlVector* pEigVal, TlDistributeMatrix* pEigVec,
...
// !!!
TlDistributeMatrix multiplicationByScaLapack(
    const TlDistributeSymmetricMatrix& X, const TlDistributeMatrix& Y) {
  // this const_cast is requiered for PGI compiler
  // "error: expression must be an lvalue or a function designator"
  // TlDistributeSymmetricMatrix& Xtmp =
  // const_cast<TlDistributeSymmetricMatrix&>(X);  TlDistributeMatrix& Ytmp =
  // const_cast<TlDistributeMatrix&>(Y);
  ...
  // const double* B = &(const_cast<TlDistributeMatrix&>(Y).m_aMatrix[0]);    //
  // DIMENSION (LDB, n)
...
TlDistributeMatrix multiplicationByScaLapack(
    const TlDistributeMatrix& X, const TlDistributeSymmetricMatrix& Y) {
  // this const_cast is requiered for PGI compiler
  // "error: expression must be an lvalue or a function designator"
  // TlDistributeMatrix& Xtmp = const_cast<TlDistributeMatrix&>(X);
  // TlDistributeSymmetricMatrix& Ytmp =
  // const_cast<TlDistributeSymmetricMatrix&>(Y);
  ...
  // const double* A = &(const_cast<TlDistributeMatrix&>(X).m_aMatrix[0]);    //
  // DIMENSION (LDA, ka)
...
TlDistributeMatrix operator*(const TlDistributeSymmetricMatrix& X,
...
TlDistributeSymmetricMatrix operator*(const double X,
                                      const TlDistributeSymmetricMatrix& Y) {
...
TlDistributeVector operator*(const TlDistributeSymmetricMatrix& A,
                             const TlDistributeVector& X) {
  // this const_cast is requiered for PGI compiler
  // "error: expression must be an lvalue or a function designator"
  TlDistributeSymmetricMatrix& Atmp =
      const_cast<TlDistributeSymmetricMatrix&>(A);
...
TlDistributeSymmetricMatrix operator*(const TlDistributeSymmetricMatrix& X,
                                      const double Y) {
...
TlDistributeMatrix TlDistributeSymmetricMatrix::choleskyFactorization(
...
// TlDistributeMatrix
// TlDistributeSymmetricMatrix::choleskyFactorization_mod0(const double
// threshold) const
// {
//     TlCommunicate& rComm = TlCommunicate::getInstance();
//     TlLogging& log = TlLogging::getInstance();
...
//         // L(0:m, pivot[m])を一時保管
//         const TlVector L_x_pm = L.getColVector(pivot[m]); // TODO:
//         バッファの摂り過ぎ
...
//             const double l_m_pi = (this->get(pivot[m], pivot[i]) - sum_ll) *
//             inv_l_m_pm; L.set(m, pivot[i], l_m_pi);
...
TlDistributeMatrix TlDistributeSymmetricMatrix::choleskyFactorization_mod(
    const double threshold) const {
  TlCommunicate& rComm = TlCommunicate::getInstance();
  TlLogging& log = TlLogging::getInstance();
...
TlDistributeMatrix TlDistributeSymmetricMatrix::choleskyFactorization_mod2(
    const double threshold) const {
  // timing data
  TlTime CD_all_time;
