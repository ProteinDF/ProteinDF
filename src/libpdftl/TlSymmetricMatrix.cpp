...
////////////////////////////////////////////////////////////////////////
//
TlSymmetricMatrix::TlSymmetricMatrix(int dim) : TlMatrix(dim, dim, NULL) {
  assert(dim >= 0);
...
// TlSymmetricMatrix& TlSymmetricMatrix::operator+=(const
// TlSparseSymmetricMatrix& rhs)
// {
//     std::cerr << "call TlSymmetricMatrix::operator+=(const
...
TlSymmetricMatrix operator+(const TlSymmetricMatrix& X,
...
bool TlSymmetricMatrix::isLoadable(const std::string& rFilePath) {
  ...
  if (ifs.fail()) {
#ifdef DEBUG
    std::cerr << "[error] TlMatrix::load(): could not open file. " << rFilePath
              << std::endl;
#endif  // DEBUG
    return false;
  }
...
bool TlSymmetricMatrix::getHeaderInfo(const std::string& filePath, int* pType,
  ...
  if (ifs.fail()) {
#ifdef DEBUG
    std::cerr << "[error] TlMatrix::getHeaderInfo(): could not open file. "
              << filePath << std::endl;
#endif  // DEBUG
    return false;
  }
  ...
  const bool answer =
      TlSymmetricMatrix::getHeaderInfo(ifs, pType, pNumOfRows, pNumOfCols);
  ifs.close();
  return answer;
}
...
bool TlSymmetricMatrix::getHeaderInfo(std::fstream& fs, int* pMatrixType,
                                      index_type* pNumOfRows,
                                      index_type* pNumOfCols) {
  return TlSymmetricMatrix::getHeaderInfo_tmpl<std::fstream>(
...
bool TlSymmetricMatrix::getHeaderInfo(std::ifstream& ifs, int* pMatrixType,
                                      index_type* pNumOfRows,
                                      index_type* pNumOfCols) {
  return TlSymmetricMatrix::getHeaderInfo_tmpl<std::ifstream>(
      ifs, pMatrixType, pNumOfRows, pNumOfCols);
}
...
bool TlSymmetricMatrix::getHeaderInfo_tmpl(StreamType& s, int* pMatrixType,
  ...
  // int case:
  {
    ...
    const std::streampos estimatedFileSize =
        sizeof(int) * 3 + sizeof(double) * row * (col + 1) / 2;
  ...
  if (answer != true) {
    ...
    const std::streampos estimatedFileSize =
        (sizeof(int) + sizeof(long) * 2) + sizeof(double) * row * (col + 1) / 2;
    if ((row == col) && (estimatedFileSize == fileSize)) {
...
bool TlSymmetricMatrix::load(const std::string& sFilePath) {
  ...
  if (ifs.fail()) {
#ifdef DEBUG
    std::cerr << "[error] TlSymmetricMatrix::load(): could not open file. "
              << sFilePath << std::endl;
#endif  // DEBUG
    return false;
  }
  ...
  if (bAnswer == false) {
    ...
    log.critical(TlUtils::format(
        "TlSymmetricMatrix::load() is not supported: %s", sFilePath.c_str()));
    std::abort();
...
bool TlSymmetricMatrix::load(std::ifstream& ifs) {
  ...
  bAnswer =
      TlSymmetricMatrix::getHeaderInfo(ifs, &nType, &numOfRows, &numOfCols);
  ...
  // const int nMatrixSize = this->m_nRows + (this->m_nRows * (this->m_nRows
  // -1)) / 2;
  if (bAnswer == true) {
    this->clear();
...
bool TlSymmetricMatrix::save(std::ofstream& ofs) const {
  ...
  // const int nMatrixSize = this->m_nRows + (this->m_nRows * (this->m_nRows
  // -1)) / 2; ofs.write(reinterpret_cast<const char*>(&(this->m_aMatrix[0])),
  // nMatrixSize);
...
#ifdef HAVE_HDF5
bool TlSymmetricMatrix::saveHdf5(const std::string& filepath,
...
bool TlSymmetricMatrix::loadHdf5(const std::string& filepath,
  ...
  if (row != col) {
    this->log_.critical(
        TlUtils::format("illegal parameter in TlSymmetricMatrix::loadHdf5() "
                        "row(%ld) != col(%ld)",
                        row, col));
  }
  this->resize(row);
  ...
  switch (mat_type) {
    ...
    default:
      this->log_.critical(TlUtils::format(
          "illegal matrix type for TlSymmetricMatrix: %d", mat_type));
      break;
  }
...
TlSymmetricMatrix& TlSymmetricMatrix::transpose() { return (*this); }
...
double TlSymmetricMatrix::getMaxAbsoluteElement(int* outRow,
                                                int* outCol) const {
...
TlMatrix multiplicationByLapack(const TlSymmetricMatrix& A, const TlMatrix& B) {
  ...
  const int LDA =
      M;  // When  SIDE = 'L' or 'l'  then LDA must be at least max(1, M),
...
TlMatrix multiplicationByLapack(const TlMatrix& B, const TlSymmetricMatrix& A) {
  ...
  const double ALPHA = 1.0;        // ALPHA specifies the scalar alpha
  const int LDA =
      N;  // When  SIDE = 'L' or 'l'  then LDA must be at least max(1, M),
          // otherwise  LDA must be at least  max(1, N).
...
bool diagonalByLapack(const TlSymmetricMatrix& inMatrix, TlVector* outEigVal,
                      TlMatrix* outEigVec) {
...
#ifdef DEBUG
    std::cerr
        << "[error] INFO != 0 at TlSymmetricMatrix::diagonalByLapack(). abort."
        << std::endl;
#endif  // DEBUG
    abort();
  }
...
TlMatrix TlSymmetricMatrix::choleskyFactorization2(
...
TlMatrix TlSymmetricMatrix::choleskyFactorization2omp(
    const double threshold) const {
  const index_type N = this->getNumOfRows();
  TlVector d = this->getDiagonalElements();
  double error = d.sum();
...
#pragma omp for schedule(runtime) reduction(+ : error)
  ...
  // TlMatrix TlSymmetricMatrix::choleskyFactorization2omp(const double
  // threshold) const
  // {
  //     const index_type N = this->getNumOfRows();
  //     TlVector d = this->getDiagonalElements();
