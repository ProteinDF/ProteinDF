...
class TlMatrix : public TlMatrixObject {
 ...
 public:
  ...
  /// @param[in] row 作成する行列の行数
  /// @param[in] col 作成する行列の列数
  TlMatrix(const TlVector& rVector, index_type row, index_type col);
 ...
 public:
  ...
  virtual TlMatrix getBlockMatrix(index_type nRow, index_type nCol,
                                  index_type nRowDistance,
                                  index_type nColDistance) const;
  ...
  virtual double get(const index_type row, const index_type col) const;
  ...
  friend TlMatrix operator*(double X, const TlMatrix& Y) { return (Y * X); };
 ...
 protected:
  ...
  bool
      isUsingMemManager_;  /// TlMemManagerオブジェクトを用いてメモリ管理を行う(true)/行わない(false)
...
template <typename T>
void TlMatrix::print(T& out) const {
