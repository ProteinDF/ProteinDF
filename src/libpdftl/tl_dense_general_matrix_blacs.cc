...
/// @file
/// OpenMPに関する注意事項
/// #pragma omp critical (TlDenseGeneralMatrix_blacs_gpmServerTasks_) は
/// this->gpmServerTasks_ メンバ変数の書き換え処理を行う箇所に設置している。
/// 同様に #pragma omp critical (TlDenseGeneralMatrix_blacs_gpmClientTasks_) は
/// this->gpmClientTasks_ メンバ変数の書き換え処理を行う箇所に設置している。

int TlDenseGeneralMatrix_blacs::systemBlockSize_ = 64;
const std::size_t TlDenseGeneralMatrix_blacs::FILE_BUFFER_SIZE =
    100 * 1024 * 1024;  // 100 MB
const TlDenseGeneralMatrix_blacs::size_type
...
void TlDenseGeneralMatrix_blacs::LocalMatrixHeader::save(std::ofstream* pOs) {
  pOs->write(reinterpret_cast<const char*>(&(this->type)), sizeof(int));
  pOs->write(reinterpret_cast<const char*>(&(this->globalRow)),
             sizeof(index_type));
  pOs->write(reinterpret_cast<const char*>(&(this->globalCol)),
             sizeof(index_type));
  pOs->write(reinterpret_cast<const char*>(&(this->myRows)),
             sizeof(index_type));
  pOs->write(reinterpret_cast<const char*>(&(this->myCols)),
             sizeof(index_type));
...
////////////////////////////////////////////////////////////////////////
TlDenseGeneralMatrix_blacs::TlDenseGeneralMatrix_blacs(const index_type row,
    ...
    : TlMatrixObject(CSFD),
      ...
      m_nRows(row),
      m_nCols(col),
      m_nBlockSize(TlDenseGeneralMatrix_blacs::systemBlockSize_),
      pData_(NULL) {
  this->initialize();
}

TlDenseGeneralMatrix_blacs::TlDenseGeneralMatrix_blacs(
...
TlDenseGeneralMatrix_blacs::TlDenseGeneralMatrix_blacs(
    ...
    for (index_type r = 0; r < numOfLocalRows; ++r) {
      ...
      if (globalRow >= globalCol) {
        ...
        const TlMatrixObject::size_type localIndex =
            this->getIndex(globalCol, globalRow);
        if (localIndex != -1) {
          assert(localIndex < this->getNumOfMyElements());
  ...
  for (int i = 0; i < numOfProcs; ++i) {
    ...
    if (sizeArray[i] > 0) {
      rComm.iSendDataX(&(indexArray[i][0]), sizeArray[i] * 2, i,
                       TAG_CONSTRUCTOR_INDEX);
  ...
  while (isFinished == false) {
    for (int i = 0; i < numOfProcs; ++i) {
      ...
      if (((state[i] & WAIT_SIZE) == WAIT_SIZE) &&
          ((state[i] & RECV_SIZE) == 0) && (rComm.test(recvSize[i]) == true)) {
      ...
      if (((state[i] & WAIT_INDEX) != 0) && ((state[i] & RECV_INDEX) == 0) &&
          (rComm.test(recvIndex[i]) == true)) {
        rComm.wait(recvIndex[i]);
        state[i] |= RECV_INDEX;
      }

      if (((state[i] & WAIT_VALUES) != 0) && ((state[i] & RECV_VALUES) == 0) &&
      ...
      if (((state[i] & RECV_INDEX) == RECV_INDEX) &&
        ...
        for (std::size_t j = 0; j < bufSize; ++j) {
          ...
          const TlMatrixObject::size_type localIndex =
              this->getIndex(globalRow, globalCol);
          assert(localIndex >= 0);
          this->pData_[localIndex] = value;
        }
        state[i] |= FINISHED;
...
TlDenseGeneralMatrix_blacs::TlDenseGeneralMatrix_blacs(
    const TlDistributedVector& rhs, const index_type row, const index_type col)
    : TlMatrixObject(CSFD),
      log_(TlLogging::getInstance()),
      m_nContext(0),
      m_nRows(row),
  ...
  TlDistributedVector rhs_tmp =
      const_cast<TlDistributedVector&>(rhs);  // for PGI compiler
...
TlDenseGeneralMatrix_blacs::TlDenseGeneralMatrix_blacs(
    ...
    : TlMatrixObject(CSFD),
      log_(TlLogging::getInstance()),
      m_nContext(0),
      m_nRows(rhs.getNumOfRows()),
      m_nCols(rhs.getNumOfCols()),
      m_nBlockSize(TlDenseGeneralMatrix_blacs::systemBlockSize_),
      pData_(NULL) {
  this->initialize();
...
void TlDenseGeneralMatrix_blacs::setSystemBlockSize(int blockSize) {
  TlDenseGeneralMatrix_blacs::systemBlockSize_ = blockSize;
}

void TlDenseGeneralMatrix_blacs::setUsingPartialIO(bool isUsePIO) {
  TlDenseGeneralMatrix_blacs::isUsingPartialIO = isUsePIO;
}

TlMatrixObject::size_type TlDenseGeneralMatrix_blacs::getNumOfElements() const {
  return TlMatrixObject::getNumOfElements_RSFD();
}

std::size_t TlDenseGeneralMatrix_blacs::getMemSize() const {
  //     std::size_t answer = sizeof(TlDenseGeneralMatrix_blacs);
  //     answer += (sizeof(int) * 2 + sizeof(double)) * this->m_nMyRows *
  //     this->m_nMyCols; return answer;
  const int blockSize = this->getBlockSize();
  const int rowCycle =
      (this->getNumOfRows() / blockSize) / this->m_nProcGridRow + 1;
  const int colCycle =
      (this->getNumOfCols() / blockSize) / this->m_nProcGridCol + 1;
...
void TlDenseGeneralMatrix_blacs::resize(const index_type row,
                                        const index_type col) {
  assert(row > 0);
  assert(col > 0);
  ...
  TlDenseGeneralMatrix_blacs tmp(*this);
...
TlDistributedVector TlDenseGeneralMatrix_blacs::getVector() const {
  ...
  return TlDistributedVector(answer,
...
TlVector_BLAS TlDenseGeneralMatrix_blacs::getColumnVector(
  ...
  if (std::binary_search(this->m_ColIndexTable.begin(),
    ...
    for (std::vector<int>::const_iterator p = this->m_RowIndexTable.begin();
         p != pEnd; ++p) {
      const int nRow = *p;
      if (nRow < nGlobalRows) {
        const TlMatrixObject::size_type local_index =
...
double TlDenseGeneralMatrix_blacs::getMaxAbsoluteElement(
    index_type* pOutRow, index_type* pOutCol) const {
  int nRow = 0;
  int nCol = 0;
...
double TlDenseGeneralMatrix_blacs::getLocalMaxAbsoluteElement(
    index_type* pOutRow, index_type* pOutCol) const {
...
double TlDenseGeneralMatrix_blacs::getRMS() const {
  TlCommunicate& rComm = TlCommunicate::getInstance();
...
// TlSparseMatrix TlDenseGeneralMatrix_blacs::getPartialMatrix(const double
// threshold)
// const
// {
...
//     const int numOfHolds = (numOfDiffMElements + proc -1) / proc; //
//     この数だけ担当する
...
// void TlDenseGeneralMatrix_blacs::getPartialMatrix(TlSparseMatrix& ioMatrix)
// const
// {
...
// 複数スレッドから同時に呼び出さない
bool TlDenseGeneralMatrix_blacs::getSparseMatrixX(TlSparseMatrix* pMatrix,
  ...
  if (pMatrix != NULL) {
// Client task通信処理
#pragma omp critical(TlDenseGeneralMatrix_blacs__getSparseMatrixX)
    {
...
void TlDenseGeneralMatrix_blacs::getPartialMatrix_ServerTasks(
    const bool isFinalize) const {
#pragma omp critical(TlDenseGeneralMatrix_blacs__getPartialMatrix_ServerTasks)
  {
    TlCommunicate& rComm = TlCommunicate::getInstance();
    // const int numOfProcs = rComm.getNumOfProc();
    ...
    if ((this->isWaitingRequestHandShake_ == true) &&
      ...
      if (this->isDebugOut_GPM_ == true) {
        std::cerr << TlUtils::format(
                         "[%d] SRV [OK]: recv session id from [%d].",
                         rComm.getRank(), requestProc)
                  << std::endl;
      }
      ...
      // #pragma omp critical (TlDenseGeneralMatrix_blacs_gpmServerTasks_)
      //         {
    ...
    for (GpmServerTasksType::iterator it = this->gpmServerTasks_.begin();
         it != itEnd; ++it) {
      ...
      if ((task.state & GPM_SERVER_RECV_NUM_OF_COMPONENTS) == 0) {
        if (this->isDebugOut_GPM_ == true) {
          std::cerr << TlUtils::format(
                           "[%d] SRV [**]: recv num of components from [%d]",
                           rComm.getRank(), proc)
                    << std::endl;
        }

        rComm.iReceiveData(task.numOfComponents, proc,
      ...
      if (((task.state & GPM_SERVER_RECV_NUM_OF_COMPONENTS) != 0) &&
        ...
        if (rComm.test(task.numOfComponents) == true) {
          ...
          if (this->isDebugOut_GPM_ == true) {
            std::cerr << TlUtils::format(
                             "[%d] SRV [OK]: recv num of components from [%d]. "
                             "num = %d",
      ...
      if (((task.state & GPM_SERVER_WAIT_NUM_OF_COMPONENTS) != 0) &&
        ...
        rComm.iReceiveDataX((int*)&(task.components[0]), task.numOfComponents,
                            proc, TAG_GPM_COMPONENTS + task.sessionId);
      ...
      if (((task.state & GPM_SERVER_RECV_COMPONENTS) != 0) &&
        ...
        if (rComm.test((int*)&(task.components[0])) == true) {
          ...
          if (this->isDebugOut_GPM_ == true) {
            std::cerr << TlUtils::format(
                             "[%d] SRV [OK]: recv components from [%d]",
                             rComm.getRank(), proc)
                      << std::endl;
          }
      ...
      if (((task.state & GPM_SERVER_WAIT_COMPONENTS) != 0) &&
        ...
        for (int i = 0; i < size; ++i) {
          ...
          const TlMatrixObject::size_type localIndex = this->getIndex(row, col);
          assert(localIndex != -1);
...
void TlDenseGeneralMatrix_blacs::getSparseMatrixX_registerTask(
    TlSparseMatrix* pMatrix) const {
  TlCommunicate& rComm = TlCommunicate::getInstance();
  const int numOfProcs = rComm.getNumOfProc();
  ...
  if (pMatrix != NULL) {
    ...
    if (this->gpmClientTasks_.find(pMatrix) == this->gpmClientTasks_.end()) {
      ...
      for (TlSparseMatrix::iterator it = pMatrix->begin(); it != pMatrix->end();
           ++it) {
        ...
        if (localIndex != -1) {
          ...
          //                     std::cerr << TlUtils::format("[%d] check
          //                     elements([%d] has (%d, %d)",
          //                                                   rComm.getRank(),
          //                                                   proc, row, col)
          //                                << std::endl;
...
int TlDenseGeneralMatrix_blacs::getPartialMatrix_getSessionId(
...
bool TlDenseGeneralMatrix_blacs::getPartialMatrix_ClientTasks(
    TlMatrixObject* pMatrix) const {
  bool answer = false;
  TlCommunicate& rComm = TlCommunicate::getInstance();
  const int numOfProcs = rComm.getNumOfProc();

  GpmClientTasks::iterator itEnd = this->gpmClientTasks_.end();
  for (GpmClientTasks::iterator it = this->gpmClientTasks_.begin(); it != itEnd;
       ++it) {
    ...
    for (int proc = 0; proc < numOfProcs; ++proc) {
      if ((task.state[proc] & GPM_CLIENT_COUNT_COMPONENTS) == 0) {
        ...
        if (size == 0) {
          task.state[proc] =
              (GPM_CLIENT_COUNT_COMPONENTS | GPM_CLIENT_SEND_SESSION_ID |
               GPM_CLIENT_WAIT_SESSION_ID | GPM_CLIENT_SEND_NUM_OF_COMPONENTS |
               GPM_CLIENT_WAIT_NUM_OF_COMPONENTS | GPM_CLIENT_SEND_COMPONENTS |
               GPM_CLIENT_WAIT_COMPONENTS | GPM_CLIENT_RECV_ELEMENT_VALUES |
      ...
      if ((task.state[proc] & GPM_CLIENT_SEND_SESSION_ID) == 0) {
        if (this->trafficControl_[proc] == NULL) {
          ...
          if (this->isDebugOut_GPM_ == true) {
            std::cerr << TlUtils::format(
                             "[%d] CLI [**]: send session id to [%d]. num = %d",
      ...
      if ((task.state[proc] & GPM_CLIENT_SEND_NUM_OF_COMPONENTS) == 0) {
        if (this->trafficControl_[proc] == pMatrix) {
          ...
          if (this->isDebugOut_GPM_ == true) {
            std::cerr << TlUtils::format(
                             "[%d] CLI [**]: send num of components to [%d]. "
                             "num = %d",
                             rComm.getRank(), proc, task.numOfComponents[proc])
                      << std::endl;
          }
      ...
      if (((task.state[proc] & GPM_CLIENT_SEND_NUM_OF_COMPONENTS) != 0) &&
        ...
        if (this->trafficControl_[proc] == pMatrix) {
          if (rComm.test(task.numOfComponents[proc]) == true) {
            ...
            if (this->isDebugOut_GPM_ == true) {
              std::cerr << TlUtils::format(
                               "[%d] CLI [OK]: send num of components to [%d].",
                               rComm.getRank(), proc)
                        << std::endl;
            }
#endif  // NDEBUG
      ...
      if ((task.state[proc] & GPM_CLIENT_SEND_COMPONENTS) == 0) {
        if (this->trafficControl_[proc] == pMatrix) {
          ...
          rComm.iSendDataX((int*)&(task.components[proc][0]),
                           task.numOfComponents[proc], proc,
      ...
      if ((task.state[proc] & GPM_CLIENT_RECV_ELEMENT_VALUES) == 0) {
        if (this->trafficControl_[proc] == pMatrix) {
          ...
          rComm.iReceiveDataX((double*)&(task.elementValues[proc][0]), size,
                              proc,
...
void TlDenseGeneralMatrix_blacs::mergeSparseMatrix(const TlSparseMatrix& M) {
...
void TlDenseGeneralMatrix_blacs::mergeMatrix_common(
    const std::vector<std::vector<index_type> >& indexArrays,
  ...
  for (int proc = 0; proc < numOfProcs; ++proc) {
    ...
    rComm.iSendData(numOfIndeces[proc], proc,
                    TAG_TLDISTRIBUTE_MATRIX__MERGE__NUM_OF_INDECES);
    if (size > 0) {
      assert(values[proc].size() == size / 2);
      rComm.iSendDataX((index_type*)&(indexArrays[proc][0]), size, proc,
                       TAG_TLDISTRIBUTE_MATRIX__MERGE__INDECES);
      rComm.iSendDataX((double*)&(values[proc][0]), size / 2, proc,
  ...
  std::vector<unsigned int> state(numOfProcs, 0);
  state[rank] = (RECV_NUM_OF_INDECES | WAIT_NUM_OF_INDECES | RECV_INDECES |
                 WAIT_INDECES | RECV_VALUES | WAIT_VALUES | FINISHED);
  ...
  while (true) {
    for (int proc = 0; proc < numOfProcs; ++proc) {
      ...
      if ((state[proc] & RECV_NUM_OF_INDECES) != RECV_NUM_OF_INDECES) {
        rComm.iReceiveData(numOfInputIndeces[proc], proc,
                           TAG_TLDISTRIBUTE_MATRIX__MERGE__NUM_OF_INDECES);
      ...
      if (((state[proc] & WAIT_NUM_OF_INDECES) == WAIT_NUM_OF_INDECES) &&
        ...
        if (size > 0) {
          ...
          rComm.iReceiveDataX((index_type*)&(inputIndexList[proc][0]), size,
                              proc, TAG_TLDISTRIBUTE_MATRIX__MERGE__INDECES);
          state[proc] |= RECV_INDECES;
        } else {
          assert(size == 0);
          state[proc] |= (RECV_INDECES | WAIT_INDECES | RECV_VALUES |
                          WAIT_VALUES | FINISHED);
        }
      }
      ...
      if (((state[proc] & WAIT_NUM_OF_INDECES) == WAIT_NUM_OF_INDECES) &&
        ...
        if (size > 0) {
          ...
          rComm.iReceiveDataX((double*)&(inputValues[proc][0]), (size / 2),
                              proc, TAG_TLDISTRIBUTE_MATRIX__MERGE__VALUES);
...
void TlDenseGeneralMatrix_blacs::mergeSparseMatrixAsync(
  ...
  if (pMatrix != NULL) {
    ...
    for (TlSparseMatrix::const_iterator it = pMatrix->begin(); it != itEnd;
         ++it) {
      index_type globalRow = it->first.row;
      index_type globalCol = it->first.col;
      const double value = it->second;
...
void TlDenseGeneralMatrix_blacs::mergeMatrixAsync_send(
    const std::vector<std::vector<index_type> >& indexArrays,
    const std::vector<std::vector<double> >& values) {
  TlCommunicate& rComm = TlCommunicate::getInstance();
  const int numOfProcs = rComm.getNumOfProc();
  ...
  for (int proc = 0; proc < numOfProcs; ++proc) {
    ...
    if (size > 0) {
      ...
      rComm.iSendData(numOfContents[proc], proc,
                      TAG_MERGE_MATRIX_NUM_OF_CONTENTS + sessionId);
      rComm.iSendDataX(&(indexArrays[proc][0]), size * 2, proc,
                       TAG_MERGE_MATRIX_INDECES + sessionId);
      rComm.iSendDataX(&(values[proc][0]), size, proc,
                       TAG_MERGE_MATRIX_VALUES + sessionId);
    }
  }
...
void TlDenseGeneralMatrix_blacs::mergeMatrixAsync_recv(bool isFinalize) {
  TlCommunicate& rComm = TlCommunicate::getInstance();
  // const int numOfProcs = rComm.getNumOfProc();
  // const int rank = rComm.getRank();
  ...
  // セッション待ち受け
  int recvProc = 0;
#pragma omp critical(TlDenseGeneralMatrix_blacs__mergeMatrixAsync_recv)
  {
    while (true) {
      if (this->mm_isWaitingSessionId_ != true) {
        rComm.iReceiveDataFromAnySource(this->mm_waitingSessionId_,
                                        TAG_MERGE_MATRIX_SESSION_ID);
    ...
    for (MergeMatrixRecvTasks::iterator it =
             this->mergeMatrixRecvTasks_.begin();
         it != itEnd;) {
...
double TlDenseGeneralMatrix_blacs::trace() const {
  double answer = 0.0;
  const int nGlobalCols = this->m_nCols;
...
TlDenseGeneralMatrix_blacs& TlDenseGeneralMatrix_blacs::operator=(
    const TlDenseGeneralMatrix_blacs& rhs) {
  if (&rhs != this) {
...
TlDenseGeneralMatrix_blacs& TlDenseGeneralMatrix_blacs::operator=(
    const TlDenseSymmetricMatrix_blacs& rhs) {
  *this = TlDenseGeneralMatrix_blacs(rhs);
  return (*this);
}

void TlDenseGeneralMatrix_blacs::initialize() {
  TlCommunicate& rComm = TlCommunicate::getInstance();
  const int numOfProcs = rComm.getNumOfProc();
...
TlMatrixObject::index_type TlDenseGeneralMatrix_blacs::getNumOfMyElements()
    const {
  return (this->m_nMyRows * this->m_nMyCols);
}

TlMatrixObject::index_type TlDenseGeneralMatrix_blacs::getNumOfRows() const {
  return this->m_nRows;
}

TlMatrixObject::index_type TlDenseGeneralMatrix_blacs::getNumOfCols() const {
  return this->m_nCols;
}

int TlDenseGeneralMatrix_blacs::getBlockSize() const {
  return this->m_nBlockSize;
}

TlMatrixObject::size_type TlDenseGeneralMatrix_blacs::getIndex(
    const index_type nGlobalRow, const index_type nGlobalCol) const {
  TlMatrixObject::size_type answer = -1;

  std::vector<index_type>::const_iterator pRow = std::lower_bound(
      this->m_RowIndexTable.begin(), this->m_RowIndexTable.end(), nGlobalRow);
  if ((pRow != this->m_RowIndexTable.end()) && (*pRow == nGlobalRow)) {
    std::vector<index_type>::const_iterator pCol = std::lower_bound(
        this->m_ColIndexTable.begin(), this->m_ColIndexTable.end(), nGlobalCol);

    if ((pCol != this->m_ColIndexTable.end()) && (*pCol == nGlobalCol)) {
      const TlMatrixObject::size_type localRow =
          pRow - this->m_RowIndexTable.begin();
      const TlMatrixObject::size_type localCol =
          pCol - this->m_ColIndexTable.begin();
...
int TlDenseGeneralMatrix_blacs::getProcIdForIndex(
    const index_type globalRow, const index_type globalCol) const {
...
double TlDenseGeneralMatrix_blacs::get(const index_type row,
  ...
  pdelget_("A", " ", &dAnswer, this->pData_, &nGlobalRow, &nGlobalCol,
           this->m_pDESC);

  return dAnswer;
}

void TlDenseGeneralMatrix_blacs::set(const index_type row, const index_type col,
                                     const double dValue) {
  assert((0 <= row) && (row < this->m_nRows));
  assert((0 <= col) && (col < this->m_nCols));
...
void TlDenseGeneralMatrix_blacs::add(const index_type row, const index_type col,
                                     const double value) {
  assert((0 <= row) && (row < this->m_nRows));
  assert((0 <= col) && (col < this->m_nCols));
...
void TlDenseGeneralMatrix_blacs::addByList(const index_type* pIndexPairs,
...
double TlDenseGeneralMatrix_blacs::getLocal(const index_type row,
                                            const index_type col) const {
  double answer = 0.0;
  const int index = this->getIndex(row, col);
  if (index != -1) {
...
std::vector<TlDenseGeneralMatrix_blacs::index_type>
TlDenseGeneralMatrix_blacs::getRowIndexTable() const {
  return this->m_RowIndexTable;
}

std::vector<TlDenseGeneralMatrix_blacs::index_type>
TlDenseGeneralMatrix_blacs::getColIndexTable() const {
...
TlDenseGeneralMatrix_blacs operator*(const TlDenseGeneralMatrix_blacs& X,
  ...
  // TlDenseGeneralMatrix_blacs& Ytmp =
  ...
  TlDenseGeneralMatrix_blacs Z(nZRow, nZCol);
  ...
  pdgemm_("N", "N", &nZRow, &nZCol, &nXCol, &dAlpha, X.pData_, &nIX, &nJX,
          X.m_pDESC, Y.pData_, &nIY, &nJY, Y.m_pDESC, &dBeta, Z.pData_, &nIZ,
          &nIZ, Z.m_pDESC);

  return Z;
}

TlDistributedVector operator*(const TlDenseGeneralMatrix_blacs& A,
                              const TlDistributedVector& X) {
  // this const_cast is requiered for PGI compiler
  // "error: expression must be an lvalue or a function designator"
  // TlDenseGeneralMatrix_blacs& Atmp =
  // const_cast<TlDenseGeneralMatrix_blacs&>(A);
  ...
  pdgemv_(&TRANS, &M, &N, &alpha, A.pData_, &IA, &JA, A.m_pDESC,
          &(Xtmp.data_[0]), &IX, &JX, X.m_pDESC, &INCX, &beta, &(Y.data_[0]),
          &IY, &JY, Y.m_pDESC, &INCY);

  return Y;
}

// TlDistributedVector operator*(const TlDistributedVector& X, const
...
TlDenseGeneralMatrix_blacs operator*(const double X,
                                     const TlDenseGeneralMatrix_blacs& Y) {
  TlDenseGeneralMatrix_blacs ans = Y;
  ans *= X;
...
TlDenseGeneralMatrix_blacs operator*(const TlDenseGeneralMatrix_blacs& X,
                                     const double Y) {
  TlDenseGeneralMatrix_blacs ans = X;
  ans *= Y;
...
TlDenseGeneralMatrix_blacs operator+(const TlDenseGeneralMatrix_blacs& X,
                                     const TlDenseGeneralMatrix_blacs& Y) {
  TlDenseGeneralMatrix_blacs answer = X;
  answer += Y;
...
// double TlDenseGeneralMatrix_blacs::operator()(const int row,
//                                               const int col) const {
//   return this->get(row, col);
// }
//
// double& TlDenseGeneralMatrix_blacs::operator()(const index_type row,
//                                                const index_type col) {
...
double TlDenseGeneralMatrix_blacs::sum() const {
  double answer = 0.0;
  const std::size_t bufSize = this->getNumOfMyElements();
  for (std::size_t i = 0; i < bufSize; ++i) {
...
// bool TlDenseGeneralMatrix_blacs::isLoadable(std::ifstream& ifs) {
//   TlCommunicate& rComm = TlCommunicate::getInstance();
//   bool answer = false;
//
//   if (rComm.isMaster() == true) {
//     answer = TlDenseGeneralMatrix_BLAS_old::isLoadable(ifs);
//   }
//   rComm.broadcast(answer);
//   return answer;
// }
//
// bool TlDenseGeneralMatrix_blacs::isLoadable(const std::string& rFilePath) {
//   TlCommunicate& rComm = TlCommunicate::getInstance();
//   bool answer = false;
//
//   if (rComm.isMaster() == true) {
//     answer = TlDenseGeneralMatrix_BLAS_old::isLoadable(rFilePath);
//   }
//   rComm.broadcast(answer);
//   return answer;
// }

bool TlDenseGeneralMatrix_blacs::load(const std::string& sFilePath) {
...
bool TlDenseGeneralMatrix_blacs::load(std::fstream& fs) {
  TlCommunicate& rComm = TlCommunicate::getInstance();
  assert(rComm.checkNonBlockingCommunications());
  ...
  if (rComm.isMaster() == true) {
    TlMatrixUtils::getHeaderInfo(fs, &matrixType, &(rowcol[0]), &(rowcol[1]));
    assert((matrixType == TlMatrixObject::CSFD) ||
           (matrixType == TlMatrixObject::RSFD));
    // std::cerr << TlUtils::format("%d (%d, %d)", (int)matrixType, rowcol[0],
    //                              rowcol[1])
    //           << std::endl;
  }
...
// TlDenseGeneralMatrix_blacs::load_RSFD_master(index_type* pRow, index_type*
// pCol)
// {
//     TlCommunicate& rComm = TlCommunicate::getInstance();
//     const int numOfProcs = rComm.getNumOfProcs();
...
//         answer[proc].push_back(TlMatrixObject::MatrixElement(currentRow,
//         currentCol, buf[i]));
//         // if (proc == 0) {
//         //     // masterが担当
//         //     const index_type index = this->getIndex(currentRow,
//         currentCol);
//         //     assert(index != -1);
...
bool TlDenseGeneralMatrix_blacs::save(const std::string& sFilePath) const {
  if (TlDenseGeneralMatrix_blacs::isUsingPartialIO == true) {
  ...
  if (rComm.isMaster() == true) {
    ...
    TlFileGenericMatrix fm(sFilePath, globalRows, globalCols);
...
TlDenseGeneralMatrix_blacs::getMatrixElementsInLocal() const {
...
void TlDenseGeneralMatrix_blacs::saveElements(
    TlDenseMatrix_IO_object* pFileMatrix,
    const std::vector<TlMatrixObject::MatrixElement>& elements) const {
  std::vector<TlMatrixObject::MatrixElement>::const_iterator itEnd =
      elements.end();
  for (std::vector<TlMatrixObject::MatrixElement>::const_iterator it =
           elements.begin();
       it != itEnd; ++it) {
...
// HDF5 ------------------------------------------------------------------------
#ifdef HAVE_HDF5
bool TlDenseGeneralMatrix_blacs::saveHdf5(const std::string& filepath,
...
TlMatrixObject::size_type TlDenseGeneralMatrix_blacs::getArrayIndex(
...
bool TlDenseGeneralMatrix_blacs::loadHdf5(const std::string& filepath,
                                          const std::string& h5path) {
  TlCommunicate& rComm = TlCommunicate::getInstance();
  assert(rComm.checkNonBlockingCommunications());
  ...
  if (rComm.isMaster() == true) {
    ...
    const size_type numOfAllElements =
        this->getNumOfRows() * this->getNumOfCols();
    int cycle = 0;
    while (isFinished == false) {
      // buffer分を一度に読み込み
      const size_type numOfLoadItems =
          std::min<size_type>((cycle + 1) * bufferCount, numOfAllElements);
...
bool TlDenseGeneralMatrix_blacs::saveLocal(const std::string& filePath) const {
  // std::cerr << "TlDenseGeneralMatrix_blacs::saveLocal() file=" << filePath <<
  // std::endl;
...
bool TlDenseGeneralMatrix_blacs::loadLocal(const std::string& filePath) {
  // std::cerr << "TlDenseGeneralMatrix_blacs::loadLocal() file=" << filePath <<
  // std::endl;
  bool answer = false;
...
bool TlDenseGeneralMatrix_blacs::saveText(const std::string& sFilePath) const {
  TlCommunicate& rComm = TlCommunicate::getInstance();
  // this->update();
  bool bAnswer = true;
...
bool TlDenseGeneralMatrix_blacs::saveText(std::ofstream& ofs) const {
  TlCommunicate& rComm = TlCommunicate::getInstance();
  // this->update();
  bool bAnswer = true;
...
TlDenseGeneralMatrix_blacs::transposeInPlace() {
  ...
  const int IA = 1;
  const int JA = 1;
  TlDenseGeneralMatrix_blacs C(M, N);
  const int IC = 1;
  const int JC = 1;
  pdtran_(&M, &N, &alpha, &(this->pData_[0]), &IA, &JA, this->m_pDESC, &beta,
          &(C.pData_[0]), &IC, &JC, C.m_pDESC);
