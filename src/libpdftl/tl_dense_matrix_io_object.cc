...
TlDenseMatrix_IO_object::TlDenseMatrix_IO_object(const TlMatrixObject::MatrixType matrixType,
                                                 const std::string& filePath, const TlMatrixObject::index_type row,
                                                 const TlMatrixObject::index_type col, const std::size_t cacheSize)
    : TlMatrixObject(matrixType, row, col), filePath_(filePath), cacheCount_(0), cacheSize_(cacheSize) {
    this->initializeCache();
}

TlDenseMatrix_IO_object::TlDenseMatrix_IO_object(const TlMatrixObject::MatrixType matrixType,
                                                 const std::string& filePath, const std::size_t cacheSize)
    : TlMatrixObject(matrixType, 1, 1), filePath_(filePath), cacheCount_(0), cacheSize_(cacheSize) {
    this->initializeCache();
}
...
void TlDenseMatrix_IO_object::createNewFile() {
    ...
    this->fs_.write(reinterpret_cast<const char*>(&this->row_), sizeof(index_type));
    this->fs_.write(reinterpret_cast<const char*>(&this->col_), sizeof(index_type));
...
void TlDenseMatrix_IO_object::finalize() const {
    ...
    for (std::list<CacheUnit>::const_iterator p = this->cache_.begin(); p != pEnd; ++p) {
        if (p->isUpdate == true) {
            this->writeDisk(*p);
        }
    }
...
void TlDenseMatrix_IO_object::set(const index_type row, const index_type col, const double value) {
    double* const pBuf = this->getCachedData(row, col);
    *pBuf = value;
}

void TlDenseMatrix_IO_object::add(const index_type row, const index_type col, const double value) {
    const double tmp = this->get(row, col) + value;
    double* const pBuf = this->getCachedData(row, col);
    *pBuf = tmp;
}

double TlDenseMatrix_IO_object::get(const index_type row, const index_type col) const {
    return this->getCachedData(row, col);
}

void TlDenseMatrix_IO_object::setRowVector(const index_type row, const TlDenseVector_Lapack& v) {
...
void TlDenseMatrix_IO_object::setColVector(const index_type col, const TlDenseVector_Lapack& v) {
...
TlDenseVector_Lapack TlDenseMatrix_IO_object::getRowVector(const index_type nRow) const {
...
TlDenseVector_Lapack TlDenseMatrix_IO_object::getColumnVector(const index_type nCol) const {
...
double* TlDenseMatrix_IO_object::getCachedData(const index_type row, const index_type col) {
...
double TlDenseMatrix_IO_object::getCachedData(const index_type row, const index_type col) const {
    static const unsigned long localIndexBit = (static_cast<unsigned long>(1) << CACHE_GROUP_BIT) - 1;
...
void TlDenseMatrix_IO_object::updateCache(const size_t index) const {
    ...
    std::list<CacheUnit>::iterator found = std::find_if(this->cache_.begin(), pCacheUnitEnd, CacheUnitComp(group));
    ...
    if (found != pCacheUnitEnd) {
        ...
        const std::fstream::pos_type pos =
            std::fstream::pos_type(this->startPos_) + std::fstream::pos_type(group) * std::fstream::pos_type(blockSize);
...
void TlDenseMatrix_IO_object::writeDisk(const TlDenseMatrix_IO_object::CacheUnit& cu) const {
    std::string stateStr = "";
...
TlDenseMatrix_IO_object& TlDenseMatrix_IO_object::operator*=(const double coef) {
    // ToDo 高速化
    const index_type numOfRows = this->getNumOfRows();
    const index_type numOfCols = this->getNumOfCols();
...
TlDenseGeneralMatrix_Lapack TlDenseMatrix_IO_object::getBlockMatrix(const index_type row, const index_type col,
...
void TlDenseMatrix_IO_object::block(const TlMatrixObject::index_type row, const TlMatrixObject::index_type col,
