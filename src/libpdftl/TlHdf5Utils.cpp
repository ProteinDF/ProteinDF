...
void TlHdf5Utils::createDataSet_common(const std::string& path,
    ...
    H5::DataSet dataSet =
        this->file_.createDataSet(path.c_str(), memType, memSpace);
}
...
void TlHdf5Utils::write(const std::string& path, const std::string& values) {
    ...
    H5::DataSet dataSet =
        this->file_.createDataSet(path.c_str(), dataType, dataSpace);
...
void TlHdf5Utils::write(const std::string& name, const double* pValues,
                        std::size_t size) {
    this->write_common<double>(name, pValues, H5::PredType::NATIVE_DOUBLE,
                               size);
}
...
void TlHdf5Utils::setAttr(const std::string& path, const std::string& attrName,
    ...
    H5::Attribute attr =
        dataSet.createAttribute(attrName.c_str(), memType, memSpace);
...
bool TlHdf5Utils::hasGroup(const H5::Group* pParent, const std::string& name) {
...
bool TlHdf5Utils::hasObject(const H5::Group* pParent, const std::string& name,
...
bool TlHdf5Utils::hasChildObject(const H5::Group* pParent,
    ...
    for (hsize_t i = 0; i < numOfObjs; ++i) {
        ...
        const std::string objName =
            this->getObjnameByIdx(pParent, i, TlHdf5Utils::initStrLength_);
        if (objName == name) {
...
std::string TlHdf5Utils::getObjnameByIdx(const H5::Group* pParent,
                                         const hsize_t index,
                                         const std::size_t size) {
    std::string answer;
