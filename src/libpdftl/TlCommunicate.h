...
public:
...
#ifdef HAVE_EIGEN
    int broadcast(TlDenseGeneralMatrix_Eigen* pMatrix, int root = 0);
    int broadcast(TlDenseSymmetricMatrix_Eigen* pMatrix, int root = 0);
    int broadcast(TlDenseVector_Eigen* pMatrix, int root = 0);
#endif  // HAVE_EIGEN
    ...
    //  int sendData(const TlVector_BLAS& data, int destination = 0, int tag =
    //  0); int sendData(const TlDenseGeneralMatrix_BLAS_old& data, int
    //  destination = 0,
    //               int tag = 0);
    ...
    int sendData(const TlDenseSymmetricMatrix_Lapack& rData,
                 int nDestination = 0, int nTag = 0);
    int sendData(const TlDenseVector_Lapack& data, int destination = 0,
                 int tag = 0);
    ...
    //  int receiveData(TlDenseGeneralMatrix_BLAS_old& data, int src, int tag =
    //  0); int receiveData(TlDenseSymmetricMatrix_BLAS_Old& rData, int nSrc,
    //                  int nTag = 0);
    ...
    int receiveDataFromAnySource(unsigned int& data, int* pSrc,
                                 int* pTag = NULL);
    ...
    // タグ付き
    //  int receiveDataFromAnySource(TlDenseGeneralMatrix_BLAS_old& rData, int*
    //  pSrc,
    //                               int tag = 0);
    ...
    int iSendData(const std::vector<int>& data, int destination = 0,
                  int tag = 0);
    ...
    int receiveDataX(double* pData, const std::size_t size, const int src,
                     const int tag = 0);
    int receiveDataX(TlMatrixObject::MatrixElement* pData,
                     const std::size_t size, const int src, const int tag = 0);
    int receiveDataX(TlVectorObject::VectorElement* pData,
                     const std::size_t size, const int src, const int tag = 0);
...
protected:
    ...
    template <typename T>
    int receiveDataFromAnySource(std::vector<T>& data,
                                 const MPI_Datatype mpiType, int* pSrc,
                                 int* pTag);
    ...
    int iReceiveDataFromAnySourceX(T* pData, const MPI_Datatype mpiType,
                                   const std::size_t start,
                                   const std::size_t end, const int tag);
...
private:
    ...
    template <typename T>
    int broadcast(std::valarray<T>& data, const MPI_Datatype mpiType,
                  const std::size_t start, const std::size_t end, int root);
...
private:
    ...
    public:
        ...
        NonBlockingCommParam(const std::vector<uintptr_t>& reqs,
                             const int in_tag, const unsigned int in_property)
            : requests(reqs),
