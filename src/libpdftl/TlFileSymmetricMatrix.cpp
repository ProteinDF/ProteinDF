...
{
    // TlFileMatrix::open() is not called, because base constructer was called using (initialize = false)!
    this->open();
}
...
void TlFileSymmetricMatrix::open()
{
    if (TlFile::isExistFile(this->filePath_) == false) {
        ...
        this->fs_.write(reinterpret_cast<const char*>(&this->numOfRows_), sizeof(index_type));
        this->fs_.write(reinterpret_cast<const char*>(&this->numOfCols_), sizeof(index_type));
...
TlMatrixObject::size_type TlFileSymmetricMatrix::getIndex(index_type row, index_type col) const
{
