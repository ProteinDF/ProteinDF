...
const std::string TlGetopt::operator[](const std::string& sKey) const
{
    ...
    std::map<std::string, std::string>::const_iterator p = this->m_Data.find(sKey);
...
void TlGetopt::parseArgv(int argc, char* argv[], const char* list)
{
    ...
    bool bOptionSection = true; // optionのparseをしている時はtrue, 引数の時はfalseを表すフラグ
    std::string sOptArg = "";   // optionの引数用フラグ兼ハッシュのkey。""の場合flagなし
