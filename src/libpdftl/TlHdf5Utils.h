...
class TlHdf5Utils {
   ...
   public:
    ...
    void write(const std::string& name, const double* pValues,
               std::size_t size);
   ...
   public:
    ...
    void get(const std::string& path, double* pOut,
   ...
   protected:
    ...
    bool hasChildObject(const H5::Group* pParent, const std::string& name,
                        const H5O_type_t requestType);
    std::string getObjnameByIdx(const H5::Group* pParent, const hsize_t index,
                                const std::size_t size);
