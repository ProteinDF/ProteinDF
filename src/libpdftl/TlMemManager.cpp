...
void TlMemManager::setParam(const std::size_t needMemSize, const std::string& baseFileName) {
    ...
    TlMemManager::filePath_ =
        TlUtils::format("%s.%s%d.map", baseFileName.c_str(), TlSystem::getHostName().c_str(), TlSystem::getPID());
...
TlMemManager& TlMemManager::getInstance() {
    if (TlMemManager::instance_ == NULL) {
        if (TlMemManager::isSetParam_ == false) {
            ...
            log.warn(
                "please call TlMemManager::setParam() function for "
                "performance.");
        }
...
char* TlMemManager::allocate(const std::size_t size) {
    ...
    std::sort(this->freeMemList_.begin(), this->freeMemList_.end(), MemItemSortFunctor_size());

    // 空きメモリリストから必要量のメモリを確保する
    std::vector<MemItem>::iterator p = std::lower_bound(this->freeMemList_.begin(), this->freeMemList_.end(),
...
void TlMemManager::deallocate(char* p) {
    ...
    std::sort(this->freeMemList_.begin(), this->freeMemList_.end(), MemItemSortFunctor_index());
...
void TlMemManager::memMap() {
    ...
    // message
    this->log_.info(TlUtils::format("TlMemManager::memMap() file create: path=%s, size=%lu", this->filePath_.c_str(),
                                    TlMemManager::mmapLength_));

    TlMemManager::beginMmap_ = (char*)TlSystem::newMmap(this->mmapLength_, fd);
...
void TlMemManager::debugOutUseMemList() const {
    std::string str = "";
    for (std::map<std::size_t, std::size_t>::const_iterator it = this->useList_.begin(); it != this->useList_.end();
         ++it) {
