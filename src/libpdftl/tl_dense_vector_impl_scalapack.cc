...
double TlDenseVector_ImplScalapack::operator*(
  ...
  pddot_(&N, &DOT, Xtmp.pData_, &IX, &JX, Xtmp.pDESC_, &INCX, Ytmp.pData_, &IY,
         &JY, Ytmp.pDESC_, &INCY);
  ...
  if (bIsFail) {
    if (rComm.isMaster() == true) {
      this->log_.critical(TlUtils::format(
          "[error] TlDistributedVector::load(): could not open file. : %s",
          sFilePath.c_str()));
    }
    abort();
  }
  ...
  if (rComm.isMaster() == true) {
    ...
    while (isFinished == false) {
      ...
      for (std::size_t i = 0; i < bufferCount; ++i) {
        ...
        elements[rank].push_back(TlVectorObject::VectorElement(count, buf[i]));
      ...
      // proc=0分データの書き込み
      {
        ...
        for (TlVectorObject::index_type i = 0; i < sizeList; ++i) {
          const TlVectorObject::index_type globalIndex = elements[0][i].index;
...
TlDenseVector_ImplScalapack::getVectorElementsInLocal() const {
  ...
  const TlVectorObject::size_type numOfMyElements = this->getNumOfMyElements();
