...
const std::size_t TlDistributeMatrix::FILE_BUFFER_SIZE =
    100 * 1024 * 1024;  // 100 MB
const TlDistributeMatrix::size_type TlDistributeMatrix::MAX_LOOP =
...
////////////////////////////////////////////////////////////////////////
TlDistributeMatrix::TlDistributeMatrix(const index_type row,
...
TlDistributeMatrix::TlDistributeMatrix(const TlDistributeVector& rhs,
  ...
  TlDistributeVector rhs_tmp =
      const_cast<TlDistributeVector&>(rhs);  // for PGI compiler
...
TlDistributeVector TlDistributeMatrix::getVector() const {
  ...
  return TlDistributeVector(answer,
...
double TlDistributeMatrix::getMaxAbsoluteElement(index_type* pOutRow,
                                                 index_type* pOutCol) const {
  int nRow = 0;
  int nCol = 0;
...
double TlDistributeMatrix::getLocalMaxAbsoluteElement(
...
// TlSparseMatrix TlDistributeMatrix::getPartialMatrix(const double threshold)
// const
// {
...
// 複数スレッドから同時に呼び出さない
bool TlDistributeMatrix::getSparseMatrixX(TlSparseMatrix* pMatrix,
...
void TlDistributeMatrix::getPartialMatrix_ServerTasks(
...
void TlDistributeMatrix::getSparseMatrixX_registerTask(
...
bool TlDistributeMatrix::getPartialMatrix_ClientTasks(
...
void TlDistributeMatrix::mergeMatrix_common(
...
void TlDistributeMatrix::mergeSparseMatrixAsync(const TlSparseMatrix* pMatrix,
                                                bool isFinalize) {
  if (pMatrix != NULL) {
...
void TlDistributeMatrix::mergeMatrixAsync_send(
    const std::vector<std::vector<index_type> >& indexArrays,
    const std::vector<std::vector<double> >& values) {
  TlCommunicate& rComm = TlCommunicate::getInstance();
  const int numOfProcs = rComm.getNumOfProc();
...
TlDistributeMatrix& TlDistributeMatrix::operator=(
    const TlDistributeMatrix& rhs) {
  if (&rhs != this) {
...
TlDistributeMatrix& TlDistributeMatrix::operator=(
    const TlDistributeSymmetricMatrix& rhs) {
...
int TlDistributeMatrix::getBlockSize() const { return this->m_nBlockSize; }

TlMatrixObject::size_type TlDistributeMatrix::getIndex(
...
int TlDistributeMatrix::getProcIdForIndex(const index_type globalRow,
...
double TlDistributeMatrix::get(const index_type row,
...
void TlDistributeMatrix::set(const index_type row, const index_type col,
                             const double dValue) {
  assert((0 <= row) && (row < this->m_nRows));
  assert((0 <= col) && (col < this->m_nCols));
...
void TlDistributeMatrix::add(const index_type row, const index_type col,
                             const double value) {
  assert((0 <= row) && (row < this->m_nRows));
  assert((0 <= col) && (col < this->m_nCols));
...
double TlDistributeMatrix::getLocal(const index_type row,
                                    const index_type col) const {
  double answer = 0.0;
  const int index = this->getIndex(row, col);
  if (index != -1) {
...
TlDistributeMatrix operator*(const TlDistributeMatrix& X,
...
TlDistributeVector operator*(const TlDistributeMatrix& A,
...
// TlDistributeVector operator*(const TlDistributeVector& X, const
...
double& TlDistributeMatrix::operator()(const index_type row,
                                       const index_type col) {
...
bool TlDistributeMatrix::load(const std::string& sFilePath) {
  ...
  if (bAnswer != true) {
    if (rComm.isMaster() == true) {
      std::cerr << "TlDistributeMatrix::load() is not supported: " << sFilePath
                << std::endl;
    }
    std::abort();
  }
...
TlDistributeMatrix::getMatrixElementsInLocal() const {
...
void TlDistributeMatrix::saveElements(
    TlFileMatrix* pFileMatrix,
...
TlMatrixObject::size_type TlDistributeMatrix::getArrayIndex(
...
bool TlDistributeMatrix::loadHdf5(const std::string& filepath,
                                  const std::string& h5path) {
  TlCommunicate& rComm = TlCommunicate::getInstance();
  assert(rComm.checkNonBlockingCommunications());
...
bool TlDistributeMatrix::saveLocal(const std::string& filePath) const {
  // std::cerr << "TlDistributeMatrix::saveLocal() file=" << filePath <<
  // std::endl;
...
bool TlDistributeMatrix::loadLocal(const std::string& filePath) {
  // std::cerr << "TlDistributeMatrix::loadLocal() file=" << filePath <<
  // std::endl;
  bool answer = false;
...
bool inverseByScaLapack(TlDistributeMatrix& X) {
  ...
  //   if (rComm.isMaster() == true){
  //     std::cout << "TlDistributeMatrix::inverseMatrix() time:" << (dEndTime -
  //     dStartTime) << std::endl;
  //   }
