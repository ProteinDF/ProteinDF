...
TlPseudoYaml::TlPseudoYaml(const TlSerializeData& data) : data_(data) {}

TlPseudoYaml::TlPseudoYaml(const TlPseudoYaml& rhs) : data_(rhs.data_) {}
...
std::string TlPseudoYaml::str() const { return this->str(this->data_, 0); }

std::string TlPseudoYaml::str(const TlSerializeData& data,
    ...
    switch (data.getType()) {
        ...
        default: { ans += data.getStr() + "\n"; } break;
    }

    return ans;
}

std::string TlPseudoYaml::str_array(const TlSerializeData& data,
                                    const int level) const {
    ...
    for (TlSerializeData::ArrayConstIterator p = data.beginArray();
        ...
        if ((childType == TlSerializeData::ARRAY) ||
            (childType == TlSerializeData::MAP)) {
            ans += "\n";
        }
...
std::string TlPseudoYaml::str_map(const TlSerializeData& data,
                                  const int level) const {
    const std::string indent = TlUtils::repeat("  ", level);
    std::string ans = "";
...
TlSerializeData TlPseudoYaml::getSerializeObject() const { return this->data_; }
