...
class TlSerializeDataMapIterator {
   ...
   public:
    ...
    TlSerializeDataMapIterator& operator=(
        const TlSerializeDataMapIterator& rhs) {
        if (&rhs != this) {
            this->it_ = rhs.it_;
        }
        return *this;
...
class TlSerializeData {
   ...
   public:
    typedef TlSerializeDataVectorIterator<ArrayContainerType::iterator,
                                          TlSerializeData>
        ArrayIterator;
    typedef TlSerializeDataVectorConstIterator<
        ArrayContainerType::const_iterator, TlSerializeData>
        ArrayConstIterator;
...
TlSerializeData::TlSerializeData(const std::vector<T>& rhs)
    ...
    for (typename std::vector<T>::const_iterator it = rhs.begin(); it != itEnd;
         ++it) {
        this->pushBack(*it);
    }
}
