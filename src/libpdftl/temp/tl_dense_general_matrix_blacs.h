...
#ifndef TL_DENSE_GENERAL_MATRIX_BLACS_H
#define TL_DENSE_GENERAL_MATRIX_BLACS_H
...
class TlDenseGeneralMatrix_blacs : public TlMatrixObject {
 ...
 public:
  struct LocalMatrixHeader {
   public:
    LocalMatrixHeader(int inType = 0, index_type inGlobalRow = 0,
                      index_type inGlobalCol = 0, index_type inMyRows = 0,
                      index_type inMyCols = 0, int inRank = 0,
                      int inNumOfProcs = 0, int inProcGridRow = 0,
                      int inProcGridCol = 0, int inMyProcRow = 0,
                      int inMyProcCol = 0, int inBlockSize = 0)
        : type(inType),
          globalRow(inGlobalRow),
          globalCol(inGlobalCol),
          myRows(inMyRows),
          myCols(inMyCols),
          rank(inRank),
          numOfProcs(inNumOfProcs),
          procGridRow(inProcGridRow),
          procGridCol(inProcGridCol),
          myProcRow(inMyProcRow),
          myProcCol(inMyProcCol),
 ...
 public:
  ...
  explicit TlDenseGeneralMatrix_blacs(int row = 1, int col = 1);

  /// コピーコンストラクタ
  TlDenseGeneralMatrix_blacs(const TlDenseGeneralMatrix_blacs& rhs);

  /// 対称行列からの変換コンストラクタ
  TlDenseGeneralMatrix_blacs(const TlDenseSymmetricMatrix_blacs& rhs);

  /// ベクトルからの変換コンストラクタ
  TlDenseGeneralMatrix_blacs(const TlDistributedVector& rhs, const int row,
                             const int col);

  TlDenseGeneralMatrix_blacs(
      const TlDenseGeneralMatrix_arrays_RowOriented& rhs);

  virtual ~TlDenseGeneralMatrix_blacs();
 ...
 public:
  ...
  virtual TlDistributedVector getVector() const;
  ...
  /// @return 要素の絶対値の最大値
  virtual double getMaxAbsoluteElement(index_type* pOutRow = NULL,
                                       index_type* pOutCol = NULL) const;
 ...
 protected:
  ...
  /// @param[out] outCol 該当要素の列
  /// @return 要素の絶対値の最大値
  virtual double getLocalMaxAbsoluteElement(index_type* pOutRow = NULL,
                                            index_type* pOutCol = NULL) const;
 ...
 public:
  ...
  /// dot積を返す
  const TlDenseGeneralMatrix_blacs& dotInPlace(
  ...
  TlDenseGeneralMatrix_blacs& operator=(
  ...
  TlDenseGeneralMatrix_blacs& operator*=(
      const TlDenseSymmetricMatrix_blacs& rhs);
  TlDenseGeneralMatrix_blacs& operator*=(double dCoef);
  TlDenseGeneralMatrix_blacs& operator/=(double dCoef);
 ...
 public:
  ...
  virtual void addByList(const index_type* pIndexPairs, const double* pValues,
 ...
 public:
  ...
  virtual const TlDenseGeneralMatrix_blacs& transposeInPlace();
 ...
 protected:
  ...
  ///
  /// @param[in] indexArrays
  /// プロセス番号ごとに行列インデックス(行、列)を順に格納する。(i1, j1, i2, j2,
  /// ...)
 ...
 protected:
  ...
  friend TlDenseGeneralMatrix_blacs operator*(
      const TlDenseGeneralMatrix_blacs& X, const TlDenseGeneralMatrix_blacs& Y);
  ...
  friend TlDenseGeneralMatrix_blacs operator*(
      double X, const TlDenseGeneralMatrix_blacs& Y);
  friend TlDenseGeneralMatrix_blacs operator*(
      const TlDenseGeneralMatrix_blacs& X, double Y);
...
// =============================================================================
// inline double TlDenseGeneralMatrix_blacs::getDirect(const index_type row,
// const
...
void TlDenseGeneralMatrix_blacs::print(T& out) const {
  ...
  for (int ord = 0; ord < nNumOfCols; ord += 10) {
    ...
    for (int i = 0; i < nNumOfRows; ++i) {
      ...
      for (int j = ord; ((j < ord + 10) && (j < nNumOfCols)); ++j) {
        std::string s = TlUtils::format(" %10.6lf", this->get(i, j));
...
#endif  // TL_DENSE_GENERAL_MATRIX_BLACS_H
