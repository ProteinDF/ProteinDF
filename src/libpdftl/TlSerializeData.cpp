...
bool TlSerializeData::operator==(const TlSerializeData& rhs) const {
    ...
    if (this->getType() == rhs.getType()) {
        switch (this->getType()) {
            ...
            case DOUBLE:
                answer =
                    (std::fabs(this->scalar_.double_ - rhs.scalar_.double_) <
...
void TlSerializeData::clearChildren() {
    ...
    for (MapContainerType::iterator p = this->map_.begin();
         p != this->map_.end(); ++p) {
        delete p->first;
        // p->first = NULL;
...
TlSerializeData::MapContainerType::iterator TlSerializeData::find(
    ...
    if (this->map_.size() > 0) {
        ...
        for (MapContainerType::iterator p = this->map_.begin(); p != pEnd;
             ++p) {
    ...
    switch (this->getType()) {
        ...
        case MAP: {
            ...
            for (MapConstIterator p = this->beginMap(); p != pEnd; ++p) {
                ...
                ans += TlUtils::format("\"%s\": \"%s\", ", key.c_str(),
                                       value.c_str());
            }
            ans = ans.substr(0, ans.size() - 2);  // remove ", ".
            ans += "}";
