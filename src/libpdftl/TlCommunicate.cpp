...
int TlCommunicate::reduce_SUM(TlSparseMatrix& rMatrix, int root) {
    ...
    if (myRank == root) {
        ...
        for (TlSparseMatrix::const_iterator it = rMatrix.begin(); it != itEnd;
             ++it) {
            // buf[count] = TlMatrixElement(it->first.row, it->first.col,
            // it->second);
...
//   if (nErr == 0) {
//     nErr = this->sendDataX(data.data_, MPI_DOUBLE, 0, dim, destination,
//     nTag);
//   }
//
//   return nErr;
...
//   }
//
//   err = this->sendDataX(data.data_, data.getNumOfElements(), destination,
//   tag); this->log_.debug(TlUtils::format(
//       "TlCommunicate::sendData(TlDenseGeneralMatrix_BLAS_old): dest=%d, "
//       "tag=%d, size=%ld, err=%d.",
...
//   if (nErr == 0) {
//     nErr = this->sendDataX(data.data_, MPI_DOUBLE, 0,
//     data.getNumOfElements(),
//                            nDestination, nTag);
//   }
//
//   return nErr;
...
//   this->log_.debug(TlUtils::format(
//       "TlCommunicate::recvData(TlDenseGeneralMatrix_BLAS_old): src=%d,
//       tag=%d, " "row=%d, col=%d, err=%d.", src, tag, numOfRows, numOfCols,
//       err));
//
//   delete[] pHeader;
//   pHeader = NULL;
...
//   err = this->receiveDataX(data.data_, MPI_DOUBLE, 0,
//   data.getNumOfElements(),
//                            src, tag);
//   this->log_.debug(TlUtils::format(
//       "TlCommunicate::recvData(TlDenseGeneralMatrix_BLAS_old): src=%d,
//       tag=%d, " "size=%ld, err=%d.", src, tag, data.getNumOfElements(),
//       err));
//
//   return err;
// }

// int TlCommunicate::receiveData(TlDenseSymmetricMatrix_BLAS_Old& rData, int
// nSrc,
//                                int nTag) {
//   std::size_t dim = 0;
...
// int TlCommunicate::receiveDataFromAnySource(TlDenseGeneralMatrix_BLAS_old&
// data,
//                                             int* pSrc, int tag) {
//   assert(pSrc != NULL);
//
...
//
//   data.resize(numOfRows, numOfCols);
//   return this->receiveDataX(data.data_, MPI_DOUBLE, 0,
//   data.getNumOfElements(),
//                             src, tag);
// }
...
void TlCommunicate::register_MatrixElement() {
    ...
    // MPI_Datatype types[3] = {MPI_UNSIGNED_LONG, MPI_UNSIGNED_LONG,
    // MPI_DOUBLE};
