...
int TlCommunicate::barrier(bool isDebugOut) const {
  ...
  if (isDebugOut == true) {
    this->log_.debug(
        TlUtils::format("barrier called. times=%ld", this->counter_barrier_));
  }
...
#pragma omp critical(TlCommunicate_nonBlockingCommParamTable_update)
  {
    if (this->nonBlockingCommParamTable_.find(key) !=
      ...
      const char isSendRecv =
          ((param.property & NonBlockingCommParam::SEND) != 0) ? 'S' : 'R';
      this->log_.critical(
          TlUtils::format("[%5d/%5d WARN] non-blocking table collision(%c) "
                          "found in TlCommunicate: tag=%d, line=%d",
                          this->getRank(), this->getNumOfProcs() - 1,
                          isSendRecv, param.tag, line));
      answer = false;
    }
  }
...
#pragma omp critical(TlCommunicate_nonBlockingCommParamTable_update)
  {
    if (this->nonBlockingCommParamTable_.empty() == false) {
      ...
      for (NonBlockingCommParamTableType::const_iterator it =
           ...
           it != itEnd; ++it) {
        const char isSendRecv =
            ((it->second.property & NonBlockingCommParam::SEND) != 0) ? 'S'
                                                                      : 'R';
        log.warn(
            TlUtils::format("[%5d/%5d WARN] rest waiting communication(%c) in "
                            "TlCommunicate: TAG=%d",
                            this->getRank(), this->getNumOfProcs() - 1,
...
int TlCommunicate::reduce(T* pData, const MPI_Datatype mpiType,
...
#ifdef DIV_COMM
  {
    ...
    for (long i = 0; i < tmp.quot; ++i) {
      ...
      answer = MPI_Reduce(pData + startIndex, pBuf, bufCount, mpiType, mpiOp,
                          root, MPI_COMM_WORLD);
      std::copy(pBuf, pBuf + bufCount, pData + startIndex);
      if (answer != 0) {
        this->log_.critical(TlUtils::format("MPI error. %s:%d answer=%d",
    ...
    if (tmp.rem != 0) {
      ...
      answer = MPI_Reduce(pData + startIndex, pBuf, remain, mpiType, mpiOp,
                          root, MPI_COMM_WORLD);
...
int TlCommunicate::reduce_MAXLOC(double* pValue, int* pIndex, int root) {
  ...
  const int answer = MPI_Reduce(&in, &out, 1, MPI_DOUBLE_INT, MPI_MAXLOC, root,
                                MPI_COMM_WORLD);
  *pValue = out.value;
  *pIndex = out.index;
...
int TlCommunicate::allReduce_SUM(int& rData) {
  ...
  int nReceive = 0;
  int nError =
      MPI_Allreduce(&rData, &nReceive, nSize, MPI_INT, MPI_SUM, MPI_COMM_WORLD);
...
int TlCommunicate::allReduce_SUM(std::vector<T>& data,
...
#ifdef DIV_COMM
  {
    ...
    const int bufCount =
        static_cast<int>(this->workMemSize_ / sizeof(T));  // T型配列の配列数
    ...
    for (long i = 0; i < tmp.quot; ++i) {
      std::size_t startIndex = start + static_cast<std::size_t>(bufCount * i);
      answer = MPI_Allreduce(&(data[startIndex]), &(buf[0]), bufCount, mpiType,
                             MPI_SUM, MPI_COMM_WORLD);
    ...
    if (tmp.rem != 0) {
      ...
      std::size_t startIndex =
          start + static_cast<std::size_t>(bufCount * tmp.quot);
      answer = MPI_Allreduce(&(data[startIndex]), &(buf[0]), remain, mpiType,
                             MPI_SUM, MPI_COMM_WORLD);
...
int TlCommunicate::allReduce(T* pData, const MPI_Datatype mpiType,
...
#ifdef DIV_COMM
  {
    ...
    for (long i = 0; i < tmp.quot; ++i) {
      std::size_t startIndex = start + static_cast<std::size_t>(bufCount * i);
      answer = MPI_Allreduce(pData + startIndex, pBuf, bufCount, mpiType, mpiOp,
                             MPI_COMM_WORLD);
      std::copy(pBuf, pBuf + bufCount, pData + startIndex);
      if (answer != 0) {
        this->log_.critical(TlUtils::format("MPI error. %s:%d answer=%d",
    ...
    if (tmp.rem != 0) {
      ...
      std::size_t startIndex =
          start + static_cast<std::size_t>(bufCount * tmp.quot);
      answer = MPI_Allreduce(pData + startIndex, pBuf, remain, mpiType, mpiOp,
                             MPI_COMM_WORLD);
      std::copy(pBuf, pBuf + remain, pData + startIndex);
      if (answer != 0) {
        this->log_.critical(TlUtils::format("MPI error. %s:%d answer=%d",
...
#else
  {
    T* pBuf = new T[end - start];
    answer =
        MPI_Allreduce(pData, pBuf, end - start, mpiType, mpiOp, MPI_COMM_WORLD);
    std::copy(pBuf, pBuf + (end - start), pData);
    delete[] pBuf;
    pBuf = NULL;
  }
...
int TlCommunicate::allReduce_MAX(int& rData) {
  ...
  const int count = 1;
  int nError =
      MPI_Allreduce(&rData, &receive, count, MPI_INT, MPI_MAX, MPI_COMM_WORLD);
...
int TlCommunicate::allReduce_MIN(int& rData) {
  ...
  const int count = 1;
  int nError =
      MPI_Allreduce(&rData, &receive, count, MPI_INT, MPI_MIN, MPI_COMM_WORLD);
...
int TlCommunicate::sendData(double data, int nDestination, int nTag) {
  int nErr =
      MPI_Send((void*)&data, 1, MPI_DOUBLE, nDestination, nTag, MPI_COMM_WORLD);
  return nErr;
}
...
#else
  {
    answer = MPI_Send((void*)&(data[0]), end - start, mpiType, destination, tag,
                      MPI_COMM_WORLD);
  }
#endif  // DIV_COMM
...
int TlCommunicate::sendData(const std::valarray<double>& data,
...
#ifdef DIV_COMM
  {
    ...
    for (long i = 0; i < tmp.quot; ++i) {
      std::size_t startIndex = start + static_cast<std::size_t>(bufCount * i);
      answer = MPI_Send((void*)&(data_tmp[startIndex]), bufCount, MPI_DOUBLE,
                        destination, tag, MPI_COMM_WORLD);
      if (answer != 0) {
        std::cerr << " MPI error. " << __FILE__ << ":" << __LINE__
                  << " anwer=" << answer << std::endl;
    ...
    if (tmp.rem != 0) {
      ...
      std::size_t startIndex =
          start + static_cast<std::size_t>(bufCount * tmp.quot);
      answer = MPI_Send((void*)&(data_tmp[startIndex]), remain, MPI_DOUBLE,
                        destination, tag, MPI_COMM_WORLD);
      if (answer != 0) {
        std::cerr << " MPI error. " << __FILE__ << ":" << __LINE__
                  << " anwer=" << answer << std::endl;
...
int TlCommunicate::receiveData(int& rData, int nSrc, int nTag) {
  MPI_Status status;
  int nErr =
      MPI_Recv((void*)&rData, 1, MPI_INT, nSrc, nTag, MPI_COMM_WORLD, &status);
...
int TlCommunicate::receiveData(long& rData, int nSrc, int nTag) {
  MPI_Status status;
  int nErr =
      MPI_Recv((void*)&rData, 1, MPI_LONG, nSrc, nTag, MPI_COMM_WORLD, &status);
...
int TlCommunicate::receiveData(double& rData, int nSrc, int nTag) {
  MPI_Status status;
  int nErr = MPI_Recv((void*)&rData, 1, MPI_DOUBLE, nSrc, nTag, MPI_COMM_WORLD,
                      &status);
...
//                                const std::size_t start, const std::size_t
//                                end, const int src, const int tag)
// {
//     int answer = 0;
//     const long length = static_cast<long>(end - start);
//     const int bufCount = static_cast<int>(this->workMemSize_ /
//     sizeof(double)); const ldiv_t tmp = std::ldiv(length, bufCount);
//     MPI_Status status;

//     // 作業用メモリ分のループ
//     for (long i = 0; i < tmp.quot; ++i) {
//         const std::size_t startIndex = start +
//         static_cast<std::size_t>(bufCount * i); answer =
//         MPI_Recv((void*)(pData + startIndex), bufCount, MPI_DOUBLE, src, tag,
//         MPI_COMM_WORLD, &status); if (answer != 0) {
//             std::cerr << " MPI error. " << __FILE__ << ":" << __LINE__
//                       << " anwer=" << answer
//                       << std::endl;
//         }
...
//         const int remain = tmp.rem;
//         const std::size_t startIndex = start +
//         static_cast<std::size_t>(bufCount * tmp.quot); answer =
//         MPI_Recv((void*)(pData + startIndex), remain, MPI_DOUBLE, src, tag,
//         MPI_COMM_WORLD, &status); if (answer != 0) {
//             std::cerr << " MPI error. " << __FILE__ << ":" << __LINE__
//                       << " anwer=" << answer
//                       << std::endl;
...
int TlCommunicate::receiveData(std::valarray<double>& data,
...
#ifdef DIV_COMM
  {
    ...
    for (long i = 0; i < tmp.quot; ++i) {
      std::size_t startIndex = start + static_cast<std::size_t>(bufCount * i);
      answer = MPI_Recv((void*)&(data[startIndex]), bufCount, MPI_DOUBLE, src,
    ...
    if (tmp.rem != 0) {
      ...
      std::size_t startIndex =
          start + static_cast<std::size_t>(bufCount * tmp.quot);
      answer = MPI_Recv((void*)&(data[startIndex]), remain, MPI_DOUBLE, src,
                        tag, MPI_COMM_WORLD, &status);
      if (answer != 0) {
        std::cerr << " MPI error. " << __FILE__ << ":" << __LINE__
                  << " anwer=" << answer << std::endl;
...
int TlCommunicate::receiveData(TlSparseMatrix& rData, int src, int tag) {
  ...
  if (err == 0) {
    ...
    err = this->receiveDataX(&(buf[0]), this->MPI_MATRIXELEMENT, 0, numOfSize,
                             src, tag);
...
int TlCommunicate::receiveDataX(T* pData, const MPI_Datatype mpiType,
...
#ifdef DIV_COMM
  {
    ...
    for (long i = 0; i < tmp.quot; ++i) {
      std::size_t startIndex = start + static_cast<std::size_t>(bufCount * i);
      answer = MPI_Recv((void*)(pData + startIndex), bufCount, mpiType, src,
                        tag, MPI_COMM_WORLD, &status);
      if (answer != 0) {
        std::cerr << " MPI error. " << __FILE__ << ":" << __LINE__
                  << " anwer=" << answer << std::endl;
    ...
    if (tmp.rem != 0) {
      ...
      std::size_t startIndex =
          start + static_cast<std::size_t>(bufCount * tmp.quot);
      answer = MPI_Recv((void*)(pData + startIndex), remain, mpiType, src, tag,
                        MPI_COMM_WORLD, &status);
      if (answer != 0) {
        std::cerr << " MPI error. " << __FILE__ << ":" << __LINE__
                  << " anwer=" << answer << std::endl;
...
// int TlCommunicate::receiveDataFromAnySource(TlSparseSymmetricMatrix& rData,
// int* pSrc, int* pTag)
// {
...
//                 rData.set(std::pair<unsigned long, double>(index[i],
//                 value[i]));
//             }
//         }
//     }
...
#else
  {
    answer = MPI_Recv((void*)(pData), end - start, mpiType, MPI_ANY_SOURCE, tag,
                      MPI_COMM_WORLD, &status);
    src = status.MPI_SOURCE;
  }
#endif  // DIV_COMM
...
#ifdef DIV_COMM
  {
    ...
    if (tmp.quot != 0) {
      for (long i = 0; i < tmp.quot; ++i) {
        std::size_t startIndex = start + static_cast<std::size_t>(bufCount * i);
        MPI_Request* pRequest = new MPI_Request;
        answer = MPI_Isend((void*)&(data[startIndex]), bufCount, mpiType,
...
//     int err = MPI_Isend(pHeader, 3, MPI_UNSIGNED_LONG, dest, tag,
...
#ifdef DIV_COMM
  {
    ...
    if (tmp.quot != 0) {
      for (long i = 0; i < tmp.quot; ++i) {
        std::size_t startIndex = start + static_cast<std::size_t>(bufCount * i);
        MPI_Request* pRequest = new MPI_Request;
        answer = MPI_Irecv(&(data[startIndex]), bufCount, mpiType, src, tag,
                           MPI_COMM_WORLD, pRequest);
        requests.push_back(reinterpret_cast<uintptr_t>((void*)pRequest));
        if (answer != 0) {
          std::cerr << " MPI error. " << __FILE__ << ":" << __LINE__
...
//         this->nonBlockingCommParamTable_[key] =
//         NonBlockingCommParam(NonBlockingCommParam::SEND,
//                                                                      requests);
//     }
...
#pragma omp critical(TlCommunicate_nonBlockingCommParamTable_update)
  {
    ...
    if (it != this->nonBlockingCommParamTable_.end()) {
      ...
      for (std::vector<uintptr_t>::iterator req = it->second.requests.begin();
           req != reqEnd; ++req) {
      ...
      std::cerr << "PROGRAM ERROR(TlCommunicate::cancel()): unknown key. file:"
...
#pragma omp critical(TlCommunicate_nonBlockingCommParamTable_update)
  {
    ...
    if (it != this->nonBlockingCommParamTable_.end()) {
      ...
      std::cerr << "PROGRAM ERROR(TlCommunicate::test()): unknown key. file:"
                << __FILE__ << ", l." << __LINE__ << std::endl;
      std::abort();
    }
  }
...
#pragma omp critical(TlCommunicate_nonBlockingCommParamTable_update)
  {
    ...
    if (it != this->nonBlockingCommParamTable_.end()) {
      ...
      this->log_.critical(TlUtils::format(
          "TlCommunicate::wait(): cannot find: %ld", this->getRank(), key));
    }
  }
...
int TlCommunicate::finalize() {
  ...
  // std::cerr << TlUtils::format("TlCommunicate::finalize(): rank=%d",
...
#ifdef DIV_COMM
  {
    ...
    if (tmp.rem != 0) {
      ...
      std::size_t startIndex =
          start + static_cast<std::size_t>(bufCount * tmp.quot);
      answer =
          MPI_Bcast(&(data[startIndex]), remain, mpiType, root, MPI_COMM_WORLD);
      if (answer != 0) {
        std::cerr << " MPI error. " << __FILE__ << ":" << __LINE__
                  << " anwer=" << answer << std::endl;
      }
...
int TlCommunicate::broadcast(std::valarray<double>& data,
...
#ifdef DIV_COMM
  {
    ...
    for (long i = 0; i < tmp.quot; ++i) {
      ...
      answer = MPI_Bcast((void*)(&data[startIndex]), bufCount, MPI_DOUBLE, root,
                         MPI_COMM_WORLD);
      if (answer != 0) {
        std::cerr << " MPI error. " << __FILE__ << ":" << __LINE__
                  << " anwer=" << answer << std::endl;
    ...
    if (tmp.rem != 0) {
      ...
      const std::size_t startIndex =
          start + static_cast<std::size_t>(bufCount * tmp.quot);
      answer = MPI_Bcast((void*)(&data[startIndex]), remain, MPI_DOUBLE, root,
                         MPI_COMM_WORLD);
      if (answer != 0) {
        std::cerr << " MPI error. " << __FILE__ << ":" << __LINE__
                  << " anwer=" << answer << std::endl;
...
#else
  {
    answer = MPI_Bcast((void*)(&data[0]), end - start, MPI_DOUBLE, root,
                       MPI_COMM_WORLD);
  }
#endif  // DIV_COMM
...
int TlCommunicate::broadcast(T* pData, const MPI_Datatype mpiType,
...
#ifdef DIV_COMM
  {
    ...
    for (long i = 0; i < tmp.quot; ++i) {
      const std::size_t startIndex =
          start + static_cast<std::size_t>(bufCount * i);
      answer = MPI_Bcast((void*)(pData + startIndex), bufCount, mpiType, root,
                         MPI_COMM_WORLD);
      if (answer != 0) {
        std::cerr << " MPI error. " << __FILE__ << ":" << __LINE__
                  << " anwer=" << answer << std::endl;
    ...
    if (tmp.rem != 0) {
      ...
      const std::size_t startIndex =
          start + static_cast<std::size_t>(bufCount * tmp.quot);
      answer = MPI_Bcast((void*)(pData + startIndex), remain, mpiType, root,
                         MPI_COMM_WORLD);
      if (answer != 0) {
        std::cerr << " MPI error. " << __FILE__ << ":" << __LINE__
                  << " anwer=" << answer << std::endl;
  ...
  {
    answer =
        MPI_Bcast((void*)(pData), end - start, mpiType, root, MPI_COMM_WORLD);
  }
#endif  // DIV_COMM
