...
void TlMmapMatrixObject::getHeaderInfo() {
  ...
  if (TlMatrix::getHeaderInfo(this->filePath_, &matrixType, &numOfRows,
                              &numOfCols)) {
    this->numOfRows_ = numOfRows;
    this->numOfCols_ = numOfCols;
  }

  this->fileSize_ = sizeof(int) * 3 + sizeof(double) *
                                          std::size_t(this->getNumOfRows()) *
                                          std::size_t(this->getNumOfCols());
}
...
void TlMmapMatrixObject::resize(const index_type newRow,
...
std::size_t TlMmapMatrixObject::getMemSize() const { return 0; }

double TlMmapMatrixObject::get(const index_type row,
...
void TlMmapMatrixObject::set(const index_type row, const index_type col,
                             const double value) {
  assert((0 <= row) && (row < this->getNumOfRows()));
  assert((0 <= col) && (col < this->getNumOfCols()));
...
void TlMmapMatrixObject::add(const index_type row, const index_type col,
                             const double value) {
  assert((0 <= row) && (row < this->getNumOfRows()));
  assert((0 <= col) && (col < this->getNumOfCols()));
...
bool TlMmapMatrixObject::load(const std::string& path) { return true; }

bool TlMmapMatrixObject::save(const std::string& path) const { return true; }
