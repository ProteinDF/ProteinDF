...
// properties
// ----------------------------------------------------------------------------
TlMatrixObject::index_type TlSparseGeneralMatrix_ImplEigen::getNumOfRows()
    const {
    return this->matrix_.rows();
}

TlMatrixObject::index_type TlSparseGeneralMatrix_ImplEigen::getNumOfCols()
    const {
    return this->matrix_.cols();
}

void TlSparseGeneralMatrix_ImplEigen::resize(
    const TlMatrixObject::index_type newRow,
    const TlMatrixObject::index_type newCol) {
    this->matrix_.resize(newRow, newCol);
}

double TlSparseGeneralMatrix_ImplEigen::get(
    const TlMatrixObject::index_type row,
    const TlMatrixObject::index_type col) const {
    return this->matrix_.coeff(row, col);
}

void TlSparseGeneralMatrix_ImplEigen::set(const TlMatrixObject::index_type row,
                                          const TlMatrixObject::index_type col,
                                          const double value) {
    this->matrix_.coeffRef(row, col) = value;
}

void TlSparseGeneralMatrix_ImplEigen::add(const TlMatrixObject::index_type row,
                                          const TlMatrixObject::index_type col,
                                          const double value) {
    this->matrix_.coeffRef(row, col) += value;
}

void TlSparseGeneralMatrix_ImplEigen::mul(const TlMatrixObject::index_type row,
                                          const TlMatrixObject::index_type col,
                                          const double value) {
    this->matrix_.coeffRef(row, col) *= value;
}
...
bool TlSparseGeneralMatrix_ImplEigen::load(const std::string& path) {
    ...
    const TlMatrixUtils::FileSize headerSize = TlMatrixUtils::getHeaderInfo(
        path, &matrixType, &row, &col, &numOfItems);
    if (headerSize > 0) {
        this->resize(row, col);
        ...
        if (!fs.fail()) {
            ...
            for (std::size_t i = 0; i < numOfItems; ++i) {
                ...
                TlMatrixObject::index_type col;
                double value;
                fs.read(reinterpret_cast<char*>(&row),
                        sizeof(TlMatrixObject::index_type));
                fs.read(reinterpret_cast<char*>(&col),
                        sizeof(TlMatrixObject::index_type));
...
bool TlSparseGeneralMatrix_ImplEigen::save(const std::string& path) const {
    ...
    if (!fs.fail()) {
        ...
        fs.write(reinterpret_cast<const char*>(&numOfRows),
                 sizeof(TlMatrixObject::index_type));
        fs.write(reinterpret_cast<const char*>(&numOfCols),
                 sizeof(TlMatrixObject::index_type));
        fs.write(reinterpret_cast<const char*>(numOfItems),
                 sizeof(std::size_t));
