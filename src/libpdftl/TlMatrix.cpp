...
TlMatrix::TlMatrix(const TlVector& vct, const index_type row,
...
TlMatrix::~TlMatrix() { this->clear(); }

int TlMatrix::getNumOfRows() const { return this->m_nRows; }

int TlMatrix::getNumOfCols() const { return this->m_nCols; }
...
double TlMatrix::get(const index_type row, const index_type col) const {
  return this->data_[this->index(row, col)];
}

void TlMatrix::set(const index_type row, const index_type col,
                   const double value) {
...
void TlMatrix::add(const index_type row, const index_type col,
                   const double value) {
  const size_type index = this->index(row, col);
...
void TlMatrix::setBlockMatrix(const index_type row, const index_type col,
...
void TlMatrix::addBlockMatrix(const int row, const int col,
                              const TlMatrix& matrix) {
  const int row_distance = matrix.getNumOfRows();
  const int col_distance = matrix.getNumOfCols();
...
bool TlMatrix::isLoadable(std::ifstream& ifs) {
  ...
  if (bHeader == true) {
    if ((matrixType == TlMatrixObject::RSFD) ||
        (matrixType == TlMatrixObject::CSFD)) {
      bJudge = true;
    }
  }
...
bool TlMatrix::isLoadable(const std::string& rFilePath) {
  ...
  if (ifs.fail()) {
#ifdef DEBUG
    std::cerr << "[error] TlMatrix::isLoadable(): could not open file. "
              << rFilePath << std::endl;
#endif  // DEBUG
    return false;
  }
...
bool TlMatrix::getHeaderInfo(const std::string& filePath, int* pMatrixType,
...
bool TlMatrix::getHeaderInfo(std::fstream& fs, int* pMatrixType,
                             index_type* pNumOfRows, index_type* pNumOfCols) {
  return TlMatrix::getHeaderInfo_tmpl<std::fstream>(fs, pMatrixType, pNumOfRows,
                                                    pNumOfCols);
}

bool TlMatrix::getHeaderInfo(std::ifstream& ifs, int* pMatrixType,
                             index_type* pNumOfRows, index_type* pNumOfCols) {
  return TlMatrix::getHeaderInfo_tmpl<std::ifstream>(ifs, pMatrixType,
                                                     pNumOfRows, pNumOfCols);
}
...
bool TlMatrix::getHeaderInfo_tmpl(StreamType& s, int* pMatrixType,
  ...
  // int case:
  {
    ...
    const std::streampos estimatedFileSize =
        sizeof(int) * 3 + sizeof(double) * nRows * nCols;
...
bool TlMatrix::load(const std::string& sFilePath) {
  ...
  if (ifs.fail()) {
    this->log_.critical(
        TlUtils::format("[error] TlMatrix::load(): could not open file: %s.",
                        sFilePath.c_str()));
    return false;
  }
  ...
  if (bAnswer != true) {
    std::cerr << "TlMatrix::load() is not supported: " << sFilePath
              << std::endl;
    return false;
  }
...
bool TlMatrix::load(std::ifstream& ifs) {
  ...
  // enum {RSFD, CSFD, RLHD, CLHD, RUHD, CUHD, RSFS, CSFS, RLHS, CLHS, RUHS,
  // CUHS, LHSC}; binary mode
  {
    // read header
    int matrixType = 0;
    index_type nRows = 0;
    ...
    switch (matrixType) {
      case TlMatrixObject::RSFD:
        ...
        this->initialize();
        {
          ...
          ifs.read(reinterpret_cast<char*>(&readBuf[0]),
      ...
      case TlMatrixObject::CSFD:
        ...
        this->m_nCols = nCols;
        this->initialize();
        ifs.read(reinterpret_cast<char*>(this->data_),
                 this->getNumOfElements() * sizeof(double));
        break;

      default:
        std::cerr << TlUtils::format(
                         "this matrix type is not supported. type=%d",
                         matrixType)
                  << std::endl;
        bAnswer = false;
        break;
    ...
    // if (bAnswer == true) {
    //
    //     // 本当は大きめのバッファで読み取りたいが、row oriented
    //     だから変換しないといけない
    //     //const int nMatrixSize = this->m_nRows * this->m_nCols;
    //     //ifs.read(reinterpret_cast<char*>(&(this->m_aMatrix[0])),
    //     sizeof(double) * nMatrixSize); for (int row = 0; row < nRows; ++row)
    //     {
...
bool TlMatrix::save(std::ofstream& ofs, const MatrixType saveType) const {
  ...
  switch (saveType) {
    ...
    case CSFD:
      ofs.write(reinterpret_cast<char*>(this->data_),
                this->getNumOfElements() * sizeof(double));
      break;
...
#ifdef HAVE_HDF5
bool TlMatrix::saveHdf5(const std::string& filepath,
...
bool TlMatrix::loadHdf5(const std::string& filepath,
  ...
  switch (matrixType) {
    ...
    default:
      this->log_.critical(
          TlUtils::format("illegal matrix type for TlMatrix: %d", matrixType));
      break;
  }
...
TlVector multiplicationByLapack(const TlMatrix& A, const TlVector& X) {
  ...
  // *  m by n matrix.
  dgemv_(&TRANS, &M, &N, &alpha, const_cast<TlMatrix&>(A).data_, &LDA,
         const_cast<TlVector&>(X).data_, &INCX, &beta, Y.data_, &INCY);
...
TlMatrix multiplicationByLapack(const TlMatrix& X, const TlMatrix& Y) {
  ...
  dgemm_(&TRANSA, &TRANSB, &M, &N, &K, &alpha, const_cast<TlMatrix&>(X).data_,
         &LDA, const_cast<TlMatrix&>(Y).data_, &LDB, &beta, answer.data_, &LDC);
...
bool solveLinearLeastSquaresProblemByLapack(const TlMatrix& inA,
                                            const TlMatrix& inB, TlMatrix* pX) {
  bool answer = true;
  TlLogging& log = TlLogging::getInstance();
