...
TlMmapMatrix::TlMmapMatrix(const std::string& filePath, const index_type row,
...
std::size_t TlMmapMatrix::getMemSize() const { return 0; }
...
void TlMmapMatrix::set(const index_type row, const index_type col,
                       const double value) {
  assert((0 <= row) && (row < this->getNumOfRows()));
  assert((0 <= col) && (col < this->getNumOfCols()));
...
void TlMmapMatrix::add(const index_type row, const index_type col,
                       const double value) {
  assert((0 <= row) && (row < this->getNumOfRows()));
  assert((0 <= col) && (col < this->getNumOfCols()));
...
bool TlMmapMatrix::load(const std::string& path) { return true; }

bool TlMmapMatrix::save(const std::string& path) const { return true; }
