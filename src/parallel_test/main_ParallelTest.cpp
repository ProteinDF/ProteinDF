...
int main(int argc, char* argv[]) {
  ...
  //
  {
    ...
    for (int i = 0; i < numOfProcs; ++i) {
      if (i == rComm.getRank()) {
        std::cerr << TlUtils::format("PID %d on %s as rank %d", getpid(),
                                     hostname.c_str(), rComm.getRank())
                  << std::endl;
      }
      rComm.barrier();
    }
...
void testCommunicate_sendInt() {
  ...
  for (int i = 1; i < proc; i++) {
    if (rank == i) {
      ...
      if (rData != (100 + i)) {
        std::cout << "recv(int) = " << rData << ", expect = " << (100 + i)
                  << std::endl;
        bIsPassed = false;
      }
    }
...
void testCommunicate_sendLong() {
  ...
  for (int i = 1; i < proc; i++) {
    if (rank == i) {
      ...
      if (rData != (1000 + i)) {
        std::cout << "recv(long) = " << rData << ", expect = " << (1000 + i)
                  << std::endl;
        bIsPassed = false;
      }
    }
  ...
  for (int i = 1; i < proc; i++) {
    if (rank == i) {
      ...
      if (rData != sExpectData) {
        std::cout << "recv(string) = " << rData << ", expect = " << sExpectData
                  << std::endl;
        bIsPassed = false;
      }
    }
  ...
  for (int i = 1; i < proc; i++) {
    if (rank == i) {
      ...
      if (std::fabs(rData - (100 + i)) > 1.0E-16) {
        std::cout << "recv(double) = " << rData << ", expect = " << (100 + i)
                  << std::endl;
        bIsPassed = false;
      }
    }
