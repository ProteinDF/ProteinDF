  ...
  for (int r = 0; r < nSize; ++r) {
    for (int c = 0; c <= r; ++c) {
      if (fabs(A(r, c) - exactA(r, c)) > 1.0E-5) {
        std::cout << TlUtils::format(
                         "(%2d, %2d): actual= %f, expect= %f, err= %e", r, c,
                         A(r, c), exactA(r, c), fabs(A(r, c) - exactA(r, c)))
                  << std::endl;
        bIsPassed = false;
        break;
      }
  ...
  for (int r = 0; r < 9; ++r) {
    for (int s = 0; s < 9; ++s) {
      ...
      if (fabs(c - exact_c) > 1.0E-5) {
        ...
        if (rComm.isMaster() == true) {
          std::cout << TlUtils::format(
                           "(%2d, %2d): actual= %+e, expect= %+e, err= %+e", r,
  ...
  for (int r = 0; r < dim; ++r) {
    for (int s = 0; s <= r; ++s) {
      ...
      if (std::fabs(a - exact_a) > 1.0E-12) {
        bIsPassed = false;
        std::cout << TlUtils::format(
                         "(%2d, %2d): actual= %f, expect= %f, err= %e", r, s, a,
                         exact_a, fabs(a - exact_a))
                  << std::endl;
      }
    }
  }
