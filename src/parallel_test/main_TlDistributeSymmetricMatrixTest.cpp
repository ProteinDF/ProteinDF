...
void makeMatrixA(TlDistributeSymmetricMatrix* pDistA,
                 TlDenseSymmetricMatrix_BLAS_Old* pA);
void makeMatrixB(TlDistributeSymmetricMatrix* pDistB,
                 TlDenseSymmetricMatrix_BLAS_Old* pB);
...
void testCopyConstructer() {
    ...
    if (std::fabs(B(10, 10) - 10.0) > 1.0E-16) {
        bIsPassed = false;
        std::cout << "B(10, 10) = " << B(10, 10) << ", exact = 10.0"
                  << std::endl;
    }
    if (std::fabs(B(17, 4) - 56.0) > 1.0E-16) {
        bIsPassed = false;
        std::cout << "B(17,  4) = " << B(17, 4) << ", exact = 56.0"
                  << std::endl;
    }
    if (std::fabs(B(4, 17) - 56.0) > 1.0E-16) {
        bIsPassed = false;
        std::cout << "B( 4, 17) = " << B(4, 17) << ", exact = 56.0"
                  << std::endl;
    }
    if (std::fabs(B(21, 18) - (-2.5)) > 1.0E-16) {
        bIsPassed = false;
        std::cout << "B(21, 18) = " << B(21, 18) << ", exact = -2.5"
                  << std::endl;
    }
    if (std::fabs(B(18, 21) - (-2.5)) > 1.0E-16) {
        bIsPassed = false;
        std::cout << "B(18, 21) = " << B(18, 21) << ", exact = -2.5"
                  << std::endl;
    }
...
void testMulti1() {
    ...
    TlDenseSymmetricMatrix_BLAS_Old exactA(dim);
    {
        double count = 1.0;
        for (int i = 0; i < dim; ++i) {
            for (int j = 0; j <= i; ++j) {
    ...
    TlDenseSymmetricMatrix_BLAS_Old exactB(dim);
    {
        double count = 1.82;
        for (int i = 0; i < dim; ++i) {
            for (int j = 0; j <= i; ++j) {
    ...
    for (int r = 0; r < nSize; ++r) {
        for (int c = 0; c <= r; ++c) {
            if (fabs(A(r, c) - exactA(r, c)) > 1.0E-5) {
                std::cout << TlUtils::format(
                                 "(%2d, %2d): actual= %f, expect= %f, err= %e",
                                 r, c, A(r, c), exactA(r, c),
                                 fabs(A(r, c) - exactA(r, c)))
                          << std::endl;
                bIsPassed = false;
                break;
            }
...
void testInverse() {
    ...
    // TlDenseSymmetricMatrix_BLAS_Old exactB = exactA;
...
void testInverse2() {
    ...
    TlDenseSymmetricMatrix_BLAS_Old exactAinv = exactA;
    exactA.inverse();
        ...
        for (int s = 0; s < nSize; ++s) {
            ...
            if (fabs(c - exact_c) > 1.0E-9) {
                ...
                if (rComm.isMaster() == true) {
                    std::cout
                        << TlUtils::format(
                               "(%2d, %2d): actual= %f, expect= %f, err= %e", r,
                               s, c, exact_c, fabs(c - exact_c))
                        << std::endl;
                }
                rComm.barrier();
            }
...
void testDiagonal() {
    ...
    TlDenseSymmetricMatrix_BLAS_Old exactA(size);
    {
        double count = 1.0;
        for (int i = 0; i < size; ++i) {
            for (int j = 0; j <= i; ++j) {
    ...
    for (int i = 0; i < nSize; ++i) {
        if (fabs(eigVal[i] - exactEigVal[i]) > 1.0E-9) {
            ...
            if (rComm.isMaster() == true) {
                std::cout << TlUtils::format(
                                 "eigval[%2d]: actual= %f, expect= %f, err= %e",
                                 i, eigVal[i], exactEigVal[i],
            ...
            if ((fabs(eigvec[s] - exact_eigvec[s]) > 1.0E-9) &&
                ...
                if (rComm.isMaster() == true) {
                    std::cout
                        << TlUtils::format(
                               "(%2d, %2d): actual= %f, expect= %f, err= %e", r,
                               s, eigvec[s], exact_eigvec[s],
                               fabs(eigvec[s] - exact_eigvec[s]))
                        << std::endl;
                }
                rComm.barrier();
            }
...
void testDot() {
    ...
    TlDenseSymmetricMatrix_BLAS_Old exactB;
...
void testSum() {
    ...
    if (std::fabs(sum - exactSum) > 1.0E-12) {
        bIsPassed = false;
        std::cout << TlUtils::format("actual= %f, expect= %f, err= %e", sum,
                                     exactSum, fabs(sum - exactSum))
                  << std::endl;
    }
