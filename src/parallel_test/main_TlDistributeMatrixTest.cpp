        ...
        for (int j = 0; j < row; ++j) {
            ...
            if (fabs(a - ea) > 1.0E-5) {
                ...
                if (rComm.isMaster() == true) {
                    std::cout
                        << TlUtils::format(
                               "(%2d, %2d): actual= %f, expect= %f, err= %e", i,
                               j, a, ea, fabs(a - ea))
                        << std::endl;
                }
            }
...
void testMulti2() {
    ...
    for (int r = 0; r < M; ++r) {
        ...
        if (fabs(c - exact_c) > 1.0E-5) {
            bIsPassed = false;
            std::cout << TlUtils::format("[%2d]: actual = %f, expect = %f", r,
                                         c, exact_c)
                      << std::endl;
        }
    }
...
void testGetMaxAbsoluteElement() {
    ...
    if ((row != 3) || (col != 17)) {
        if (rComm.isMaster() == true) {
            std::cout << TlUtils::format(
                             "(%2d, %2d): actual= %f, expect= %f, err= %e", row,
                             col, v, 51.0, fabs(v - 51.0))
                      << std::endl;
        }
        bIsPassed = false;
    }
...
void testGetPartialMatrix2() {
    ...
    while (true) {
        if (rComm.getRank() == target) {
            ...
            //             std::cerr << "is complete? "<< isComplete <<
            //             std::endl;
            if (isComplete == true) {
    ...
    if (rComm.getRank() == target) {
        if (fabs(B(1, 0) - 0.0) > 1.0E-10) {
            std::cerr << TlUtils::format("B(1, 0)=%f(actual), %f(expect)",
                                         B(1, 0), 0.0)
                      << std::endl;
            bIsPassed = false;
        }
        if (fabs(B(3, 17) - (-51.0)) > 1.0E-10) {
            std::cerr << TlUtils::format("B(3, 17)=%f(actual), %f(expect)",
                                         B(3, 17), -51.0)
                      << std::endl;
            bIsPassed = false;
        }
        if (fabs(B(3, 9) - 33.0) > 1.0E-10) {
            std::cerr << TlUtils::format("B(3, 9)=%f(actual), %f(expect)",
                                         B(3, 9), 33.0)
                      << std::endl;
            bIsPassed = false;
        }
