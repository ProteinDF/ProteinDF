...
TlDenseGeneralMatrix_Lapack DfConverge_DIIS::buildBMatrix(
    ...
    for (int i = 0; i < cycles; ++i) {
        ...
        for (int j = 0; j < i; ++j) {
            const MatrixType r_j =
                this->getResidual<MatrixType, SymmetricMatrixType>(
                    runType, startItr + j);
...
void DfConverge_DIIS::convergeKSMatrix(const DfObject::RUN_TYPE runType) {
    ...
    if (itr >= this->startIterationOfDIIS_) {
        ...
        if (maxError < 0.1) {
            ...
            this->log_.info(TlUtils::format("itr=%d, start=%d, last=%d", itr,
                                            startItr, last));

            // 係数行列の取得
            const TlDenseGeneralMatrix_Lapack c =
                this->getCoef<MatrixType, SymmetricMatrixType>(runType,
                                                               startItr, last);
...
void DfConverge_DIIS::convergePMatrix(const DfObject::RUN_TYPE runType) {
    this->log_.info(
        TlUtils::format("converge dennsity matrix using DIIS. #%d/%d",
                        this->m_nIteration, this->startIterationOfDIIS_));
    const int itr = this->m_nIteration;

    bool updateByDIIS = false;
    if (itr >= this->startIterationOfDIIS_) {
        // 残差ベクトルを求める
        MatrixType r = this->getResidual<MatrixType, SymmetricMatrixType>(
            runType, itr - 1);
        ...
        if (maxError < 0.1) {
            ...
            // 係数行列の取得
            const TlDenseGeneralMatrix_Lapack c =
                this->getCoef<MatrixType, SymmetricMatrixType>(
        ...
        this->log_.info(TlUtils::format(
            "the iteration, %d, is lower than start number, %d.", itr,
