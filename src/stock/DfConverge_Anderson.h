...
void DfConverge_Anderson::convergeKSMatrix(const DfObject::RUN_TYPE runType) {
    ...
    if (this->m_nIteration >= (nAndersonStartIterationNumber - 1)) {
        VectorType nextY1 =
            this->getVectorOfKSMatrix<SymmetricMatrixType, VectorType>(
                runType, nIteration);
        nextY1.save("fl_Work/fl_Vct_anderson." + TlUtils::xtos(nIteration));
    }
...
void DfConverge_Anderson::convergePMatrix(const DfObject::RUN_TYPE runType) {
    ...
    if (this->m_nIteration >= nAndersonStartIterationNumber) {
        // do Anderson
        VectorType Y0;
        Y0.load("fl_Work/fl_Vct_anderson." +
                TlUtils::xtos(this->m_nIteration - 1));
        ...
        VectorType Y1;
        Y1.load("fl_Work/fl_Vct_anderson." +
                TlUtils::xtos(this->m_nIteration - 2));
...
VectorType DfConverge_Anderson::anderson(const VectorType& X1,
                                         ...
                                         const VectorType& Y0,
                                         const VectorType& Y1) {
    const double beta = this->m_dDampingFactorOfAnderson;  // damping factor of
                                                           // Anderson's method
    ...
    double theta = 0.0;
    {
        ...
        r1.save("fl_Work/fl_Vct_andersonr1." +
                TlUtils::xtos(this->m_nIteration));

        // r^(n-2) = Y^(n-1) - X^(n-2)
        const VectorType r2 = Y1 - X2;
        r2.save("fl_Work/fl_Vct_andersonr2." +
                TlUtils::xtos(this->m_nIteration));

        const VectorType r12 = r1 - r2;
        r12.save("fl_Work/fl_Vct_andersonr12." +
                 TlUtils::xtos(this->m_nIteration));
