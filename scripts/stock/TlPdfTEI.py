...
class TlPdfTEI:
    ...
    #_re_number_of_basis  = re.compile("\s*(\d+)\s*basis functions")
    # [control-norb]                                                      [17]
    _re_number_of_basis  = re.compile("\s*\[control-norb\]\s+\[(\d+)\]")

    # ( 30   6  17  15) [ERI_SPPP] =  6.2086326513168963e-06
    _re_two_electron_integrals = re.compile("\(\s*(\d+)\s*(\d+)\s*(\d+)\s*(\d+)\s*\)\s+\[(\S+)\]\s*=\s*(\S+)")
    ...
    def get_twoei_type_name_from_key(self, i, j, k, l):
        key = (i, j, k, l)
        if self._twoei.has_key(key):
            return self._twoei_type[key]
        key = (i, j, l, k)
        if self._twoei.has_key(key):
            return self._twoei_type[key]
        key = (j, i, k, l)
    ...
    def get_K_matrix(self, max_index):
        ...
        for i in seq_i:
            ...
            for j in seq_j:
                ...
                for k in seq_k:
                    for l in seq_l:
                        #tmp += (self.get_twoei_from_key(i, k, j, l) * -0.5)
                        tmp += (self.get_twoei_from_key(i, k, j, l) * -1.0)
    ...
    def _parse_two_electron_integrals(self, line):
        ...
        if (matchObj != None):
            ...
            type_name = str(matchObj.group(5))
            t = (i -1, j -1, k -1, l -1)
            v = float(matchObj.group(6).replace("D", "E"))
            self._twoei[t] = v
            self._twoei_type[t] = type_name
            #print "PDF(%3d %3d | %3d %3d) = % e" % (i, j, k, l, v)
