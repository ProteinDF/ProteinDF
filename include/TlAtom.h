...
public:
    ...
    explicit TlAtom(const std::string& symbol = "",
                    const TlPosition& pos = TlPosition(),
                    double charge = 0.0);
...
public:
    ...
    void rotateBy(const TlMatrix& rot);
...
{
    return m_dVdw_radii[this->getType()];
}

inline TlPosition TlAtom::getPosition() const
{
    return m_position;
}
...
inline void TlAtom::rotateBy(const TlMatrix& rot)
{
    this->m_position.rotateBy(rot);
}

inline std::string TlAtom::getName() const
{
    return this->m_name;
}

inline void TlAtom::setName(const std::string& name)
{
    this->m_name = name;
}
